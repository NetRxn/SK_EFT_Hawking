...
-- Phase 5q.F W5 (2026-06-15): the geometric Smith map s:Ω₅^{Spin-ℤ₄}→Ω₄^{Pin⁺} as a TYPED HOMOMORPHISM
-- on the genuine DataBordismGrp carriers (DR Smith_sequence.md §5.2 scope). smithDataHom (grade transport,
-- floor-surviving ℤ/16) + the Smith square abkGrade∘smithDataHom=η-grade + the floor-quotient Smith iso
-- ℤ/16≃ℤ/16 + the SW operative content (smith_w2_vanishes) + §7 the genuine MANIFOLD layer (bricks 1-2:
-- PD(a) a real codim-1 SingularManifold over arbitrary M). The de-thinned [M]↦[PD(a)] at the typed scope.
...
-- Phase 5q.H (2026-07-04, Option-A foundation brick 3): the integral intersection FORM — cupH24 paired
...
import SKEFTHawking.SingularRelCohomSetCongrMkInt
import SKEFTHawking.SingularIndUfCapAgreeInt
import SKEFTHawking.SingularCoverPartitionCapBoundaryInt
...
-- Phase 5q.F (w₂-foundation, brick 6c-c7c.2): barycentric subdivision stays in a convex set — the
...
-- Phase 5q.H · E1 (CSC-PD tower): the per-compact fundamental cycle family fundCycleW (from a global
-- ancestor) + relBoundaries_monoInt + the relB-form cycle rung. Integral mirror of SingularOpenDualityCycle.
...
-- Phase 5q.H · E1 (CSC-PD tower): RelativeChainInt is PROJECTIVE (split injection chainIncl) — the crux
-- enabling brick for the field-UC-free relative-cohomology MV exactness (Hom-dualize the homology MV SES).
import SKEFTHawking.SingularRelativeChainProjectiveInt
...
-- Phase 5q.F (w₂-foundation, brick 6c-c8.3): the barycentric-subdivision diameter estimate (analytic
-- heart of excision) — barycenter-to-vertex bound ‖barycenter v − vⱼ‖ ≤ (n/(n+1))·diam (the i=j term
-- vanishes), the contraction factor n/(n+1)<1 driving diam(Sdᵐ pieces)→0.
import SKEFTHawking.SingularSubdivisionDiameter
-- Phase 5q.F (w₂-foundation, brick 6c-c8.4): singular excision — joins the subdivision chain homotopy to
...
-- II.4.3. The banked local tooling carries it: isotropic_padicInt_of_unit_det is NOT even-specific
-- (rank >= 3, odd p, unit det), so the odd places are free at both ranks; R is indefiniteness; and
-- the place 2 is free by reciprocity -- a TERNARY form's local obstruction is a single Hilbert
-- symbol, so rank 3 needs no discriminant hypothesis, while rank 4 with det = +1 is
-- square-discriminant and reuses quaternary_sqdisc_solvable_of_local_no_two verbatim (evenness was
-- only ever used in weakIsotropic_rank_four to produce det = 1). (wt1-built.)
...
import SKEFTHawking.SingularCapCrossTriple
...
import SKEFTHawking.PinPlusCylinderWAdmPinnedBaseWu
...
import SKEFTHawking.PoincareLefschetzRelFundClassCylinderPuncturedPieceIso
...
import SKEFTHawking.PoincareLefschetzRelFundClassCylinderPuncturedOverlapPair
import SKEFTHawking.PoincareLefschetzRelFundClassCylinderPuncturedFlankCount
...
import SKEFTHawking.PoincareLefschetzRelFundClassCylinderFlankReduce
...
import SKEFTHawking.PinPlusCylComponentExcisionBridgeConsume
...
import SKEFTHawking.PinPlusTraceCapstoneCoverGlueSeamCorrector
...
import SKEFTHawking.PinPlusTraceDiskControlledRep
import SKEFTHawking.PinPlusTraceDiskSphereCycle
...
import SKEFTHawking.PinPlusTraceExplicitDiskChain
...
import SKEFTHawking.PinPlusKTTowerInhabit
...
import SKEFTHawking.PinPlusKTSpinSigmaNovikovTowerBridge
...
import SKEFTHawking.PinPlusTraceCapstoneCollarPairHandle
...
-- SingularFaceLocalHomologyVanish — degree-general boundary-FACE local-homology vanishing
...
-- Phase 5q.H — ambient-generic: closed `n`-manifolds are homologically `n`-dimensional
-- (`Hₚ(M;ℤ) = 0` for `p > n`, `Hₙ(M;ℤ)` free), off the Poincaré-duality lane's Hatcher-3.27
-- good-compact stage run at `K = univ`. No orientation, no duality.
