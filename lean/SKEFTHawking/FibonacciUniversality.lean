...
/- The arithmetic facts in this file are over the two-level cyclotomic tower
   K = Q(ζ₅, √φ) = `QCyc5Ext` and are **kernel-checked modulo `native_decide`**
   (ADR-002 #2). Unlike the base-field `QCyc5` headlines in `FibonacciBraiding`
   (which are hardened kernel-pure via the `ext`/`powerTable` template), the
   `QCyc5Ext` proofs nest the base-`QCyc5` `powerTable` reduction inside the
   degree-2 `mulReduce2` tower, and the fully-symbolic `ext <;> simp` expansion
   exceeds the heartbeat budget; Pipeline Invariant #10 forbids raising
   `maxHeartbeats`. A future Route-1′ metaprogram (ADR-002 2026-05-29 addendum)
   would discharge these symbolically — until then they retain the compiler-trust
   evaluator. The D4 universal-gate headline itself is established literature
   (FLW 2002); the formalization, not the fact, is the contribution (ADR-002 Q3). -/
