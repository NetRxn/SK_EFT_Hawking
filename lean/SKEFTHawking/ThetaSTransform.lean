...
/-- **The Fourier-integral exponent is the dual lattice-theta phase** (brick B): the `[Θ2]` exponent
`(∑(c ᵥ* (√G)⁻¹_ℂ)ᵢ²)/(4·(-iπσ))` with `cᵢ = 2πi(-nᵢ)` equals `π·i·(-1/σ)·nᵀG⁻¹n` — the summand phase of
`latticeTheta G⁻¹ (-1/σ)`. Via `sum_sq_vecMul_sqrtInv_eq` (⟹ `cᵀG⁻¹c`), the scalar `c = -2πi·n`
(`(-2πi)² = -4π²`), the ℝ→ℂ cast of `nᵀG⁻¹n`, and complex division (`I⁻¹ = -I`, `σ ≠ 0`). -/
theorem gaussianCM_exponent_eq {d : ℕ} {σ : ℂ} {G : Matrix (Fin d) (Fin d) ℝ} (hG : G.PosDef)
    (hσ : 0 < σ.im) (n : Fin d → ℤ) :
    (∑ i, ((fun j => 2 * (π : ℂ) * I * ((-(n j) : ℝ) : ℂ)) ᵥ*
        ((CFC.sqrt G)⁻¹.map (Complex.ofReal))) i ^ 2) / (4 * (-((π : ℂ) * I * σ)))
      = (π : ℂ) * I * (-1 / σ)
  ...
  -- c = (-(2πI)) • ñ
  have hcsmul : (fun j => 2 * (π : ℂ) * I * ((-(n j) : ℝ) : ℂ))
      = (-(2 * (π : ℂ) * I)) • (fun i => (n i : ℂ)) := by
    funext j; simp only [Pi.smul_apply, smul_eq_mul]; push_cast; ring
  rw [sum_sq_vecMul_sqrtInv_eq hG, hcsmul, Matrix.mulVec_smul, smul_dotProduct,
    dotProduct_smul, smul_eq_mul, smul_eq_mul]
  have hcast : (fun i => (n i : ℂ)) ⬝ᵥ (G⁻¹.map (Complex.ofReal)) *ᵥ (fun i => (n i : ℂ))
      = (((fun i => (n i : ℝ)) ⬝ᵥ G⁻¹ *ᵥ (fun i => (n i : ℝ)) : ℝ) : ℂ) := by
    simp only [Matrix.mulVec, dotProduct, Matrix.map_apply, Complex.ofReal_sum, Complex.ofReal_mul]
    push_cast
    rfl
  rw [hcast]
  set Q : ℂ := (((fun i => (n i : ℝ)) ⬝ᵥ G⁻¹ *ᵥ (fun i => (n i : ℝ)) : ℝ) : ℂ) with hQ
  have hden : (4 : ℂ) * (-((π : ℂ) * I * σ)) ≠ 0 := by
    simp [hπ, hσ0, Complex.I_ne_zero]
  have hkey : (-(2 * (π : ℂ) * I)) * ((-(2 * (π : ℂ) * I)) * Q) = -(4 * (π : ℂ) ^ 2) * Q := by
    linear_combination (4 * (π : ℂ) ^ 2 * Q) * Complex.I_sq
