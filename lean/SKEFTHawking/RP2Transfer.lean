...
open SKEFTHawking.RP2PointSet SKEFTHawking.RP2Covering
open SKEFTHawking.SingularHomologyMod2 SKEFTHawking.SingularFunctoriality

namespace SKEFTHawking.RP2Transfer

/-- The covering map as a continuous map of `TopCat` carriers. -/
noncomputable def mkC : C(↑(TopCat.of S2), ↑(TopCat.of RP2)) :=
  ⟨Quotient.mk (MulAction.orbitRel ℤˣ S2), continuous_quotient_mk'⟩
...
noncomputable def rlP {n : ℕ}
    ...
    C(stdSimplex ℝ (Fin (n + 1)), RP2) :=
  (TopCat.of RP2).toSSetObjEquiv (op (SimplexCategory.mk n)) σ

/-- **The fiber is the antipodal pair**: any two points over the same class differ by `±1`. -/
theorem fiber_pair {e y : S2}
    (h : Quotient.mk (MulAction.orbitRel ℤˣ S2) e
      = Quotient.mk (MulAction.orbitRel ℤˣ S2) y) :
    e = y ∨ e = (-1 : ℤˣ) • y := by
...
noncomputable def liftSimplex {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) (e₀ : S2)
    (he : Quotient.mk (MulAction.orbitRel ℤˣ S2) e₀ = rlP σ (bary n)) :
    (TopCat.toSSet.obj (TopCat.of S2)).obj (op (SimplexCategory.mk n)) :=
  ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n))).symm
    (rp2_isCoveringMap.existsUnique_continuousMap_lifts (rlP σ) (bary n) e₀ he).exists.choose

/-- The lift's defining properties, packaged: it sits over `σ` and hits `e₀` at the barycenter. -/
theorem liftSimplex_spec {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) (e₀ : S2)
    (he : Quotient.mk (MulAction.orbitRel ℤˣ S2) e₀ = rlP σ (bary n)) :
    ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n))
        (liftSimplex σ e₀ he)) (bary n) = e₀ ∧
      Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
        ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) (liftSimplex σ e₀ he))
        = rlP σ := by
  have hspec :=
    (rp2_isCoveringMap.existsUnique_continuousMap_lifts (rlP σ) (bary n) e₀ he).exists.choose_spec
...
theorem liftSimplex_unique {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) (e₀ : S2)
    (he : Quotient.mk (MulAction.orbitRel ℤˣ S2) e₀ = rlP σ (bary n))
    (τ : (TopCat.toSSet.obj (TopCat.of S2)).obj (op (SimplexCategory.mk n)))
    (hτ0 : ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) (bary n) = e₀)
    (hτ : Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
      ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) = rlP σ) :
    τ = liftSimplex σ e₀ he := by
  have huniq :=
    (rp2_isCoveringMap.existsUnique_continuousMap_lifts (rlP σ) (bary n) e₀ he).unique
      (y₁ := (TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ)
      (y₂ := (TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) (liftSimplex σ e₀ he))
      ⟨hτ0, hτ⟩ (liftSimplex_spec σ e₀ he)
  exact ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n))).injective huniq
...
noncomputable def outFiber {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) : S2 :=
  Quotient.out (rlP σ (bary n))

theorem mk_outFiber {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
    Quotient.mk (MulAction.orbitRel ℤˣ S2) (outFiber σ) = rlP σ (bary n) :=
  Quotient.out_eq _

theorem mk_neg_outFiber {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
    Quotient.mk (MulAction.orbitRel ℤˣ S2) ((-1 : ℤˣ) • outFiber σ) = rlP σ (bary n) :=
...
theorem neg_ne_outFiber {n : ℕ}
  ...
  have h2 : (outFiber σ : EuclideanSpace ℝ (Fin (2 + 1)))
      + (outFiber σ : EuclideanSpace ℝ (Fin (2 + 1))) = 0 := by
    nth_rewrite 1 [← hcoe]
    exact neg_add_cancel _
  have h3 : (2 : ℝ) • (outFiber σ : EuclideanSpace ℝ (Fin (2 + 1))) = 0 := by
    rw [two_smul]; exact h2
  have h0 : (outFiber σ : EuclideanSpace ℝ (Fin (2 + 1))) = 0 :=
...
theorem liftPlus_ne_liftMinus {n : ℕ}
  ...
  have hval := congrArg
    (fun τ => ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) (bary n)) h
  simp only at hval
...
noncomputable def transferChain (n : ℕ) :
    SingularChain (TopCat.of RP2) n →ₗ[ZMod 2] SingularChain (TopCat.of S2) n :=
...
theorem mapChain_transferChain (n : ℕ) (c : SingularChain (TopCat.of RP2) n) :
...
theorem liftSimplex_eq_of_agree {n : ℕ}
    (τ τ' : (TopCat.toSSet.obj (TopCat.of S2)).obj (op (SimplexCategory.mk n)))
    (hover : mapSimplex mkC τ = mapSimplex mkC τ')
    (d : stdSimplex ℝ (Fin (n + 1)))
    (hd : ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) d
      = ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ') d) : τ = τ' := by
  have hcomp : Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
      ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ)
      = Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
        ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ') := by
    have h1 := congrArg ((TopCat.of RP2).toSSetObjEquiv (op (SimplexCategory.mk n))) hover
    rw [mapSimplex, mapSimplex, Equiv.apply_symm_apply, Equiv.apply_symm_apply] at h1
    exact congrArg DFunLike.coe h1
  have := rp2_isCoveringMap.eq_of_comp_eq
    (((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ)).continuous
    (((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ')).continuous
    hcomp d hd
  exact ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n))).injective
...
theorem face_lift_mem_pair {n : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk (n + 1))))
    (i : Fin (n + 2))
    (τ : (TopCat.toSSet.obj (TopCat.of S2)).obj (op (SimplexCategory.mk (n + 1))))
  ...
  have h1 : rlP F = mkC.comp ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τF) := by
    rw [rlP, ← hover, mapSimplex, Equiv.apply_symm_apply]
  set y := ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τF) (bary n) with hy
  have hmk : Quotient.mk (MulAction.orbitRel ℤˣ S2) y = rlP F (bary n) :=
    (congrFun (congrArg DFunLike.coe h1) (bary n)).symm
  have hcirc : Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
      ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τF) = rlP F :=
...
theorem chainBoundary_transferChain (n : ℕ) (c : SingularChain (TopCat.of RP2) (n + 1)) :
    chainBoundary (TopCat.of S2) n (transferChain (n + 1) c)
      = transferChain n (chainBoundary (TopCat.of RP2) n c) := by
  induction c using Finsupp.induction_linear with
...
theorem mem_pair_of_pushforward {n : ℕ}
  ...
  have h1 : rlP F = mkC.comp ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) := by
    rw [rlP, hF, mapSimplex, Equiv.apply_symm_apply]
  set y := ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) (bary n) with hy
  have hmk : Quotient.mk (MulAction.orbitRel ℤˣ S2) y = rlP F (bary n) :=
    (congrFun (congrArg DFunLike.coe h1) (bary n)).symm
  have hcirc : Quotient.mk (MulAction.orbitRel ℤˣ S2) ∘
      ((TopCat.of S2).toSSetObjEquiv (op (SimplexCategory.mk n)) τ) = rlP F :=
    (congrArg DFunLike.coe h1).symm
...
theorem mapChain_apply_pair {n : ℕ} (c : SingularChain (TopCat.of S2) n)
...
theorem transferChain_apply_liftPlus {n : ℕ} (c : SingularChain (TopCat.of RP2) n)
    (β : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
...
theorem mem_range_transferChain_of_mapChain_eq_zero {n : ℕ}
    (c : SingularChain (TopCat.of S2) n) (hc : mapChain mkC n c = 0) :
    c ∈ LinearMap.range (transferChain n) := by
  classical
  suffices H : ∀ (N : ℕ) (c : SingularChain (TopCat.of S2) n), c.support.card ≤ N →
