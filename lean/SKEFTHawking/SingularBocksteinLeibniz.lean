...
/-! ## §3. `Sq¹ = (·)²` on `1`-cocycles (pointwise) -/

/-- At `(1,1)` the AW front face is the `δ₂`-face (`{0,1}`-edge). -/
theorem frontFace_one_one {X : TopCat}
    (σ : (TopCat.toSSet.obj X).obj (op (SimplexCategory.mk (1 + 1)))) :
    frontFace (p := 1) (q := 1) σ = face 2 σ := by
  show (TopCat.toSSet.obj X).map (frontIncl 1 1).op σ
    = (TopCat.toSSet.obj X).map (SimplexCategory.δ 2).op σ
  have h : frontIncl 1 1 = SimplexCategory.δ 2 := by
    apply SimplexCategory.Hom.ext
    ext i
    fin_cases i <;> rfl
  rw [h]

/-- At `(1,1)` the AW back face is the `δ₀`-face (`{1,2}`-edge). -/
theorem backFace_one_one {X : TopCat}
    (σ : (TopCat.toSSet.obj X).obj (op (SimplexCategory.mk (1 + 1)))) :
    backFace (p := 1) (q := 1) σ = face 0 σ := by
  show (TopCat.toSSet.obj X).map (backIncl 1 1).op σ
    = (TopCat.toSSet.obj X).map (SimplexCategory.δ 0).op σ
  have h : backIncl 1 1 = SimplexCategory.δ 0 := by
    apply SimplexCategory.Hom.ext
    ext i
    fin_cases i <;> rfl
  rw [h]

/-- **`Sq¹a = a ⌣ a` pointwise on `1`-cocycles**: the 4-case parity check
`half(v̂₀ − v̂₁ + v̂₂) = v₂·v₀` under `v₀ + v₁ + v₂ = 0`. -/
theorem Sq1cochain_one_cocycle_eq_cup_self {X : TopCat}
    (a : SingularCochain X 1) (ha : coboundary X 1 a = 0)
    (σ : (TopCat.toSSet.obj X).obj (op (SimplexCategory.mk (1 + 1)))) :
    Sq1cochain a σ = cup a a σ := by
  have hc : a (face 0 σ) + a (face 1 σ) + a (face 2 σ) = 0 := by
    have h0 := congrFun ha σ
    rw [coboundary_apply, Fin.sum_univ_three] at h0
    exact h0
  rw [Sq1cochain_apply, coboundary4_apply, Fin.sum_univ_three, cup_apply,
    frontFace_one_one, backFace_one_one]
  simp only [lift_apply]
  revert hc
  generalize a (face 0 σ) = v₀
  generalize a (face 1 σ) = v₁
  generalize a (face 2 σ) = v₂
  revert v₀ v₁ v₂
  decide
