...
43. `6ed-honeycomb-phase-chart-gl2z-invariant` [refutation]
   The Bloch-phase honeycomb structure factor `f(theta) = 1 + exp(i theta_1) + exp(i theta_2)` is
   independent of WHICH primitive pair the phases are read against -- i.e. every theorem about it holds for
   ANY primitive-vector choice, so `theta_i = <k, a_i>` may be declared for an arbitrary primitive `(a_1,
   a_2)`. FALSE (Phase 6ED, 2026-07-29, kernel-checked; found by adversarial review of Waves 1-2, where this
   was the stated justification for a coordinate deviation). `f` is NOT GL_2(Z)-invariant. WITNESS:
   `structureFactor_zero_set_not_shear_invariant` exhibits a genuine Dirac point K at which the EQUALLY
   PRIMITIVE sheared pair `(a_1, a_1 + a_2)` gives `f != 0` -- so under that chart
   `structureFactor_eq_zero_iff` MISIDENTIFIES the Dirac set outright, reporting a genuine Dirac point as
   gapped. Numerically the sheared chart also disagrees on |f| (0.6468 vs 1.3115 at a sample momentum).
   SCOPE -- the theorems themselves are true; they are statements about `theta in R^2`. What is false is the
   IDENTIFICATION claim. `f` is the honeycomb structure factor precisely for the pair `(a_1, a_2) = (d_1 -
   d_3, d_2 - d_3)` built from the nearest-neighbour differences -- equal length, 60 degrees apart -- and
   for the six relabelings generated by `theta -> (theta_2, theta_1)` and `theta -> (-theta_1, theta_2 -
   theta_1)` (`structureFactor_norm_swap`, `_rotate`). THE REPAIR, and the real generality content the false
   claim was standing in for: the admissible chart is now a nameable hypothesis `IsHoneycombChart`, and
   `isHoneycombChart_of_neighbours` proves the honeycomb's own 120-degree bond geometry FORCES it -- so a
   consumer supplying genuine nearest-neighbour vectors gets the chart for free and never has to check it.
   CONSEQUENCE (Wave 2): 'the quadratic form `q1^2 - q1 q2 + q2^2` is isotropic in the physical frame' is
   likewise chart-specific -- true for the 60-degree pair, and a 120-degree pair sweeps a factor of 3 across
   directions. That claim is now the theorem `quadForm_of_chart`, stated against `IsHoneycombChart`, not
   prose.
...
/-- NO-GO [`6ed-honeycomb-phase-chart-gl2z-invariant`] — do NOT re-derive. FALSE: The Bloch-phase honeycomb structure factor `f(theta) = 1 + exp(i theta_1) + exp(i theta_2)` is independent of WHICH primitive pair the phases are read against -- i.e. Backing refutation: `SKEFTHawking.GrapheneBand.structureFactor_zero_set_not_shear_invariant`. -/
alias nogo_structureFactor_zero_set_not_shear_invariant := SKEFTHawking.GrapheneBand.structureFactor_zero_set_not_shear_invariant
