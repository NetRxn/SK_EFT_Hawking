...
namespace SKEFTHawking.RP4ProjectionFormula
...
theorem backFace_liftPlus_ne_backFace_liftMinus {p q : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP4)).obj (op (SimplexCategory.mk (p + q)))) :
  ...
  have h2 := congrArg ((TopCat.of S4).toSSetObjEquiv (op (SimplexCategory.mk q))) h
...
theorem backFace_lift_mem_pair {p q : ℕ}
    (σ : (TopCat.toSSet.obj (TopCat.of RP4)).obj (op (SimplexCategory.mk (p + q))))
    (τ : (TopCat.toSSet.obj (TopCat.of S4)).obj (op (SimplexCategory.mk (p + q))))
...
theorem backFace_pair_value {p q : ℕ} (y : SingularCochain (TopCat.of S4) q)
    (σ : (TopCat.toSSet.obj (TopCat.of RP4)).obj (op (SimplexCategory.mk (p + q)))) :
...
theorem cochainTransfer_cup_pullback {p q : ℕ}
    (a : SingularCochain (TopCat.of RP4) p) (y : SingularCochain (TopCat.of S4) q) :
...
end SKEFTHawking.RP4ProjectionFormula
