...
/-- **Is this declaration COMPILER-GENERATED rather than author-written?**

    Answered STRUCTURALLY, from Lean's own predicates, so consumers stop inferring it from
    the shape of a name. Downstream Python previously matched `.casesOn` / `.injEq` /
    `._eq_1` with a regex; every count that filtered on it inherited that guess.

    Sources, because no single predicate covers the family:
    * `Name.isInternalDetail` — `_`-prefixed parts, `eq_N`, `match_N`, `proof_N`, `omega_N`,
      numeric components (so `._eq_1`, `.proof_2`, `._sunfold`);
    * `isAuxRecursor` — the `@[auxRecursor]` family (`casesOn`, `recOn`, …);
    * `isNoConfusion` — `noConfusion` ONLY, see below;
    * an explicit suffix leg for the families the three env predicates miss.

    ⚠️ **THE SUFFIX LEG IS LOAD-BEARING; the docstring used to claim coverage this
    predicate did not have.** It said `isNoConfusion` covers "`noConfusion` /
    `noConfusionType`" — `Lean.isNoConfusion` returns **false** for `noConfusionType`.
    And `.injEq` was named in the motivation as one of the three families this field
    replaces the downstream regex for, while no leg matched it at all. Measured on the
    live extraction (2026-08-10), the field marked these COMPILER-GENERATED
    declarations as author-written:

        noConfusionType  498
        injEq (any)      417   (400 of them `.mk.injEq`)
        below              3
        brecOn             3
        UNION            921

    So the field billed as strictly better than the regex was LESS complete on two of
    the three families it cites, and any consumer switching to it inflated
    author-written counts by 921. `below`/`brecOn` are `@[auxRecursor]` for inductives
    but not for structures, which is why they leak here.

    Constructors, recursors and `Quot` never reach here: `shouldSkip` drops them earlier. -/
private def isCompilerSuffix (n : Name) : Bool :=
  match n with
  | .str _ s => s == "noConfusionType" || s == "injEq" || s == "below" || s == "brecOn"
  | _        => false

private def isAutogenerated (env : Environment) (n : Name) : Bool :=
  n.isInternalDetail || isAuxRecursor env n || isNoConfusion env n || isCompilerSuffix n
...
private def buildDeclJsonStr (env : Environment) (name : Name) (immDeps : Std.HashMap Name (Array Name))
  ...
  let json := Json.mkObj [
    ...
    -- Compiler-generated vs author-written, decided by Lean rather than by a downstream
    -- regex over the name. See `isAutogenerated`.
    ("autogen", Json.bool (isAutogenerated env name)),
