...
/-- **Is this declaration COMPILER-GENERATED rather than author-written?**
    ...
    Three sources, because no single predicate covers the family:
    * `Name.isInternalDetail` — `_`-prefixed parts, `eq_N`, `match_N`, `proof_N`, `omega_N`,
      numeric components (so `._eq_1`, `.proof_2`, `._sunfold`);
    * `isAuxRecursor` — the `@[auxRecursor]` family (`casesOn`, `recOn`, `brecOn`, `below`, …);
    * `isNoConfusion` — `noConfusion` / `noConfusionType`.
