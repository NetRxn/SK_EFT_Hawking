...
# Phase 6x Track T-B.5.2 substantive substrate — Read-Rezayi SU(2)_5 generator case analysis

For any `X ∈ 𝔰𝔲(2)` with `X ≠ 0`, at least one of `H_SU`, `T_RR5` is a
...
at phase `π/14` (level k=5) instead of `π/8`, REUSING the H_SU lemmas
from Clifford+T verbatim (since `σ_5_1 = H_SU` is alphabet-independent).

## Strategy

Same as Clifford+T:
  - **T_RR5's bad set**: `T_RR5 · X = X · T_RR5` iff `X = c · paulI_z`
    for some `c : ℝ`. Anti-commute case is trivial: `T_RR5` NEVER
    anti-commutes with any non-zero `X ∈ ts`.
  - **H_SU's coverage of T_RR5's bad set**: for `X = c · paulI_z` with
...
import SKEFTHawking.FKLW.ReadRezayiK5NonCommuting
...
/-! ## Sub-lemma 1: T_RR5 never anti-commutes with non-zero ts elements -/
theorem T_RR5_mat_never_anticommute_ts
    {X : Matrix (Fin 2) (Fin 2) ℂ}
    (_hX : X ∈ tracelessSkewHermitian (Fin 2))
    (hX_ne : X ≠ 0) :
    T_RR5_mat * X ≠ -(X * T_RR5_mat) := by
  intro h_anticom
  apply hX_ne
  have h_sum_zero : T_RR5_mat * X + X * T_RR5_mat = 0 := by
    rw [h_anticom]; exact neg_add_cancel _
  set α : ℂ := Complex.exp (-(Complex.I * (Real.pi : ℂ) / 14)) with hα_def
  set β : ℂ := Complex.exp (Complex.I * (Real.pi : ℂ) / 14) with hβ_def
  have hα_ne : α ≠ 0 := Complex.exp_ne_zero _
  have hβ_ne : β ≠ 0 := Complex.exp_ne_zero _
  -- α + β = 2·cos(π/14) ≠ 0.
  have hαβ_ne : α + β ≠ 0 := by
    have h_two_cos := Complex.two_cos ((Real.pi / 14 : ℝ) : ℂ)
    have h_eq_β : Complex.exp (((Real.pi / 14 : ℝ) : ℂ) * Complex.I) = β := by
      rw [hβ_def]; congr 1; push_cast; ring
    have h_eq_α : Complex.exp (-((Real.pi / 14 : ℝ) : ℂ) * Complex.I) = α := by
      rw [hα_def]; congr 1; push_cast; ring
    rw [h_eq_β, h_eq_α] at h_two_cos
    have h_sum_eq : α + β = 2 * Complex.cos ((Real.pi / 14 : ℝ) : ℂ) := by
      rw [add_comm, ← h_two_cos]
    rw [h_sum_eq, ← Complex.ofReal_cos]
    have h_cos_pos : (0 : ℝ) < Real.cos (Real.pi / 14) := by
      apply Real.cos_pos_of_mem_Ioo
      constructor
      · have h_pi := Real.pi_pos; linarith
      · have h_pi := Real.pi_pos; linarith
    have h_cos_ne : (Real.cos (Real.pi / 14) : ℂ) ≠ 0 := by
  ...
  have h_T00 : T_RR5_mat ⟨0, by decide⟩ ⟨0, by decide⟩ = α := rfl
  have h_T01 : T_RR5_mat ⟨0, by decide⟩ ⟨1, by decide⟩ = 0 := rfl
  have h_T10 : T_RR5_mat ⟨1, by decide⟩ ⟨0, by decide⟩ = 0 := rfl
  have h_T11 : T_RR5_mat ⟨1, by decide⟩ ⟨1, by decide⟩ = β := rfl
...
/-! ## Sub-lemma 2: T_RR5 commutator characterization -/
theorem T_RR5_mat_commute_ts_iff_paulI_z
    {X : Matrix (Fin 2) (Fin 2) ℂ}
    (hX : X ∈ tracelessSkewHermitian (Fin 2)) :
    T_RR5_mat * X = X * T_RR5_mat ↔
  ...
  have hαβ_distinct : α ≠ β := by
    intro h_eq
    apply exp_I_pi_14_ne_exp_neg_I_pi_14
  ...
  · have h_diff_zero : T_RR5_mat * X - X * T_RR5_mat = 0 := sub_eq_zero.mpr h_comm
    have h_X01_zero : X ⟨0, by decide⟩ ⟨1, by decide⟩ = 0 := by
  ...
  · rintro ⟨c, hc⟩
    ...
    ext i j
    fin_cases i <;> fin_cases j <;>
      simp [T_RR5_mat, SU2LieAlgebra.paulI_z, SKEFTHawking.σ_z,
...
with `X ≠ 0`, at least one of `H_SU_mat`, `T_RR5_mat` is a unitary that
...
theorem exists_readRezayiK5_generator_not_commute_not_anticommute
    {X : Matrix (Fin 2) (Fin 2) ℂ}
    (hX : X ∈ tracelessSkewHermitian (Fin 2))
    (hX_ne : X ≠ 0) :
    ∃ g : Matrix (Fin 2) (Fin 2) ℂ,
      (g = H_SU_mat ∨ g = T_RR5_mat) ∧
      g * X ≠ X * g ∧
      g * X ≠ -(X * g) := by
  by_cases h_comm_T : T_RR5_mat * X = X * T_RR5_mat
  · obtain ⟨c, h_X_eq⟩ := (T_RR5_mat_commute_ts_iff_paulI_z hX).mp h_comm_T
  ...
  · refine ⟨T_RR5_mat, Or.inr rfl, h_comm_T, ?_⟩
    exact T_RR5_mat_never_anticommute_ts hX hX_ne
