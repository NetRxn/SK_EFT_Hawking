...
= U`, with `synth U h := h.choose` extracting a witness by `Classical.choice`. This increment re-bases
it onto a *structural* channel-rep membership: every exactly-representable `U` has its channel rep `Û`
in the finitely-generated submonoid of `GL(64)` generated by the gates' channel reps — the concrete
`64 × 64` matrix monoid in which Mukhopadhyay's reduction (§3.4, §4.2) actually searches.

The Mukhopadhyay dossier (Q1.4) is explicit that the *converse* — a full decidable `⟺` membership over
the Clifford+CCZ set — is NOT available from arXiv:2401.08950 (Lemma 3.10 gives only the necessary
`ℤ[1/2]`-entry condition; the converse "every `ℤ[1/2]`-channel-rep unitary is exactly implementable" is
unproved in the source). So the honest re-base is this NECESSARY structural condition (a certificate:
`Û ∉ channelGenMonoid ⟹ U` is not exactly representable), together with the conjugation-action
characterization of `Û` (`channelRep_mulVec_repr`, L.A.2c). The dyadic-`ℤ[1/2]`-entry refinement
(Lemma 3.10, via the per-generator Theorem 3.8 entry analysis) and the full Fact 3.9 `⟺` (the converse
imported from Gosset–Kliuchnikov–Mosca–Russo, arXiv:1308.4134) are the deeper number-theoretic layer,
documented as an optional follow-on (consistent with the dossier's tractability verdict). -/

/-- **The channel-rep generating submonoid**: the submonoid of `64 × 64` matrices generated by the
channel reps `(gateMatrix g)̂` of the Clifford+CCZ gates. Mukhopadhyay's exact synthesis operates
entirely inside this submonoid (channel-rep land, §3.4 / §4.2). -/
noncomputable def channelGenMonoid :
    Submonoid (Matrix (Fin 4 × Fin 4 × Fin 4) (Fin 4 × Fin 4 × Fin 4) ℂ) :=
  Submonoid.closure (Set.range fun g : CliffordCCZGate => channelRep (gateMatrix g))

/-- Each gate's channel rep is a generator of `channelGenMonoid`. -/
theorem channelRep_gateMatrix_mem (g : CliffordCCZGate) :
    channelRep (gateMatrix g) ∈ channelGenMonoid :=
  Submonoid.subset_closure ⟨g, rfl⟩

/-- The channel rep of any gate word lies in `channelGenMonoid` (homomorphism + closure). -/
theorem channelRep_interp_mem (gs : List CliffordCCZGate) :
    channelRep (interp gs) ∈ channelGenMonoid := by
  induction gs with
  | nil => rw [interp_nil, channelRep_one]; exact Submonoid.one_mem _
  | cons g gs ih =>
    rw [interp_cons, channelRep_mul]
    exact Submonoid.mul_mem _ (channelRep_gateMatrix_mem g) ih

/-- **Structural re-base of `IsExactlyCliffordCCZ` onto the channel rep**: every exactly
Clifford+CCZ-representable unitary `U` has its channel representation `Û` in the finitely-generated
submonoid `channelGenMonoid` of `GL(64)` — the concrete matrix monoid Mukhopadhyay's reduction
searches. This replaces the trivial `∃-gate-word` predicate's opacity with a structural channel-rep
membership *necessary* condition / certificate (`Û ∉ channelGenMonoid ⟹ U` not exactly representable).
The converse `⟺` is not available from the source (dossier Q1.4); see the section docstring. -/
theorem isExactlyCliffordCCZ_channelRep_mem {U : Matrix (Fin 8) (Fin 8) ℂ}
    (hU : IsExactlyCliffordCCZ U) : channelRep U ∈ channelGenMonoid := by
  obtain ⟨gs, hgs⟩ := hU
  rw [← hgs]
  exact channelRep_interp_mem gs
