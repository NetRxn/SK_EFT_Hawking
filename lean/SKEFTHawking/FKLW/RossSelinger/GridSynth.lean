...
entries collapse to `normSq (mk u k) + normSq (mk t k) = 1` (via `normSq_mk` + `mk_add` +
`sqrt2_pow_two_mul`); the two off-diagonal entries vanish by commutativity (`ring`, since the
runtime `ZOmegaSqrt2` is a `CommRing`). With `det (assembleUnitary u t k) = 1` (next), `M ∈
...
theorem isUnitaryT_assembleUnitary (u t : ZOmega) (k : ℕ)
  ...
  have hcol : normSq (mk u k) + normSq (mk t k) = 1 := by
