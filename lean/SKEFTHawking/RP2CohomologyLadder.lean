...
namespace SKEFTHawking.RP2CohomologyLadder
...
/-- **`H¹(S²; ℤ/2) = 0`** — UC-duality against the degree-1 homological vanishing. (The surface
ladder needs only the degree-1 rung; `H²(S²) ≠ 0` is the top.) -/
theorem sphere_cohomology_eq_zero {k : ℕ} (h1 : 1 ≤ k) (h3 : k ≤ 1)
    (ω : Cohomology (TopCat.of S2) k) : ω = 0 := by
  obtain ⟨N, rfl⟩ : ∃ N, k = N + 1 := ⟨k - 1, by omega⟩
  apply (ucDualEquiv (TopCat.of S2) N).injective
  rw [map_zero]
  apply LinearMap.ext
  intro x
  rw [SKEFTHawking.RP2SphereHomology.sphere_homology_eq_zero (N + 1) h1 h3 x, map_zero,
    LinearMap.zero_apply]

/-! ## §3. The ladder start: `τ^* = 0` on `H⁰` (M3-f) -/

/-- Path-connectedness of `S2` (the sphere in `ℝ³`, dimension `≥ 2`). -/
noncomputable instance : PathConnectedSpace S2 := by
  have h : IsPathConnected (Metric.sphere (0 : EuclideanSpace ℝ (Fin 3)) 1) :=
...
theorem cochainTransfer_zero_of_cocycle
    (y : SingularCochain (TopCat.of S2) 0)
    (hy : coboundaryₗ (TopCat.of S2) 0 y = 0) :
    cochainTransfer 0 y = 0 := by
  funext σ
  show y (liftPlus σ) + y (liftMinus σ) = (0 : SingularCochain (TopCat.of RP2) 0) σ
...
theorem cohomologyTransfer_zero_eq_zero (y : Cohomology (TopCat.of S2) 0) :
    cohomologyTransfer 0 y = 0 := by
  obtain ⟨y, rfl⟩ := Submodule.Quotient.mk_surjective _ y
  show cohomologyTransfer 0 (Cohomology.mk (TopCat.of S2) 0 y) = 0
...
/-! ## §4. The ladder: `δSᵏ(1) ≠ 0` (`k ≤ 2`) and `Hᵏ(ℝP²) = span{δSᵏ(1)}` (`k ≤ 1`) -/

/-- `ℝP²` is path-connected — the continuous image of the path-connected `S²`. -/
noncomputable instance : PathConnectedSpace RP2 := by
  rw [pathConnectedSpace_iff_univ]
  have h2 : ⇑mkC '' Set.univ = (Set.univ : Set RP2) := by
...
/-- **`δS` is injective in degrees `k ≤ 1`**: `ker δS = im τ^*`, and `τ^*` vanishes — at `k = 0`
by constancy, at `k = 1` because its domain `H¹(S²)` is zero. -/
theorem smithCoConnecting_injective {k : ℕ} (hk : k ≤ 1) :
...
/-- **`δS` is surjective onto degrees `k + 1 ≤ 1`**: `im δS = ker π^*`, and `π^*` lands in the
vanishing `Hᵏ⁺¹(S²)`. -/
theorem smithCoConnecting_surjective {k : ℕ} (h1 : k + 1 ≤ 1) :
...
noncomputable def xpow : (k : ℕ) → Cohomology (TopCat.of RP2) k
  | 0 => unitClass (TopCat.of RP2)
  | k + 1 => smithCoConnecting k (xpow k)

@[simp] theorem xpow_succ (k : ℕ) : xpow (k + 1) = smithCoConnecting k (xpow k) := rfl

/-- **The ladder classes are nonzero through degree 2** — `δS`-injectivity walks the
nonvanishing up from the unit. -/
theorem xpow_ne_zero {k : ℕ} (hk : k ≤ 2) : xpow k ≠ 0 := by
  induction k with
...
/-- **`Hᵏ(ℝP²; ℤ/2) = span{δSᵏ(1)}` for `k ≤ 1`** — surjectivity transports the degree-0 span
up the ladder. With `xpow_ne_zero`, `H⁰` and `H¹` are each one-dimensional with basis the
ladder class; in particular `H¹(ℝP²; ℤ/2) ≅ ℤ/2` with generator `xpow 1`. -/
theorem cohomology_eq_smul_xpow : ∀ {k : ℕ}, k ≤ 1 → ∀ (w : Cohomology (TopCat.of RP2) k),
...
end SKEFTHawking.RP2CohomologyLadder
