/-
# Phase 5q.H (E1 CSC-PD tower) — the assembled integral σ÷16 leg, resting on `hcoreG` + orientation (integral)

The crispest end-to-end statement of the integral Poincaré-duality → `16 ∣ σ` leg: feeding the
openDuality-univ (2,1)-window bijectivity (`openDuality_univ_bij_of_hcoreGInt`, from `pdWindowPInt_univ` with
`hproj`/`hfree`/`hbaseConvG` all discharged) into the assembled cap-equiv → unimodular-intersection-form →
`sixteen_dvd_latticeSig_of_capEquiv` chain. The whole leg now rests on EXACTLY:
* `hcoreG` — the torsion-safe MV connecting core (brick 6b, the one remaining PD input);
* the orientation input `hcyc` + `hloc` (the fundamental cycle restricts to the local generator);
* the finite data: the `H₂`-free Kronecker pairing (`kron`/`hkron`) + its basis `B`, the spin-Wu even-form
  datum `D`, and the topological-Rokhlin factor `htopo` (`2 ∣ σ/8`, = E2's Guillou–Marin content).

Kernel-pure (`{propext, Classical.choice, Quot.sound}`); no `sorry`/`native_decide`/`maxHeartbeats`/axiom.
-/
import Mathlib
import SKEFTHawking.SingularOpenDualityUnivBijInt
import SKEFTHawking.SingularIntCapEquivAssembly

open SKEFTHawking.SingularHomologyInt
open SKEFTHawking.SingularCohomologyInt
open SKEFTHawking.SingularRelHomologyInt
open SKEFTHawking.SingularEuclideanCapIsoInt
open SKEFTHawking.SingularOpenDualityMVConnSquareInt (castChainInt chainBoundary_castChainInt_eq_zero)
open SKEFTHawking.SingularPDWindowInt (HcoreG)
open SKEFTHawking.SingularIntCapEquivAssembly (sixteen_dvd_latticeSig_of_capEquiv)
open SKEFTHawking.SingularOpenDualityUnivBijInt (openDuality_univ_bij_of_hcoreGInt)

namespace SKEFTHawking.SingularSixteenDvdLegInt

/-- **The assembled integral σ÷16 leg** — `16 ∣ σ` for a closed charted 4-manifold, resting on EXACTLY the
torsion-safe connecting core `hcoreG` (brick 6b) + the orientation input (`hcyc`/`hloc`) + the finite
Kronecker/basis/spin-Wu/topological-Rokhlin data. `hproj`/`hfree`/`hbaseConvG` are all discharged; the
openDuality-univ bijectivity threads from `pdWindowPInt_univ` via `openDuality_univ_bij_of_hcoreGInt`. -/
theorem sixteen_dvd_latticeSig_of_hcoreGInt {M : Type} [TopologicalSpace M] [T2Space M] [CompactSpace M]
    [ChartedSpace (EuclideanSpace ℝ (Fin 4)) M]
    (zM : SingularChainInt (TopCat.of M) (1 + 0 + 3))
    (hzM : chainBoundary (TopCat.of M) (1 + 0 + 2) zM = 0)
    (hcoreG : HcoreG M zM hzM)
    (hcyc : castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 0 + 1 by omega) zM
      ∈ cycles (TopCat.of M) (2 + 2))
    (hloc : ∀ x : M, SKEFTHawking.IntOrientationSection.restrictHomologyToPointInt
        (X := TopCat.of M) x (2 + 2)
        (Homology.mk (TopCat.of M) (2 + 2)
          ⟨castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 0 + 1 by omega) zM, hcyc⟩)
      = (SKEFTHawking.SingularBaseCaseD0Int.localIsoComplInt x).symm 1)
    (kron : Homology (TopCat.of M) 2 ≃ₗ[ℤ] Module.Dual ℤ (Cohomology (TopCat.of M) 2))
    (hkron : ∀ (h : Homology (TopCat.of M) 2) (b : Cohomology (TopCat.of M) 2),
      kron h b = kroneckerHInt 2 b h)
    (B : IntH2Basis (TopCat.of M))
    (D : SpinWuDatum (intFundamentalClassOfHomology (Homology.mk (TopCat.of M) (2 + 1 + 1)
        ⟨castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 1 by omega) zM,
          chainBoundary_castChainInt_eq_zero (b := 2 + 1) (by omega) (by omega) zM hzM⟩)))
    (htopo : (2 : ℤ) ∣ latticeSig (interMatrix (intFundamentalClassOfHomology
        (Homology.mk (TopCat.of M) (2 + 1 + 1)
          ⟨castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 1 by omega) zM,
            chainBoundary_castChainInt_eq_zero (b := 2 + 1) (by omega) (by omega) zM hzM⟩)) B) / 8) :
    (16 : ℤ) ∣ latticeSig (interMatrix (intFundamentalClassOfHomology
        (Homology.mk (TopCat.of M) (2 + 1 + 1)
          ⟨castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 1 by omega) zM,
            chainBoundary_castChainInt_eq_zero (b := 2 + 1) (by omega) (by omega) zM hzM⟩)) B) :=
  sixteen_dvd_latticeSig_of_capEquiv
    (isOpen_univ : IsOpen (Set.univ : Set ↑(TopCat.of M)))
    (castChainInt (show (1 : ℕ) + 0 + 3 = 2 + 1 + 1 by omega) zM)
    (chainBoundary_castChainInt_eq_zero (b := 2 + 1) (by omega) (by omega) zM hzM)
    (openDuality_univ_bij_of_hcoreGInt zM hzM hcoreG hcyc hloc) kron hkron B D htopo

end SKEFTHawking.SingularSixteenDvdLegInt
