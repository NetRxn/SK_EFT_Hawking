...
namespace SKEFTHawking.RP4Covering
...
instance : IsCancelSMul ℤˣ S4 where
  ...
  right_cancel' u v x h := by
    have hcoe := congrArg Subtype.val h
    simp only [smul_coe] at hcoe
    have hx : (x : EuclideanSpace ℝ (Fin (4 + 1))) ≠ 0 := by
      intro h0
...
theorem rp4_isCoveringMap :
    IsCoveringMap (Quotient.mk (MulAction.orbitRel ℤˣ S4)) :=
  (isQuotientCoveringMap_quotientMk_of_properlyDiscontinuousSMul (G := ℤˣ)
    (E := S4)).isCoveringMap

end SKEFTHawking.RP4Covering
