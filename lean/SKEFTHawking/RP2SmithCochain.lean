...
open SKEFTHawking.RP2PointSet SKEFTHawking.RP2Covering SKEFTHawking.RP2Transfer
open SKEFTHawking.SingularHomologyMod2 SKEFTHawking.SingularCohomologyMod2
open SKEFTHawking.SingularFunctoriality

namespace SKEFTHawking.RP2SmithCochain
...
noncomputable def cochainTransfer (n : ℕ) :
    SingularCochain (TopCat.of S2) n →ₗ[ZMod 2] SingularCochain (TopCat.of RP2) n where
...
@[simp] theorem cochainTransfer_apply {n : ℕ} (y : SingularCochain (TopCat.of S2) n)
...
theorem face_pair_value {n : ℕ} (y : SingularCochain (TopCat.of S2) n)
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk (n + 1))))
    (i : Fin (n + 2)) :
...
theorem coboundary_cochainTransfer {n : ℕ} (y : SingularCochain (TopCat.of S2) n) :
    coboundary (TopCat.of RP2) n (cochainTransfer n y)
      = cochainTransfer (n + 1) (coboundary (TopCat.of S2) n y) := by
  funext σ
...
theorem cochainTransfer_mem_ker {n : ℕ} (y : LinearMap.ker (coboundaryₗ (TopCat.of S2) n)) :
    cochainTransfer n y.1 ∈ LinearMap.ker (coboundaryₗ (TopCat.of RP2) n) := by
  rw [LinearMap.mem_ker]
  show coboundary (TopCat.of RP2) n (cochainTransfer n y.1) = 0
  rw [coboundary_cochainTransfer,
    show coboundary (TopCat.of S2) n y.1 = coboundaryₗ (TopCat.of S2) n y.1 from rfl,
...
noncomputable def cohomologyTransfer (n : ℕ) :
    Cohomology (TopCat.of S2) n →ₗ[ZMod 2] Cohomology (TopCat.of RP2) n :=
  Submodule.liftQ _
    ((Submodule.mkQ _).comp
      (((cochainTransfer n).domRestrict (LinearMap.ker (coboundaryₗ (TopCat.of S2) n))).codRestrict
        (LinearMap.ker (coboundaryₗ (TopCat.of RP2) n)) fun y => cochainTransfer_mem_ker y))
    (by
      intro y hy
      ...
      cases n with
      | zero =>
          have hy0 : (y.1 : SingularCochain (TopCat.of S2) 0) = 0 := by
            have h0 := hy
            rwa [show coboundaryRange (TopCat.of S2) 0 = (⊥ : Submodule (ZMod 2) _) from rfl,
              Submodule.mem_bot] at h0
          rw [show coboundaryRange (TopCat.of RP2) 0 = (⊥ : Submodule (ZMod 2) _) from rfl,
            Submodule.mem_bot, hy0, map_zero]
      | succ m =>
          obtain ⟨b, hb⟩ := hy
          refine ⟨cochainTransfer m b, ?_⟩
          show coboundaryₗ (TopCat.of RP2) m (cochainTransfer m b) = cochainTransfer (m + 1) y.1
          rw [show coboundaryₗ (TopCat.of RP2) m (cochainTransfer m b)
              = coboundary (TopCat.of RP2) m (cochainTransfer m b) from rfl,
            coboundary_cochainTransfer]
          rw [show coboundary (TopCat.of S2) m b = coboundaryₗ (TopCat.of S2) m b from rfl, hb])

/-- The computation rule for `cohomologyTransfer` on a representative cocycle. -/
@[simp] theorem cohomologyTransfer_mk {n : ℕ}
    (y : LinearMap.ker (coboundaryₗ (TopCat.of S2) n)) :
    cohomologyTransfer n (Cohomology.mk (TopCat.of S2) n y)
      = Cohomology.mk (TopCat.of RP2) n ⟨cochainTransfer n y.1, cochainTransfer_mem_ker y⟩ :=
...
theorem cochainPullback_injective (n : ℕ) :
    Function.Injective (cochainPullback (X := TopCat.of S2) (Y := TopCat.of RP2) mkC n) := by
  intro a b h
  funext σ
...
theorem cochainTransfer_cochainPullback (n : ℕ) (a : SingularCochain (TopCat.of RP2) n) :
  ...
  show cochainPullback mkC n a (liftPlus σ) + cochainPullback mkC n a (liftMinus σ)
    = (0 : SingularCochain (TopCat.of RP2) n) σ
...
@[simp] theorem plusValues_apply {n : ℕ} (y : SingularCochain (TopCat.of S2) n)
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
    plusValues n y σ = y (liftPlus σ) := rfl

/-- Plus-values inverts the pullback: `plusValues (π^# a) = a`. -/
@[simp] theorem plusValues_cochainPullback {n : ℕ} (a : SingularCochain (TopCat.of RP2) n) :
...
theorem cochainPullback_plusValues {n : ℕ}
    (y : SingularCochain (TopCat.of S2) n) (hy : cochainTransfer n y = 0) :
...
noncomputable def cochainSection (n : ℕ) :
    SingularCochain (TopCat.of RP2) n →ₗ[ZMod 2] SingularCochain (TopCat.of S2) n where
  toFun g := fun σ' =>
...
open Classical in
@[simp] theorem cochainSection_apply_plus {n : ℕ} (g : SingularCochain (TopCat.of RP2) n)
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
...
open Classical in
@[simp] theorem cochainSection_apply_minus {n : ℕ} (g : SingularCochain (TopCat.of RP2) n)
    (σ : (TopCat.toSSet.obj (TopCat.of RP2)).obj (op (SimplexCategory.mk n))) :
...
theorem cochainTransfer_cochainSection {n : ℕ} (g : SingularCochain (TopCat.of RP2) n) :
...
noncomputable def connectingCochain (n : ℕ) :
    SingularCochain (TopCat.of RP2) n →ₗ[ZMod 2] SingularCochain (TopCat.of RP2) (n + 1) :=
  (plusValues (n + 1)) ∘ₗ (coboundaryₗ (TopCat.of S2) n) ∘ₗ (cochainSection n)
...
theorem cochainTransfer_coboundary_cochainSection {n : ℕ}
    (g : SingularCochain (TopCat.of RP2) n) (hg : coboundaryₗ (TopCat.of RP2) n g = 0) :
    cochainTransfer (n + 1) (coboundaryₗ (TopCat.of S2) n (cochainSection n g)) = 0 := by
  rw [show (coboundaryₗ (TopCat.of S2) n) (cochainSection n g)
      = coboundary (TopCat.of S2) n (cochainSection n g) from rfl,
...
theorem cochainPullback_connectingCochain {n : ℕ}
    ...
    cochainPullback mkC (n + 1) (connectingCochain n g)
      = coboundaryₗ (TopCat.of S2) n (cochainSection n g) :=
...
theorem connectingCochain_mem_ker {n : ℕ}
    (g : SingularCochain (TopCat.of RP2) n) (hg : coboundaryₗ (TopCat.of RP2) n g = 0) :
    coboundaryₗ (TopCat.of RP2) (n + 1) (connectingCochain n g) = 0 := by
  apply cochainPullback_injective (n + 1 + 1)
  rw [map_zero,
    show cochainPullback mkC (n + 1 + 1)
        (coboundaryₗ (TopCat.of RP2) (n + 1) (connectingCochain n g))
      = coboundary (TopCat.of S2) (n + 1)
  ...
  exact coboundary_comp_coboundary (TopCat.of S2) n _
...
theorem connectingCochain_coboundary {n : ℕ} (b : SingularCochain (TopCat.of RP2) n) :
    ∃ c, coboundaryₗ (TopCat.of RP2) (n + 1) c
      = connectingCochain (n + 1) (coboundaryₗ (TopCat.of RP2) n b) := by
  set u := cochainSection (n + 1) (coboundaryₗ (TopCat.of RP2) n b)
    + coboundaryₗ (TopCat.of S2) n (cochainSection n b) with hu
  have hker : cochainTransfer (n + 1) u = 0 := by
    rw [hu, map_add, cochainTransfer_cochainSection,
      show (coboundaryₗ (TopCat.of S2) n) (cochainSection n b)
        = coboundary (TopCat.of S2) n (cochainSection n b) from rfl,
      ← coboundary_cochainTransfer, cochainTransfer_cochainSection]
    show coboundaryₗ (TopCat.of RP2) n b + coboundaryₗ (TopCat.of RP2) n b = 0
    rw [← two_smul (ZMod 2), show (2 : ZMod 2) = 0 by decide, zero_smul]
  refine ⟨plusValues (n + 1) u, ?_⟩
  apply cochainPullback_injective (n + 1 + 1)
  rw [show cochainPullback mkC (n + 1 + 1)
        (coboundaryₗ (TopCat.of RP2) (n + 1) (plusValues (n + 1) u))
      = coboundary (TopCat.of S2) (n + 1) (cochainPullback mkC (n + 1) (plusValues (n + 1) u))
      from (coboundary_cochainPullback mkC (n + 1) (plusValues (n + 1) u)).symm,
    cochainPullback_plusValues u hker,
    cochainPullback_connectingCochain (coboundaryₗ (TopCat.of RP2) n b)
      (coboundary_comp_coboundary (TopCat.of RP2) n b),
    hu]
  show coboundaryₗ (TopCat.of S2) (n + 1)
      (cochainSection (n + 1) (coboundaryₗ (TopCat.of RP2) n b)
        + coboundaryₗ (TopCat.of S2) n (cochainSection n b))
    = coboundaryₗ (TopCat.of S2) (n + 1)
        (cochainSection (n + 1) (coboundaryₗ (TopCat.of RP2) n b))
  rw [map_add,
    show (coboundaryₗ (TopCat.of S2) (n + 1))
        ((coboundaryₗ (TopCat.of S2) n) (cochainSection n b))
      = coboundary (TopCat.of S2) (n + 1) (coboundary (TopCat.of S2) n (cochainSection n b))
      from rfl,
    coboundary_comp_coboundary (TopCat.of S2) n, add_zero]
...
noncomputable def smithCoConnecting (n : ℕ) :
    Cohomology (TopCat.of RP2) n →ₗ[ZMod 2] Cohomology (TopCat.of RP2) (n + 1) :=
  Submodule.liftQ _
    ((Submodule.mkQ _).comp
      (((connectingCochain n).domRestrict
          (LinearMap.ker (coboundaryₗ (TopCat.of RP2) n))).codRestrict
    ...
    (by
      ...
      cases n with
      | zero =>
          have hg0 : (g.1 : SingularCochain (TopCat.of RP2) 0) = 0 := by
            have h0 := hg
            rwa [show coboundaryRange (TopCat.of RP2) 0 = (⊥ : Submodule (ZMod 2) _) from rfl,
              Submodule.mem_bot] at h0
...
theorem smithCoConnecting_cohomologyTransfer {n : ℕ} (y : Cohomology (TopCat.of S2) n) :
    smithCoConnecting n (cohomologyTransfer n y) = 0 := by
  obtain ⟨y, rfl⟩ := Submodule.Quotient.mk_surjective _ y
  show smithCoConnecting n (cohomologyTransfer n (Cohomology.mk (TopCat.of S2) n y)) = 0
  ...
  show coboundaryₗ (TopCat.of RP2) n (plusValues n u) = connectingCochain n (cochainTransfer n y.1)
  apply cochainPullback_injective (n + 1)
  rw [show cochainPullback mkC (n + 1) (coboundaryₗ (TopCat.of RP2) n (plusValues n u))
      = coboundary (TopCat.of S2) n (cochainPullback mkC n (plusValues n u)) from
  ...
  show coboundaryₗ (TopCat.of S2) n (cochainSection n (cochainTransfer n y.1) + y.1)
    = coboundaryₗ (TopCat.of S2) n (cochainSection n (cochainTransfer n y.1))
...
theorem exact_cohomologyTransfer_smithCoConnecting (n : ℕ) :
  ...
  constructor
  · intro h
    have hb : connectingCochain n g.1 ∈ coboundaryRange (TopCat.of RP2) (n + 1) := by
      have h2 : smithCoConnecting n (Cohomology.mk (TopCat.of RP2) n g) = 0 := h
      rw [smithCoConnecting_mk, cmk_eq_zero_iff] at h2
      exact h2
    obtain ⟨c, hc⟩ := hb
    have hycyc : cochainSection n g.1 + cochainPullback mkC n c
        ∈ LinearMap.ker (coboundaryₗ (TopCat.of S2) n) := by
      rw [LinearMap.mem_ker, map_add,
        show (coboundaryₗ (TopCat.of S2) n) (cochainPullback mkC n c)
          = cochainPullback mkC (n + 1) (coboundaryₗ (TopCat.of RP2) n c) from
          coboundary_cochainPullback mkC n c,
        hc,
        show (coboundaryₗ (TopCat.of S2) n) (cochainSection n g.1)
          = cochainPullback mkC (n + 1) (connectingCochain n g.1) from
          (cochainPullback_connectingCochain g.1 (LinearMap.mem_ker.mp g.2)).symm,
        ← two_smul (ZMod 2), show (2 : ZMod 2) = 0 by decide, zero_smul]
    refine ⟨Cohomology.mk (TopCat.of S2) n ⟨_, hycyc⟩, ?_⟩
    rw [cohomologyTransfer_mk]
    refine congrArg (Cohomology.mk (TopCat.of RP2) n) (Subtype.ext ?_)
...
theorem cohomologyPullback_smithCoConnecting {n : ℕ} (w : Cohomology (TopCat.of RP2) n) :
    cohomologyPullback mkC (n + 1) (smithCoConnecting n w) = 0 := by
  obtain ⟨g, rfl⟩ := Submodule.Quotient.mk_surjective _ w
  show cohomologyPullback mkC (n + 1)
    (smithCoConnecting n (Cohomology.mk (TopCat.of RP2) n g)) = 0
...
theorem exact_smithCoConnecting_cohomologyPullback (n : ℕ) :
  ...
  constructor
  · intro h
    have hb : cochainPullback mkC (n + 1) x.1 ∈ coboundaryRange (TopCat.of S2) (n + 1) := by
      have h2 : cohomologyPullback mkC (n + 1)
          (Cohomology.mk (TopCat.of RP2) (n + 1) x) = 0 := h
      rw [cohomologyPullback_mk, cmk_eq_zero_iff] at h2
      exact h2
    obtain ⟨w, hw⟩ := hb
    have hgcyc : cochainTransfer n w ∈ LinearMap.ker (coboundaryₗ (TopCat.of RP2) n) := by
      rw [LinearMap.mem_ker,
        show (coboundaryₗ (TopCat.of RP2) n) (cochainTransfer n w)
          = coboundary (TopCat.of RP2) n (cochainTransfer n w) from rfl,
        coboundary_cochainTransfer,
        show coboundary (TopCat.of S2) n w = coboundaryₗ (TopCat.of S2) n w from rfl,
        hw, cochainTransfer_cochainPullback]
    refine ⟨Cohomology.mk (TopCat.of RP2) n ⟨_, hgcyc⟩, ?_⟩
    ...
    have hker : cochainTransfer n u = 0 := by
      rw [hu, map_add, cochainTransfer_cochainSection, ← two_smul (ZMod 2),
        show (2 : ZMod 2) = 0 by decide, zero_smul]
    refine ⟨plusValues n u, ?_⟩
    show coboundaryₗ (TopCat.of RP2) n (plusValues n u)
      = connectingCochain n (cochainTransfer n w) - x.1
    apply cochainPullback_injective (n + 1)
    rw [map_sub,
      show cochainPullback mkC (n + 1) (coboundaryₗ (TopCat.of RP2) n (plusValues n u))
        = coboundary (TopCat.of S2) n (cochainPullback mkC n (plusValues n u)) from
        (coboundary_cochainPullback mkC n (plusValues n u)).symm,
      cochainPullback_plusValues u hker,
      cochainPullback_connectingCochain (cochainTransfer n w)
        (LinearMap.mem_ker.mp hgcyc), hu, ← hw]
    show coboundaryₗ (TopCat.of S2) n (cochainSection n (cochainTransfer n w) + w)
      = coboundaryₗ (TopCat.of S2) n (cochainSection n (cochainTransfer n w))
        - coboundaryₗ (TopCat.of S2) n w
    rw [map_add, sub_eq_add']
  · rintro ⟨g, hg⟩
    rw [← hg]
    exact cohomologyPullback_smithCoConnecting g

end SKEFTHawking.RP2SmithCochain
