...
theorem exists_unitary_traceNorm_eq_re_trace {M : Matrix ι ι ℂ} (hM : IsUnit M) :
  ...
  · -- v4.32: `congr 1` under `Complex.re` blows the whnf budget here. Rewrite `W * M = |M|` as a
    -- matrix identity first, so no congruence closure is needed.
    have hWM : P⁻¹ * Mᴴ * M = P := by
      rw [Matrix.mul_assoc, ← hPP, ← Matrix.mul_assoc, hPiP, Matrix.one_mul]
    rw [traceNorm_eq_trace_absOp, hWM, ← hPdef]
