...
SORRY_GAPS: list[SorryGap] = [
    ...
    SorryGap(module="SKEFTHawking.ChiralityWall", name="tpf_evades_at_least_two",
             priority=1, description="TPF evasion margin quantified (renamed from tpf_evasion_margin in Phase 5v Wave 1a)",
             strategy_hint="arithmetic", filled=True),
    ...
    SorryGap(module="SKEFTHawking.GloriosoLiu.Axioms",
             ...
             filled=True),  # Phase 6n Session 5 (2026-05-04): Stage 2-3 + 2-3b SUBSTANTIVE shipped. Stage 2-3 (initial commit `1b87c4a`): predicates invoke real SKDoubling preds; zero-action witness via kmsForZeroAction + all-zero FirstOrderCoeffs. Stage 2-3b (KMS framework finding `temporary/working-docs/phase6n/6n_gamma_kms_framework_finding.md`): hasDynamicalKMS re-defined to algebraic-FDR form (Nonempty (KMSSymmetry action β) is generically vacuous for non-trivial dissipative actions; replaced with ∃ c : FirstOrderCoeffs with action.lagrangian-match AND FirstOrderKMS c β). Companion theorem SKEFTAxioms_for_dissipative ships as the non-trivial existence witness (any DissipativeCoeffs with γ_1, γ_2 ≥ 0 maps to a valid GL skeleton via the inverted firstOrder_uniqueness coefficient mapping). Renamed from SKEFTAxioms_well_posed.
    ...
    SorryGap(module="SKEFTHawking.GloriosoLiu.Phase1Reconciliation",
             ...
             priority=2,
             description="Phase 1 4-of-9 Aristotle partition (run 270e77a0; counterexample c=(0,0,0,0,0,0,0,1,0)) "
                         "recovered as theorem of first-order projection. LOAD-BEARING for I1 §3 prose update "
