...
ARISTOTLE_THEOREMS = {
    ...
    # ⚠️ PROVENANCE NOTE (2026-08-13, statement-substance review I1 §4): the run below
    #    proved the ORIGINAL `tpf_violates_C2` / `tpf_outside_gs_scope` statements, which
    #    were `∃ N, N > c.local_dim` shapes whose `tpf` binder was decorative. Both have
    #    since been RESTATED against `TPFConstruction.LocalSpace` and re-proved MANUALLY
    #    in Lean (kernel-pure: propext, Classical.choice, Quot.sound). The run IDs are
    #    therefore HISTORICAL for these two names, not a machine check of the current
    #    statements. Lead decision required: re-submit the new statements to Aristotle,
    #    or move these two out of the machine-verified count. `c1_implies_i2` is unchanged.
...
EXISTENTIAL_WITNESS_REGISTRY: dict[str, dict[str, str]] = {
    ...
    # ── REMEDIATED 2026-08-13 (statement-substance review I1 §4). All three were
    #    escapes whose conclusion contained no project symbol. Each has been RESTATED
    #    against the object it is named for and is no longer existential, so each
    #    leaves this check's population once `lean_deps.json` is regenerated. The
    #    entries are retained (not deleted) so the gate is green against BOTH the
    #    pre- and post-regeneration `lean_deps.json`; delete them at the next sweep. ──
    'onsager_infinite_dimensional': {
        'status': 'anchored',
        'witness': 'NO LONGER EXISTENTIAL. Now `¬ FiniteDimensional ℂ onsagerAlgebra`, '
                   'where `onsagerAlgebra` is the Lie subalgebra of `L(gl₂) = gl₂ ⊗ '
                   'ℂ[t,t⁻¹]` generated by the Davies generators built in '
                   'OnsagerAlgebra.lean §4b (`onsagerA`, `onsagerG`, whose Davies '
                   'relations are proved, not assumed). Was `∀ n, ∃ m, m > n` — the '
                   'Archimedean property of ℕ, with no project symbol.',
        'claims': 'the Onsager algebra is infinite-dimensional',
        'proves': 'exactly that, of a constructed algebra: the (0,1)-entry map carries '
                  'it onto all of ℂ[t,t⁻¹], which is free on a ℤ-indexed basis',
    },
    'tpf_violates_C2': {
        'status': 'anchored',
        'witness': 'NO LONGER EXISTENTIAL. Now `IsEmpty (tpf.LocalSpace ≃ₗ[ℂ] '
                   'FermionicFockSpace c.n_modes)` — both binders appear. '
                   '`TPFConstruction` now carries its local Hilbert space as a field '
                   'with `local_infinite_dim`, witnessed by `tpfRotor` on ℓ²(ℤ). Was '
                   '`∃ N, N > c.local_dim`, in which `tpf` was decorative.',
        'claims': 'the TPF construction violates condition C2',
        'proves': 'that, via the finite-dimensionality of the fermionic Fock space',
    },
    'tpf_outside_gs_scope': {
        'status': 'anchored',
        'witness': 'NO LONGER EXISTENTIAL. Now `¬ I3_finite_dim_local tpf.LocalSpace ∧ '
                   'IsEmpty (tpf.LocalSpace ≃ₗ[ℂ] FermionicFockSpace h.c2.n_modes)` — '
                   'stated against the bundle `h` and the construction `tpf`. Was '
                   '`∃ k, k > h.local_dim`, in which `tpf` was decorative.',
        'claims': 'the TPF construction lies outside Golterman-Shamir scope',
        'proves': 'that, by failing I3 and C2 of any bundle simultaneously',
    },
...
KERNEL_NOGO_REGISTRY: dict[str, dict] = {
    'fk-mod2-membrane-index-cannot-reach-nonorientable-stratum': {
        ...
        'false_statement': 'The Freedman-Kirby MOD-2 membrane index D.F + O(D) + d(C) -- blueprint Route A nodes [G2]/[Q1]/[Q2], FK Lemma 2.6.1 -- produces the pin-minus class of a NONORIENTABLE characteristic surface, so building out the membrane layer suffices to supply `hgm` on the whole char-pair carrier. FALSE (2026-07-28, wt2, lead-verified: I re-derived the polarization step and checked the RP2 instantiation myself). Any mu : (iota -> ZMod 2) -> ZMod 2 satisfying the [Q2] refinement identity mu (x+y) = mu x + mu y + B x y FORCES B v v = 0 for every v: put x = y = v, use mu 0 = 0 and 2 * mu v = 0 in ZMod 2. So a mod-2 index can only refine an ALTERNATING form, i.e. it reaches the ORIENTABLE stratum and kernel-checkably only that one. Instantiated at stdQuadratic 1 -- the RP2 surface half of the (RP4, RP2) generator, where B 1 1 = 1 -- `no_mod2_index_on_rp2` is a flat refutation by `decide`. NOT FIXABLE BY MORE MEMBRANE WORK: the obstruction is the polarization identity itself, not a missing lemma or a weak construction. CONSEQUENCE FOR ROUTE A: reaching the nonorientable stratum requires the Guillou-Marin Z/4-RESOLUTION index. ** RESOLVED 2026-07-28 (3e15199c): the split is now PINNED from the primary source, read directly by the lead -- Klug arXiv:2011.12418v3 SECTION 6 p. 18, following Matsumoto [Mat86] and Guillou-Marin [GM77]. NOTE the earlier version of this entry cited section 2; that was WRONG -- section 2 is the ORIENTABLE mod-2 case and Klug states the orientability assumption there explicitly. The split is e_F(x) = n(D) + 2(D.F) + 2 d(C) (mod 4), coefficients 1,2,2, with the half-integrality living in n(D) -- the framing, counting right-handed HALF-twists under the convention that (r_D, C, e1, e2) agrees with the ambient orientation of X -- and NOT in a doubled D.F, so the shape this entry warned against guessing was indeed the wrong one. Built as a construction in GMZ4MembraneIndex.lean; corpus note at Lit-Search/Phase-5qH/Klug-GuillouMarin-Z4-membrane-index-coefficient-split.md. THIS REFUTATION IS UNAFFECTED: the mod-2 index still cannot reach the nonorientable stratum, and `GMZ4MembraneIndex.orientable_framing_forces_alternating` re-derives exactly this obstruction whenever the framing is a whole-twist count -- the kernel-checked statement that the half-twists are the load-bearing part. ** ALSO SETTLED HERE (a second, independent refutation): with the normal-framing comparison as FREE data, [Q1] in its natural universal shape is FALSE -- `indexWellDefined_false_of_framedMembrane` shows the 2-pi twist leaves the membrane untouched and flips the index. This is the same failure mode `CharSurfaceFKVacuity` found one level up, reappearing at the framing; the repair is a rigidity tie (NormalFramingTie), and `not_tied_twist` plus `trivialTie_rigid_but_uninhabited` show both that the repair bites and that inhabitedness is separately load-bearing. The same theorem is the P5 repackaging check: the membrane layer is strictly MORE data than the enhancement it produces, which a field-carrying MembraneIndexData could never be. SCOPE: this refutes the mod-2 index on the nonorientable stratum and the free-framing [Q1]; it does NOT refute [FK] itself, nor the mod-2 index on the orientable stratum, where it is exactly right.',
        'memory': '[[project_5qH_nonhausdorff_substrate_bug]]',
    },
    'k3-e1-forall-orientation-routes-are-vacuous': {
        'fork_id': 'k3-e1-forall-orientation-routes-are-vacuous',
        'backing_theorems': [
            'SKEFTHawking.IntOrientationScaling.not_forall_intOrientation_isUnimodular',
            'SKEFTHawking.KummerK3E1Repair.not_forall_intOrientation_pdInput',
            'SKEFTHawking.KummerK3E1Repair.not_kummerK3E1Residuals',
            'SKEFTHawking.KummerK3E1Repair.not_forall_latticeSig_neg16',
            'SKEFTHawking.KummerK3E1Repair.not_forall_gram',
            'SKEFTHawking.KummerK3E1Repair.not_forall_geoData',
            'SKEFTHawking.KummerK3ForallOrientationFalse.forall_orientation_kummerSubForm_family_false',
            'SKEFTHawking.IntOrientationPrimitive.not_isPrimitiveClass_smul',
            'SKEFTHawking.KummerK3E1Repair.not_forall_intOrientationPrim_latticeSig_neg16',
        ],
        'nogo_kind': 'refutation',
        'false_statement': 'A geometric hypothesis about the welded Kummer K3 may be quantified over ALL integral orientations -- the shape `forall o : IntOrientation KummerK3, <Gram / unimodularity / signature / cap-dual-span statement at o>` used by EVERY published entry point into `Nonempty KummerK3E1Atoms`. FALSE (2026-07-28, solo lead build, kernel-checked). TWO INDEPENDENT DEFECTS, both rooted in `IntOrientation` carrying only {fundClass, redCompat}. (1) SCALING (the destructive one, `IntOrientationScaling`): redCompat says the mod-2 reduction of fundClass is the canonical class; reduction is Z-linear into a ZMod 2-module, so `n . fundClass` satisfies it for EVERY ODD n, and `IntOrientation.smulOdd` constructs it. Scaling the fundamental class scales the intersection form, hence the Gram, hence multiplies det by n^rank. At n = 3, rank 22: a determinant cannot be a unit both before and after, so `forall o, IsUnimodular (interMatrix [M]_o B)` is false for ANY carrier with an inhabited IntOrientation and positive rank. This is a defect of the STRUCTURE, not of K3. (2) SIGN (`IntOrientationReverse` / `KummerK3ForallOrientationFalse`): reversal is a genuine second orientation (redCompat cannot see a sign) and negates the form, so any forall-o hypothesis carrying a sign-bearing Gram demands both G and -G. This one is IRREDUCIBLE -- no strengthening of IntOrientation can rescue it. WHAT IS REFUTED AT K3 (an orientation exists unconditionally via KummerK3SeamTransport.nonempty_intOrientation_kummerK3_uncond, so all of this is unconditional): `pdInput` (hence `KummerK3E1Residuals` is UNINHABITABLE), `hsig` (forall o, latticeSig (kummerK3Gram o) = -16), `hgram`, `hk3`, `hfam`, and `hgeo` (the terminal cap-dual datum, whose spanning half alone already yields pdInput). Therefore `nonempty_kummerK3E1Atoms_of_pd`, `_of_gram`, `_of_hk3`, `_of_stable`, `_of_stable_of_geometric`, and all five `KummerK3E1Unconditional` entry points are VACUOUSLY TRUE -- provable, but undischargeable. WHAT IS NOT REFUTED: `KummerK3E1Atoms` itself bundles ONE orientation with PD against that orientation; that shape is correct and reachable. THE REPAIR (built in the same turn, `KummerK3E1Repair` section 2): every route restated at a single supplied orientation -- `nonempty_kummerK3E1Atoms_at`, `_of_hk3_at`, `_of_capDualSpan_at`, `kummerK3_hk3_at`. These are NOT refutable by the argument above (one orientation carries no reversal and no rescaling). The repair also exposes an over-specification the forall-o shape had hidden: integral PD at ONE orientation is by itself the whole E1 atom triple -- no evenness, no Gram, no lattice theory -- so the Gram lane serves the sigma = -16 congruence that OTHER consumers want, not the E1 atoms. DO NOT re-derive a forall-o geometric hypothesis on this carrier; pin the orientation. ** THE DATUM REPAIR (same day, IntOrientationPrimitive.lean): pinning the QUANTIFIER is not enough, because the orientation an in-tree producer hands you is an arbitrary inhabitant of the defective structure -- possibly 3 . [K3], at which integral PD is FALSE. So "produce o from the seam route, then prove PD at it" was never viable either. `IntOrientationPrim` = `IntOrientation` + `IsPrimitiveClass fundClass` (not a proper multiple). `not_isPrimitiveClass_smul` shows `smulOdd` does NOT lift for |n| >= 2, so the SCALE defect is closed; `IntOrientationPrim.reverse` shows reversal DOES lift, so the SIGN ambiguity correctly survives -- and `not_forall_intOrientationPrim_latticeSig_neg16` confirms `hsig` stays FALSE even on the strengthened structure (do NOT read the strengthening as reviving forall-o for sign-carrying claims). Unimodularity, by contrast, is sign-invariant at even rank (`isUnimodular_kummerK3Gram_reverse_iff`), so pdInput is a LIVE target once the orientation is primitive. The strengthened datum costs NOTHING to produce: the degree-4 route was already choosing a basis vector, and `nonempty_intOrientationPrim_of_free_nontrivial` has exactly the hypotheses of the unstrengthened producer -- instantiated unconditionally at the welded K3 by `nonempty_intOrientationPrim_kummerK3_uncond`.',
        'memory': '[[project_5qH_nonhausdorff_substrate_bug]]',
    },
    ...
    'k0_to_k1_transport_refuted': {
        'fork_id': 'k0-to-k1-transport-refuted',
        'backing_theorems': [
            'SKEFTHawking.PinPlusRegularitySeparation.no_generic_zero_to_one_transport',
            'SKEFTHawking.PinPlusRegularitySeparationCarrier.exists_carrier_element_not_smooth',
        ],
        'nogo_kind': 'refutation',
        'false_statement': "Every k = 0 charted space over the R^4 model is also a k >= 1 manifold -- i.e. the `k := 0` binder in the live KT provider is harmless generality that transports to the smooth category for free. FALSE (kernel refutation, 2026-07-21). Mathlib registers an UNCONDITIONAL `instance : IsManifold I 0 M` for every charted space (contDiffGroupoid 0 I = continuousGroupoid H), so at k = 0 the IsManifold field is not weak but FREE (PinPlusRegularityFence.isManifoldZero_free). And no generic transport can repair it: no_generic_zero_to_one_transport refutes `forall M, IsManifold (R^4) 0 M -> IsManifold (R^4) 1 M` via TwistedR4 = R^4 with atlas {id, twist 0}, whose transition carries the kink min t (t/2) with one-sided derivatives 1 and 1/2 at 0 (not_differentiableAt_kinkInv). At the CARRIER level, exists_carrier_element_not_smooth exhibits a legal `SingularManifold PUnit 0 (R^4)` element -- built by adjoining one twisted chart to the project's own RP4 -- that is not a k >= 1 manifold. CONSUMER CHECK (why this is not idle): CharPairWProviderPerOp (R^4) 0 is hard-wired in 45 files and a tree-wide grep finds NO provider at k >= 1; residualProv (PinPlusKTAssemblyResiduals.lean:70) is the only provider and is declared at literal 0. The C0 generality is not unexercised -- it is the only thing exercised. SCOPE (worker-stated, lead-confirmed; DO NOT OVERSTATE): this separates OBJECT CLASSES, not bordism groups. Bordism could still identify the extra objects, and whether the k = 0 GROUP differs is OPEN and NOT claimed here; Kirby-Siebenmann is not formalized. TwistedRP4 IS homeomorphic to RP4 and does admit some smooth structure -- what fails is that its ATLAS, the datum SingularManifold actually bundles, is not C^1. So this upgrades the roadmap-leg-2 fence from 'the conclusion is uninterpreted' to 'the k := 0 binder is not harmless generality and no generic transport can exist'. It is NOT a refutation of the KT Z/16 mathematics.",
        'memory': '[[project_5qH_nonhausdorff_substrate_bug]]',
    },
    ...
    'honeycomb_phase_chart_gl2z_invariant': {
        'fork_id': '6ed-honeycomb-phase-chart-gl2z-invariant',
        'backing_theorems': [
            'SKEFTHawking.GrapheneBand.structureFactor_zero_set_not_shear_invariant',
            'SKEFTHawking.GrapheneBand.isHoneycombChart_of_neighbours',
        ],
        'nogo_kind': 'refutation',
        'false_statement': "The Bloch-phase honeycomb structure factor "
            "`f(theta) = 1 + exp(i theta_1) + exp(i theta_2)` is independent of WHICH primitive pair "
            "the phases are read against -- i.e. every theorem about it holds for ANY "
            "primitive-vector choice, so `theta_i = <k, a_i>` may be declared for an arbitrary "
            "primitive `(a_1, a_2)`. FALSE (Phase 6ED, 2026-07-29, kernel-checked; found by "
            "adversarial review of Waves 1-2, where this was the stated justification for a "
            "coordinate deviation). `f` is NOT GL_2(Z)-invariant. WITNESS: "
            "`structureFactor_zero_set_not_shear_invariant` exhibits a genuine Dirac point K at which "
            "the EQUALLY PRIMITIVE sheared pair `(a_1, a_1 + a_2)` gives `f != 0` -- so under that "
            "chart `structureFactor_eq_zero_iff` MISIDENTIFIES the Dirac set outright, reporting a "
            "genuine Dirac point as gapped. Numerically the sheared chart also disagrees on |f| "
            "(0.6468 vs 1.3115 at a sample momentum). SCOPE -- the theorems themselves are true; they "
            "are statements about `theta in R^2`. What is false is the IDENTIFICATION claim. `f` is the "
            "honeycomb structure factor precisely for the pair `(a_1, a_2) = (d_1 - d_3, d_2 - d_3)` "
            "built from the nearest-neighbour differences -- equal length, 60 degrees apart -- and for "
            "the six relabelings generated by `theta -> (theta_2, theta_1)` and "
            "`theta -> (-theta_1, theta_2 - theta_1)` (`structureFactor_norm_swap`, `_rotate`). "
            "THE REPAIR, and the real generality content the false claim was standing in for: the "
            "admissible chart is now a nameable hypothesis `IsHoneycombChart`, and "
            "`isHoneycombChart_of_neighbours` proves the honeycomb's own 120-degree bond geometry "
            "FORCES it -- so a consumer supplying genuine nearest-neighbour vectors gets the chart for "
            "free and never has to check it. CONSEQUENCE (Wave 2): 'the quadratic form "
            "`q1^2 - q1 q2 + q2^2` is isotropic in the physical frame' is likewise chart-specific -- "
            "true for the 60-degree pair, and a 120-degree pair sweeps a factor of 3 across "
            "directions. That claim is now the theorem `quadForm_of_chart`, stated against "
