...
Current scale: **4385 theorems** (4362 substantive + 23 placeholder as tracked in `constants.PLACEHOLDER_THEOREMS`), **1 axiom** (`gapped_interface_axiom`, eliminability: hard), across **187 modules**, **0 sorry project-wide**. 322 Aristotle-proved (44 runs, all closed; subsequent gap closures via interactive MCP tooling). 75 test files (3254 fast + 66 slow tests), 128 figure functions in `visualizations.py` (114 PNGs in `figures/`), 56 notebooks, 103 Python source modules, 32 papers + 1 short formalization note, 165+ CITATION_REGISTRY entries. Phase 6 (a/b/c/d/f) end-to-end ship: 7 new core papers (paper27, paper29, paper32, paper34–38) + `note_rt_ch_bounds` + 16 Phase-6 Lean modules covering BBN, equivalence principle, strong-CP / topological dark energy, QEC holography bridge, center-symmetry confinement, chiral SSB / GMOR, CFL chiral Lagrangian, RT/Casini-Huerta bounds, energy conditions, plus the Track A–C Phase 6a Wave-1–5 backbone (linearized EFE, FLRW dynamics, gravitational waves, BH entropy + thermodynamics).
...
**Last verified:** 2026-04-28-1200. **Section 2 module table re-synced 2026-04-28** (16 Phase 6a/6b/6c/6d/6f rows added + post-strengthening counts updated for the Phase-6c/6d cohort). Counts in this section are a rolling snapshot; `docs/counts.json` (regenerated by `scripts/update_counts.py`) is the single source of truth for live numbers.
...
| EnergyConditions | 542 | 9 | 0 | **6f W3** | **Phase 6f Wave 3** (Stage 1–2 ship): WEC/NEC/DEC/SEC predicates on abstract `StressEnergyTensor : Vec4 → Vec4 → ℝ` over `Vec4 = Fin 4 → ℝ`; signature `−+++` with explicit time-direction parameter; `IsNull` / `IsTimelike` / `IsFutureDirectedTimelike` predicates with load-bearing non-zero / sign hypotheses. Three chain implications (`DEC_implies_WEC`, `WEC_implies_NEC_under_continuity`, `DEC_implies_NEC_under_continuity`) and four counterexample-witness theorems (`cosmologicalLambda_WEC` / `_NEC` / `_violates_SEC`; `ghostScalar_violates_NEC`). Per the Phase 6f deep-research audit §3E: **first formalization of these predicates with chain implications + explicit counterexample witnesses across all surveyed proof assistants** (Lean / Coq / Isabelle/AFP / HOL Light / HOL4 / Mizar / Agda). Lorentzian-manifold version deferred to Phase 6f.4 once Mathlib Lorentzian-metric infrastructure lands. (**ALL PROVED, zero sorry**) |
...
|----------|-------|--------|
| **Python Source Modules** | 103 | Complete (Phases 1–6f) |
| **Test Files** | 75 | 3254 fast + 66 slow tests (3 `slow`-tagged Lean ExtractDeps consumers, 2 `importorskip("torch")` modules) |
| **Notebooks** | 56 | Phases 1–5x (Technical + Stakeholder) |
| **Lean Modules** | 187 | All build clean (8451/8451 jobs); per `docs/counts.json` 2026-04-28 |
| **Lean Theorems** | 4385 (4362 substantive + 23 placeholder) | **0 sorry project-wide**, **1 axiom** (`gapped_interface_axiom` in `SPTClassification.lean`). Phase 6a/6b/6c/6d/6f cohort shipped end-to-end: 7 core papers + `note_rt_ch_bounds` + 16 Phase-6 modules. Phase 5s research-grade sorries in `CenterFunctorZ2Equiv` were closed earlier; H_CF2 OPTIONAL per `CenterFunctor.lean:75`. |
| **Aristotle-proved** | 322+ | 44 runs (no Aristotle calls in Phase 6 — all interactive MCP closure) |
| **Paper Drafts** | 32 + 1 short formalization note + prediction tables | Phase 6 cohort uses `\input{counts.tex}` macroification; count-literal drift eliminated. |
| **Pipeline Figure Functions** | 128 in `visualizations.py` (114 PNGs in `figures/`) | All generated; Phase 6 figures (paper27 acoustic-vs-Schwarzschild T_H, paper29 BBN heatmap, paper32 ρ_DE-vs-Λ_QCD scan, paper34 EP matrix, paper35 code-distance bars, paper36 Polyakov loop, paper37 GMOR, paper38 ℤ_3 cube-roots, RT/CH knife-edge) shipped. |
| **Validation Checks** | 22+ | All passing (validate.py 22/22 PASS at last full run; CHECK 15d `claim_clusters_fresh` introduced Phase 5v Wave 10) |
