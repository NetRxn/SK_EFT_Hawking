...
- **Pattern summary:** A bundle lead dispatched to "redraft bundle X at Stage 10" is given two jobs in one agent lifetime — (a) analysis: read the substrate and the primary sources, verify every apex theorem's STATEMENT, file findings, render tables and figures; and (b) integration: write the assembled manuscript. Analysis is unbounded and open-ended; integration is bounded and comes last. **Analysis expands to consume the entire runway, and integration is what gets dropped.** Both leads produced excellent (a) — D3 filed 2 CRITICAL / 3 BLOCKER / 8 REQUIRED and landed `tables.py` + 4 re-rendered figures; D2 filed 4 CRITICAL / 3 MAJOR — and neither wrote a single line into `paper_draft.tex`. Both left the superseded draft deliberately untouched rather than half-replaced, which is the *right* call at the moment of running out, and both said so honestly. The defect is in the dispatch, not the agents.
- **Why it is not self-announcing:** the lead's report reads like success. It is full of verified findings, real artifacts and committed work, and the one sentence saying the manuscript was not assembled is easy to read past. The failure is only visible by checking `paper_draft.tex`'s mtime against the branch head. **Nothing in the pipeline asserts that a Stage-10 redraft produced a changed manuscript** — the stage's own deliverable is the one thing unverified.
- **Aggravating factor — the prose is stranded, not lost, but only barely.** Section drafters hold no write tools by design (`paper-drafter` returns prose to the lead), so when a lead exits before integrating, the only surviving copy of verified section prose is the ephemeral agent transcript under the session scratch directory. For D3 that was ten transcripts totalling ~29 MB; they were recovered and persisted into the worktree by hand. A session restart before that recovery would have destroyed a full day of verified drafting with no trace in git.
- **Pipeline stage affected:** Stage 10 (drafting/integration), and the wave-dispatch design that feeds it.
- **Proposed structural prevention:** three layers, cheapest first.
  - **(a) Assemble incrementally, never at the end.** Instruct the lead to write each section into `paper_draft.tex` as it returns, rather than accumulating sections and integrating last. This converts an all-or-nothing terminal step into a monotone one whose partial results survive. Cheapest fix and it alone would have prevented both instances.
  - **(b) Split the role.** Dispatch a bundle *analyst* (substrate + sources + findings + figures) and a bundle *assembler* (manuscript) as separate agents with separate runways. Applied to wave 2's recovery and is what produced the assembled drafts.
  - **(c) Gate it.** A Stage-10 completion claim should be checkable: assert that the bundle's `paper_draft.tex` changed in the redraft's own commit range. A stage whose deliverable is a manuscript should not be recordable as complete with the manuscript untouched — this is the "assert the decider, not the proxy" shape, where filed findings and committed figures are the proxy and the changed manuscript is the decider.
- **Owner:** lead. **Target:** (a) applied to wave 3 dispatch immediately; (b) applied to the wave-2 recovery; (c) to be filed as an infra finding after wave 3, once the check's population is measured rather than assumed.
- **Evidence:** `redraft/D3` at `d8ea552f` and `redraft/D2` at `2530373b` — both commits carry findings, tables and figures; neither touches `paper_draft.tex`. Recovered transcripts persisted at `papers/D3/_section_transcripts/` and `papers/D2/_sections/`.
- **Severity:** high. It does not corrupt anything, which is exactly why it survives review: the wave reports as productive, the findings are real, and the deliverable is simply absent.
