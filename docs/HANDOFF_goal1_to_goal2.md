...
⚠️ **Do not justify this item as "structural vs. name-matching".** `isAutogenerated`
(`ExtractDeps.lean`) is `isInternalDetail || isAuxRecursor || isNoConfusion ||
isCompilerSuffix`, and `isCompilerSuffix` is a hardcoded suffix list — `noConfusionType`,
`injEq`, `below`, `brecOn` — present because, in its own words, "no single predicate covers the
family". Its docstring also records that an earlier version overclaimed coverage and inflated
author-written counts by 921 for consumers who trusted it. Several of the 2,405 missing
declarations (`deriving` products `.repr` / `.decEq` / `.ofNat` / `.sizeOf_spec`; equation
lemmas `.eq_def` / `.congr_simp`; inductive-elaboration products `.ctorIdx` / `.enumList`) will
most likely need new suffix legs in Lean too. **The item relocates suffix matching into the
producer; it does not remove it.**

**The argument that does survive is DEFAULT-CORRECTNESS.** Wherever the classification lives,
applying it at the producer means no consumer can get it wrong — six sites became six defects
because the default was wrong, not because the classifier was. Decide the item on that basis.
If the detection extension proves harder than the guard it replaces, `theorem_census_agrees`
stays as the mechanism and this item closes as not-worth-it, which is a legitimate outcome.
