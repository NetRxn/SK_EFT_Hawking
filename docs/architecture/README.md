...
| Which cited sources do we actually **hold**, at what fidelity, and what may a claim rest on? Where may a loop fetch from? | [`../adrs/ADR-014-source-acquisition-and-citation-fidelity.md`](../adrs/ADR-014-source-acquisition-and-citation-fidelity.md) (normative) + [`../dev-loops/RESEARCH_LADDER_AND_WEB_EGRESS.md`](../dev-loops/RESEARCH_LADDER_AND_WEB_EGRESS.md) (the egress guard's spec) + [`../SOURCE_ACQUISITION_REGISTER.md`](../SOURCE_ACQUISITION_REGISTER.md) (derived). ⚠️ **This row was missing until 2026-08-15, and its absence had the cost the row below predicts.** With no document owning *do we hold this source*, the check that walks the population measured file-presence instead of fidelity — an `abstract.txt` counted as a held source — and the repair was written into a **manuscript** as four disclosures that we cite what we have not read. A fix with nowhere to be specified goes into the artifact it was found in. |
