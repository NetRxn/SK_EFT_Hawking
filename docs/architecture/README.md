...
| Where does a loop's **durable memory** live — the lab notebook, what it may hold, and why a worker often cannot see it? | [`../dev-loops/HARNESS_GUIDE.md`](../dev-loops/HARNESS_GUIDE.md) §Lab notebooks (operator's copy). ⚠️ **This row was missing until 2026-08-17, and its absence had the cost this table keeps predicting.** The notebook is the project's cross-session process memory — the home of *log tried-and-FAILED so we never repeat post-compact* — and **no document owned it**. Two consequences, both measured: `**/LAB_NOTEBOOK*.md` is gitignored, so **no notebook exists in any worktree**, and every subagent this project dispatches runs in one; meanwhile the notebook tooling resolves against the repo root, so **writes land on main while reads fail in the worktree**. An agent consults the tracker, finds nothing, proceeds uninformed, and still contributes to it. Separately, Phase 5q.T ran for months with no notebook at all and accumulated three route errors in its roadmap — a variance refutation, a stale entry-state, and a sidedness error — none of which anything caught, because [`END_TO_END_MAP.md`](END_TO_END_MAP.md) records that **nothing validates a roadmap**. A tracker the workers cannot read cannot prevent any of that. |
