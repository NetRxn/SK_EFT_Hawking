...
| Where does a loop's **durable memory** live — the lab notebook, what it holds, and where a worker can read it? | [`../dev-loops/HARNESS_GUIDE.md`](../dev-loops/HARNESS_GUIDE.md) §8b (operator's copy). ⚠️ **Notebooks are gitignored, so they exist only in the main checkout and never in a worktree.** The notebook tooling resolves this itself — `notebook_lib` asks whether a directory *carries a notebook*, not whether the directory exists, so `/skeft-qa:notebook` and the FRONTIER extraction reach the main checkout from inside a worktree. A **direct file read** still does not: cite a notebook to a worker by absolute path, and confirm against the main checkout before reporting one absent. A lab notebook is the standard tracker for a **unit of development work**, not a feature of `/goal` — a managed goal loop is the preferred way to run one, not the condition for having one. **A notebook's home is therefore wherever its work lives, at any depth**: when a roadmap grows past what can be reasoned about correctly, the work splits into a sub-roadmap that carries its own notebook beside it, so notebook homes **nest**. Enumerate them by finding the notebooks (`find … -name 'LAB_NOTEBOOK*.md'`), never by listing one level of a directory — `.gitignore`'s `**/` matches at any depth and so must any scan. Roadmaps do not substitute for a notebook, because [`END_TO_END_MAP.md`](END_TO_END_MAP.md) records that **nothing validates a roadmap**. |
| What must a Stage-10 **dispatch brief** contain, and where must each of its claims come from? | [`../../.claude/plugins/skeft-qa/skills/paper-authoring/references/dispatch-brief.md`](../../.claude/plugins/skeft-qa/skills/paper-authoring/references/dispatch-brief.md) (the author's side) + the [`paper-drafter`](../../.claude/plugins/skeft-qa/agents/paper-drafter.md) agent (the receiving side); normative decision [`../adrs/ADR-011-manuscript-quality-layer.md`](../adrs/ADR-011-manuscript-quality-layer.md) §C6/§D5. ⚠️ **This row was missing until 2026-08-17, and its absence had the cost the rows above predict.** A brief is an instruction that nothing gates, so with no document owning *what a brief must contain*, the discipline accumulated inside brief prose itself and was retyped per dispatch — improvable only by whoever happened to remember it. The rule is that every factual claim a brief makes names the artifact it was derived from. |
