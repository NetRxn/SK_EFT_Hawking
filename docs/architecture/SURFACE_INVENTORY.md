...
| 55 | `bundle_abstract_length` | `bundles_readiness` | Every bundle's abstract is within the abstract ceiling its declared venue imposes |
| 56 | `bundle_reviewer_stage_ordering` | `bundles_readiness` | No bundle records a Stage-13 verdict before Stages 9 and 10 are green |
| 57 | `bundle_prose_em_dash_free` | `bundles_readiness` | No bundle draft contains an em-dash in prose a reader will see |
| 58 | `bundle_reader_facing_voice` | `bundles_readiness` | No bundle draft narrates its own process to the reader — neither its correction history nor its unfinished diligence |
| 59 | `bundle_sentence_length` | `bundles_readiness` | No bundle draft grows its stock of sentences a reader must re-read to parse |
| 60 | `bundle_figure_adequacy` | `bundles_readiness` | Every bundle carries at least the figures its tier owes a reader |
| 61 | `bundle_structural_coherence` | `bundles_readiness` | Every bundle has a closing section, a bibliography, and a readable section count |
| 62 | `bundle_lean_module_coverage` | `bundles_readiness` | Lean modules a bundle registers as contributing are reached by its draft, by module name or by citing their theorems (ratcheted) |
| 63 | `notebook_stored_outputs_current` | `freshness` | Bundle companion notebooks' STORED outputs equal what their code produces |
| 64 | `notebook_markdown_retracted_claims` | `freshness` | Bundle companion notebooks' MARKDOWN cells carry no retracted claim |
| 65 | `readiness_verdicts_agree` | `bundles_readiness` | The heatmap and the submission gate return the same per-bundle verdict |
| 66 | `readiness_submission_gate` | `bundles_readiness` | Every paper has all P1 readiness gates passed (Phase 5v Wave 4) |
| 67 | `citation_primary_sources_present` | `citations` | Every cited external bibitem has a primary-source cache, its fidelity tier is decided by what the file holds rather than by its extension so an abstract-only or metadata-only cache is flagged rather than counted as the source, and exemption is claimed by a declared citation_class rather than inferred from absent metadata |
| 68 | `provenance_doi_in_registry` | `citations` | PARAMETER_PROVENANCE source DOIs resolve to CITATION_REGISTRY bibkeys |
| 69 | `bundle_consistency` | `bundles_readiness` | Cross-bundle clusters' member sentences agree on numerical content across bundle boundaries |
| 70 | `bundle_source_freshness` | `freshness` | Bundle source-paper mtime ≤ bundle last_lift; flag stale bundles |
| 71 | `bibitem_title_primary_source` | `citations` | Registry titles match primary-source cache PDF page-1 titles (drift detector) |
| 72 | `quantum_network` | `physics` | QN Python formulas satisfy the QuantumNetwork Lean theorem identities |
...
| `D7` | 1 | 17 | Classical Simulability and Quantum Advantage via Tensor Netw |
