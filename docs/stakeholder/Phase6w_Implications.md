...
|---|---|---|
| **6w.1** | **Kibble-Zurek-Unruh bridge foundation** (Tindall-Sels Science 392, 868 (2026)): module `KibbleZurekUnruh.lean` separating the four Kibble-Zurek exponents built from the same (ν, z) — freeze-out time, freeze-out length, defect density in d dimensions, and the reciprocal-convention correlation collapse — with kernel-checked reduction laws between them. Only the freeze-out-time exponent μ_t = νz/(1+νz) lies in (0,1) universally; explicit witnesses show the other three exceed 1. Headline `kzm_freezeOutTime_exceeds_horizonTime_of_one_lt_kappa`: under τ_Q = 1/κ, the freeze-out time κ^{-μ_t} exceeds the horizon-crossing time κ^{-1} when κ > 1 and falls below it when κ < 1. Companion `kzm_unruh_thermal_matches_hawking`: substantive 2π·T_H = κ via `field_simp` (NOT rfl). | D1 §9.1 new subsection + E1 §6 + E2 §7 |
...
**Tindall-Sels Kibble-Zurek-Unruh bridge (Wave 6w.1) gives the project its first cross-check against a classical-simulability calibration anchor.** Tindall et al. simulated a 300+ qubit disordered TFIM with a belief-propagation tensor-network algorithm and extracted a Kibble-Zurek exponent μ ≈ 2.70–2.75 from the scaling collapse of the correlation function. That exponent is defined in the reciprocal convention `ξ̂ ~ τ_Q^{1/μ}`, so it is the inverse of the standard KZM freeze-out-LENGTH exponent — neither a defect density nor the freeze-out-TIME exponent νz/(1+νz) that the project's substrate bounds in (0,1). The Lean module now carries both quantities under separate names with the reduction law `kzmCorrelationCollapseExponent_mul_freezeOutLengthExponent` connecting them, and `kzmCorrelationCollapseExponent_three_dim_ising_mem_Ioo` places ν = 0.63, z = 1 at 2.587, inside the μ = 2.6 ± 0.3 band the source quotes. That conversion, not a numerical coincidence, is what makes the cross-check meaningful.
