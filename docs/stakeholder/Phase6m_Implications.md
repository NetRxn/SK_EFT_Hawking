...
**Second honest correction (review-2026-06-05 D5-EV1, applied 2026-06-10):** the Verlinde conjunct of the 2026-05-08 refactor was itself a units conflation — Verlinde's quantitative ledger is not a Bayes factor but the Halenka-Miller galaxy-cluster mass-density exclusion (V. Halenka & C. J. Miller, PRD **102**, 084007 (2020), arXiv:1807.01689: 23 relaxed clusters, weak-lensing + X-ray; ruled out at >5σ *under nominal profile assumptions only*, weakening to compatibility once systematics are included; involves neither the CMB nor the Bullet cluster). The Lean module was updated accordingly: constant `verlinde_cmb_bullet_sigma` → `halenka_miller_cluster_nominal_sigma`, theorem `verlinde_2017_no_go_via_cmb_bullet_cluster_halenka_miller` → `verlinde_2017_no_go_via_cluster_mass_densities_halenka_miller` (now compared against the σ-scale `five_sigma_threshold`, not the |log𝓑|-scale `jeffreys_decisive_threshold`), and the Bayes-decisive cohort aggregator is now `both_decisive_bayes_bounds_exceed_jeffreys_decisive` (Tsallis + Odintsov, 2-of-4); `all_quantitative_bounds_disfavoured` carries the 4-of-4 mixed-threshold closure with per-candidate unit coherence (σ / log𝓑 / ΔAIC / log𝓑). The 8/8 NO-GO closure again survives at the *disfavoured* level.
