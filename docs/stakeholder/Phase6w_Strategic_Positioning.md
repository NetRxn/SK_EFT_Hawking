...
1. **Tindall-Sels et al. classical simulation of D-Wave Advantage2 disordered TFIM** (*Science* 392, 868 (2026); DOI 10.1126/science.adx2728; arXiv:2503.05693) — belief-propagation tensor-network algorithm classically extracts a Kibble-Zurek correlation-collapse exponent on a 300+ qubit disordered transverse-field Ising model. **The largest published direct head-to-head between a quantum annealer and a classical tensor-network method to date.**
...
4. **Zurek 1985 Kibble-Zurek mechanism** (*Nature* 317, 505) — the foundational theoretical anchor for the freeze-out scaling family, of which the correlation-collapse exponent Tindall-Sels report is the reciprocal-convention member. The cycle KZM theory → analog-Hawking surface gravity → SK-EFT WKB modified unitarity needs a formal bridge.
...
**Tindall-Sels KZM-Unruh bridge (Wave 6w.1).** Kernel-verified headline pair `kzm_freezeOutTime_exceeds_horizonTime_of_one_lt_kappa` / `kzm_freezeOutTime_below_horizonTime_of_kappa_lt_one` (the surface gravity decides the direction, and the inequality reverses at κ = 1) + companion `kzm_unruh_thermal_matches_hawking` (substantive 2π·T_H = κ via `field_simp`, NOT rfl-trivial) + concrete 1D-TFIM ν=z=1 → μ_t=1/2 numerical witness. The module separates the four Kibble-Zurek exponents that the literature all calls "μ" — freeze-out time, freeze-out length, defect density, correlation collapse — and proves the reduction laws between them, so that a value quoted in one convention converts rather than compares. What Tindall-Sels measure is the correlation-collapse exponent in the reciprocal convention (μ ≈ 2.70–2.75); `kzmCorrelationCollapseExponent_three_dim_ising_mem_Ioo` is the quantitative check against their quoted band. **First kernel-verified KZM-Unruh bridge with substantive thermal-bridge identity.**
...
**2. The Kibble-Zurek-Unruh bridge supplies the project's first cross-check against an experimental anchor outside SK-EFT scope.** The exponent Tindall-Sels report is the correlation-collapse exponent (1+νz)/ν in the reciprocal convention, not the freeze-out-time exponent νz/(1+νz) the project's substrate bounds in (0,1); the two are related by `kzmCorrelationCollapseExponent_mul_freezeOutLengthExponent` together with `kzmFreezeOutTimeExponent_eq_z_mul_length`, and it is that conversion that makes the cross-check well posed. The 1D-TFIM ν=z=1 → μ_t=1/2 witness anchors the theory side; `kzmCorrelationCollapseExponent_three_dim_ising_mem_Ioo` anchors the measurement side. This is the project's first formal bridge of this kind — analog-Hawking physics calibrated against a non-analog-Hawking classical-simulation extraction.
