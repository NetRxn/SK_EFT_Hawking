...
## Amendment A (2026-06-29) — first live-run findings + D7 staged-environment pre-clean

The first live submission under this ADR (job `946db1c6-b120-4191-a620-dfc8b27e88e2`, the two open L2 sorries) ran as a **5-turn interactive dashboard conversation** (orig submit → 2× "what would it take" analyses → "try A/B/C" → "work it via Approach D", ending OUT_OF_BUDGET) and was **retrieved + analyzed 2026-06-29**. The full per-task event/CoT logs were dumped under `docs/aristotle_results/run_20260629_054537_946db1c6/{extracted,logs}/` (gitignored). Reviewing the *full reasoning* (not just the per-turn summaries) revised two beliefs baked into the original ADR and motivates one new decision.

### Empirical findings

1. **The toolchain mismatch is NOT a barrier to the prover running** — correcting D6's framing that 4.28↔4.29 drift is the dominant exposure. Aristotle ran the prover on our project under **both** toolchains: task 3 via `lake update mathlib` (fetched our exact **v4.29.1** + cache; network was available), and task 5 by **porting to v4.28.0** (toolchain-agnostic `change` + `split_ifs <;> omega` fixes to the `SimplexCategory` API divergence in `SingularCohomologyMod2`/`SingularRelativeHomologyMod2`/`SingularCSCMayerVietoris{,Middle}`). A cross-version submission is therefore **not** doomed; the prover initializes either way.
2. **The dominant *avoidable* cost was build-environment setup, not the mathematics.** The only build fix Aristotle needed was dropping the **unfetchable `require Physlib` / `require repl`** blocks (our staging copies the manifest verbatim, so the staged project pins packages absent in Aristotle's sandbox → `lake build` dies on *"Physlib: package directory not found"*) plus a manifest consistent with the mathlib pin. In task 5 roughly **one-third of the budget went to this porting/setup** before any proof search — a direct contributor to the OUT_OF_BUDGET on the actual goal.
3. **The crux is route-invariant (confirms the Consequences risk note).** Five materially-different attempts all reduced to the same cap / Mayer–Vietoris-connecting cross-realization brick (`hcross`); Aristotle independently rediscovered the project's own verdict that the homology-class `of_crossRealization` route is banned/circular. Approach D ran **out of budget, not into a wall**: it built+verified a homology-level closer (`mem_boundaries_of_mk_eq`), discharged the two cycle obligations, and reduced the whole goal to the single class square `[seam] = [pd]` — the scaffold has since been **grafted into `SingularConnSquareCloseNC.lean`** (builds clean under our 4.29.1; the lone `sorry` is now exactly that square).

### D7 — Pre-clean the staged build environment (closure-aware require/manifest prune + staged-build gate)

Promote what D6 dismissed as an "optional diagnostic" to a **standard part of `stage_minimal_project`**, because the live run showed env-setup is the dominant avoidable budget cost. Three pieces, all operating **only on the throwaway staged copy** (`STAGING_DIR/stage_*`), never the real repo:

- **(i) Closure-aware `require` pruning.** `_staged_lakefile` drops a `[[require]]` block **iff no module in the staged import-closure imports its package.** This is **not** a blanket strip: `Physlib` is genuinely imported by `SKEFTHawking/QuantumNetwork/{QuantumFDTFloor,ThermalAssignmentFloor}.lean`, so it must be **kept** for a QuantumNetwork closure but is correctly **dropped** for the L2 (singular-homology) closure. `repl` (the lean-lsp REPL dependency) is imported by no source, so it is always droppable from a submission.
- **(ii) Manifest trim.** A new `_staged_manifest` drops the pinned entries for the pruned requires, leaving `mathlib` + its transitive deps. It is **derived from the real `lake-manifest.json` each stage**, so a future mathlib-pin bump is tracked automatically — there is no hand-maintained second copy to drift.
- **(iii) Staged-build gate.** After staging, run `lake build` on the throwaway staged dir under **our own 4.29.1**, and submit only if it is green. A staged project that builds for us builds for Aristotle (same toolchain, reached via `lake update`), so the prover spends ≈0 budget on environment setup. This is the definitive submission-side de-risk; its cost is one local build, not Aristotle's budget.

**Not a liability:** the real `lakefile.toml` / `lake-manifest.json` are untouched and keep `Physlib` (QuantumNetwork) and `repl` (lean-lsp-mcp); the transform touches only the disposable staged copy; and we do **not** bake in Aristotle's v4.28 *source* ports — D7 is **config-only** (requires + manifest), so the staged proof stays faithful to 4.29.1 and grafts back cleanly.

### Reconciliation with D6

D6's core stance is **refined, not overturned**, by separating two boundaries:

- **Graft-back boundary (D6 stands):** we still treat the 4.28↔4.29 mismatch as a known, tolerated risk and rely on the **D2 verify-then-graft gauntlet** to reject any returned proof that does not `lake build` + kernel-verify on our 4.29.1. A toolchain-induced failure still costs a *run*, not substrate integrity.
- **Submission boundary (D7 adds):** the **staged-build gate** removes the avoidable *environment-setup* tax that D6 had written off as optional. It is distinct from — and does **not** reintroduce — a 4.28.0-compatibility pre-check (we still do not do that; we let Aristotle resolve our 4.29.1). D6 ("no mandatory 4.28 pre-build") and D7 ("yes, a 4.29.1 staged-build gate, env-layer only") are therefore complementary, not contradictory.
