...
- **Status:** **PROPOSED (2026-06-13).** Records the decision, arising from the 2026-06-13 whole-substrate weakness audit, to install five standing automated gates (R1–R5) that move detection of the *proof-substance* and *assumption-disclosure* failure classes left from the Stage-13 fresh-context review into Stages 2/5/7, to re-open the two QI items that name these classes and re-close them via **structural prevention** (Stage-14 closure pathway #2) rather than per-finding supersession, and to give Pipeline Invariants #4 and #9 automated teeth plus a new Invariant #16. Execution tracked by [SubstrateIntegrityGates roadmap](../roadmaps/SubstrateIntegrityGates_Roadmap.md).
