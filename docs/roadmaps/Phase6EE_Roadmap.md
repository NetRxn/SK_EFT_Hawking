...
> **⚠️ GUARDRAIL — coordinate the Kramers statement with `MajoranaKramers`; complement, never duplicate.** The repo already owns `kramers_anticommutation` and `kramers_pfaffian_definite_sign` (module `MajoranaKramers`, plus `kramers_holds_hs_matrix` in `HubbardStratonovichRHMC`). Those are **Majorana/DIII superconductor** statements, not two-level control, so this phase's time-reversal-protected-doublet statement is genuinely new — but the antiunitary-symmetry machinery is shared. Read `MajoranaKramers.lean` at Stage 2, reuse its `Θ`-algebra rather than rebuilding it, and record which branch applies. *(Added 2026-07-30 by pre-arm plan-currency check — same shape as the 6CA/6ED QWZ guardrail, which worked.)*
