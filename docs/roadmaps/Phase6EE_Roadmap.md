...
- [x] `detector_chain_ceiling` — the 6EC bolometric floor composed end-to-end (the deepest chain, now grounded in dependency containment rather than asserted — `bolometer_error_floor` calls `error_floor_from_budget` + `nep_quadrature_add`, so the word is checkable: detector NEP → filter → Gaussian error → fidelity);
