...
**2026-04-30 PHASE 6g FULLY CLOSED — Wave 9 (4 sessions, substantive curve-theoretic finish-up across 6g.2 + 6g.6 + 6g.3 + 6g.4) + Wave 10 (1 session, substantive concrete-PDE 6g.5 distillation) SHIPPED end-to-end.** Five new modules: `RiccatiComparison.lean` (W9-S1, curve-pullback Gronwall-style Riccati comparison closing the parked Mathlib-PR-style follow-on from the 2026-04-30 6g.2-curve wave), `MazurSigmaModelRigidity.lean` (W9-S2, Mazur monotone-rigidity at the 1D distillation + Ernst-potential coincidence corollary), `HawkingPenroseSingularityCurveTheoretic.lean` (W9-S3, SEC-variant timelike Raychaudhuri-curve composition + de Sitter counterexample at curve scope), `AreaTheoremCurveTheoretic.lean` (W9-S4, area-evolution monotone-rigidity at null-generator scope), `WaveEquation1D.lean` (W10, 1D wave-equation Fourès-Bruhat distillation with explicit substantive solutions + linearity + cross-bridge to existing CauchyProblem.IsLocallyWellPosed). **26 substantive theorems** post-audit / 5 structures / 6 defs / 0 sorry / 0 new axioms / 3-cut ruthless wave-end audit (W9-S3 P3 alias-wrap + P5 trivial-rename + W9-S4 P3 trivial restatement) / Library 8490 jobs PASS clean. **First-formalization in any proof assistant** of: Raychaudhuri-Riccati comparison via `Convex.mul_sub_le_image_sub_of_le_deriv` + reciprocal-derivative bound, Mazur σ-model 1D distillation + Ernst-potential coincidence, timelike Hawking-Penrose curve composition, area-evolution-ODE monotone-rigidity, and 1D wave-equation Cauchy distillation with explicit substantive solution witnesses. Mathlib upstream PRs deferred per user "build-locally PR-later" policy. Bundle-target: lifts as **D3 §24 (W9-S1) + §25 (W9-S3) + §26 (W9-S4) + §27 (W9-S2) + §28 (W10)** plus **I1 sidebar** (W10 PDE distillation as methodology infrastructure). User authorization: standing per `memory:feedback_phase6g_standing_authorization.md` plus 2026-04-30 explicit user direction "let's start by finishing all the 6g work — no upstream PRs, but we can build in our own repo with aim to eventually pr upstream".

**2026-04-30 SHIPPING UPDATE — Substantive curve-theoretic 6g.2 re-engagement SHIPPED via 5-session multi-session wave on Wave-8 5b Levi-Civita backbone.** Five new modules: `LorentzianBundle.lean`, `NullGeodesic.lean`, `RaychaudhuriEquation.lean`, `FocalPoint.lean`, `PenroseSingularityCurveTheoretic.lean` / **25 substantive theorems** + 5 structures + 2 defs / ~1909 LOC / 0 sorry / 0 new axioms / 2-cut ruthless wave-end audit (S1 P3+P7 plumbing) / Library 8485 jobs PASS clean. **First-formalization in any proof assistant** of: bundle-level Lorentzian metric typeclass (Mathlib has IsContinuousRiemannianBundle Gouëzel 2025; no Lorentzian analog), vector-field null auto-parallel congruence with metric-compat-driven null-character preservation, Raychaudhuri focusing inequality at abstracted-trace + curve-level scope, focal-point existence at abstracted-comparison-hypothesis scope, curve-theoretic Penrose composition theorem at abstracted-comparison-hypothesis scope. **Architectural note:** wave ships at TWO parallel substantive scopes — manifold-level abstract focusing inequality (Session 3) + curve-level Riccati-comparison + focal-point bridge (Sessions 4-5); curve-pullback bridge between them parked as Mathlib-PR follow-on. Bundle-target: lifts as D3 §24 (correctness-push) + I1 sidebar (Mathlib-PR-shape infrastructure). User authorization: standing per `memory:feedback_phase6g_standing_authorization.md`.
...
### Wave 2 — `PenroseSingularity.lean` (6g.2) [Pipeline: Stages 1–12] — **SHIPPED 2026-04-29 (Stages 1-3+5; correctness-push)** + **SUBSTANTIVE CURVE-THEORETIC RE-ENGAGEMENT SHIPPED 2026-04-30 (5-session multi-session wave)**
...
**Substantive curve-theoretic re-engagement shipped 2026-04-30** — the curve-theoretic gap (focal-point ⟹ geodesic-incompleteness) is now closed at the abstracted-comparison-hypothesis + structural-Prop scope via a 5-session multi-session wave on the Wave-8 5b Levi-Civita backbone. 5 new modules / 25 substantive theorems / 5 structures / 2 defs / ~1909 LOC / 0 sorry / 0 new axioms / 2-cut wave-end audit. Modules (in order of dependency):

| Session | Module | Headline content | Subst thms |
|---------|--------|------------------|-----------:|
| S1 | `LorentzianBundle.lean` | Bundle Lorentzian metric typeclass + bridge to W8-5b `leviCivita_unique_of_isLeviCivita` | 5 |
| S2 | `NullGeodesic.lean` | Vector-field null auto-parallel congruence + metric-compat null-character preservation | 4 |
| S3 | `RaychaudhuriEquation.lean` | Raychaudhuri focusing inequality `d_k θ ≤ -θ²/(n-1)` + n=4 specialization | 5 |
| S4 | `FocalPoint.lean` | Focal-configuration predicate + Riccati-comparison bridge + cross-bridge to existing 6g.2 §3 `IsNullGeodesicallyIncomplete` | 6 |
| S5 | `PenroseSingularityCurveTheoretic.lean` | Wave composition: `IsCurveTheoreticPenroseHypothesis` ⟹ incompleteness at `lam_focus = -3/θ₀` | 5 |

**Wave architecture:** ships at TWO parallel substantive scopes — manifold-level abstract focusing inequality (S3) + curve-level Riccati-comparison + focal-point bridge (S4+S5). Curve-pullback that composes them is Mathlib-PR follow-on. Library 8485 jobs PASS clean. See `memory:project_phase6g2_curve_theoretic_shipped.md`.
...
**O.6 (NEW, opened 2026-04-29 catch-up; RESOLVED 2026-04-29 same day at algebraic-precedent scope; strengthened 2026-04-30 — 2 retroactive cuts):** Riemann/Ricci/scalar curvature on Mathlib's now-landed `CovariantDerivative` API — the SK-EFT-owned upstream contribution per Phase 6f audit §4. **RESOLVED:** Phase 6f.7 catch-up wave shipped 2026-04-29 — two new modules `LorentzianMetric.lean` + `RiemannianConnection.lean` (originally 13 substantive thms / 0 sorry / 0 new axioms; 11 after the 2026-04-30 W7+W8 reflexive strengthening pass cut `leviCivita_unique_at_koszul` (P5 trivial-trans on def-as-equality predicate) + `lorentzian_metric_nonzero` (P3 corollary `g 0 0 ≠ 0` of predicate's first conjunct via `rw + norm_num`)) at the algebraic-precedent + Mathlib-PR-shape stub scope. Headline: `riemannian_not_lorentzian_signature` falsifier + `christoffelKoszul_isTorsionFree` (Koszul-formula torsion-freeness under metric-partial symmetry) + `leviCivitaRiemannQuadratic_zero_of_partialZero` (flat Lorentzian space has zero Christoffel-quadratic curvature). **Substantive Levi-Civita uniqueness** (every torsion-free metric-compatible connection equals the Koszul Christoffel — Wald §3.1 / Kobayashi-Nomizu Thm IV.2.2) **deferred to Wave 8 Session 5** (bundle-level Koszul-bilinear-form argument over arbitrary smooth sections). Paper44 shipped as I1 sidebar primary + D3 §22.5 supplement. **Bundle-level Mathlib-PR upgrade (consuming Bonn `CovariantDerivative` + `TensorialAt.mkHom₂` machinery directly with discharged Bianchi identities) parked as future wave.** See memory `project_phase6f_w7_shipped.md`, `project_phase6f_w8_strengthening_pass_shipped.md`, and Phase 6f roadmap top.
