...
1. **Bridge** `traceNorm (PosSemidef A) = A.trace.re` — needs spectral mapping `eigenvalues(A²) = (eigenvalues A)²` or `tr|A| = tr A` via `cfc |·|`. Linchpin; everything below depends on it. *Likely route:* redefine/relate `traceNorm` on Hermitians to `(cfc |·| A).trace` where `cfc |·|` is identity on PSD spectrum.
