...
**Novelty claim — SCOPED 2026-07-28 (Stage-13 round 2).** What is *verified*: **Mathlib at pin `5e932f97` carries no** Bhattacharyya / Hellinger / Le Cam bound, no `erf`/`erfc`/Gaussian CDF, and no Neyman–Pearson or likelihood-ratio structure (declaration-level grep, zero hits); and **this project carries none** either. What is *not yet verified*: the broader "no theorem prover has this" claim. No search of Isabelle/AFP, Coq/`infotheo`, Agda, or Mizar has been performed — `isa-afp.org` is denied by this repo's own egress whitelist, so it cannot be checked from inside the loop. Mathlib *does* carry an abstract decision-theoretic scaffold (`Probability.Decision.Risk`: `bayesRisk`/`minimaxRisk` over `Kernel`s, `ℝ≥0∞`-valued, with data processing) — that is not a discrimination *bound*, but it means the whitespace is about the bound layer, not about decision theory. **PRE-SUBMISSION GATE for D12:** the unqualified prior-art claim must not be lifted into any draft until an AFP / `infotheo` search is documented; until then the scoped form above is the only one supported by evidence.
