...
Clean whitespace: no theorem prover has a kernel-checked Poisson discrimination-**bound** family (Bhattacharyya coefficient closed form, Le Cam two-point bound, dark-baseline exact optimum) packaged as reusable metrology substrate. **Precisified 2026-07-28 (Stage 13):** Mathlib at pin *does* carry a decision-theoretic scaffold — `Mathlib.Probability.Decision.Risk` (`bayesRisk`, `minimaxRisk`, `ℝ≥0∞`-valued over `Kernel`s, with a data-processing lemma). What is absent is the **bound layer**: declaration-level grep over `Mathlib/Probability/` returns zero hits for Hellinger, Bhattacharyya, or Le Cam, and zero for `erf`/`erfc`/Gaussian CDF anywhere in Mathlib. The whitespace claim is about the bounds, not about decision theory as such.
