...
- **Wave 5 (`BHThermodynamicsFourLaws`) initial ship 2026-04-26-0830 → SUPERSEDED 2026-04-26-2230 (full rewrite around Balbinot 2005).** The initial ship used the deep-research-supplied Schottky framework `T_H = T_H,0(1 − (M/M_c)²)` attributed to Jacobson-Koike 2002 cond-mat/0205174 Eq. (13). Stage 9 figure-reviewer flagged a slope-sign annotation contradicting the plotted curve. **Verbatim TeX-source verification** (saved at `Lit-Search/Phase-6a/primary-sources/{jk0205174, jv9801308, v0301043, balbinot}/`) revealed the deep research **conflated two different analog systems**:
  ...
  - **Balbinot 2005 (gr-qc/0405098 = PRD 71, 064019, sonic/acoustic, constant-c Laval nozzle)**: `T(t) = (ℏc/2π)·κ·[1 − (563/720π)·ε·κ³·c·A_0·t]`, asymptotic `t ~ 1/T³`, `T → 0` at infinite time (**cools** as evaporates, near-extremal-RN analog) — Balbinot **explicitly contrasts** with `³He-A` moving-wall as having opposite behavior
  - The project's own `src/wkb/backreaction.py` (Phase 1-2 anchor, citing Balbinot 2005) computes the acoustic `κ(t) ~ κ_0·exp(-t/τ_cool)` time-evolution. The Wave 5 deep research did NOT reference this existing project anchor.
...
- **Wave 5 corrected (re-ship target 2026-04-26-2230):** rewrite around the acoustic time-evolution (Balbinot 2005) as primary anchor + Schwarzschild Hawking 1975 contrast. **Regime partition shifts from sign-of-`dT_H/dM` to sign-of-`dT_H/dt` under evaporation** — Schwarzschild dT/dt > 0 (heats, finite t-evap), acoustic dT/dt < 0 (cools, infinite t-evap). Both regime sides primary-source-grounded. M_c remains project-original ansatz `(N_f·Λ_UV)/(12π·α_ADW)`. JK 2002 cited as the **explicit contrast case** per Balbinot's own §"Fate of the acoustic black hole". ⚠️ **Superseded 2026-08-15:** JV 1998 was grouped here in error and is a COOLING case, not a contrast case — see the attribution correction below. Lean module ~80% rewrite, paper27 substantial revision, figure replace, citations + tests + memory updates. Stage-14 process review filed at `papers/AutomatedReviews/2026-04-26-2230-wave5-process/deep_research_analog_conflation.md` documenting the deep-research-conflation failure mode (severity: critical; surfaces QI candidate `qi-deep-research-analog-conflation`). New memory `feedback_deep_research_analog_conflation.md` captures the lesson: TeX-verify every load-bearing primary source before integrating; cross-check existing project Python anchors.
...
**Goal:** Bardeen-Carter-Hawking four laws as formal theorems in emergent framework, with regime-partition reconciling Schwarzschild evaporation (Hawking 1975) and acoustic evaporation (Balbinot 2005).

**Prerequisites:** Waves 1, 3 complete. `KerrSchild.lean` provides stationary BH solutions. `SecondOrderSK.lean` provides FDR infrastructure. **`src/wkb/backreaction.py`** (Phase 1-2) is the existing project anchor for the acoustic time-evolution `κ(t) ~ κ_0·exp(-t/τ_cool)`, citing Balbinot et al. PRD 71, 064019 (2005). Wave 5's role is to formalize this existing computation in Lean and combine it with classical Schwarzschild thermodynamics to produce the regime-partition theorem.

**Primary-source anchor (corrected 2026-04-26; scope + JV-1998 attribution corrected 2026-08-15):** Balbinot, Fagnocchi, Fabbri, "Quantum effects in Acoustic Black Holes: the Backreaction" PRD 71, 064019 (2005), arXiv:gr-qc/0405098. **Scope: a SONIC hole in a Laval nozzle, one-dimensional flow, CONSTANT sound speed — not a Bose-Einstein condensate; the authors explicitly defer the condensate case ("the sound velocity c is not constant, but proportional to ρ^(1/2). This will be discussed elsewhere").** Eq. (Tsonic) gives `T(t) = (ℏc/2π)·κ·[1 − (563/720π)·ε·κ³·c·A_0·t]`; extrapolation `t ~ 1/T³` ⇒ T → 0 at infinite time (analog of near-extremal RN). Balbinot §"Fate of the acoustic black hole" explicitly contrasts the sonic hole with the `³He-A` thin-film moving wall (Jacobson-Koike 2002 cond-mat/0205174 — that system has non-vanishing end-temperature, opposite behavior). ⚠️ **Attribution correction 2026-08-15:** Jacobson-Volovik 1998 (PRD 58, 064021) is NOT a heating contrast case. It is a planar soliton in BULK ³He-A whose horizon exists only for `v > c_⊥`; Eq. (5.7) sends `κ → 0` as back-reaction slows the soliton to `c_⊥`, and its §VIII "cools as it evaporates and approaches an extremal black hole" AGREES with its own equations. It sits on the COOLING side with Balbinot. The heating law `T_H(v) = T_H(0)(1 − v²/c_⊥²)` belongs to the JK-2002 THIN FILM (`v < c_⊥`); the two ³He-A systems had been swapped. TeX source preserved at `Lit-Search/Phase-6a/primary-sources/Balbinot2005PRD.source/`.
...
- `lean/SKEFTHawking/BHThermodynamicsFourLaws.lean`
  ...
  - 1st law: `dM = κ/(8πG) dA + Ω dJ` — formalized over stationary axisymmetric BH; substrate-corrected variant for the acoustic regime
  - 2nd law: Glorioso-Liu SK-EFT entropy-current monotonicity (KMS Z₂ + unitarity ⇒ ∂_μs^μ ≥ 0; **no NEC invoked**)
  - 3rd law: Conditional — Israel strong form preserved in the acoustic regime via Balbinot's `t ~ 1/T³` (asymptotic in infinite time); classical regime carries Kehle-Unger 2022 caveat for charged-matter sectors
  ...
  - **Correctness-push theorem:** `regime_partition_criterion` — `b.M > M_c p → dT_H/dt > 0` (Schwarzschild heats during evaporation, Hawking 1975) `∧ b.M < M_c p → dT_H/dt < 0` (acoustic cools, Balbinot 2005). Sign-of-`dT_H/dt` flips at M_c. Both branches primary-source-grounded.
...
**O.3 — RESOLVED 2026-04-26 (rewrite).** 6a.5 regime separation framing: the genuine partition is **sign-of-`dT_H/dt` under Hawking evaporation**, not sign-of-`dT_H/dM`. Above M_c (Schwarzschild regime, Hawking 1975): `dT/dt > 0` (heats), finite t-evap. Below M_c (acoustic / near-extremal-RN regime, Balbinot 2005 PRD 71, 064019): `dT/dt < 0` (cools), infinite t-evap. M_c is project-original ansatz; both regime sides have primary-source grounding. JK 2002 (³He-A THIN-FILM moving wall) is explicitly NOT the cooling-regime anchor — Balbinot's own paper documents the contrast. ⚠️ **Corrected 2026-08-15:** JV 1998 (bulk moving soliton) is on the COOLING side and was grouped with JK 2002 in error.
