...
| **W3** | Teleportation (PROBE-GATED): Mathlib Haar lemma `∫_{S²}(⟨ψ|σ_k|ψ⟩)²dμ=1/3`? If tractable → Horodecki `f_avg=(2F+1)/3` + `F>1/2` utility threshold. If hard → algebraic skeleton + Haar fact as a TRACKED HYPOTHESIS (Prop arg), NOT a project axiom | ✅ DONE — **probe-gate → tracked hypothesis** (`Teleportation.lean`, kernel-pure, ZERO new axioms) |
| **W4** | Packaging: D6 §6 + preprint extended with W1–W3 + fresh-context Stage-13 → FULLY CLEAN | ✅ DONE (D6 §6 + preprint §3b, LaTeX compile PASS; Stage-13 GREEN — `docs/audits/stage13_phase6AC_2026-06-01.md`) |
...
- **BB84 key rate** `r=1−2h₂(e)`: Shor & Preskill, *Phys. Rev. Lett.* **85**, 441 (2000). (The other 6AC citations — Fortescue–Lo PRL 98/PRA 78, D'Hondt–Panangaden, Smolin–Verstraete–Winter PRA 72, Horodecki PRA 60, Massar–Popescu PRL 74 — are catalogued in `Lit-Search/Phase-6AA/6AA-network-protocol-exact-formulas.md`.)
...
## 🎯🎯 PHASE 6AC DONE (2026-06-01)
All DONE gates met: W1–W4 shipped kernel-clean; BB84 key-rate formula + positive-key threshold proven (crossover via IVT, not hardcoded); GHZ-vs-W comparison proven; teleportation proven under a documented Haar tracked-hypothesis (zero new axioms); all 3 new modules root-imported + counts regenerated (765 mod / 10025 thm / 0 axiom / 0 sorry); D6 §6 + preprint §3b extended (LaTeX compile-clean); **Stage-13 GREEN — FULLY CLEAN** (the lone provenance advisory remediated). Not pushed.

## Progress log
- **W3 — DONE** (`Teleportation.lean`, kernel-pure, **zero new axioms**): probe confirmed Mathlib at pin has sphere-Haar machinery (`Measure.toSphere`, `integral_fun_norm_addHaar`) but NOT the Pauli-quadratic integral `∫_{S²}(⟨ψ|σ_k|ψ⟩)²dμ=1/3` → tracked-hypothesis route. `HaarPauliConstant c := c = 1/3` (the discharged analytic gate, NOT an axiom), `teleportAvgFidelity F c := F+(1−F)·c` (algebraic skeleton), `teleportAvgFidelity_horodecki` (= `(2F+1)/3` under `hc`), `teleportAvgFidelity_one` (c-independent), `teleportAvgFidelity_classical_at_half` (= 2/3, Massar–Popescu), `teleportAvgFidelity_strictMono`, **`teleport_beats_classical_iff`** (`f_avg>2/3 ↔ F>1/2`, the utility threshold), `teleport_useful_over_chain` (composes shipped `endToEnd_teleportation_useful`). Counts 765 mod / 10025 thm.
