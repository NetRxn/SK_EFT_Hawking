...
### Substrate-breadth extensions (PLANNED, authorized 2026-06-29 and 2026-07-27)

Three series of public-substrate phases extend the verified-physics core into adjacent computational-chemistry / condensed-matter / device-metrology domains, made fast-lane by the PhysLib dependency (Schmüdgen unbounded spectral theory, full CPTP/Choi/Kraus, a proven 1D tight-binding band model, Schur triangulation, POVM + hypothesis-testing). Each is its own double-letter phase (à la 6AA/6AB, independent of the unrelated single-letter 6A/6b/6c) with numeric waves. The rosters are **authorized in `PAPER_STRATEGY` (D10 + D11, 2026-06-29; D12 + the 6ED→D11 source addition, 2026-07-27)**; the remaining on-disk scaffolding (`_VALID_BUNDLE_TARGETS`, `papers/D10`+`papers/D11`+`papers/D12`) executes at **first content-lift** per `BUNDLE_LIFT_PROCEDURE` (Pipeline Invariant #14, the D6–D9 precedent), not at planning time. Phases authorized but not yet scheduled for execution.

16. **`6B*` — computational quantum chemistry & open quantum systems** → bundle **D10** (authorized 2026-06-29): **6BA** NEGF/Landauer transport (`Phase6BA_Roadmap.md`) · **6BB** Hohenberg–Kohn DFT foundations — Kato self-adjointness + HK uniqueness/variational on PhysLib spectral theory (`Phase6BB_Roadmap.md`) · **6BC** Lindblad/GKSL open-system dynamics on PhysLib CPTP (`Phase6BC_Roadmap.md`) · **6BD** Jordan–Wigner/Trotter — *DEPRIORITIZED; novelty refuted by QBlue, Rocq arXiv:2509.18583 → first-in-Lean only; bundle home D8* (`Phase6BD_Roadmap.md`).
17. **`6C*` — topological band theory & metamaterials** → bundle **D11** (authorized 2026-06-29; 6CC may attach to **D2**): **6CA** Chern number + bulk–boundary, W3 DEEP→conditional (`Phase6CA_Roadmap.md`) · **6CB** acoustic/phononic band gaps — PhysLib TB port + Rayleigh/Courant–Fischer (`Phase6CB_Roadmap.md`) · **6CC** SPT/Pin⁺ condensed-matter reframe — *consumes the existing Pin⁺ ℤ/16 arc; CONDITIONAL on the disclosed Kirby–Taylor landmark (item 15); coordinate with the active 5q.G `/goal`, do not fork; Kitaev = 2D class-D* (`Phase6CC_Roadmap.md`) · **6CD** non-Hermitian/exceptional points — PhysLib Schur + build the finite-dim JNF/EP primitive (`Phase6CD_Roadmap.md`) · **6CE** effective-medium Maxwell–Garnett — algebraic path only, two-scale is a substrate-stall (`Phase6CE_Roadmap.md`). The two deep waves (6CA W3, 6CC W1) are the only ones matching the 5qF/5qG-L2 weakness; both ship conditional/temporary (tracked-Prop landmark, **no new axiom**).
18. **`6E*` — verified device-physics metrology** → bundle **D12** (authorized 2026-07-27; **6ED → D11**): **6EA** Poisson/Gaussian discrimination floors — Bhattacharyya/Le Cam universal floor + dark-baseline optimum + kernel-checked folklore-`e^(−N_diff)` refutations + shot/thinning algebra + the PhysLib POVM/hypothesis-testing seam (`Phase6EA_Roadmap.md`; series head — execute first, widest consumption) · **6EB** ENBW/NEP/matched-filter floors — `ENBW·T ≥ 1/2` realizability + plane-transfer/quadrature algebra + matched-filter optimality (`Phase6EB_Roadmap.md`) · **6EC** electrothermal detector physics — linearized bolometer ETF, stability iff `ℒ > −1`, `(1+ℒ)` responsivity correction, phonon/Johnson NEP floors (`Phase6EC_Roadmap.md`) · **6ED** graphene electronic structure — honeycomb tight-binding, Dirac dispersion with explicit remainders, Berry phase, **Haldane concrete Chern witness** via the shipped FHS machinery, *coordinate with 6CA's gated QWZ spike* — **sources D11**, not D12 (`Phase6ED_Roadmap.md`; fully parallelizable) · **6EE** two-level control + composite readout ceilings — RWA explicit-remainder bound, signed calibration algebra, Kramers degeneracy, end-to-end fidelity ceilings (`Phase6EE_Roadmap.md`; series capstone/barrier).
