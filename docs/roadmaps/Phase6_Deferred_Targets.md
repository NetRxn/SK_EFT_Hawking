...
### Substrate-breadth extensions (PLANNED, authorized 2026-06-29)

Two series of public-substrate phases extend the verified-physics core into adjacent computational-chemistry / condensed-matter domains, made fast-lane by the PhysLib dependency (Schmüdgen unbounded spectral theory, full CPTP/Choi/Kraus, a proven 1D tight-binding band model, Schur triangulation). **Phases authorized 2026-06-29**; each is its own double-letter phase (à la 6AA/6AB, independent of the unrelated single-letter 6A/6b/6c) with numeric waves. The roster is **authorized in `PAPER_STRATEGY` (D10 + D11, 2026-06-29)**; the remaining on-disk scaffolding (`_VALID_BUNDLE_TARGETS`, `papers/D10`+`papers/D11`) executes at **first content-lift** per `BUNDLE_LIFT_PROCEDURE` (Pipeline Invariant #14, the D6–D9 precedent), not at planning time. Phases authorized but not yet scheduled for execution.
