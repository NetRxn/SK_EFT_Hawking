...
| **Wave 2c.4a-R4.2.d.3** | Path-(i) Phase D3: closed-subgroup-of-SU(2) classification | ⏳ **NEXT-SESSION ENTRY**. With D2 in hand (non-commutation + non-centrality + same-trace-different-axis structure for σ_Fib_1_SU, σ_Fib_2_SU), rule out remaining closed subgroups: (a) center {±I} — already ruled out by D2 non-centrality; (b) 1-tori — ruled out by non-commutation (both generators in common torus ⟹ commute); (c) finite binary subgroups {Z_n, BD_4n, 2T, 2O, 2I} — need infinite-order witness (e.g., conjugate σ_Fib_1_SU^k · σ_Fib_2_SU has irrational rotation angle) OR cardinality bound (subgroup containing two generators of order 20 must have |G| > 120); (d) normalizers N(T) of maximal tori — dimension 1 + |Z/2|, need to show generators not in a common N(T). Substantial substrate ship: closed-subgroup classification for SU(2) is a Mathlib gap. Alternative: direct HBS-style accumulation argument via specific braid-word infinite-order witness. ~500-1500 LoC multi-session. | n/a | **D.2** | none |
