...
| **Wave 2d.3-followup** | Qubit Bloch-sphere balanced commutator (D-N §4.1 Eq. 10-13, Z-axis case) | ✅ **SHIPPED 2026-05-14** (this session): `FKLW/QubitBalancedCommutator.lean` (~260 LoC). **Headline**: `qubit_balanced_commutator_z_axis` — for any `θ ∈ [0, 1]`, exists hermitian F, G with ‖F‖, ‖G‖ ≤ √(θ/2) and `F·G - G·F = -(θ·i)•σ_z = -i·H` (H := θ·σ_z). Explicit construction: F := √(θ/2)·σ_y, G := √(θ/2)·σ_x. **Substrate** (10 thms): `σ_{x,y,z}_norm_le_one` (Pauli `linftyOpNorm` ≤ 1) + `smul_σ_{x,y,z}_norm_le` (scalar bound `‖α • σ‖ ≤ \|α\|`) + `smul_σ_{x,y,z}_isHermitian` + `σ_y_hermitian` (inline, complement to `PauliMatrices.lean`) + `balanced_commutator_z_core` (core identity via `comm_σ_x_σ_y`). **General-axis case** documented as predicate `QubitBalancedCommutatorGeneralAxis` (deferred ~200-400 LoC SU(2) Bloch substrate). Zero new axioms. | n/a | **D.2** | none |
| **Wave 2d.5-followup-full** | Substantive Dawson-Nielsen single-step refinement for qubit Z-axis | ✅ **Z-AXIS CASE SHIPPED 2026-05-14** (this session): `FKLW/SKZAxisStep.lean` (~220 LoC). **Headline**: `dn_lemma3_z_axis` — `‖exp(iF)·exp(iG)·exp(-iF)·exp(-iG) - exp(i·H)‖ ≤ 320·(θ/2)^{3/2}` for the explicit construction `F = √(θ/2)·σ_y, G = √(θ/2)·σ_x, H = θ·σ_z, ∀ θ ∈ [0, 1]`. **Composition** of Wave 2d.3-followup (`qubit_balanced_commutator_z_axis`) + Wave 2d.2-followup-R5.2b (`bch_order_2_cubic_thm`) into the substantive D-N Lemma 2 + Lemma 3 for the Z-axis case. Predicate scaffold `DNSingleStepZAxis` + discharge `dnSingleStepZAxis_of_θ_le_one`. **Deferred**: (a) general-axis case (requires Wave 2d.3-followup-general), (b) ε-net base case (Wave 2d.3 `EpsilonNet.lean`), (c) iterated recursion + `SolovayKitaevWithLengthBound` discharge. Zero new axioms. | n/a | **D.2** | none |
| **Wave 3a.2.2c-followup** | Split-braid Frobenius infrastructure for Rouabah 30-crossing Hadamard | ✅ **STRUCTURAL INFRASTRUCTURE SHIPPED 2026-05-14** (this session): `FKLW/RouabahSplitBraid.lean` (~200 LoC). **Headline**: `fibRep3Qubit_rouabah_eq_split` — the load-bearing structural lemma factoring `fibRep3Qubit rouabah_hadamard` into a 15-deep prefix (`fibRep3Qubit rouabah_first15`) plus a 15-deep `foldl` continuation via `List.foldl_append`. **Critical**: this avoids the need for any unproven `Mat2K_40_Ext` monoid laws — the factoring is purely algebraic on the underlying `List BraidLetter`. **Substrate**: `rouabah_first15` + `rouabah_last15` half-decompositions + `rouabah_split_eq` (via `List.take_append_drop`) + predicate `RouabahHadamardFrobValue` + biconditional `rouabahHadamardFrobValue_split_form` (reduces full-form to split-form discharge). **Empirically verified** (via lean_run_code): each individual 15-deep half is `native_decide`-tractable in seconds. **Deferred to follow-up sub-wave**: (a) Python-precomputed QCyc40Ext literal `frob_value` (~30-50 LoC), (b) the final single `native_decide` discharge on the split-form (combined 15+15+Frobenius may need additional optimization due to intermediate coefficient growth), (c) real-projection ordering on QCyc40Ext for threshold comparison with `(657/100000)²`. Quality improvement, not blocking — Hadamard ε~6.57e-3 already established via Rouabah paper citation. Zero new axioms. | n/a | **D.2** | none |
...
| **Wave 2d.2-followup-R5.2c** | Optimize K ≤ 4 (D-N original) | ⛔ **ARCHITECTURE-BOUND** (2026-05-14 analysis, defer indefinitely). Detailed analysis: tightening `exp(δ) ≤ 3` to `exp(δ) ≤ Real.exp 1 ≈ 2.72` brings K from 320 → ~256 (~20% reduction). Strengthening hypothesis from `δ ≤ 1` to `δ ≤ 1/2` brings K → ~46. **D-N's K ≤ 4 requires a fundamentally different proof architecture** — MVT-style integral estimate on the path `s ↦ exp(siF)·exp(siG)·exp(-siF)·exp(-siG)·exp(s²[F,G])` with `f(0) = 1`, `f'(0) = f''(0) = 0`, and `‖f(1) - 1‖ ≤ (1/6)·sup_{s∈[0,1]}‖f'''(s)‖`. The current 5/2 norm bound on `T2pos F = 1 + iF - F²/2` (tight at δ=1) is the binding constraint via the (5/2)^k factors in the telescope. **Decision**: defer; K=320 (R5.2a) + K=320 (R5.2b) are sufficient for R5.3 + all downstream consumers (`AharonovAradLemma6.lean` consumes `bch_order_2_cubic_thm` cleanly). Estimated cost for full K ≤ 4 refactor: ~300-500 LoC (MVT substrate + new proof). | n/a | **D.2** | none |
...
| **Wave 2c.4a-R5.4** | AA Bridge Lemma 6.2 (LieSpan + small elements fill nbhd of 1) | ⛔ **RETIRED 2026-05-13 PM (R2 soundness audit)**. The target axiom `aa_residual_interior_at_one_for_hom` was shown UNSOUND for `d ≥ 3` via SO(d) ⊂ SU(d) counterexample (Funar / Burau-at-real-q / Hecke-reduction representations satisfy `LieSpanProp + h_inf` but `closure ⊆ SO(d) ⊊ SU(d)`). The axiom was DELETED in commit `f44c60d` and replaced with axiom-free `aa_residual_interior_at_one_from_closure_eq_univ` (which takes `closure = univ` directly as hypothesis). R5.4's target therefore no longer exists. **Substantive density now caller burden** (i.e., R4.2.d for concrete Fibonacci, which is the active arc). | n/a | **D.2** | none |
| **Wave 2c.4a-R5.5** | Compose R5.1 + R5.2 + R5.3 + R5.4 → discharge `aa_residual_interior_at_one_for_hom` | ⛔ **RETIRED 2026-05-13 PM (R2 soundness audit)**. Target axiom DELETED in commit `f44c60d`; nothing to compose. Project axiom count is 1 (`gapped_interface_axiom`, physics-substantive permanent scaffolding). R5.1–R5.3 remain valuable in-tree substrate (BCH cubic infrastructure + topological closure / AccPt-at-1) and stay shipped. | n/a | **D.2** | none |
...
| **Wave 2c.4a-R4.2.d.2** | Path-(i) Phase D2: individual-generator trace formulas + F-conjugacy + non-centrality | ✅ **SHIPPED 2026-05-13 PM** (this commit): `FKLW/FibSU2Density.lean` +~210 LoC. 13 new substantive theorems, all standard-kernel-only. Headlines: (1) **`R1_C_ne_Rtau_C`** — R-eigenvalues distinct, extracted as standalone fact via R1^5 = 1 vs Rτ^5 = -1. (2) `σ_Fib_1_trace : tr(σ_Fib_1) = R_1 + R_τ` (rfl-level). (3) `σ_Fib_2_trace : tr(σ_Fib_2) = R_1 + R_τ` — same trace via `(φInv_C² + φInv_C) · (R_1 + R_τ)` collapsed by `linear_combination (R1_C + Rtau_C) * φInv_C_sq_add_self`. (4) **`σ_Fib_1_SU_mat_trace_eq : tr(σ_Fib_1_SU_mat) = exp(-7πi/10) + exp(7πi/10)`** — exponential form of the SO(3) rotation-angle invariant; angle 7π/5 (period 20 in SU(2) matches D1's `σ_Fib_1_SU_mat^20 = I`). (5) **`σ_Fib_2_SU_mat_eq_F_conj : σ_Fib_2_SU_mat = F_C · σ_Fib_1_SU_mat · F_C`** — the F-conjugacy relation, via `← Matrix.smul_mul, ← Matrix.mul_smul`. (6) `σ_Fib_2_SU_mat_trace_eq_σ_Fib_1_SU_mat_trace` — same trace via `Matrix.trace_mul_cycle + F_C_sq + one_mul`. (7) Non-centrality chain: `σ_Fib_1_SU_mat_diag_ne` (diagonal entries ω·R_1 ≠ ω·R_τ via `mul_left_cancel₀` + R1_C_ne_Rtau_C), `σ_Fib_1_SU_mat_ne_smul_one` (no scalar form), `σ_Fib_1_SU_mat_ne_one`, `σ_Fib_1_SU_mat_ne_neg_one`. (8) σ_Fib_2 analogue via F-conjugacy lift: if σ_Fib_2_SU_mat = c·I then F·σ_Fib_1·F = c·I, wrap both sides by F to get σ_Fib_1 = F·(c·I)·F = c·(F·F) = c·I, contradicting σ_Fib_1_SU_mat_ne_smul_one. (9) SU(2) group-level: `σ_Fib_1_SU_ne_one`, `σ_Fib_2_SU_ne_one` (note: SU(2) qua subgroup has no Neg instance; the `≠ -I` content lives at matrix-algebra level). Build: 8617 jobs / 0 new sorry / 0 new axioms. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.3a** | Path-(i) Phase D3.a: conjugation analysis + N(T) ruleout | ✅ **SHIPPED 2026-05-13 PM** (this commit): `FKLW/FibSU2Density.lean` §6, +~190 LoC. 7 substantive theorems, 4 of them headline (3 are private helpers). All standard-kernel-only. Headlines: (1) **`ω_Fib_C_sq_mul_R1_C_sq_ne_one : ω² · R_1² ≠ 1`** via `(ω²·R_1²)^5 = ω^10·R_1^10 = (-1)·1 = -1`. (2) **`σ_Fib_2_apply_00_ne_zero : σ_Fib_2[0,0] ≠ 0`** via factor `σ_Fib_2[0,0] = φInv · (φInv·R_1 + R_τ)` → 5th-power argument → `φInv^5 = 1` contradicts `φInv^5 < 1` (real golden-ratio < 1). (3) **`σ_Fib_SU_mat_not_conj_inverts : σ_Fib_2_SU_mat·σ_Fib_1_SU_mat ≠ star σ_Fib_1_SU_mat·σ_Fib_2_SU_mat`** — the N(T)-ruleout headline. Proof: project to entry [0,0]. After algebra, equality forces `(ω·R_1)² = (ω·R_1)·star(ω·R_1) = ‖ω·R_1‖² = 1`, i.e., `ω²·R_1² = 1`, contradicting (1). **Density implication**: closed subgroups of SU(2) of dim 1 are tori T and normalizers N(T). For ⟨σ_Fib_1_SU, σ_Fib_2_SU⟩ ⊆ N(T): either σ_Fib_2_SU ∈ T (forces commute, ⊥ §3) or σ_Fib_2_SU ∈ N(T)\T (forces `σ_Fib_2·σ_Fib_1·σ_Fib_2⁻¹ = σ_Fib_1⁻¹` via Weyl-group inversion, equivalent to the D3.a inequality being an equality, ⊥). Hence ⟨σ_Fib_1_SU, σ_Fib_2_SU⟩ ⊄ N(T) for any T. **Combined with D2**: center {±I} + 1-tori already ruled out → only finite binary subgroups remain (D3.b) + SU(2) itself. Build: 8617 jobs / 0 new sorry / 0 new axioms. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.3b** | Path-(i) Phase D3.b: order analysis + finite-subgroup ruleout | ✅ **SHIPPED 2026-05-13 PM** (this commit): `FKLW/FibSU2Density.lean` §7, +~165 LoC. 7 substantive theorems (4 headline + 3 private helpers). All standard-kernel-only. Headlines: (1) **`σ_Fib_1_SU_mat_pow_4_ne_one : σ^4 ≠ I`** — rules out element order 4. Via slick reduction `(ω·R_1)^4 = R_1` (private `ω_R1_pow_4_eq_R1`, factored through `ω^4·R_1^3 = ζ_5·ζ_5^9 = 1` private cyclotomic identity), then `R_1 ≠ 1` via bridge identity `R_1^2 + R_1^3 = 1/φ` ⟹ `2 ≠ 1/φ`. (2) **`σ_Fib_1_SU_mat_pow_10_eq_neg_one : σ^10 = -I`** — concrete 10th-power value via `ω^10 = -1` + `σ_Fib_1^10 = I`. (3) **`σ_Fib_1_SU_mat_pow_10_ne_one : σ^10 ≠ I`** — derived from (2) since -I ≠ I. (4) **`σ_Fib_1_SU_mat_has_period_20`** — packaged `σ^20 = I ∧ σ^10 ≠ I ∧ σ^4 ≠ I`. Combined: order of σ_Fib_1_SU in M_2(ℂ) is exactly 20 (divisors of 20 not dividing 4 or 10 = {20}). **Finite-subgroup ruleout (proof-by-docstring)**: Z_n abelian ⊥ §3 non-commute; BD_4n outside-cyclic order 4 ≠ 20 ⊥; 2T (max 6) < 20; 2O (max 8) < 20; 2I (max 10) < 20. Together with D2 (center+1-tori out) and D3.a (N(T) out), **only SU(2) itself remains as a closed subgroup of SU(2) containing both generators** — the informal density result. Formal closure-eq-univ deferred to D4. Build: 8617 jobs / 0 new sorry / 0 new axioms. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.1** | Path-(i) Phase D4.1: closure-as-subgroup substrate (general-purpose `IsTopologicalGroup` for SU(n,ℂ) + `H_Fib` packaging) | ✅ **SHIPPED 2026-05-14 AM** (this commit): `FKLW/FibSU2Density.lean` §10, +~140 LoC. 7 substantive declarations, all standard-kernel-only (`propext + Classical.choice + Quot.sound`). **General-purpose substrate (not in Mathlib4 as of v4.29.0)**: (1) **`su_continuousInv (n : ℕ)`** : `ContinuousInv ↥(Matrix.specialUnitaryGroup (Fin n) ℂ)` — proof: `(A : SU(n))⁻¹ = star A` (definitional in Mathlib); `star` continuous on `Matrix` (upstream `Matrix.instContinuousStar`); subtype-mk lifts (4-line proof body). (2) **`su_isTopologicalGroup (n : ℕ)`** : combines `Submonoid.continuousMul` (upstream) with new `su_continuousInv`. **Fibonacci packaging**: (3) **`H_Fib := ρ_Fib_SU2.range.topologicalClosure`** — the Fibonacci closure subgroup of SU(2), a closed `Subgroup`. (4) `H_Fib_isClosed`. (5) **`σ_Fib_1_SU_mem_H_Fib`** + **`σ_Fib_2_SU_mem_H_Fib`** — both generators in `H_Fib` via R4.2.c apply-σⱼ lemmas + `Subgroup.le_topologicalClosure`. (6) **`H_Fib_eq_top_iff_closure_eq_univ`** — the `Subgroup`-eq-⊤ ↔ `Set`-eq-univ lift lemma via `Subgroup.topologicalClosure_coe + MonoidHom.coe_range + Subgroup.coe_top`. (7) **`fibonacci_density_from_H_Fib_eq_top`** — full `DenseInSpecialUnitary` from `H_Fib = ⊤` via composition with D1's `fibonacci_density_conditional`. Build: 8617 jobs / 0 new sorry / 0 new axioms. **Residual D4 hypothesis now articulated at subgroup level** as `H_Fib = ⊤`; the D1-D3.b structural ruleouts (order-20 + non-commute + non-N(T) + non-scalar) are now potentially actionable at the subgroup level. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.2** | Path-(i) Phase D4.2: Subgroup-level structural ruleouts (lift D1-D3.b matrix facts to Subgroup level) | ✅ **SHIPPED 2026-05-14 AM** (same commit-cluster as D4.1): `FKLW/FibSU2Density.lean` §11, +~130 LoC. 9 substantive theorems, all standard-kernel-only. (1) **`σ_Fib_1_SU_pow_20_eq_one : σ_Fib_1_SU ^ 20 = 1`** in SU(2) via `Subtype.ext + SubmonoidClass.coe_pow` lift of D1's matrix-level `σ_Fib_1_SU_mat_pow_20`. (2) **`σ_Fib_1_SU_pow_10_ne_one`** + (3) **`σ_Fib_1_SU_pow_4_ne_one`** — lifts of D3.b matrix ruleouts. (4) **`σ_Fib_1_SU_has_period_20`** — packages the three pow-facts: σ has order exactly 20 in SU(2) qua group (only divisor of 20 not dividing 4 or 10 is 20). (5) **`H_Fib_isCompact`** — closed subset of compact SU(2) via `instCompactSpaceSpecialUnitaryGroup` (`SpecialUnitaryTopology.lean`). (6) **`H_Fib_ne_bot`** — σ_Fib_1_SU ∈ H_Fib but ≠ 1 (D2's `σ_Fib_1_SU_ne_one`). (7) **`H_Fib_not_abelian`** — ∃ x y ∈ H_Fib with x*y ≠ y*x via D1's `σ_Fib_SU_not_commute`. (8) **`H_Fib_contains_period_20_element`** — ∃ u ∈ H_Fib with `u^20=1 ∧ u^10≠1 ∧ u^4≠1`, witnessed by σ_Fib_1_SU. (9) `H_Fib_not_subset_singleton_id` — H_Fib not contained in `{1}` pointwise (stronger form of #6). **Density implication**: H_Fib is now provably a **non-trivial, non-abelian, compact subgroup of SU(2) containing an element of order exactly 20** — precisely the Subgroup-level preconditions for the Cartan-classification argument. Build: 8617 jobs / 0 new sorry / 0 new axioms (unchanged). | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.3.a** | Path-(i) Phase D4.3.a: finite-case cardinality bounds for H_Fib (Lagrange-based) | ✅ **SHIPPED 2026-05-14 AM** (this commit): `FKLW/FibSU2Density.lean` §12, +~190 LoC. 13 substantive theorems all standard-kernel-only. (1) **`σ_Fib_1_SU_orderOf : orderOf σ_Fib_1_SU = 20`** via divisors-of-20 enumeration + `orderOf_dvd_iff_pow_eq_one`. (2) Private `σ_Fib_2_SU_mat_pow_eq_F_conj` + `σ_Fib_2_SU_mat_pow_eq_one_iff` via F²=I telescope induction. (3) Matrix-level period 20 for σ_Fib_2 (`σ_Fib_2_SU_mat_pow_20/_10_ne_one/_4_ne_one`). (4) SU(2)-level lifts (`σ_Fib_2_SU_pow_*`). (5) **`σ_Fib_2_SU_orderOf = 20`** via same divisor enumeration. (6) **`σ_Fib_2_SU_not_mem_zpowers_σ_Fib_1_SU`** : forces commutation if true → contradicts D1. (7) `zpowers_σ_Fib_1_SU_le_H_Fib` + **`zpowers_σ_Fib_1_SU_lt_H_Fib`** strict containment. (8) `H_Fib_card_ge_20_if_finite` via `Subgroup.orderOf_le_card`. (9) **`H_Fib_card_ge_40_if_finite`** via Lagrange `20 ∣ |H_Fib|` + strict-containment `20 < |H_Fib|` → smallest multiple ≥ 40. **Density implication**: any finite SU(2) subgroup containing H_Fib has cardinality ≥ 40. Combined with D4.2's non-abelian: rules out cyclic (abelian), 2T (24<40), 2O (max element 8<20), 2I (max element 10<20). Only finite candidates left: BD_{4n} with n ≥ 10 (cyclic part ⊇ ⟨σ_Fib_1_SU⟩). Build: 8617 jobs / 0 new sorry / 0 new axioms. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.3.b** | Path-(i) Phase D4.3.b: two-cyclic-subgroup structure + intersection cardinality bound | ✅ **SHIPPED 2026-05-14 AM** (this commit): `FKLW/FibSU2Density.lean` §13, +~140 LoC. 7 theorems all standard-kernel-only. (1) `zpowers_σ_Fib_2_SU_le_H_Fib` + (2) `σ_Fib_1_SU_not_mem_zpowers_σ_Fib_2_SU` + (3) `zpowers_σ_Fib_2_SU_lt_H_Fib` + (4) `Nat_card_zpowers_σ_Fib_2_SU = 20` — symmetric mirrors of D4.3.a for σ_2. (5) `zpowers_σ_Fib_1_SU_ne_zpowers_σ_Fib_2_SU` — two cyclic subgroups distinct. (6) **`inter_zpowers_lt_zpowers_σ_Fib_1_SU`** : strict containment ⟨σ_1⟩ ∩ ⟨σ_2⟩ < ⟨σ_1⟩ via inf_eq_left + non-commute contradiction. (7) **`inter_zpowers_card_le_10`** : |⟨σ_1⟩ ∩ ⟨σ_2⟩| ≤ 10 via "proper divisor of 20" combining `Subgroup.card_dvd_of_le` + `Set.Finite.card_lt_card` + `IsOfFinOrder.finite_powers` + interval_cases. **Density implication**: H_Fib contains TWO distinct order-20 cyclic subgroups with intersection ≤ 10 elements. Informally rules out BD_{4n} (forces both σ_1, σ_2 into cyclic part, hence commute, contradicts D1). Formal closure needs D4.3.c. Build: 8617 jobs / 0 new sorry / 0 new axioms. | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.3.c+** | Path-(i) Phase D4.3.c+: scalar-centralizer argument + complete Cartan substrate | ⏳ **DEFERRED 2026-05-14** (analysis-in-roadmap, multi-session). With D4.3.b shipped (`inter_zpowers_card_le_10`), the next substantive tightening is `inter_zpowers_card_le_2` via scalar-centralizer argument: u ∈ ⟨σ_1⟩ ∩ ⟨σ_2⟩ commutes with both σ_1 (diagonal) AND σ_2 (F-conjugate of diag with nonzero off-diagonal) → u must be diagonal AND scalar in 2x2 → u ∈ {a·I : a ∈ QCyc40Ext} → for u ∈ SU(2), det(u) = 1 forces a² = 1, hence u ∈ {I, -I}. Combined with D4.3.b chain: |H_Fib| ≥ 200 in finite case. **Implementation cost (per 2026-05-14 analysis)**: ~200-300 LoC of substrate, factored into (1) diagonal-from-commute-with-diag (~50 LoC), (2) scalar-from-commute-with-σ_2-AND-diagonal (~50 LoC, requires the σ_2[0][1] = φ⁻³/²(R₁-Rτ) ≠ 0 lemma at QCyc40Ext level via `decide`), (3) scalar-in-SU(2)-is-±I (~30 LoC, det(u) = 1 + a² = 1), (4) lifting from matrix to Subgroup level (~50-100 LoC), (5) combining via Lagrange (~30 LoC). D4.3.d+ then completes finite ruleout + positive-dim case (Cartan substrate). Multi-session ~500-1500 LoC remaining. **Constraint**: scoped HBS axiom is OFF THE TABLE per 2026-05-14 directive. **Note**: D4.3.b's |inter| ≤ 10 is already substantive structural progress; D4.3.c is optional sharpening that doesn't change the formal posture (both leave D4.3.d+ Cartan as the missing piece for unconditional discharge). | n/a | **D.2** | none |
| **Wave 2c.4a-R4.2.d.4.3.d.1** | Path-(i) Phase D4.3.d.1: identity-component substrate (entry point for Cartan classification) | ✅ **SHIPPED 2026-05-14 PM** (this session): `FKLW/CartanSubstrate.lean` (~340 LoC including docstrings, ~180 LoC code). 11 substantive declarations all standard-kernel-only (`propext + Classical.choice + Quot.sound`). **§1 generic substrate (4 reusable lemmas parametric in `G`)**: (1) `Subgroup.compactSpace_of_isClosed_in_compact` (closed subgroup of compact group is compact). (2) `Subgroup.finite_of_isClosed_of_discreteTopology` (closed + DiscreteTopology ⇒ Finite). (3) **`Subgroup.connectedComponentOfOne_eq_bot_of_finite`** (finite subgroup of T1 group has trivial identity component, via `Finite.instDiscreteTopology` + `totallyDisconnectedSpace_iff_connectedComponent_singleton`). (4) `Subgroup.finite_iff_discreteTopology_of_isClosed_in_compact_T1` (iff form). **§2 H_Fib substrate**: `H_Fib_compactSpace` instance + **`H_Fib_idComponent : Subgroup H_Fib`** := `Subgroup.connectedComponentOfOne H_Fib` + properties (`one_mem`, `isClosed`, `isConnected`). **§3 specialization**: `H_Fib_idComponent_eq_bot_of_finite`. **§4 dichotomy headlines**: (5) `H_Fib_finite_iff_discreteTopology`. (6) **`H_Fib_dichotomy_discrete_or_accPt`** — substantive headline: either H_Fib is discrete-and-finite-with-≥40 (via D4.3.a's `H_Fib_card_ge_40_if_finite`), or 1 is an AccPt of H_Fib (via R5.1's `one_accPt_of_infinite_closed_subgroup`). (7) **`H_Fib_idComponent_ne_bot_implies_infinite`** — contrapositive of §3 (the converse direction "infinite ⇒ idComponent ≠ ⊥" is the Cartan gap). (8) **`H_Fib_finite_card_ge_40_and_idComponent_bot_or_infinite`** — packaged composition of D4.3.a cardinality bound with identity-component substrate. **Architectural deferral (D4.3.d.2+)**: Cartan's closed-subgroup theorem + 1-parameter subgroup theorem (von Neumann) + maximal-torus classification — Mathlib4 v4.29.0 gaps, ~300-1500 LoC if shipped directly (could be upstreamed to Mathlib). Build: 8617 → 8621 jobs (1 new module) / 0 new sorry / 0 new axioms (project count unchanged at 1 = `gapped_interface_axiom`). | n/a | **D.2** | none |
...
### 2026-05-14 PM — Wave 2c.4a-R4.2.d.4.3.d.1 SHIPPED

**Ship**: `FKLW/CartanSubstrate.lean` (~340 LoC incl docstrings, ~180 LoC code), the Cartan-substrate / identity-component infrastructure for the formal `H_Fib = ⊤` discharge.

**Headline theorems (all standard-kernel-only via `lean_verify`)**:

1. **`Subgroup.connectedComponentOfOne_eq_bot_of_finite`** (§1, generic) — finite subgroup of T1 topological group has trivial identity component. Proof: `Finite + T1 ⇒ DiscreteTopology` (Mathlib `Finite.instDiscreteTopology`) ⇒ `connectedComponent x = {x}` ⇒ `connectedComponentOfOne = ⊥`.

2. **`H_Fib_idComponent : Subgroup H_Fib`** (§2) — the Mathlib `Subgroup.connectedComponentOfOne H_Fib` specialized to our Fibonacci closure subgroup. Properties: `one_mem`, `isClosed`, `isConnected` shipped.

3. **`H_Fib_dichotomy_discrete_or_accPt`** (§4, substantive headline) — either H_Fib has discrete topology (hence finite with `40 ≤ |H_Fib|` via D4.3.a's `H_Fib_card_ge_40_if_finite`), or 1 is an accumulation point of H_Fib in SU(2) (via R5.1 AALemma6's `one_accPt_of_infinite_closed_subgroup`). Clean binary substantive choice composing 4 prior waves.

4. **`H_Fib_idComponent_ne_bot_implies_infinite`** (§4) — the contrapositive direction of "finite ⇒ trivial id-component". The CONVERSE direction is exactly the Mathlib4 v4.29.0 Cartan gap (profinite groups counterexample).

5. **`H_Fib_finite_card_ge_40_and_idComponent_bot_or_infinite`** (§4) — packaged composition: either H_Fib is finite with `40 ≤ |H_Fib|` AND identity component is trivial, or H_Fib is infinite.

**Architectural deferral (D4.3.d.2+)**: Cartan's closed-subgroup theorem + 1-parameter subgroup theorem (von Neumann) + maximal-torus classification — Mathlib4 v4.29.0 gaps, ~300-1500 LoC if shipped directly (could be upstreamed). The remaining D4.3.d work is now structurally well-articulated: convert `H_Fib_idComponent ≠ ⊥` into "H_Fib has positive Lie-group dimension", then use the existing D3.a `N(T)` ruleout + D2 non-centrality to force `H_Fib = SU(2)`.

**Build**: 8617 → 8621 jobs (1 new module). Zero sorry, zero new axioms (project count UNCHANGED at 1 = `gapped_interface_axiom`).

**Strengthening audit**: zero P2/P3/P4/P5 anti-patterns. The dichotomy headline produces either a numerical `40 ≤ Nat.card H_Fib` witness or an `AccPt` topological witness — not a P3 multiplication-only tautology, not a P5 conditional-violator tautology. Cross-module bridge integrity ✓ (substantively calls `H_Fib_card_ge_40_if_finite`, `H_Fib_isClosed`, `H_Fib_isCompact`, `one_accPt_of_infinite_closed_subgroup`, plus Mathlib substrate).

**Pre-Phase-6p axiom count: 1 → 1 (unchanged)**.
