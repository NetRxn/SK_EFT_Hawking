...
| **Wave 2c.4a-R4.2.d.3b** | Path-(i) Phase D3.b: finite-subgroup ruleout | ⏳ **NEXT-SESSION ENTRY**. Rule out finite binary subgroups of SU(2): cyclic Z_n (abelian, ruled out by non-commute), binary dihedral BD_4n (outside-cyclic elements have order 4; σ_Fib_2_SU has order 20 → in cyclic part → commute, ⊥), binary tetrahedral 2T (max element order 6 < 20), binary octahedral 2O (max 8 < 20), binary icosahedral 2I (max 10 < 20). Element-order facts for binary polyhedral groups are a Mathlib4 substrate gap; either ship them locally or use direct calculation (the order-20 element's existence rules out 2T/2O/2I directly). After D3.b: closure must be SU(2) (only remaining closed subgroup). ~200-500 LoC. | n/a | **D.2** | none |
