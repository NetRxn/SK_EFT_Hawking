...
| **Wave 2c.4a-R4.2.d.4.3.b+** | Path-(i) Phase D4.3.b+: rule out BD_{4n} candidate + complete Cartan substrate | ⏳ **NEXT-SESSION ENTRY**. With D4.3.a shipped, only finite candidates left are binary dihedral BD_{4n} (n ≥ 10) where σ_Fib_1_SU sits in the cyclic Z_{2n} part. D4.3.b would prove σ_Fib_2_SU also has order 20 (DONE in D4.3.a) AND must be in the cyclic part of BD_{4n} (since outside-cyclic elements have order exactly 4 in BD_{4n}, contradicting σ_Fib_2_SU's order 20). But then σ_Fib_2_SU is in cyclic ⟨σ_Fib_1_SU⟩-like subgroup, forcing commutation — contradicts D1. This requires the "BD_{4n} outside-cyclic order is 4" structural fact (Mathlib substrate gap). D4.3.c+ would complete the positive-dim case (compact 0-dim ⟹ finite, positive-dim ⟹ 1-param subgroup ⟹ torus containment ⟹ N(T) ruleout from D3.a). Multi-session ~500-1500 LoC. **Constraint**: scoped HBS axiom is OFF THE TABLE per 2026-05-14 directive. | n/a | **D.2** | none |
