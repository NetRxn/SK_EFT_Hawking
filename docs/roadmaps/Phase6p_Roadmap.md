...
| **Wave 2c.4a-R4.2.d** | Density discharge: `closure(Set.range ρ_Fib_SU2) = Set.univ` | ⏳ **NEXT-SESSION ENTRY** (R4.2.c ✅ done, `ρ_Fib_SU2` now exists in SU(2)). Substantial substrate ship OR scoped axiom; user sign-off required for axiom path. **Path (i) constructive**: Weyl-equidistribution (Mathlib has via `AddCircle`) + Euler-axes covering for SU(2). Requires substrate ship: 2 non-commuting infinite-order SU(2) rotations with non-parallel irrational-angle axes generate dense subgroup. ~500-1500 LoC. Mathlib lacks: matrix log with Lipschitz bounds, classification of closed subgroups of SU(2), Tits alternative / free-group-in-Lie-group. **Path (ii) scoped axiom**: ship `axiom ρ_Fib_SU2_dense : closure (Set.range (fun (g : BraidGroup 3) => (ρ_Fib_SU2 g : Matrix _ _ _))) = (Set.univ : Set (Matrix.specialUnitaryGroup (Fin 2) ℂ))` (or analogous) citing Hormozi-Bonesteel-Simon 2007 + Bonesteel et al. 2005 + FLW 2002 + Reichardt 2005. ~5 LoC + AXIOM_METADATA entry. Project axiom count: 1 → 2 (cited, specific, primary-source-backed). | n/a | **D.2** | none |
