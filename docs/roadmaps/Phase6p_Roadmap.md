...
| **Wave 3a.2.2c-followup** | Full 30-deep Rouabah Frobenius `native_decide` over Mat2K_40_Ext | ⏳ **NOT STARTED — REVISED COST ESTIMATE 2026-05-14 AM**. Attempted single-shot `native_decide` on `frobNormSq_ext (fibRep3Qubit rouabah_hadamard) hadamardTarget_ext = <literal>` exceeds 2-minute timeout. Root cause: 30 layers of 2×2 matrix multiplication over QCyc40Ext, where each QCyc40Ext multiplication expands to 4 QCyc40 multiplications, and the intermediate matrix-entry coefficients grow unboundedly under accumulation (rational coefficients in the degree-16 cyclotomic basis can double bit-width per layer). Per RouabahExplicit.lean §9 docstring (anticipated risk R1): fall back to **split-braid into halves** with intermediate-norm-bound lemmas (~150-250 LoC, not the original ~30 LoC). Substrate (Mat2K_40_Ext + fibRep3Qubit + frobNormSq_ext + F²=I + Yang-Baxter) remains ready. Quality improvement, not blocking — Hadamard ε~6.57e-3 already established via Rouabah paper citation. | n/a | **D.2** | none |
