...
| **Wave 2c.4a-R4.2.d.4** | Path-(i) Phase D4: final closure → DenseInSpecialUnitary | ⏳ **NEXT-SESSION ENTRY**. With D2 + D3.a + D3.b shipped, all closed subgroups of SU(2) except SU(2) itself have been ruled out via informal argument (proof-by-docstring). D4 lifts this to formal `closure(range ρ_Fib_SU2) = univ` then mechanical composition with `fibonacci_density_conditional` (shipped D1) → full `DenseInSpecialUnitary 3 2 _` for ρ_Fib_SU2. Substantive step requires either (a) Mathlib4 closed-subgroup classification for SU(2) (currently substrate gap — Cartan's closed-subgroup theorem + classification of dim-0 + dim-1 + dim-3 closed subgroups of SU(2)), or (b) direct accumulation argument using the structural facts (order-20 + non-commute + non-N(T) + non-scalar) to conclude closure = univ via Mathlib's topological closure machinery. Approach (b) likely ~500-1500 LoC of in-tree topological substrate. | n/a | **D.2** | none |
