...
| **Wave 2c.4a-R4.2.c** | det normalization + `ρ_Fib_SU2 : BraidGroup 3 →* SU(2)` MonoidHom | ⏳ **NEXT-SESSION ENTRY** (R4.2.b.3 ✅ done, YB matrix relation now available). Add `ω = exp(πi/10)` det-normalization (`ω · σ_Fib_1, ω · σ_Fib_2 ∈ SU(2)`); verify Yang-Baxter survives det-rescaling (`σ_Fib_yang_baxter` lifts via scalar multiplication since YB is multilinear); apply `braidGroup3HomFromPair` (R4.1) to get `ρ_Fib_SU2 : BraidGroup 3 →* Matrix.specialUnitaryGroup (Fin 2) ℂ`. ~100-150 LoC. | n/a | **D.2** | none |
