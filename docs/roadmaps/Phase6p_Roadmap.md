...
| **Wave 2c.4a-R4.2.d.4.2+** | Path-(i) Phase D4.2+: discharge `H_Fib = ⊤` | ⏳ **NEXT-SESSION ENTRY**. With D4.1 shipped, the residual unconditional density target is `H_Fib = ⊤` (a `Subgroup` equality). Substantive step requires either (a) Mathlib4 closed-subgroup classification for SU(2) (currently substrate gap — Cartan's closed-subgroup theorem + classification of dim-0 + dim-1 + dim-3 closed subgroups of SU(2)), or (b) direct accumulation argument using the structural facts (order-20 + non-commute + non-N(T) + non-scalar) to conclude `H_Fib = ⊤` via Mathlib's topological closure machinery. Approach (b) likely ~500-1500 LoC of in-tree topological substrate. Alternative escalation path: scoped HBS axiom on `H_Fib = ⊤` (with explicit user sign-off, citing Hormozi-Bonesteel-Simon 2007). | n/a | **D.2** | none |
