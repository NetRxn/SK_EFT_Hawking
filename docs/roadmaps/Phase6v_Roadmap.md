...
| **9.A** ✅ | A | Full k-dependent Hamiltonian bridge + Majorana-basis sewing matrix (SHIPPED 2026-05-26 PM) | `BdGHamiltonianNbRe.lean` (extended ~260 LoC) | (1) ✅ `H_BdG_NbRe sc k1 k2 k3` with sin-k kinetic; (2) ✅ `H_BdG_NbRe_TRInvariant_at_TRIM_holds`; (3) ✅ `bdGSewingMatrix` (Majorana-basis); (4) ✅ antisymmetric by construction (honest disclosure: from-TRS-derivation requires band-basis eigendecomposition, future-wave); (5) ✅ NbRe `pfaffianFin4 = -2`, Nb `= +1`; (6) ✅ sign-level bridge to substrate `pfaffianSignAtTRIM` |
| **9.E** ✅ | E | TRIM refactor + concrete orthorhombic NbRe model (SHIPPED 2026-05-26 PM) | `TRIMParameterization.lean` (extended ~190 LoC; §7 refactor via generic bridge, not in-place) | (1) ✅ `[Fintype TRIM] [DecidableEq TRIM]` parameterization via `pfaffianSignAtGeneric` + `fuKaneInvariantGeneric`; (2) ✅ hexagonal `(TRIM := Fin 4)` bridge: `fuKaneInvariantGeneric_hex_nbRe = -1`, `_elementalNb = +1`; (3) ✅ `orthorhombicNbReParameters` Ima2 capsule (Tc=8.5K, Triplet, noncentrosymmetric); (4) ✅ `orthorhombicNbRe_fuKaneInvariant_neg_one = -1` DERIVED via structural rule (NOT hand-crafted) |
| **9.C** ✅ | C | η-invariant / Z₁₆ derivation route (SHIPPED 2026-05-26 PM) | `NbReDIIIToPinPlusZ16.lean` (extended ~110 LoC) | (1) ✅ `nbReEtaInvariant sc := ZMod.toAddCircle (diiiBdGToZ16 sc)`; (2) ✅ `nbRe_nbReEtaInvariant_ne_zero` via biconditional + injectivity; (3) ✅ `elementalNb_nbReEtaInvariant_eq_zero`; (4) ✅ `diiiBdGToZ16_derived_from_eta_invariant` biconditional (via `ZMod.toAddCircle_injective`) |
| **9.D** ✅ | D | 3D winding-number universality theorem (SHIPPED 2026-05-26 PM) | `NbReWindingNumber.lean` (extended ~75 LoC) | ✅ Cheaper-alternative path taken: `IsSatoFujimotoIntegerWinding` predicate + `windingNumberSurrogate_isSatoFujimoto` (universal over all 6 channel-centrosym quadrants) + `windingNumber_uniqueness_mod_2` universality + biconditional characterization. Full BZ-integral construction deferred as future-wave follow-up (~400-800 LoC) |
