...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — THE REMAINING STRUCTURAL BRICK** dnStepFG_sud (S82) + the (B) discharge (S102) +
    h_regime from IFT `matrixLog` to `matrixLogConcrete d Δ := matrixMercatorLog(Δ−1)`, with the regime
    now on the CONCRETE ball `‖Δ−1‖ ≤ d·2ε₀ < 1`. The large structural refactor; ε₀_sud
    (≈1/(8·1024·d^16)) is tiny enough for all smallness bounds (incl. `d²·2ε₀ ≤ 1/8`). With all four
    regime conjuncts now concrete, R4 is the last step to UNCONDITIONAL `h_regime`.
