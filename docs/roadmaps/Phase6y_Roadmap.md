...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **✅✅✅ S140 (`9d0d931`, `GenericSUdSkHeadlineCascadeConcrete.lean`) — R4 CAPSTONE: UNCONDITIONAL
      ...
      **(D) density + length-polylog ONLY — NO `h_regime`**; base finder `findNearestInCover_SUd` + (B)
    ...
    - **✅ Length-composition DONE (2026-05-28, commit `9343c15`)**: `concrete_length_polylog_holds`
      (`GenericSUdConcreteLengthPolylog.lean`) discharges the cascade's `h_length_polylog` (F#4 conjunct) by
      composing `skLengthPolylogBound_sud_holds` ∘ S141 ∘ S55 base-finder bound; `skHeadline_FreeGroup_SUd_from_density`
      reduces the headline to (D) density + word-length bundle (kernel-only).
    - **✅ Per-alphabet headline-of-witness DONE (2026-05-28, commit `45b793c`)**:
      ...
      single input — a `ClosureDenseWitness` for the alphabet (densityFromWitness is unconditional since Cartan
      S.2g is discharged). (B), regime, F#4 length, density-from-witness all internal; kernel-only.
    - **✅ AS-SHIPPED UPDATE (2026-05-30, post adversarial review) — the (D) witnesses are SHIPPED
      UNCONDITIONALLY; the "SOLE substantive frontier" text below is STALE.** The `ClosureDenseWitness`
      discharge described as remaining was subsequently completed (2026-05-28): `cliffordCCZSU8ClosureDenseWitness`
      / `trappedIonSU4ClosureDenseWitness` are real `def`s (`CliffordCCZSU8WitnessFull.lean` /
      `TrappedIonSU4WitnessFull.lean`), the tracked Props `*_v4_witness_tracked` are now DISCHARGED theorems,
      and the per-alphabet UNCONDITIONAL headlines ship as
      **`cliffordCCZSU8_solovayKitaev_headline_unconditional`** / **`trappedIonSU4_solovayKitaev_headline_unconditional`**
      (+ `trappedIonSU4FullHeadline_holds`) — kernel-pure, verified by the 6y adversarial review
      (`docs/validation/phase6uxyz_adversarial_review_2026_05_30.md`). NOTE: there are **no** theorems matching
      the `solovayKitaev_dawson_nielsen_quantitative_su{4,8}_..._tight` pattern named in the stale text below —
      use the `*_headline_unconditional` names. HONEST-SCOPE caveat (carried over): the SU(8) instance ships
      **Clifford+T at SU(8)** — its density comes from the `{H,T,CNOT}` sub-alphabet and **CCZ is present in the
      10-token alphabet but UNUSED in the witness construction** (verified 0 refs); the faithful *literal*
      Clifford+CCZ density (CCZ-essential, no `T`) is **Phase 6z**'s deliverable (shipped there as
      `cliffordCCZLiteral_dense`), NOT 6y's. The genuine remaining 6y frontier is only the M-S substantive
      Mathlib extraction.
    - **(HISTORICAL, pre-2026-05-28 plan) REMAINING — the (D) `ClosureDenseWitness` discharge.** The named
      UNCONDITIONAL headlines now follow IMMEDIATELY
      from `trappedIonSU4_headline_of_witness` / `cliffordCCZSU8_headline_of_witness` once the per-alphabet
      `ClosureDenseWitness` is constructed UNCONDITIONALLY (then the tracked Props
      `trappedIonSU4_v4_witness_tracked` / `cliffordCCZSU8_v4_witness_tracked`). This is the deep universality
      math — Brylinski-Brylinski 2002 (SU(4) MS+1Q entangler universality) / Aaronson-Gottesman 2004 (SU(8)
      Clifford+CCZ) — requiring the `ClosureDenseWitness` fields: spanning 𝔰𝔲(d) tangents + flow-line
      containment `exp(t·Xᵢ) ∈ H_of_G gs`. [Resolved per the AS-SHIPPED UPDATE above.]
    - **Deep-research blueprint REQUESTED 2026-05-28** for the (D) witness discharge (per CLAUDE.md hard-proof
      protocol): `Lit-Search/tasks/submitted/phase6y_closure_dense_witness_su4_su8_blueprint.md` — asks for the
      explicit 𝔰𝔲(4)/𝔰𝔲(8) tangent set, the `hX_spans` finite-linear-algebra certificate, and the `hX_flow`
    ...
    - **🎯 T-A2′ SU(8) RESOLUTION (2026-05-28, user-authorized) — ship A′ now + DR-spike literal.** The
      ...
      literal `{H,S,CNOT,CCZ}`**. **Honest labeling (REQUIRED):** A′ ships "**Clifford+T at SU(8)**" — its
      ...
      EntanglerSpread,FactorConj,TangentSpan}.lean`). A′ therefore does **not** deliver instance (2)'s "CCZ
      compiled not primitive" intent; that is the literal follow-on. The reason literal is hard is NOT Trotter:
      `{H,S,CNOT,CCZ}` is all finite-order (no per-qubit ∞-order gate), so the FIRST continuous flow must be
      manufactured by a **von Neumann/Kronecker accumulation** of a CCZ·Clifford product word with an
      irrational eigen-angle — Trotter only propagates flows, it cannot seed them. (The prior DR blueprint's
      cheap route-(a) Trotter cost assumed continuous-parameter gates — true for SU(4) MS, false here.)
    - **Deep-research SPIKE REQUESTED 2026-05-28** (literal Clifford+CCZ seed):
      `Lit-Search/Tasks/phase6y_literal_cliffordCCZ_su8_irrational_seed_spike.md` — asks for the explicit
      irrational-angle seed element `g(H,S,CNOT,CCZ) ∈ SU(8)`, its irrationality proof (cyclotomic/Niven), the
      Lean-v4.29.1 formalization strategy (`AddSubgroup.dense_or_cyclic`, `IsPrimitiveRoot`,
      `Polynomial.cyclotomic`, torus-closure → one-parameter subgroup), and how it composes with the
      already-shipped (reusable, alphabet-agnostic) A′ conjugation spread + Trotter propagation. Returns →
      literal `{H,S,CNOT,CCZ}` witness → faithful instance (2) as a tracked strengthening.
  ...
  **⚠️⚠️ Crux (iii) ROOT-CAUSE finding (Sessions 112-113 attempts — instance-pollution diamond)**: the per-term derivative `HasDerivAt (fun s : ℝ => c_n • ((↑s:ℂ)•X)^(n+1)) (((-1)^n·(↑s)^n) • X^(n+1)) s` is mathematically clean (coefficient `(n+1)·c_n = (-1)^n` collapses; assemble via `Complex.ofRealCLM.hasDerivAt` + `HasDerivAt.pow` + `HasDerivAt.smul_const`). **THE DEEP BLOCKER (diagnosed via `lean_run_code`)**: `Matrix.linftyOpNormedAddCommGroup` (a *local instance* the whole file needs for `‖·‖`) supplies its OWN `AddCommGroup` and hence its own ℝ-module path (`NormedSpace.complexToReal`-derived, `inst.toModule`), which is a **NON-DEFEQ diamond** with the standard `Matrix.addCommGroup`/Pi ℝ-module that the goal and the `Complex.ofRealCLM`/`smul_const` calculus machinery resolve to. Concretely: `smul_const`'s output `HasDerivAt` lands in `linftyOpNormedAddCommGroup.toAddCommGroup`+`inst2.toModule`, but the goal (post-`rw`) sits in `Matrix.addCommGroup`+default `module` — `exact` fails with a genuine type mismatch, and the surfaced "`IsScalarTower ℝ ℂ (Matrix ℂ)` not synthesized" is a *symptom* of `smul_const` searching the tower over the wrong module path. **This is instance pollution specific to doing CALCULUS (ℝ-module + norm together) on `Matrix ℂ` under the linftyOp norm** — bare `inferInstance` finds each instance, but `smul_const` mixes the two module diamonds. **✅ RESOLVED (Session 112, `ec55dbb`)**: option (c) — bundle the ℂ-smul into a continuous ℝ-linear map `g : ℂ →L[ℝ] Matrix ℂ`, `g z = z • X^(n+1)` (`(id ℂ).smulRight (X^(n+1)) |>.restrictScalars ℝ`), and differentiate via `g.hasFDerivAt.comp_hasDerivAt` — sidesteps `smul_const` and the module diamond entirely. `hasDerivAt_matrixMercatorLog_term` SHIPPED (per-term derivative `d/dt[c_n•((↑t)•X)^(n+1)] = ((-1)^n·(↑t)^n)•X^(n+1)`, kernel-only). **✅ CRUX (iii) COMPLETE (Sessions 113-114, `78f93bb`/`61a02bd`)**: `summable_matrixMercatorLog_deriv_series` + `tsum_matrixMercatorLog_deriv_eq` (`∑' ((-1)^n(↑t)^n)•X^(n+1) = (1+(↑t)•X)⁻¹·X` via `geom_series_eq_inverse`) + `hasDerivAt_matrixMercatorLog_path` (`d/dt[matrixMercatorLog((↑t)•X)] = (1+(↑t)•X)⁻¹·X` for `|t|·‖X‖<1`, assembled via `hasDerivAt_tsum_of_isPreconnected` on `ball 0 R`, `|t|<R<1/‖X‖`). **✅ CRUX (iv) COMPLETE (Session 115, `df209e7`)**: `hasDerivAt_exp_path` — `d/dt[exp(A t)] = exp(A t₀)·A'` for a commuting path (eventual-commute form, S116 `b842a73`). **✅✅ BRICK 2 COMPLETE — EXP/LOG ROUND-TRIP SHIPPED (Session 118, `6867dc8`)**: `exp_matrixMercatorLog : ‖X‖<1 → NormedSpace.exp (matrixMercatorLog X) = 1 + X`. Built across S116-S118: `hasDerivAt_exp_neg_matrixMercatorLog_path` (u'(t)) + `hasDerivAt_one_add_smul` (v'(t)=X) + `hasDerivAt_round_trip_factor` (f'≡0 via product rule + `(1+Y)⁻¹·X·(1+Y)=X` cancellation) + `Convex.is_const_of_fderivWithin_eq_zero` on `ball 0 (1/‖X‖)` + extraction via `exp_add_of_commute`. The Mercator log is now a CONCRETE-RADIUS right-inverse of exp (shifted by 1) on the NAMED ball `‖X‖<1` — the concrete-radius analog of the existential IFT `expAmbient_matrixLog`. **Remaining brick 3**: identify `matrixMercatorLog(Δ−1) = matrixLog d Δ` for `Δ` near `1` (via local injectivity of exp near 0 — both `mLog(Δ−1)` and `matrixLog d Δ` exponentiate to `Δ`, and exp is injective on a concrete ball — using `exp_matrixMercatorLog` + `expAmbient_matrixLog`) + a concrete ball ⊆ `target`, discharging the regime θ-bound (`norm_matrixMercatorLog_le_two_mul`, K=2) + `Δ∈target` on the concrete calibration ball. **Historical re-scope options (now moot for crux iii, but relevant if crux iv hits the same diamond — it won't, same CLM fix applies)**: (a) reconcile the diamond — prove `linftyOpNormedAddCommGroup.toAddCommGroup`'s ℝ-module is defeq/`Subsingleton`-equal to the standard one and force one path via `@`-explicit instances or `convert ... using 1` + `AddCommGroup`-congruence (painful but local); (b) **PIVOT to a non-derivative round-trip proof** (Cauchy product of the exp/log series, or the dense-diagonalizable + continuity route) that avoids ℝ-module calculus entirely — likely the cleaner path given the diamond; (c) prove the round-trip in a norm whose `NormedAddCommGroup` shares the standard Matrix `AddCommGroup` (e.g. a Pi/Frobenius norm) then transfer the equation (norm-independent) back. The mathematical skeleton (Neumann `(1+t•X)⁻¹` off-the-shelf; `hasDerivAt_tsum_of_isPreconnected` on `ball 0 (1/‖X‖)`) is unchanged once the module path is settled.
  2. **(D-SU4) Density witness at SU(4) trapped-ion** — ✅ SHIPPED 2026-05-28 (`TrappedIonSU4WitnessFull.lean`, `trappedIonSU4ClosureDenseWitness` + `trappedIonSU4_solovayKitaev_headline_unconditional`, kernel-pure).
  3. **(D-SU8) Density witness at SU(8)** — ✅ SHIPPED 2026-05-28 (`CliffordCCZSU8WitnessFull.lean`, `cliffordCCZSU8_solovayKitaev_headline_unconditional`, kernel-pure) — at the **Clifford+T** alphabet (CCZ present-but-unused); the faithful literal Clifford+CCZ density is Phase 6z's `cliffordCCZLiteral_dense`.
