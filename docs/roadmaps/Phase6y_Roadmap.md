...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **REMAINING R4 sub-bricks** (final induction stretch, S89-S102 analog onto the concrete recursion):
      concrete `skApproxC_generic_sud_concrete_succ_rho_val` (ρ_hom unfolding, S92 analog) + concrete
      `skApproxC_sud_succ_error_le_combine` (S95 analog, structural telescoping `≤ Cstab+Ccubic`);
      concrete `skApproxC_sud_succ_super_quad_valid` (S101 analog — composes √ε-norm S135 + S136 cubic
      term + the log-agnostic stability-term S96 + numeric chain S99, both reused as-is); concrete
      **(B) bound** `SkApproxCSuperQuadraticBound_generic_sud_concrete_holds` (S102 analog, Nat.rec wrap)
      with the regime hypothesis in CONCRETE-log form — **dischargeable UNCONDITIONALLY via S120-S128**
      (no longer a threaded existential), discharging the θ-bound from `regime_thetabound_concrete` (S120)
      + the IH; then the concrete cascade (`skHeadline_FreeGroup_SUd_cascade_B_discharged` analog).
      ε₀_sud (≈1/(8·1024·d^16)) tiny enough for `(n+2)²·2ε₀ ≤ 1/8`. Last step ⟹ S.6/T-X′.5 reduce to
      (D) + length only.
