...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **REMAINING R4 sub-bricks**: re-point the rest of the (B) chain via `dnStepFG_fromLog` — concrete
      cubic remainder (S86 analog), F/G-norm bound (S85 analog), super-quad induction (S89-S102 analog),
      and the (B) discharge → then an UNCONDITIONAL `h_regime` (concrete-log form) feeding the cascade
      (`skHeadline_FreeGroup_SUd_cascade_B_discharged` analog, regime stated with `matrixMercatorLog`).
      ε₀_sud (≈1/(8·1024·d^16)) is tiny enough for `(n+2)²·2ε₀ ≤ 1/8`. This is the last step to
      UNCONDITIONAL `h_regime` ⟹ S.6/T-X′.5 reduce to (D) + length only.
