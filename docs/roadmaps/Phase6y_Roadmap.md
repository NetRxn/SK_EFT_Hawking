...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **REMAINING R4 — THE FINAL BRICK**: the concrete **(B) bound**
      `SkApproxCSuperQuadraticBound_generic_sud_concrete_holds` (S102 analog, Nat.rec wrap over recursion
      depth applying S138 at each level). **The payoff: it takes NO `h_regime` hypothesis** — the per-level
      θ-bound is discharged internally from `regime_thetabound_concrete` (S120) + the IH, and the
      calibration `(m+2)²·‖V_n−U‖≤1/8` from `‖V_n−U‖≤ε_k≤2ε₀_sud` + `(m+2)²·2ε₀_sud≤1/8`. Then the concrete
      cascade (`skHeadline_FreeGroup_SUd_cascade_B_discharged` analog) yields an **UNCONDITIONAL** SU(d)
      headline (no threaded existential regime). ε₀_sud (≈1/(8·1024·d^16)) tiny enough for all bounds.
      Last step ⟹ S.6/T-X′.5 reduce to (D) + length only.
