...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **REMAINING R4 sub-bricks**: a concrete recursion `skApproxC_generic_sud_concrete` (calling
      `dnStepFG_sud_concrete`) + the super-quad **induction telescoping** (S89-S102 analog: multi-level
      error accumulation `‖ρ_hom(skApproxC_concrete n U) − U‖ ≤ ε_seq` from the three concrete single-step
      bounds) → concrete **(B) bound** → UNCONDITIONAL `h_regime` (concrete-log form) feeding the cascade
      (`skHeadline_FreeGroup_SUd_cascade_B_discharged` analog). ε₀_sud (≈1/(8·1024·d^16)) tiny enough for
      `(n+2)²·2ε₀ ≤ 1/8`. The induction is the bulk of the remaining work. Last step ⟹ S.6/T-X′.5 reduce
      to (D) + length only.
