...
**Truly remaining substantive work (post S108 — (B) is DISCHARGED; 4 named items + 1 review)**:
  ...
  **Re-point sub-brick breakdown (S119 scouting — for efficient next-session execution):**
  ...
  - **R4 RE-POINT — IN PROGRESS (foundation + exp-delta SHIPPED S129/S130)**. Key structural insight
    ...
    - **REMAINING for S.6 + per-alphabet T-X′.5** (no existential blocker remains): (a) the **word-length
      polylog bound** for the concrete recursion `skApproxC_generic_sud_concrete` (transfers structurally
      from the IFT `skLength_sud`/`skLength_at_skLevel_polylog_le_generic` — the word recurrence is
      log-independent, uniform `5^n`-style growth); (b) the **(D) density witness** (from S.5's Cartan-v4
      closure for generic SU(d); Brylinski-Brylinski SU(4) / Aaronson-Gottesman SU(8) for the per-alphabet
      density witnesses); then the named S.6 + T-A1′.5 + T-A2′.5 headline theorems instantiate
