...
- ✅ **6AG-4 / Ask 4 DONE — bridges PROVEN, Haar formula FENCED (2026-06-02, commit `5f5ad76d`).** `GateFidelityBridge.lean`: `one_sub_sqrtFidelity_output_le_diamondDist` (`1−√F((K₁⊗id)ρ,(K₂⊗id)ρ) ≤ diamondDist K₁ K₂`, composing shipped FvdG-lower `one_sub_sqrtFidelity_le_traceDist` with `le_diamondDist`) + contrapositive `sqrtFidelity_output_ge_one_sub_diamondDist`. Root-imported, kernel-pure. **`avgGateFidelity_eq=(d·F_e+1)/(d+1)` for general d FENCED** (Explore fan-out grep-verified, 2026-06-02): Mathlib v4.29.1 has Haar-on-groups + sphere measure (`HaarToSphere`) but **NO Weingarten calculus, NO unitary t-design, NO pure-state twirl second-moment closed form** ⟹ the (d·F_e+1)/(d+1) Horodecki/Nielsen identity is not formalizable without a substantial new Weingarten build; single-qubit instance already shipped (`HaarPauli.teleportAvgFidelity_horodecki_unconditional`).
...
- ⏳ **Remaining public fence: Ask-4 Haar `avgGateFidelity_eq` — toehold confirmed soft.** Scout found the Gaussian→sphere route (`integral_fun_norm_addHaar` polar decomposition + Mathlib Gaussian integrals) bypasses the absent Weingarten machinery; reduces to ONE missing lemma (degree-4 complex sphere moment = Gaussian Wick contraction in n variables, ~50–200 LOC self-contained measure theory). Not a wall — a discrete build.
