...
- **Session 4 — `BundleRiemann.lean`.** Tensoriality discharges via `TensorialAt.mkHom₃` (build from `mkHom₂` if not already in Mathlib) + bundled `CovariantDerivative.riemann` + first Bianchi at bundle level for torsion-free + specialization theorem bridging bundle-level Riemann to Sessions 1+2 coordinate Riemann when the manifold is `EuclideanSpace ℝ (Fin 4)`. Target ~8–12 substantive theorems. **STATUS: PENDING (blocker analysis below).** **2026-04-30 scoping pass identified two upstream-Mathlib gaps** that the original Session 4 plan undercounted: (a) `TensorialAt.mkHom₃` is **NOT** in Mathlib's pinned commit `8850ed93` — only `mkHom` and `mkHom₂` are; `mkHom₃` is a ~80-line trilinear-map analog of `mkHom₂` (~50 lines) that needs to be built before bundled `riemann` can be defined. (b) Mathlib's `VectorField.LieBracket` API has `leibniz_identity_mlieBracket_apply` (Leibniz form) and `mlieBracket_swap_apply` (antisymmetry) but **NOT** a direct `mlieBracket_neg_left`/`mlieBracket_neg_right` lemma; deriving the inner-bracket antisymmetry needed for the cyclic Jacobi `[X,[Y,Z]] + [Y,[Z,X]] + [Z,[X,Y]] = 0` requires either (i) constructing `mlieBracket_neg` from `mlieBracket_smul_right` with `f = -1` constant + `mdifferentiable_const` + Pi.neg_apply machinery (~30 lines), or (ii) a careful direct derivation routing all rewrites through `mlieBracket_swap_apply` only on the outer bracket and using algebraic regrouping to avoid inner antisymmetry. Both paths are realistic but neither is a 30-minute deliverable — Session 4 is a 1.5-2 hr session, not a single-response sub-task. **Strategic split applied 2026-04-30:** Session 4a = cyclic Jacobi for `mlieBracket` (independent Mathlib-PR-shaped contribution) + abstract bundle-level first Bianchi for `bundleRiemannAux` parameterized by section-level torsion-free + cov-additivity hypotheses. Session 4b = `mkHom₃` + bundled `riemann` + smooth-section bookkeeping that discharges Session 4a's abstract hypotheses + specialization-to-coord theorem. **Both Session 4a + 4b status: PENDING — gated on user authorization for the Session 4 time allocation.**
- **Session 5 — Levi-Civita uniqueness + Mathlib PR.** `IsLeviCivita` predicate + uniqueness at bundle level (Koszul-bilinear-form argument over smooth vector fields) + Bonn outreach (the staged email script in `temporary/working-docs/phase6f1_curvature_architecture_decision.md`) + Mathlib PR submission. Paper44 closes out as a real Mathlib-contribution bundle. **STATUS: PENDING.**
...
- [ ] (Session 4) Full coord-level differential second Bianchi `∇_λ R + ∇_μ R + ∇_ν R = 0` for the entire Riemann (cubic-Γ + bilinear pieces) via specialization from Session 3 bundle-level Jacobi.
- [ ] (Session 4) Cross-bridge: 6f.2 `∇^μ G_{μν} = 0` discharge using above.
...
- [ ] (Session 4) `TensorialAt.mkHom₃` (build or import).
- [ ] (Session 4) Bundled `CovariantDerivative.riemann` via `mkHom₃`.
- [ ] (Session 4) Bundle-level first Bianchi for torsion-free.
- [ ] (Session 4) Specialization theorem: bundle Riemann on `EuclideanSpace ℝ (Fin 4)` reduces to coordinate Riemann from Sessions 1+2.
- [ ] (Session 5) `IsLeviCivita` predicate + uniqueness at bundle level.
...
- **Session 4:** `BundleRiemann.lean` + (possibly) `TensorialAtMkHom3.lean` (~700 LOC target); paper44 §6 rewrite to bundle scope; roadmap state checkboxes flipped; memory `project_phase6f_w8_session4_shipped.md`.
- **Session 5:** Module polish for upstream; outreach email sent; Mathlib PR draft + submission; paper44 final state reflects PR target; memory `project_phase6f_w8_session5_shipped.md`.
