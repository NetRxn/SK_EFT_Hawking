...
**REMAINING W3:** (a) add `prose`/`tier` fields + merge the doc's 4 cosmology Props (H_VestigialModeIsGraviton, H_DESICompatibility, H_RT_Formula_Valid, TPFConjecture) + the Phase-6r SymTFT Props into HYPOTHESIS_REGISTRY; (b) register the 21 gap Props (or downgrade local ones into `TRACKED_HYPOTHESIS_NON_LOAD_BEARING`); (c) build `scripts/render_tracked_hypotheses.py` + `tracked_hypotheses_fresh`; (d) escalate `tracked_hypothesis_ledger` to hard-fail (`passed = not gap`); (e) add Invariant #16.
...
| W3 tracked-hyp ledger sweep (R3) + Inv #16 | IN PROGRESS |
| W4 formula content-grounding (R1) | NOT STARTED |
