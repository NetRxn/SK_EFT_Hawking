...
>
> **Deferred (tracked, not lost):** reviewer items I4 (`phononLimited_iff_psd_lt` should compare NEPs not NEP², and needs an inhabitation witness), I5 (`matchedBudget_antitone_psd` in 6EB + a `phonon_only_error_floor`, which is what makes the phase's *"no thermal detector of this class beats this floor"* thesis a theorem), I7–I10, and M1–M8. Third phase of the `6E*` series (*verified device-physics metrology*). Consumes 6EB (NEP/ENBW layer, **COMPLETE 2026-07-29**); consumed by 6EE (composite ceilings). See `Phase6EA_Roadmap.md` for the series framing.
