...
| **M1** | dead `hGne` **dropped** (not kept-and-disclosed) from `responsivity_etf_correction`, `nep_bare_understates_by_one_plus_loopGain`, `johnsonNEP_naive_understates_by_one_sub_loopGain` — following Wave 1's `effectiveTimeConstant_eq_div` precedent that unused positivity wrongly suggests a stability fact | branch disclosed instead by `responsivity_of_G_eq_zero` |
...
2026-07-29 physics correction had already renamed it to `johnsonNEP_naive_understates_by_one_sub_loopGain`
