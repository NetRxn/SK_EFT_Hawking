...
| 6z.4 conjugation spread | ⏳ **SOLE REMAINING FRONTIER (~400 LoC, multi-session)** — witness `X = {Cⱼ X₀ Cⱼ⁻¹}` (flows free via `flow_conj_mem`, DONE & ready); `hX_spans` needs the Clifford orbit of the unknown `X₀` to span 𝔰𝔲(8). **CLEAN FORMALIZABLE ROUTE (2026-05-28):** (1) 63 Paulis `P_v` (v≠0) are a basis of 𝔰𝔲(8) — **`suEightTangent_spans` + `kronK8_linearIndependent` DONE** (`CliffordCCZSU8TangentSpan.lean`); (2) `Ad_{P_w}(P_v)=(−1)^⟨w,v⟩ P_v` (Pauli commutation — common eigenvectors); (3) characters `χ_v` distinct (symplectic non-deg) ⟹ any Pauli-invariant subspace is a coordinate subspace `⊕_{v∈S}ℝP_v`; (4) Clifford permutes the 63 lines transitively (finite `decide`-able orbit of the 12 generators); (5) ⟹ irreducible ⟹ orbit of any nonzero `X₀` spans. Steps 2–4 = the genuine rep-theory foundation (not in Mathlib). NB transitivity alone ≠ irreducibility; the distinct-characters step (3) is what makes (4) suffice. |
