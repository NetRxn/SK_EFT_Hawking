...
| 6z.2 first flow | 🟡 IN-PROGRESS (von-Neumann route). **2a ✅ SHIPPED** (`CliffordCCZSU8SeedFlow.lean`, `40c0592`, kernel-only): `H_of_G_literal_infinite` + `seedSU8_accPt_one` (`1 ∈ AccPt H_of_G`). **2b NEXT (large/multi-session)**: SU(d) von-Neumann 1-param-subgroup theorem — generalize `OneParameterSubgroupSU2` (`su2Log`/`expAmbient`/BW-on-𝔰𝔲(d)-sphere/integer-rounding) using `GenericSUd` matrix-log, consuming `seedSU8_accPt_one` → `∃ X₀≠0∈𝔰𝔲(8), ∀t exp(t•X₀)∈H_of_G`. |
