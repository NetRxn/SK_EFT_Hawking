...
| 6z.2 first flow | ✅ **COMPLETE** (von-Neumann route). **2a** (`CliffordCCZSU8SeedFlow.lean`, `40c0592`): `H_of_G_literal_infinite` + `seedSU8_accPt_one` (`1 ∈ AccPt H_of_G`). **2b** (`GenericSUdOneParameterSubgroup.lean`, inc3 `eae04fb` / inc3b `00ea621` / inc4 `8e6998d` / inc5 `84f755f`, all kernel-only): the full SU(d) von-Neumann 1-param-subgroup engine — near-verbatim SU(2)→SU(d) port (`su2Log`→`matrixLog d`, `expAmbient`→`expAmbient d`), capstone `vonNeumann_assemble_explicit_X_SUd` (`1∈AccPt H → ∃X≠0∈𝔰𝔲(d), ∀t ∃M∈H, M.val=exp((t:ℂ)•X)`). **2 first flow** (`CliffordCCZSU8SeedFlow.lean`, `3232576`): `seedSU8_first_flow` instantiates at d=8/`H_of_G` → `∃ X₀≠0∈𝔰𝔲(8), ∀t ∃M∈H_of_G, M.val=exp((t:ℂ)•X₀)`. lake build clean 8334. |
| ~~6z.3 SU(d) Trotter~~ | ❌ DROPPED (Gate 2 = BEST) |
| 6z.4 conjugation spread | ✅ **COMPLETE 2026-05-29** (`bb9aae0`, single /goal session, all kernel-pure). The Clifford-adjoint irreducibility on 𝔰𝔲(8), then orbit-spanning + witness, exactly along the 2026-05-28 route: (1) 63 Paulis basis (`suEightTangent_spans`, DONE); (2) `Ad_{P_w}(P_v)=(−1)^⟨w,v⟩P_v` (`kronK8_conj`); (3) char orthogonality `∑_w χ_aχ_b=64δ` (`sigmaSign8_orth`) → partial twirl (`pauli_twirl`); (4) Clifford transitivity on 63 lines (`clifford_label_transitive`, kernel `decide`). **Modules:** `GenLift` (6 H/S 3-qubit tableau lifts) → `LineTransport` (9 corollaries) → `Transport` (`all_lines_conj`: one line ⟹ all, via `ReflTransGen` involution-symmetry — NOT a 63-start decide, Inv #10) → `ConjClosure` (Ad-homomorphism + phase-blind involution conj) → `PauliWords` (single-qubit Paulis as forward H/S words, `conjP{1,2,3}`) → `KronK8Closure` (`conj_kronK8_closed`) → **`Irreducible` (`twirl_extract` + `clifford_irreducible_spans`: any nonzero Clifford-invariant submodule ⊇ 𝔰𝔲(8))** → `OrbitWitness` (`cliffOrbit_spans_su8`: orbit of seed spans; `Hlit` a `def` not abbrev + 9 closures as refine goals to avoid whnf-explosion) → `OrbitProps` (orbit elements skew/trace/flow via `flow_conj_mem`). |
| 6z.5 headline + Stage-13 | ✅ **COMPLETE 2026-05-29** (`bb9aae0`). `CliffordCCZSU8Density.lean`: `cliffordCCZLiteral_H_of_G_eq_top` (`H_of_G = ⊤`) + **`cliffordCCZLiteral_dense` (`IsDenseInSUd_gs`: literal ⟨H,S,CNOT,CCZ⟩ no-T dense in SU(8))**. Finite spanning subfamily (`Submodule.subset_span_finite_of_subset_span`) + `Finset→Fin` reindex (`cliffOrbit_sum_reindex`) → `ClosureDenseWitness` → unconditional `CartanFinalStep_SUd_v4_holds` / `H_of_G_eq_top_of_witness` / `densityFromWitness`. Full build 8926 jobs; 0 axioms (Inv #15), 0 sorries, no maxHeartbeats (Inv #10); headlines kernel-pure {propext, Classical.choice, Quot.sound}. **Stage-13 fresh-context adversarial review: GREEN — 0 BLOCKER / 0 REQUIRED / 0 ADVISORY** (statement honesty, vacuity, soundness red-flags, logical-gap hunt, irreducibility-hole hunt all PASS; CCZ confirmed essential — used only in the seed, not in the spreading). **⚠️ Closeout correction (2026-05-29, post-assessment):** the `bb9aae0` ship delivered the *density* (`cliffordCCZLiteral_dense`) but NOT the named headline `SolovayKitaevHeadline_FreeGroup_SUd` for the literal alphabet that this wave's title promised — it stopped at the witness, and the only shipped `…_headline_of_witness` consumer is hardcoded to the universal Clifford+CCZ+`T` set. **Gap closed** in new leaf `CliffordCCZSU8LiteralHeadline.lean`: `cliffordCCZLiteral_solovayKitaev_headline_unconditional` (word-length bundle `freeGroup_wordLength_su8_literal[_isFreeGroupLike]` + one application of `skHeadline_FreeGroup_SUd_from_density_auto` on `cliffordCCZLiteral_dense`). Kernel-pure `[propext, Classical.choice, Quot.sound]` (verified via `lean_verify` — **no `native_decide`** on the path); per-module `lake build` clean. Trivial wrap, no new math; the named compilation guarantee for the CCZ-essential T-free alphabet is now genuinely in-hand. |
