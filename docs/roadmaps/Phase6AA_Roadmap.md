...
- **W3 fidelity-composition core — IN PROGRESS** (`QuantumNetwork/{Swapping,Distillation}.lean`, kernel-pure): Werner swap `wernerSwapFidelity` `F_out=F₁F₂+(1−F₁)(1−F₂)/3` + `_comm` + `_mono_left/_right` over F≥1/4 (`b938428f`); **BBPSSW flagship** `bbpsswRecurrence_gt` — distillability on (1/2,1) reduces to the cubic `(1−F)(2F−1)(4F−1)>0` (`34209d9a`), plus `bbpsswSuccessProb_pos`.
- **NEXT (pending):** finish W3 — DEJMPS recurrence (Pauli-error pairing: squared pair is (00,11)=(I,Y)) + W-state Fortescue-Lo lower-bound ≥1 & finite-round D/(D+1) only (compose with `WStateQFT`); the Bell-diagonal Klein-4 swap map. Then **W2** (`NumericalBounds` Taylor-squeeze, needed for fidelity-under-decay), W1′ remaining anchors (physics link rate `τ=L/(c·p_link)`; BSM 50% as a **documented modeling constant**, NOT a tautological `≤1/2 := norm_num` — Calsamiglia-Lütkenhaus is physics, not algebra), then **W4** (model-parameterized decoherence + Werner-iterated end-to-end) and **W5** (general FIDELITY envelope + D6 §6 + Stage-13 + preprint).
