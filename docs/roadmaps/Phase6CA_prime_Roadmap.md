...
| **Q4 — Bloch-frame adapter** | ✅ 2026-07-20 | `TopologicalBand/BlochFrame.lean` + `BlochFHS.lean`: `AdmissibleBandFrame` (nonzero nearest-neighbor overlap as an EXPLICIT field — audit §3.3 honest choice, a gap does NOT imply nonzero overlaps; no global continuous frame built, per §3.2), `linkOfFrame` via phase normalization, **`blochLatticeChern_integrality` + `blochLatticeChern_rephase`** (both delegate to Q2 in 1–2 lines — integrality/gauge inherited for free), `blochProj_idem` from `blochPauli_sq` (BlochBundle genuinely load-bearing), `flatFrame` negative fixture with the lower-band eigenvector law discharged concretely. |
| **Q4-Lane-C — QWZ `C = ±1`** | 🔻 **SPIKE VERDICT: KILL/DEFER** (2026-07-20) | The "π/2-exact" repricing was WRONG and the spike caught it (its job): the d-vectors ARE exact lattice vectors, but the eigenvector entries carry `‖d‖ = √(non-square)` (Lean-verified `Irrational (√3)`, `Irrational (√5)`; a concrete overlap has irrational real part + imaginary part exactly 1), so overlap args are genuine arctans of quadratic irrationals — certification = **certified nested-radical arctan-vs-π inequalities** (the audit §3.4 deferred symbolic-inequality class). 2 of the packet's own 5 gate criteria FAIL ⟹ the packet's own stop-rule applies. **Positive findings preserved for any future attempt:** admissibility HOLDS on the 4×4 sampling (all 32 overlaps nonzero — no antipodal nearest neighbors; one-point two-chart split at the south pole suffices); `C = ±1` is TRUE (a defer, NOT a dead fork — no KERNEL_NOGO warranted); nothing downstream depends on it (non-vacuity already secured by the Q2 `Uwit` C=1 witness + the Q4 `flatFrame`). The lane's shipped deliverable is COMPLETE without QWZ; funding the certified-inequality wave is a future operator call. |
