...
- [~] **E1 · integral-topology foundation** — **integral `(co)homology → cup → cap → Kronecker → intersection form → σ÷16` + functoriality, homotopy invariance, Euclidean acyclicity, pair-LES, subdivision/excision ALL MERGED; the local iso `H₄(M|x;ℤ)≅ℤ` and the oriented fundamental class `[M]`/`intOrientation` are DONE (2026-07-05, `[M]` BUILT FROM SCRATCH — the "community-scale wall" of building 4-manifold integral topology in Lean is discharged).** IN PROGRESS (2026-07-06 CSC-PD tower → unconditional σ÷16): the integral **cohomology Mayer–Vietoris middle exactness** `(B)`, obtained by DUALISING the already-built integral homology excision iso `iotaEquivInt` (NOT rebuilding per-simplex subdivision). Shipped this session: the small-chains basis-subset split foundation (`SingularSmallChainsSplitInt`) + the reusable contraction engine "bounded-below acyclic projective ℤ-complex ⟹ Hom-dual acyclic" (`AcyclicProjectiveContractionInt`, the hard core). Remaining `(B)` work = de-risked plumbing (K = `ker(piMapInt)`) → `relCohomMv_exact_middleInt` → CSC-MV five-lemma → `intCapIso`/PD → σ÷16. Live brick state: master `LAB_NOTEBOOK_INDEX.md` FRONTIER.
