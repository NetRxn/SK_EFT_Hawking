...
> **🛑 CRITICAL SOUNDNESS FINDING (2026-07-13, KERNEL-CONFIRMED — supersedes the "`DataBordismGrp` baseline is sound" premise throughout this file AND the execution map).** The wide-latitude `hbound` attack discovered — and the lead independently kernel-verified — that the in-tree `Bordism` relation (`BordismGroup.lean:37-42`) has **NO `T2Space W` field**, so the non-Hausdorff bug-eyed interval **collapses** it: `DataBordismGrp` at `k=0` equals its grade (`dataBordismGMTied_mk_eq_iff_grade16_eq`), `BordismGrp X 0 I` is trivial, `[ℝP⁴]=0`. **⇒ the whole `DataBordismGrp`-based baseline is NON-FAITHFUL: L2/L3 (`omega4PinPlusGMTied_equiv_zmod16_via_kt_of_grade0`, the σ-route door, Freeze B, …) hold true-but-VACUOUSLY** — the `hbound` completeness they require is FREE on the degenerate relation (`grade0_eq_zero_of_nonHausdorff` IS that `hbound`, on the identical carrier, proven with zero geometric input). Every claim below treating the tied/thin `DataBordismGrp` carrier as a genuine faithful ℤ/16 is SUPERSEDED (incl. this session's "converged to `hbound`" framing — that node is *free*, not hard). **SURVIVES (genuine — 0 refs to the bad relation):** the E1 integral topology, the K3 *lattice*, the Rokhlin/Brown/ABK algebra, the D³/S²×D³ manifold atlas. **Genuine target = the honest `T2 + smooth (k=∞)` FAITHFUL carrier** (Fable's `T2TangentialBordism` repair: `hboundT2` genuinely OPEN; surjectivity onto ℤ/16 non-vacuous, lead-re-verified `{propext, Classical.choice, Quot.sound}`). Full record: no-go `nonhausdorff-bordism-collapse` (`SETTLED_FORKS.md` + `KERNEL_NOGO_REGISTRY` + generated `lean/SKEFTHawking/KernelNoGos.lean`) + notebook `LAB_NOTEBOOK.md` "★★★ SOUNDNESS FINDING". **⚠ STRATEGIC RE-BASE (T2+smooth restatement scope, this roadmap's rewrite) PENDING OPERATOR DIRECTION — §§ below are NOT yet corrected for it.**

> **📖 PROCESS NOTE — this doc OVER-PRESCRIBED routes; being corrected (operator-directed 2026-07-13).** Route / carrier / keystone SELECTION is the job of the **LIVE** layer — the notebook INDEX (`docs/dev-loops/Phase5qH/LAB_NOTEBOOK_INDEX.md`) + the machine atlas frontier (`/skeft-qa:frontier`) — NOT this durable strategic tracker. Everything below that names a specific carrier/route/keystone (the σ-route, the E1–E5 DAG-as-*status*, Freeze A/B, the 2026-07-06 keystone re-anchor, "converged to `hbound`") is HISTORICAL route-state: read for rationale, not current status. **The recurring failure this fixes (git history 07-03→07-13: repeated "re-anchor / resolve-keystone-contradiction / supersede-state-block" commits):** the durable decision that actually MATTERED — **§9.2 (2026-07-03): "ℤ/16 is a smooth-category statement; the capstone is targeted at the `k=∞` instantiation; the C⁰ fork must NOT be silently conflated"** — got BURIED under route-prescription, and the loop built the entire σ-route on the very `k=0` `DataBordismGrp` carrier §9.2 warned against. The 2026-07-13 soundness finding merely **kernel-re-proved §9.2**. The genuine target has ALWAYS been the smooth (`k≥1`) + T2 **faithful** carrier. **The durable-only rewrite** (objective + hard constraints [kernel-purity · no-axiom-without-signoff · smooth+T2+faithful carrier] + verified-source index + negative frontier + readiness gates; ALL route-state stripped out to the notebook/frontier) **is DEFERRED to the operator's fresh planning session.**
...
- [x] **E2 Rokhlin — COMPLETE + kernel-pure + WIRED to the Rokhlin node (2026-07-13).** Algebra (Brown metabolic, Gauss sums, surgery reduction = KT-LMS Lemma 3.7, GM null-bordant case); `TaylorKernelVanishing` (Taylor Thm 1.1) refactored to the EXACT split `⟺ KernelIsotropic ∧ KernelSpinVanishing` + the surface-realization split (`CharSurfaceRealization`: `ClassesEmbedded ↔ BasisEmbedded ∧ EmbeddedSumClosed`); `CharSurfaceRokhlinAssembly` **composes the leaves to `SmoothSpinManifold4.topo`** → `rokhlin_of_bounded_charSurface` (`16 ∣ latticeSig M.form` = the `hdvd` door input). Residual = exactly the [FK] Freedman–Kirby congruence (= the same `hbound` deep core; irreducibly geometric per `nogo_lattice_arf_not_sigma8`).
