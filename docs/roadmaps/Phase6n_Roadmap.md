...
| **Wave 1a** | G2 Resurgence | 🟡 **IN-PROGRESS** (1a.1 + 1a.2 + 1a.4 SHIPPED; 1a.3 Path-B verdict shipped as working doc; **1a.3 Path-A precision AUTHORIZED Session 6 by user — multi-session BdG self-energy diagram enumeration through order 7 to convert "suggestive" → "definitive" Gevrey-1-or-geometric verdict + numerical Λ_UV report**) | D1 §3-§4 (additive) — **DEFERRED** | **D.2** | none (Path A authorized) |
...
6. **Wave 1a.3 Path A precision** — ✅ AUTHORIZED Session 6. Multi-session work to ship: (a) BdG self-energy diagram enumeration through order 7; (b) cross-validation against Beliaev γ_1 + Andreev-Khalatnikov γ_2 literature anchors; (c) γ_3-γ_7 from higher-loop diagrams; (d) Padé-Borel pipeline re-run on precise γ_n; (e) definitive Gevrey-1-or-geometric verdict + Λ_UV report.
