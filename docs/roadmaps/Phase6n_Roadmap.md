...
| **Wave 1a** | G2 Resurgence | 🟡 **IN-PROGRESS** (1a.1 + 1a.2 + 1a.4 SHIPPED; 1a.3 Path-B verdict shipped; 1a.3 Path-A Stage 1 skeleton SHIPPED Session 6; 1a.3 Path-A Stage 2 γ_1 Beliaev LO SHIPPED Session 10; 1a.3 Path-A Stage 3 γ_2 NLO kinematic-dispersive SHIPPED Session 11; **1a.3 Path-A Stage 3 LEAN LIFT SHIPPED Session 12 (2026-05-05)**: NEW module `lean/SKEFTHawking/Resurgence/KinematicDispersive.lean` (~10 thms + 2 defs) lifting the closed-form `γ_n^(kin-disp)/γ_1 = (-1)^(n-1)·C(2(n-1),n-1)/16^(n-1)` to Lean substrate; central-binomial bound `C(2k,k) ≤ 4^k` via `Nat.choose_le_two_pow` → load-bearing `kinDispSeq_isGeometric : IsGeometric kinDispSeq 1 (1/4)`; convergence radius 2 · Λ_UV at Lean level. `Resurgence/Basic.lean` extended with `IsGeometric` predicate (parallel to `IsGevrey1`) + accessors + `borelTransform_bounded_of_isGeometric` cross-bridge. **Substantive cross-module verification:** 3 new integration tests in `tests/test_bdg_self_energy.py` connecting Stage-3 `gamma_kinematic_dispersive_sequence` to Wave 1a.2 `borel.py`: ratio-test signature R_n = -1/8, -3/32, ... → 0 monotonically (geometric NOT Gevrey-1); `leading_borel_singularity` returns `None` (Borel transform entire); `lambda_uv_estimate` returns `None` (no resurgence Λ_UV from kinematic alone). All headline theorems verify to `[propext, Classical.choice, Quot.sound]` only via `lean_verify`. Stage 4 next-up: γ_2^(loop) Beliaev-Galitskii 1959 2-loop self-energy + γ_3-γ_5 systematic enumeration. **32 passing tests in test_bdg_self_energy.py** (was 29 at Session 11; +3 cross-module integration); 1 Stage-4-5 skip-marked. Project pytest 4080 passed (+3 vs Session 11). **Lake build 8539 jobs** (+1 vs Session 11 8538: KinematicDispersive).) | D1 §3-§4 (additive) — **DEFERRED** | **D.2** | none |
...
| **Wave 2b** | G10-QCrooks-α | ✅ **SHIPPED** (Stage 1: 6 QuantumCrooks/ modules + parametric no-go theorem; Stage 2-3: 7th module Concrete.lean ships substantive quantum no-go via canonical 2-level Perarnau-Llobet counterexample on `Matrix (Fin 2) (Fin 2) ℝ`; **ℂ-extension SHIPPED Session 10 (2026-05-05)**: 8th module `QuantumCrooks/ConcreteComplex.lean` (~14 substantive theorems + 10 defs) ships canonical Perarnau-Llobet substrate on `Matrix (Fin 2) (Fin 2) ℂ` with σ_y Hamiltonian (purely imaginary off-diagonal) as genuinely complex example; ℂ↔ℝ cross-bridge; ℂ-form quantum no-go `perarnau_llobet_no_go_complex`. All MCP-proven, zero Aristotle escalation, zero sorry. Open extensions: arbitrary `Matrix (Fin n) (Fin n) ℂ`, reservoir-coupled forms, KMSCompatible/FDTCompatible typeclass connections.) | D1 cross-bridge / D5 — **DEFERRED** | **D.2** | none |
...
7. **Wave 1a.3 Path A Stage 4 (γ_2^(loop) + γ_3-γ_5 systematic loop enumeration)** — Stage 4 closes Wave 1a.3 at near-definitive verdict by adding the 2-loop Bogoliubov self-energy contribution at order (ξk)^6 (Beliaev-Galitskii 1959, Sov. Phys. JETP 7, 96) + γ_3-γ_5 loop coefficients via modern ERG references (Pawlowski-Strodthoff 2015, Boettcher et al.); Stage 4.5 re-runs Padé-Borel on the full (kin-disp + loop) γ_n sequence (Wave 1a.2 infrastructure now end-to-end-verified per Session 12).
8. **Wave 2b further ℂ generalization** — generalize the concrete substrate from `Matrix (Fin 2) (Fin 2) ℂ` to `Matrix (Fin n) (Fin n) ℂ` for arbitrary n; add reservoir-coupled forms; KMSCompatible/FDTCompatible typeclass connections.
9. **Wave 2c Stage 5 LDP infrastructure** — in-program build for the third Sakharov-style biconditional `AnalogHawkingSubstrate.satisfies_GLU_monotonicity ↔ EmissionSpectrum.LDP_rate_function.satisfies_GallavottiCohen`. Discrete-time / Markov-jump scope per Falasco-Esposito 2025 RMP.
10. **Phase 6o opening** — APS-η analog-horizons sub-wave + deferred-track items (G3 Carrollian, G4 Kerr-Schild, G1 Schellekens, G10-ETH-α, I3 community contribution).
...
- 🟡 **Wave 1a.3 IN-PROGRESS**: 
  ...
  - ⏳ **Path A Stage 4 (γ_2^(loop) + γ_3-γ_5 systematic loop enumeration) NOT STARTED** — Beliaev-Galitskii 1959 2-loop Bogoliubov self-energy at order (ξk)^6 + modern higher-loop ERG references (Pawlowski-Strodthoff 2015, Boettcher et al.) for γ_3-γ_5. Stage 4.5: Padé-Borel re-run on full (kin-disp + loop) γ_n sequence — Wave 1a.2 infrastructure now end-to-end-verified per Session-12 cross-module tests, ready to consume the full sequence when Stage-4 ships γ_n^(loop).
  - ⏳ **Path A Stage 5 (γ_6-γ_7 + definitive verdict) NOT STARTED** — closes Wave 1a.3 at definitive Gevrey-1-or-geometric verdict.
