...
| **Wave 1b** | G9 SymTFT audit + GD-4-factor sketch | 🟡 **IN-PROGRESS** (Stage 1+2+3+4 SHIPPED Session 6. Stage 4 ships predicate-level cross-bridges: KMSParityAlternation ↔ SecondOrderSK γ_{2,1}+γ_{2,2}=0; Z16AnomalyEta ↔ Z16AnomalyCancels. Stage 5 chiralCentralChargeMod24 ↔ Witt-class **candidate Mathlib upstream-PR opportunity** — program has track record of building Mathlib infra with intent of upstream-PR contribution (e.g., lean-tensor-categories, RingQuot work, repl pinning); Witt-group / Anderson-dual / Lagrangian-algebra would extend that pattern. Stage 5 retained as in-program build target rather than full deferral.) | D2 + D3 + D5 + F (additive) — **DEFERRED** | **D.2** | none |
...
| **Wave 2c** | G10-6n.3 Crooks-on-analog-Hawking | ✅ **SHIPPED** (Stage 1 + Stage 2-3 SHIPPED Session 6: 3 CrooksAnalogHawking/ modules — `HorizonDetailedBalance` + `GallavottiCohenSymmetry` substrate + `AnalogHawkingBiconditional.lean` ships the third Sakharov-style biconditional at predicate-bundle level (`AnalogHawkingEmissionScheme` + `monotonicityCompatibleEmission` + `gcCompatibleEmission` + biconditional theorem with `compat_hyp` precondition + non-vacuous-content partition theorem). Stage 4+ substrate-level derivation = **candidate Mathlib upstream-PR opportunity** (discrete-time / Markov-jump LDP infra per Falasco-Esposito 2025 RMP framework — buildable in-program with intent of upstream contribution). Stage 4+ retained as in-program build target rather than full Itô deferral.) | E1 + D1 — **DEFERRED** | **D.2** | none |
| **Wave 2d** | G10-6n.7 Sakharov ↔ horizon-Crooks | ✅ **SHIPPED** (Stage 1 + Stage 2-3 SHIPPED Session 6: substrate bundle + substantive biconditional `sakharov_iff_horizon_crooks` proved at Bool-projection level + ³He-A and FLS BEC compatibility theorems + horizon-Crooks-language unification of Phase 6e + Phase 6m Track C substrate-classification. Verlinde-vs-Jacobson distinction preserved.) D3+L3 reframing pre-draft on-hold per Session-5. | D3 + L3 — **DEFERRED** (pre-draft on-hold) | **D.3** | **HELD** — D3+L3 pre-draft on-hold per Session-5 user direction |
...
2. ~~**Wave 2c**~~ — ✅ Stage 1 + Stage 2-3 SHIPPED (Session 6): 3 CrooksAnalogHawking/ modules. Stage 2-3 ships the third Sakharov-style biconditional at predicate-bundle level (`AnalogHawkingEmissionScheme` + biconditional with `compat_hyp` precondition + non-vacuous-content partition). Stage 4+ substrate-level derivation deferred to domain-physics.
3. ~~**Wave 2d Lean substrate**~~ — ✅ Stage 1 + Stage 2-3 SHIPPED (Session 6): substrate bundle + substantive biconditional `sakharov_iff_horizon_crooks` proved at Bool-projection level + ³He-A and FLS BEC compatibility theorems + horizon-Crooks-language unification of Phase 6e + Phase 6m Track C substrate-classification. Verlinde-vs-Jacobson distinction preserved.
4. ~~**Wave 1b**~~ — ✅ Stage 1+2+3+4 SHIPPED (Session 6): primary-source audit verdict + 3 discrete-sector candidate predicates + 2 substantive cross-module bridges (KMSParityAlternation ↔ SecondOrderSK γ_{2,1}+γ_{2,2}=0; Z16AnomalyEta ↔ Z16AnomalyCancels). Stage 5+ chiralCentralCharge ↔ Witt-class deferred (Mathlib infra gap).
