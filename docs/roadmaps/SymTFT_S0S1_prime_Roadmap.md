...
| **S2 — object-level electric algebra** (multiplication + unit laws prefix) | ✅ 2026-07-20 (prefix); 🚧 assoc/étale residual | New leaf `SymTFT/ElectricAlgebraObject.lean`. The honest **group-algebra multiplication** `electricMul` on `unitPlusElectricObj` (`unit ⊞ electric`), defined **componentwise** as the sum of the four biproduct-corner maps `(fst⊗fst)≫λ≫inl + (fst⊗snd)≫λ≫inr + (snd⊗fst)≫ρ≫inr + (snd⊗snd)≫(e²≅1)≫inl` via `A5VacuumPlusElectric.electric_squared_iso_vacuum` — **REPLACING** (not patching) the degenerate projection-through-vacuum `unitPlusElectric_mul` (which gives `e·e = 0`, the square-zero extension; audit §1.7, falsifier 3). Substrate: `vacuumUnitIso` (`vacuumAnyon ≅ 𝟙_(Center C)` via `Center.ofBraided` `εIso` + base `unitVecGIso`), `electricInj`/`electricProj` (the `inr`/`snd` Center morphisms). **Proved:** four corner equations `electricMul_corner_11/_1e/_e1/_ee` (group-algebra fusion `1·1=1, 1·e=e, e·1=e, e·e=1`, the last the non-degenerate corner through the fusion iso); the degenerate contrast `degenerate_corner_ee` (`e·e = 0`); the **MonObj unit laws** `electricMul_one_mul`/`electricMul_mul_one` (via interchange + unitor naturality + `unitPlusElectric_total` biproduct identity — laws the degenerate multiplication FAILS, cf. A5's `unitPlusElectric_one_mul_factor`); the S1 tie `electricObject_support_eq_S1` (`{vacuum,electric} = toricElectricSupport.carrier`) + `electricObject_ee_categorifies_skeletal_fusion`. Apex `electricAlgebra_S2_closure` bundles all. **Kernel-pure `{propext, Classical.choice, Quot.sound}` (all headliners `lean_verify`-clean); slot build green (module 8271 jobs; full library `SKEFTHawking` 10128 jobs).** **Residual (next S2 session):** `mul_assoc` (⇒ the full `MonObj` instance) via 8-corner triple-tensor domain-extensionality; then `ComonObj`/left+right Frobenius/separability/connectedness/`FPdim²=4`. |
