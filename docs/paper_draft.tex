...
However, the standard calculation relies on free-field quantum field theory on a fixed curved background. The high-frequency modes that determine the Hawking temperature receive trans-Planckian corrections from unknown short-distance physics. Understanding these corrections requires either (i) a complete theory of quantum gravity, or (ii) systems where the trans-Planckian physics is known and can be studied \cite{Corley:1996,Jacobson:1996}.
...
\kappa = \frac{dv}{dx}\bigg|_{x_H} = \frac{v_H c_s}{n_H} \frac{dn}{dx}\bigg|_{x_H} + \text{(flow profile term)}.
\end{equation}
\todo{Clarify exact definition of $\kappa$ in BEC context; relate to experimental Trento setup.}
...
\end{equation}
where $\Box = \partial_t^2 - c_s^2 \nabla^2$ is the d'Alembertian, $u = \vec{v}/c_s$ is the normalized velocity, and $\gamma_{1,2}$ are the transport coefficients (with dimensions of $[\text{time}]^{-1}$). The Hermitian conjugate terms ensure proper closure under complex conjugation.
...
\end{equation}
where $\sigma$ is the scattering cross-section, $\eta$ is the shear viscosity (or bulk viscosity in a superfluid), and $m$ is the atomic mass. \todo{Compute explicit expressions for $\gamma_1, \gamma_2$ in terms of $^{87}$Rb and $^{23}$Na scattering lengths and temperatures.}
...
For small-amplitude perturbations around the background, the phonon field $\phi(\vec{r}, t)$ satisfies a modified wave equation in the presence of dissipation. In the WKB approximation, we write
\begin{equation}
\label{eq:wkb_ansatz}
...
\end{equation}
\todo{Derive full dispersion relation including both dispersive and dissipative corrections; perform WKB analysis near the horizon.}
...
With dissipation, the matching conditions are modified: instead of requiring purely ingoing or outgoing waves, we allow exponentially decaying modes. This subtly changes the relative amplitude between the Hawking-radiated modes and the vacuum fluctuations. \todo{Work out the detailed matching procedure; compute the Bogolubov coefficient $\beta_k$ in the presence of dissipation.}
...
\hline
Scattering length $a_s$ & 102 & $a_0$ \\
Temperature $T$ & 100 & nK \\
Density $n$ & $10^{15}$ & cm$^{-3}$ \\
Speed of sound $c_s$ & $\sim 1.4$ & mm/s \\
Healing length $\xi$ & $\sim 0.2$ & $\mu$m \\
Surface gravity $\kappa$ & $\sim 2 \times 10^2$ & s$^{-1}$ \\
\hline
\end{tabular}
\caption{Parameters for Steinhauer's $^{87}$Rb sonic black hole.}
\label{tab:rb_params}
\end{table}

From kinetic theory in the collisionless (Knudsen) regime transitioning to hydrodynamics, we estimate
\begin{equation}
...
A natural way to enhance dissipative corrections is to use a spin-sonic (spin-flow-acoustic) system, where phonons propagate in a spin-dependent effective metric. \todo{Coordinate with Trento experimental group on realistic parameters.}
...
\end{equation}
\todo{Refine this estimate using actual $^{23}$Na scattering data and Trento trap design.}
which is now potentially experimentally accessible if noise can be reduced to the $\sim 10^{-3}$ level.

\subsection{Formal Verification}

The core mathematical structures underlying the main result have been formally verified in Lean~4 proof code (v4.28.0, Mathlib). Of the 12 original proof gaps (``sorry'' placeholders), 7 have been machine-verified via the Aristotle automated theorem prover across two batches:
...
\item SK uniqueness witness: existence of dissipative coefficients satisfying normalization and positivity constraints.
\end{itemize}

The remaining 5 gaps (all priority-3) involve WKB asymptotics across the horizon, d'Alembertian operator infrastructure, and the fluctuation-dissipation relation from KMS symmetry. These encode genuinely analytic content that documents the mathematical boundary of formal verification for this class of problems.
...
In the higher-dimensional AdS/CFT context, dissipative effects in the boundary theory (e.g., bulk viscosity) leave signatures in the gravitational dual as ingoing or outgoing radiation. Similarly, dissipation in a BEC analog system can be viewed as a source term on the gravity side. Our results suggest that dissipative corrections to Hawking radiation follow a universal pattern governed by the surface gravity and the damping timescale.
...
We thank \todo{advisors, collaborators}. Computational work was aided by Lean 4 formal verification. This work is supported by \todo{funding agencies}.
\end{acknowledgments}
...
\bibitem{Son:2002} D.~T.~Son, \textit{Phys.\ Rev.\ D} \textbf{78}, 046003 (2008). [Check year; foundational ref on superfluid EFT.]
...
\bibitem{Steinhauer:2019} J.~Steinhauer, \textit{Nat.\ Phys.} \textbf{12}, 959 (2016). [Ref for first analog Hawking detection.]

\bibitem{Berti:2025} E.~Berti et al., \textit{Phys.\ Rev.\ Lett.} \textbf{134}, 161001 (2025). [Recent review of dispersive corrections.]
