...
| **K8b** | `hk3 : IntCongr(interMatrix, k3Form)` — TWO candidate routes, scope at dispatch: (i) extend `EvenUnimodularHyperbolic` normal-form to σ ≠ 0 with E₈ blocks (pure algebra; classification: rank-22 even unimodular indefinite σ=−16 is UNIQUE = k3Form — Milnor–Husemoller); (ii) the explicit Kummer-lattice integer base-change (22×22, decidable arithmetic, no native_decide — structured lemmas). Route (i) is reusable forever; **Fable-class if (i)** | `k3Form` + `IsEvenUnimodular` + `IntCongr` + the σ=0 normal form (fact 2) | the E₈-block classification OR the explicit base change |
