...
## 6ed-honeycomb-phase-chart-gl2z-invariant
- verdict: dead
- tier: agent-reviewed
- authored_by: kernel-no-go
- killed_by: Phase 6ED Waves 1–2 adversarial review (2026-07-29), remediated in-tree the same day — kernel-checked, lead-verified
- reason: the Bloch-phase honeycomb structure factor `f(θ) = 1 + e^{iθ₁} + e^{iθ₂}` is **not** independent of which primitive pair the phases are read against — it is **not `GL₂(ℤ)`-invariant**, so "every theorem holds for *any* primitive-vector choice" is **FALSE**. `structureFactor_zero_set_not_shear_invariant` exhibits a genuine Dirac point `K` at which the *equally primitive* sheared pair `(a₁, a₁+a₂)` gives `f ≠ 0` — under that chart `structureFactor_eq_zero_iff` **misidentifies the Dirac set**, reporting a real Dirac point as gapped. SCOPE: the theorems are true (they are statements about `θ ∈ ℝ²`); the false part was the *identification* claim, which had been the stated justification for a coordinate deviation. **THE REPAIR is the real generality content the false claim stood in for:** the admissible chart is now the named hypothesis `IsHoneycombChart` (equal length, 60°), and `isHoneycombChart_of_neighbours` proves the honeycomb's own **120° bond geometry forces it** — so a consumer supplying genuine nearest-neighbour vectors gets the chart for free. CONSEQUENCE for Wave 2: "the quadratic form `q₁²−q₁q₂+q₂²` is isotropic in the physical frame" is likewise chart-specific (a 120° pair sweeps a factor of 3 across directions); that is now the theorem `quadForm_of_chart`, stated against `IsHoneycombChart`, not prose. See `KERNEL_NOGO_REGISTRY['honeycomb_phase_chart_gl2z_invariant']`.
- memory: [[project_vectorC_public_6E_series]]
- created_ts: 2026-07-29T00:00:00Z
