...
| **a required trailing delimiter** | a declaration scanner matched `name` only when followed by a space or colon, so a name ending its line — signature beginning below — was invisible. 7.2% of one Lean corpus | anchor on what actually ends the token, and remember that **end-of-line ends it too**: `(?:\s|:\|$)` |
| **a second defect mistaken for the first** | a fully-qualified query returned nothing. That empty was first cited as proof of an unrelated line-break bug, then — when it survived that fix — written off as the tool's shape. It was a SECOND bug: the query was matched against source text, where the namespace never appears | run the healthy control (it was empty too, which is what made "by design" plausible), then ask why the healthy case is empty. A limitation that no one designed is a defect wearing a limitation's clothes |
...
| **a config record read as runtime state** | a staleness detector compared the *install record* against HEAD. A refresh moves that record immediately while every already-running process keeps executing the old cache, so the detector reported "nothing stale" — and deleted its own amendment document — for exactly the window the amendment existed to cover | ask what is RUNNING, not what is configured: a live process's argv carries what it actually loaded. Config state and runtime state are separate questions and need separate instruments |
| **hex order read as age order** | picking the "oldest" of several SHAs with a string sort. Hashes sort by their digits, so `0a…` reads as older than `f9…` regardless of date — and it is right about half the time, which is worse than always wrong | order by ancestry (`git rev-list --count <sha>..HEAD`), and when seeding a test for this, force the hex to fall the trapping way — a detector that fires on half of runs is not a detector |
