...
Sources: Phases 6BA, 6BB, 6BC. Authorized 2026-06-29; all Lean substrate, kernel-pure, zero project-local axioms. **On-disk scaffolding (`_VALID_BUNDLE_TARGETS` entry, `papers/D10/`) executes at first content-lift** per `BUNDLE_LIFT_PROCEDURE` — not at authorization time (avoids standing up an empty bundle).
...
Sources: Phases 6EA, 6EB, 6EC, 6EE (`6E*` series head: `roadmaps/Phase6EA_Roadmap.md`). Authorized 2026-07-27; Lean substrate, kernel-pure. **On-disk scaffolding (`_VALID_BUNDLE_TARGETS` entry, `papers/D12/`) executes at first content-lift** per `BUNDLE_LIFT_PROCEDURE` — not at authorization time.
