...
The graph is assembled at several sites that each derive from scratch — the pre-gate view inside `extract_readiness_gate_nodes`, the real node list, and the edge extractors — so a single build used to call each argument-free extractor **1-8 times** (most twice; five ran once, where the memo only costs a copy). `build_graph._BUILD_MEMO` derives each one once per build instead, cutting `build_graph_json()` from 14.4 s to 7.1 s (measured 2026-08-14) with a graph identical modulo per-build timestamps (`built_at`, `last_evaluated`).

⛔ **The memo lives only for the dynamic extent of one `build_graph_json()` call, and that boundary is the whole safety argument — do not widen it.** Outside a build every extractor computes normally, so a direct caller such as `bundle_readiness.load_findings_by_paper` always reads live state. A cache that outlived a build would answer a post-seeding read from a pre-seeding snapshot, which would silently defeat the seeded-mutation tests that make this repo's non-vacuity claims worth anything. Build-scoping does not solve cache invalidation — it removes the problem, because nothing on disk can change while one build is on the stack. Extending it to a cross-build cache means keying on everything the graph reads — which the dashboard cache now does for its own purposes, at ~18 ms; the reason not to do it here is that build-scoping needs no key at all, not that a key is unaffordable. Guarded by `tests/test_build_graph_memo.py`.

### Dashboard Graph Cache — the staleness key

`provenance_dashboard` serves `/api/graph*` from an in-process cache invalidated by `_graph_fingerprint()`. **The key is derived from the read set the last build actually opened** — captured through an `open` audit hook, plus every repo `.py` currently imported, since a module already in `sys.modules` is never re-opened — plus the sorted listing of every ancestor directory of those files. Cost ~15 ms against a 7.1 s rebuild.
...
| `BLOCKED_BY` | ReviewFinding | ReviewFinding | This finding waits on another, or on an external release condition (`run:` `phase:` `pub:` `research:`) | ADR-012 D10 | ⏳ — **emitter live, zero edges today** (`blocked_by` is forward-only). The consumer is `build_graph.finding_is_dispatchable`, called today by `review_runner.emit_finding_brief` (which reports DISPATCHABLE) and by orchestration, which is not built. **Do not read this row as gated.** An entry naming no node, an unrecognised scheme, or a known scheme with an EMPTY value is reported by `review_severity_declared` at a ceiling of zero — it does not raise, because raising here propagated out of `build_graph_json()` and would have taken the whole derived-graph layer down on one hand-typed id |
