...
Two figures published above are WRONG. Both were re-derived after the verbatim-extraction
repairs (`c7148779`, `c5f384b4`, `39e7ac3a`) and after PR-review pass 3 (R2, R6) challenged them.
The corrections are recorded here rather than silently edited into place.

## C6 — "D11 and D12 reference zero Lean declarations" is FALSE

§M2 reported per-bundle module counts of **D11 = 9, D12 = 0** and concluded the late D-tier
containers were *"barely attached to the substrate at all."* That was an artifact of the
measurement, not a property of the drafts: D11 and D12 write every Lean reference through
`\newcommand{\thm}[1]{{\def\_{\char`\_\allowbreak}\texttt{#1}}}`, a line-breaking wrapper the
extractor of the day could not see.

Re-measured with the structural extractor:

| bundle | declarations cited | of which theorems | modules reached |
|---|---|---|---|
| D6 | 153 | 146 | 58 |
| D7 | 11 | 11 | 3 |
| D8 | 37 | 35 | 28 |
...
**No bundle references zero declarations.** D7 is the thinnest at 11, and even that is not
"detached". The §Context sentence *"D11 and D12 reference zero Lean declarations"* is withdrawn,
and with it the inference that late authorization produces substrate-detached containers. What
survives is the *length* finding (M1), which was measured from PDFs and is unaffected.

## C7 — "the audit's ~340 is low by 4–5×" was a UNIT SWAP, not a correction

§M2 graded the audit ❌ for reporting *"162 `PinPlus*`"* against a measured **2 914**. The audit's
own text says **162 `PinPlus*.lean` MODULES**. Measured now:

| quantity | value |
|---|---|
| `PinPlus*.lean` **files** on disk | **164** |
| modules containing a `PinPlus*` declaration | 180 |
| `PinPlus*` **declarations** | 3 078 |

**164 against the audit's 162 — a 1.2 % difference. The audit was right.** The re-measurement
compared *declarations* to *modules* and reported the ratio as the audit's error.

The same swap infects the headline. The audit's *"~340 modules across ten arcs"* counts a named,
enumerated set of arcs; §M2's **1 403–1 633** counts the complement of the homed set across the
whole tree. Those answer different questions, and the ❌ verdict against the audit does not stand.
**What survives is the substantive point, restated in the audit's own unit:** a large majority of
project modules reach no bundle draft, and the un-homed population is much larger than the ten
named arcs.

The `2 914` figure is also inflated — the autogenerated-companion filter leaves structure and
inductive companions in (R6 measured ~30 %; the count above is 3 078 under the same filter after
the module recovery).
...
| claim | status |
|---|---|
| M1 — Tier-1 at 186/480 pp; fill monotone in charter size; charter is a tier template | **UNAFFECTED** (PDF-derived) |
| M4a — D6 ∩ D9 = 78 theorems | **UNAFFECTED**, and independently reproduced by R6 |
| M5 — 17 stub sections, immaterial | **UNAFFECTED** |
| M6 — absorption trigger dead, 9 bundles | **UNAFFECTED**, and independently reproduced by R5 |
| M2 — "~340 low by 4–5×" | **WITHDRAWN** (unit swap) |
| §Context — "D11/D12 reference zero" | **WITHDRAWN** (extraction artifact) |

**The D6+D9 merge case, which is what the portfolio decision actually turns on, is untouched by
both corrections.**
