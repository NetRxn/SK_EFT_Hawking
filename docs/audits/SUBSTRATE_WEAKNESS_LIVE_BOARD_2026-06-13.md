...
> **⚠️ CORRECTION (2026-06-13) — re-characterizes #1's remediation (this board originally proposed a Gauss-sum/Brown *lattice* Arf bridge as the discharge; that is FALSE/impossible).** The proposed discharge "replace `topo` with `arf : Arf(q)=0` + prove `Arf=0 → 2∣σ/8` via the Gauss-sum/Brown bridge in `ArfInvariant.lean`" **cannot be done**: `Arf(redQuad)` is **identically 0 on every even unimodular lattice** (E₈: `gaussSum = +16`, `#zeros = 136 = 2⁷+2³ ⟹ Arf = 0`, but `σ(E₈)/8 = 1` is odd), because the discriminant form `L*/L` is trivial for unimodular `L`, so the finite Arf/Gauss-sum apparatus sees only **σ mod 8**. The geometric Guillou–Marin Arf lives on a smoothly embedded **characteristic surface** (`σ ≡ Σ·Σ + 8·Arf(M,Σ) mod 16`), a different object than the lattice `Arf(redQuad)`. Machine-checked refutation: `lean/SKEFTHawking/RokhlinArfNoGo.lean` (`arfOfForm_e8lit_eq_zero`, `lattice_arf_bridge_refuted`, `redQuad_neg_eq`). **#1 is therefore NOT an achievable hard discharge** — drop its "hard discharge" ranking/score. The REAL remediation is a **disclosure/wording pass** (paper10 "unconditional 16∣σ" → "8∣σ proven outright; 16∣σ given the tracked topological factor `2∣σ/8`"), with `topo` kept as a permanent irreducible geometric hypothesis (ADR-003). The `redQuad`/`gaussSum` machinery is correct and stays; only the false σ-bridge claim is retracted. See the parallel correction in `ADR-003` Decision #2 and the `SUBSTRATE_WEAKNESS_AUDIT_2026-06-13.md` annotations.
...
|---|---|---|---|---|---|
| 1 | **1** | **Rokhlin `topo` factor** — ~~replace `topo : 2∣σ/8` with `arf : Arf(q)=0` + prove `Arf=0 → 2∣σ/8` via the Gauss-sum/Brown bridge in `ArfInvariant.lean`~~ **← FALSE/IMPOSSIBLE, retracted 2026-06-13 (see header correction + `RokhlinArfNoGo.lean`)**: `Arf(redQuad) ≡ 0` on all even unimodular lattices, so it can only ever yield σ mod 8, never the σ-mod-16 factor. **REAL remediation = disclosure/wording pass:** fix paper10 prose ("unconditional 16∣σ" → "8∣σ proven; 16∣σ given tracked topological `2∣σ/8`"); keep `topo` as a permanent irreducible **geometric** hypothesis (Guillou–Marin / Â-genus-even), NOT a discharge target. | **~~92~~ → disclosure-only** | ~~hard~~ **n/a (no hard discharge exists)** | **NOT a hard discharge — drop the ranking.** Flagship (paper10 / Z16 count) integrity is intact (8∣σ real); the fix is wording. |
...
## E. Recommendation
1. ~~**Next big `/goal` = #1 Rokhlin `topo` discharge** (B1 rank 1) — flagship, highest score, infra seeded; converts a defining-the-conclusion field into a proven Gauss-sum theorem.~~ **CORRECTION 2026-06-13: there is NO Gauss-sum discharge — the lattice Arf bridge is false (`RokhlinArfNoGo.lean`; `Arf(redQuad) ≡ 0` on even unimodular lattices ⟹ only σ mod 8). #1 is NOT a `/goal`-able hard discharge.** The ONLY action is the **disclosure/wording pass** (do now, no-regret): fix paper10's "unconditional 16∣σ" prose → "8∣σ proven outright; 16∣σ given the tracked topological factor `2∣σ/8` (geometric Â-genus-even / Guillou–Marin), carried as a structure field rather than derived." `topo` stays a permanent irreducible geometric hypothesis.
