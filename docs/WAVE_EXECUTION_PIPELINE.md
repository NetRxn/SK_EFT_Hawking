...
**Module descriptions need no maintenance here (ADR-013).** `docs/MODULE_CENSUS.md` is derived
and regenerated by `sync`, so the way to change a description is to change it **at the source** —
a Python module docstring, a shell script's leading comment block, or a notebook's opening
markdown cell, one decider per language (D2/D5/D3). The six-item *watch for* list this stage used to carry — new modules,
new theorems, new Aristotle runs, new notebooks and papers, formula changes, superseded
descriptions — was hand-enforcement of a hand catalogue, and every item on it is now answered by
a derived artifact: `docs/MODULE_CENSUS.md`, `docs/counts.json`, `lean/lean_deps.json`,
`lean/atlas_view.json`.

⚠️ **This stage is why that catalogue drifted.** Its gate was *"manual spot-check of three
count-sensitive files"* against a 319 KB hand-maintained inventory, and nothing mechanical
asserted any of its sections — measured: zero checks referenced them. That is not a discipline
failure to exhort away; it is a mechanism that could not scale to the population it governed.
