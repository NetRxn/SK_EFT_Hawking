...
def _owned_question(doc_name: str) -> str:
    """The question `README.md`'s ownership table assigns to `doc_name`.

    TODO-D8. Read from the table rather than restated here: a second copy of the
    question is the drift this contract exists to prevent.
    """
    readme = (PROJECT_ROOT / "docs" / "architecture" / "README.md").read_text(encoding="utf-8")
    for q, d in re.findall(r"^\|\s*(.+?)\s*\|\s*\[`([^`]+)`\]\([^)]+\)\s*\|\s*$",
                           readme, re.M):
        if d == doc_name:
            return q
    raise RuntimeError(
        f"{doc_name} is not assigned a question by README.md's ownership table — "
        f"refusing to emit an empty contract line.")
...
def render(inv: dict) -> str:
    ...
    a("")
    # TODO-D8: the required-content contract, emitted rather than hand-written
    # because this file is generated. `README.md`'s ownership table assigns a
    # question to each architecture document; the document must declare the same
    # question verbatim, so an assignment cannot drift away from its answer.
    a(f"> **Answers:** {_owned_question('SURFACE_INVENTORY.md')}")
    a(">")
