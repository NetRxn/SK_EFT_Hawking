...
# ==========================================================================
# ⛔ ARCHIVED + DISABLED 2026-06-25 — ADR-006. DO NOT USE THIS SCRIPT.
# ==========================================================================
# WHY THIS IS DISABLED:
#   This is the pre-rewrite, FULL-PROJECT Aristotle submission CLI. Every
#   invocation uploaded the ENTIRE Lean project (now ~1,172 files / 17 MB),
#   and `--integrate` blindly whole-file-overwrote returned files into the
#   live tree with no kernel-purity / validate / test gate. At the current
#   substrate scale these are foot-guns. It has been replaced by the safe
#   partial-submission + verify-then-graft CLI at the canonical path
#   scripts/submit_to_aristotle.py (see docs/adrs/ADR-006-aristotle-submission-rewrite.md).
#
#   This copy is RETAINED, not deleted, because it is the Methods-of-record
#   for prior papers (papers/I1, papers/paper15_methodology, papers/D1), which
#   describe this exact "the entire Lean project is submitted" process
#   verbatim. Deleting it would break the provenance of prior results.
#
# HOW TO RE-ENABLE THE FOOT-GUNS (only to reproduce the old full-project
# behaviour, e.g. to re-derive a prior paper's run): delete the no-op guard
# block below (everything between the BEGIN/END markers). The original
# full-project implementation is preserved intact beneath it and runs again
# once the guard is gone.
# --------------------------------------------------------------------------
# >>> ADR-006 NO-OP GUARD — BEGIN (delete this whole block to re-enable) >>>
import sys as _adr006_sys

_adr006_sys.stderr.write(
    "\n[ADR-006] scripts/archive/submit_to_aristotle.py is ARCHIVED + DISABLED.\n"
    "  WHY: deprecated full-project Aristotle submission + blind whole-file\n"
    "       --integrate (foot-guns at current substrate scale). Replaced by the\n"
    "       safe partial-submission + verify-then-graft CLI at the canonical\n"
    "       scripts/submit_to_aristotle.py (see docs/adrs/ADR-006-*).\n"
    "  RETAINED only as Methods-of-record for prior papers (provenance).\n"
    "  RE-ENABLE: delete the 'ADR-006 NO-OP GUARD' block at the top of this file;\n"
    "       the original implementation below is intact.\n\n"
)
raise SystemExit(2)
