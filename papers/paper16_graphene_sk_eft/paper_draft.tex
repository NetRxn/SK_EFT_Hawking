...
The dissipative correction $\delta_{\rm diss} \approx +6.5\%$ is the larger
of the two and carries the opposite sign, so the corrections partially
cancel: the viscous damping rate is
$\Gamma_H \approx \SI{1.3e11}{\per\second}$ against a surface gravity
$\kappa = \SI{2e12}{\per\second}$. The Hawking
...
for $5\sigma$ discovery. The greybody factor is computed in closed form
as $\Gamma_0 = 4 c_R v/(c_R+v)^2 \approx 0.99994$ for the Dean nozzle
(Anderson et al., PRD 87), with quasi-1D finite-width corrections
bounded at $\le 1.8\%$ at $\omega_H$, making the measurement
...
formally verified in Lean~4 (58~theorems, zero sorry) and derive from
a computation pipeline with 153~tests.
\end{abstract}
...
\subsection{Dissipative correction: comparable, and opposite in sign}
...
\end{equation}
where $\nu$ is the momentum-diffusion constant. Because the momentum
density of a relativistic fluid is $w/v_F^2$,
\begin{equation}
\nu = \frac{\eta\,v_F^2}{w} = \frac{\eta}{s T}\,v_F^2
    = 2\,\frac{\eta}{s T}\,c_s^2 ,
\end{equation}
the last equality holding for a conformal fluid ($c_s = v_F/\sqrt{2}$).
The $d = 2$ sound-attenuation bracket $[2(d-1)/d]\eta + \zeta$ is exactly
$1\cdot\eta$ with $\zeta = 0$, so no $O(1)$ factor survives; the familiar
$4/3$ is the $d = 3$ value. With $\eta/s \approx 4 \times \hbar/(4\pi k_B)$
(Majumdar~\textit{et~al.}) and $T \sim \SI{150}{\kelvin}$:
\begin{equation}
\Gamma_H \approx \SI{1.3e11}{\per\second}, \qquad
\delta_{\rm diss} \approx +6.5\%.
\end{equation}
Writing $\eta/(sT)$, a \emph{time}, in place of $\nu$ omits the
velocity$^2$ and understates $\delta_{\rm diss}$ by eleven orders of
magnitude. Restored, the dissipative correction exceeds the dispersive
$-2.8\%$ and cancels part of it, leaving a net $+3.7\%$. Graphene is
therefore the platform where the SK-EFT's dissipative channel is a
percent-level measurable rather than a formal refinement.
...
from a perfect thermal emitter. For a one-dimensional acoustic black
hole with a smooth velocity profile, Anderson, Balbinot, Fabbri, and
...
\label{eq:gamma0}
\end{equation}
which contrasts with four-dimensional Schwarzschild
($\Gamma \propto \omega^2$). For the Dean bilayer nozzle
($v \approx 0.985\,c_s$, $c_R \approx c_s$) this gives
$\Gamma_0 \approx 0.99994$. The frequency dependence in the adiabatic
regime ($D = \kappa\,l_{ee}/c_s = 0.23 < 1$) is
$\Gamma(\omega) \approx \Gamma_0[1 - (\omega/\omega_{\max})^2]$ with
$\omega_{\max} = \sqrt{\kappa c_s/l_{ee}}$ the dispersive UV cutoff, so
$\Gamma$ remains within a few per cent of $\Gamma_0$ throughout the
detection band. The quasi-1D finite-width correction
$|\delta\Gamma/\Gamma| \le (l_{ee}/W)^2 + (\omega/\omega_\perp)^2
e^{-2\pi L/W}$ bounds the 2D-to-1D deviation at $\le 1.8\%$ at
$\omega_H$; formally verified in Lean module
\texttt{QuasiOneDReduction} (theorems T1--T5; 12 theorems total in the module). Numerical predictions
below use this realistic $\Gamma(\omega)$ rather than the
$\Gamma = 1$ upper bound, giving a corrected cumulative SNR of $98.8\%$
of the step-horizon value.

The detection band is also separated from the lowest transverse-channel
cutoff $\omega_\perp(0) = c_s\,\pi/W$. For the Dean nozzle
($W = \SI{1}{\micro\meter}$, $c_s = \SI{4.4e5}{\meter/\second}$) one has
$\omega_\perp(0) \approx \SI{1.38e12}{\per\second}$, exceeding $4\,\omega_H$
across the entire $[0,\,4\omega_H]$ SNR-cumulative integration window. This
factor-of-four separation is formally verified in Lean
(\texttt{DiracFluidWKB.dean\_lowest\_channel\_above\_four\_omega\_H}) and
guarantees the quasi-1D treatment is parametrically safe: no transverse
mode is open in the detection band. The block-diagonal binding
$c_s = v_F/\sqrt{2}$ that lets all 1+1D WKB machinery apply per channel
(via \texttt{DiracFluidWKB.toExactWKB}) is similarly verified, with
the subluminal property $c_s < v_F$ shipped as
\texttt{DiracFluidWKB.soundSpeed\_lt\_vF}---the algebraic foundation
of the more-robust-horizon claim (subluminal high-momentum modes
cannot escape; cf.\ Sec.~\ref{sec:hawking}).

The thermal background is Johnson-Nyquist:
$S_{\rm thermal} = 4 k_B T_{\rm amb} \sigma_Q \approx
\SI{1.3e-24}{\ampere\squared/\hertz}$.

With the realistic $\Gamma(\omega) \approx \Gamma_0 \approx 0.99994$
[Eq.~\eqref{eq:gamma0}], the Hawking signal $\Delta S_I \approx
...
\caption{Current noise power spectral density for the Dean bilayer
nozzle (realistic greybody $\Gamma_0 \approx 0.99994$ per
Eq.~\eqref{eq:gamma0}).
...
\Gamma_0 T_H/(2T_{\rm amb})$ is approximately $8 \times 10^{-3}$
(using $\Gamma_0 \approx 0.99994$) at low frequencies, independent of
$\sigma_Q$ (the conductance cancels in the ratio; Lean theorem
\texttt{snr\_independent\_of\_sigma\_Q}).
...
\cdot \Delta f]$.
With the realistic $\Gamma_0 \approx 0.99994$
[Eq.~\eqref{eq:gamma0}, Lean
\texttt{QuasiOneDReduction.greybody\_zero\_freq\_le\_one}]:
$t_{5\sigma} < \SI{1}{\milli\second}$, essentially unchanged from the
$\Gamma=1$ step-horizon upper bound. Even with a pessimistic
$\Gamma = 0.01$ (two orders below the derived value), the 5$\sigma$
integration time remains under \SI{1}{\second}. In all physically
plausible regimes, \emph{detection is limited by systematic calibration,
not by photon statistics} (Fig.~\ref{fig:snr}).

These estimates assume the ideal radiometer equation (Gaussian noise,
independent frequency bins). Practical limiting factors include:
(i)~amplifier noise temperature $T_N \sim
...
toward $T_H$); (ii)~the residual frequency dependence of
$\Gamma(\omega)$, whose deviation from $\Gamma_0$ is bounded by
$(\omega/\omega_{\max})^2 \le 0.6\%$ at $\omega_H$ and sub-percent
everywhere in the detection band
(Lean \texttt{QuasiOneDReduction.quasi1D\_validity\_bound});
...
vs.\ frequency for the Dean bilayer nozzle (realistic
$\Gamma_0 \approx 0.99994$, quasi-1D correction
$\le 1.8\%$). The ratio is
...
Together, these capabilities could enable a measurement of analog
...
\texttt{QuasiOneDReduction} & 12 & Greybody $\Gamma_0$, quasi-1D bound \\
\texttt{DiracFluidWKB} & 14 & Subluminal, transverse-mode gap, sum-over-channels \\
\hline
Total new & 58 & Zero sorry \\
...
an observation of analog Hawking radiation in an electronic system.
...
a greybody factor $\Gamma_0 \approx 0.99994$ derived in closed form
[Eq.~\eqref{eq:gamma0}] (Table~\ref{tab:platforms}); the
...
Computation pipeline formally verified with 58~new Lean~4 theorems
(zero sorry), building on 109~reused theorems from the 1+1D BEC
framework. All numerical predictions generated by a 103-test
Python pipeline.
\end{acknowledgments}
