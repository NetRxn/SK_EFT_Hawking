...
signal manifests as excess current noise $S_{\rm Hawking} \sim
\SI{e-30}{\ampere\squared/\hertz}$ across a \SIrange{0.5}{85}{\giga\hertz}
band, extractable from the thermal Johnson-Nyquist background via broadband
correlation analysis in approximately 1~minute of bandwidth-cumulative
integration (or ${\sim}25$~minutes for $5\sigma$ discovery).
...
formally verified in Lean~4 (24~theorems, zero sorry) and derive from
a computation pipeline with 95~tests.
\end{abstract}
...
\begin{table}[t]
\caption{Analog Hawking temperature for graphene constriction geometries.
All predictions from the SK-EFT framework with formally verified correction
bounds.}
\label{tab:platforms}
\begin{ruledtabular}
\begin{tabular}{lcccc}
Geometry & $c_s$ (km/s) & $L$ (nm) & $T_H$ (K) & $D$ \\
\hline
Dean bilayer nozzle & 440 & 200 & 2.4 & 0.23 \\
Monolayer $W\sim\SI{100}{\nano\meter}$ & 710 & 100 & 8.7 & 0.76 \\
Monolayer $W\sim\SI{50}{\nano\meter}$ & 710 & 50 & 17 & 1.5$^*$ \\
BEC (Steinhauer) & $5\times 10^{-4}$ & 5000 & $3.5\times 10^{-10}$ & 0.013 \\
\end{tabular}
\end{ruledtabular}
{\footnotesize $^*$EFT breaks down ($D > 1$); non-perturbative regime.}
\end{table}
...
$\omega \sim k_B T_H / \hbar \approx \SI{51}{\giga\hertz}$. In the
Landauer-B\"uttiker scattering formalism~\cite{Blanter2000}, the excess
noise from pair production at a scattering barrier (here, the sonic
horizon) takes the form
\begin{equation}
S_{\rm Hawking}(\omega) \sim \frac{2 e^2}{\pi} \, G_Q \, \omega \, n_{\rm Hawking}(\omega),
\end{equation}
where $G_Q = \sigma_Q$ is the quantum critical conductance (sample-specific;
for the Dean device $\sigma_Q \approx 4\,e^2/h$), and
$n_{\rm Hawking}(\omega) = |\beta_\omega|^2/(1-\delta_k) + n_{\rm noise}$
includes the modified Planck factor $|\beta_\omega|^2 =
1/(e^{2\pi\omega/\kappa_{\rm eff}} - 1)$ and the FDR-mandated noise floor.
This formula is a leading-order estimate: the precise prefactor depends on
the nozzle transmission function and the number of transverse modes, which
require device-specific scattering-matrix calculation. Our predictions for
$S_{\rm Hawking}$ should be understood as order-of-magnitude estimates
pending such a calculation.

The thermal background is Johnson-Nyquist:
$S_{\rm thermal} = 4 k_B T_{\rm amb} \sigma_Q \approx
\SI{e-24}{\ampere\squared/\hertz}$.

The Hawking signal $S_{\rm Hawking} \sim \SI{e-30}{\ampere\squared/\hertz}$
sits six orders of magnitude below the thermal floor (Fig.~\ref{fig:spectrum}).
However, the Hawking excess has a \emph{distinct spectral shape}---Planck at
...
$S_{\rm Hawking}$ (blue) sits ${\sim}6$ orders below the Johnson-Nyquist
...
The per-bin signal-to-noise ratio $S_{\rm Hawking}/S_{\rm thermal} \approx
6 \times 10^{-7}$ is small, but the detection bandwidth is wide
(\SIrange{0.5}{85}{\giga\hertz}). The cumulative SNR scales as
${\rm SNR}_{\rm cumul} = {\rm SNR}_{\rm bin} \times \sqrt{\Delta f \cdot t}$,
reaching unity in approximately \SI{27}{\minute} in a single frequency bin.
Exploiting the full detection bandwidth (\SIrange{0.5}{85}{\giga\hertz},
${\sim}300$ independent bins with comparable SNR) via bandwidth-cumulative
integration reduces this to ${\sim}\SI{1}{\minute}$---a $26\times$ speedup
(Fig.~\ref{fig:snr}). These estimates assume the radiometer equation
(Gaussian noise, independent frequency bins, no systematic backgrounds).
Hydrodynamic mode coupling, 1/$f$ noise from contacts, and non-Gaussian
fluctuations near quantum criticality could increase the required time.
A $5\sigma$ discovery threshold requires $25\times$ longer, giving
${\sim}\SI{25}{\minute}$ for cumulative bandwidth integration---still
well within reach of a single measurement session.

\begin{figure}[t]
\includegraphics[width=\columnwidth]{../../figures/phase5w/fig105_graphene_snr_frequency.png}
\caption{Per-bin signal-to-noise ratio $S_{\rm Hawking}/S_{\rm thermal}$
vs.\ frequency for the Dean bilayer nozzle. The ratio is ${\sim}6\times
10^{-7}$ below $\omega_H$ and drops exponentially above. Cumulative
SNR reaches unity in ${\sim}\SI{27}{\minute}$ of broadband integration.}
\label{fig:snr}
\end{figure}
...
\item \textbf{Integration:} ${\sim}\SI{1}{\minute}$ for bandwidth-cumulative
SNR~$=1$; ${\sim}\SI{25}{\minute}$ for $5\sigma$.
...
signal is detectable in ${\sim}\SI{25}{\minute}$ at $5\sigma$.
...
Computation pipeline formally verified with 24~new Lean~4 theorems
(zero sorry), building on 109~reused theorems from the 1+1D BEC
framework. All numerical predictions generated by a 95-test
Python pipeline.
\end{acknowledgments}
