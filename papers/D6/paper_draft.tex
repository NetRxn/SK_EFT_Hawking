...
\figuredeferred{fig:hypothesis-dependency}{the figure must be generated
...
that module yet; the content is fixed, namely which of (H1)--(H4) each
capstone consumes and which two are modelling assumptions rather than
consequences}
...
arbitrary $A$ and an arbitrary base rate, a level structure satisfying all
four. It additionally makes the malignant pairs off-diagonal, which is strictly
...
\figuredeferred{fig:threshold-double-exp}{double-exponential suppression
against level, with the malignant-pair count swept over the four values AGP
report ($7\,183$, $22\,701$, $35\,235$ and $36\,511$), so the model-choice
