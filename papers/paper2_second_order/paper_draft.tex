...
\textbf{Status:} All stress tests proved. Combined tally: $35/35$ proofs across 5 Aristotle rounds (14 Phase~1 core $+$ 8 Phase~2 core $+$ 10 Round~4 stress tests $+$ 3 Round~5 total-division strengthening). Zero sorry remaining.
...
\begin{enumerate}
\item When additional imaginary monomials (e.g., $(\partial_x \psi_a)^2$ at level 2) are included, the strict constraint $\gamma_{2,1} + \gamma_{2,2} = 0$ relaxes to the inequality $(\gamma_{2,1} + \gamma_{2,2})^2 \leq 4\gamma_2 \gamma_x \beta$ (Aristotle run \texttt{3eedcabb}, theorem \texttt{relaxed\_positivity\_weakens}). A first-principles derivation of the complete second-order FDR from the CGL transformation remains open (Direction~D).
\item Can the spectral distortion $\Delta n(\omega) \propto \omega^3$ be detected in current or near-future BEC experiments? Our estimates (Table~\ref{tab:corrections}) indicate $|\delta^{(2)}| \sim 10^{-12}$--$10^{-8}$, far below current sensitivity. Spin-sonic BEC systems with enhanced $c_{\text{density}}/c_{\text{spin}}$ ratios offer the most promising avenue.
\item Do boundary terms near the horizon modify the imaginary-sector IBP reduction at second order? The analysis assumes $\psi_a \cdot \partial_t^2 \psi_a$ reduces to $-(\partial_t \psi_a)^2$ globally. If horizon boundary terms contribute, additional imaginary monomials may enter, relaxing the positivity constraint as in item~1.
\end{enumerate}

\section{Conclusion}

We have extended the SK-EFT analysis of dissipative Hawking radiation to second order in the derivative expansion, establishing the counting formula for transport coefficients, the role of spatial parity, the positivity constraint, and the frequency-dependent spectral distortion. A key finding is that the positivity constraint $\gamma_{2,1} + \gamma_{2,2} = 0$ causes the second-order correction $\delta^{(2)}(\omega)$ to vanish on the acoustic shell, making it observable only through Bogoliubov dispersion at the level of $10^{-12}$--$10^{-8}$ for current experiments. The formal verification program comprises 35 Lean~4 theorems proved across 5 Aristotle rounds, including stress tests of FDR sign sensitivity, relaxed positivity, KMS optimality, and the no-dissipation limit. This provides the theoretical framework for precision measurements of the Hawking spectrum in next-generation BEC experiments, with spin-sonic systems offering the most promising path to observing second-order effects.
