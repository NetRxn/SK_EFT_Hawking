...
\end{itemize}
These transport-coefficient counts are formally machine-verified in Lean~4.
...
\begin{equation}
(\omega - kv)^2 = k^2(1 + D^2 k^2) - i\omega \Gamma(k, \omega),
\end{equation}
where $D = \xi \kappa / c_s$ is the dimensionless adiabaticity parameter (so the
Bogoliubov quartic term carries the coefficient $D^2 = (\xi\kappa/c_s)^2$) and
\begin{equation}
...
\input{tables/table2_hierarchy.tex}
