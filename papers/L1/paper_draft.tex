...
vestigial phase, computed in the random-phase approximation (RPA)
in this project's formalization
(\texttt{VestigialSusceptibility.lean}); the underlying
...
of this Letter is that the value it fixes is incompatible with every
value the identification's own microscopic origin would predict. The
2017 sweep constrained \emph{theories}; this constrains an
\emph{identification}, which is why it is stated as a disjointness of
two intervals rather than as a surviving region of parameter space.
...
Hertz--Millis convention, as formalized in
\texttt{VestigialSusceptibility.lean}. The natural range
...
the present framework, and this path is closed in the Lean rather than
argued: \texttt{vestigial\_dispersion\_below\_ligo\_at\_inspiral\_peak}
proves that at the project's conservative upper bound
$|\gamma| \leq 10^{-30}$ on $\Gamma_H/\cGW^2$ the correction at the
$100\,\mathrm{Hz}$ inspiral peak stays inside the GW170817 tolerance,
so it does not lift the falsification. The general form is
