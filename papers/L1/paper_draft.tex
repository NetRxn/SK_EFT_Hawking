...
as the disjointness theorem
\texttt{natural\_range\_disjoint\_from\_ligo\_window}: the natural
range $[0.1, 10]$ and the GW170817-compatible window are entirely
disjoint at any speed-of-light rescaling, so the identification
admits no consistent $\chivest$ outside the fine-tuning window.
The falsification is local to the leading-
...
independence). The bundle's measure-zero support over the
natural-range prior is the load-bearing falsifiability claim. (The
...
\paragraph{Phase~5y H1 caveat as a Lean disjointness theorem.} The
...
audit as an $\exists$-absorption tautology — the existence of
\emph{one} consistent value does not encode underdetermination,
which is the substantive Phase 5y H1 finding. We replace it with
the disjointness theorem
\texttt{natural\_range\_disjoint\_from\_ligo\_window}: for $\chivest$
...
quantifies over all $c > 0$ explicitly. Together these prove the
natural range and the GW170817-compatible window are disjoint by
...
layers. The Lean layer is \texttt{GravitationalWaves.lean} (21
theorems and lemmas, zero \texttt{sorry}, zero new axioms, zero
