...
Several limitations of the present Euclidean pilot study should be
noted:

\begin{enumerate}
\item \textbf{Euclidean vs.\ Lorentzian signature.} The Euclidean
  pilot avoids the sign problem but does not directly test Lorentzian
  geometry. The transition from positive-definite to Lorentzian signature
  eigenvalues is a qualitative change that requires separate investigation.

\item \textbf{Lattice size.} The $L=4$ lattice is small. Finite-size
  scaling studies at $L = 6, 8, 12$ are needed to confirm the
  thermodynamic limit and sharpen the phase boundaries.
...
\item The Equivalence Principle is violated in the vestigial phase:
  bosons experience geometry, fermions do not.
\end{enumerate}
