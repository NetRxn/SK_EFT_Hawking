...
tetrad (Einstein-Cartan) phase ($G > \Gc$). Production Monte Carlo
simulations at $L=4,6,8$ confirm the mean-field prediction and reveal
a \emph{split transition}: the tetrad and metric susceptibility peaks
occur at different couplings at $L=6$ and $L=8$, with a splitting
$\Delta(G/\Gc) \approx 0.63$ at $L=8$, consistent with a genuine
thermodynamic vestigial phase. We discuss the implications for the
gravity hierarchy---the vestigial phase constitutes Level~1 of a
three-level sequence toward UV-complete emergent gravity---and identify
the Equivalence Principle violation as a distinctive experimental
signature.
\end{abstract}
...
\item All structural results are formally verified in Lean~4.
  ...
  with zero \texttt{sorry}. The full project contains 429 theorems +
  2 axioms across 30 modules, 99 proved by the Aristotle automated
  theorem prover~\cite{Aristotle} across 27 runs.
\end{enumerate}
