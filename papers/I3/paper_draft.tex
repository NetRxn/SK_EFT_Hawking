...
infrastructure; substantive cross-bridge typeclass
(\texttt{LDPCompatibleSKEFT}); concrete-witness instances on the
program's existing linear-response rate function; and the
upstream-coordination scoping document. All headline theorems
...
recent years. Mathlib4's measure-theory hierarchy supplies a
production-quality framework for $\sigma$-algebras, measures, integration,
$L^p$ spaces, conditional expectation in Banach spaces, sub-Gaussian
moment bounds, martingales (with optional sampling and Doob's
inequality), filtrations, adapted and predictable processes,
stopping times, the local-martingale \texttt{Locally} framework, and
Markov kernels with composition and disintegration. Degenne's recent
...
$\sim$5800 theorems were written --- which is approximately one
...
formalization;
\textbf{(iii)} Mathlib4 ships martingales, sub-Gaussian, the
...
lemma. We do not assert exhaustivity beyond Coq + Isabelle/HOL +
Lean (HOL Light, Mizar, Agda, Metamath, F\textasteriskcentered, and
domain-specific provers were not surveyed); within those three
flagship-prover ecosystems and to the best of the survey's
findings, this library contributes the first explicit
predicate-level scaffolding for the It\^o stochastic integral,
It\^o's lemma, Novikov's condition, Sanov's theorem, the
contraction principle, and Varadhan's lemma. The substantive content
behind these predicates is, of course, classical and was
established in the analytic literature decades ago; what is new
here is the formal scaffolding by which an authored Lean~4 paper
can claim a result \emph{up to the substrate level} without
asserting more than the substrate carries.
...
instance fields conjoin the LDP-rate-function predicate, the
...
\begin{itemize}
\item \textbf{12 modules} ($\sim$1000 lines of Lean source).
\item \textbf{$\sim$30 Prop-typed predicates} across the 12 modules.
\item \textbf{$\sim$30 non-vacuity witnesses} (one per
  predicate, sometimes more).
\item \textbf{12 wave-closure summary theorems} (one per module).
\item \textbf{1 substantive cross-bridge typeclass}
  (\texttt{LDPCompatibleSKEFT}) and one concrete-instance witness
  (\texttt{linearResponseRateFunctionCentered}).
...
\textbf{Cross-bridge.}
\texttt{ito\_reduces\_to\_calculus\_when\_zero\_qv} establishes that
when $\langle X\rangle = 0$ (i.e., $X$ is finite-variation),
It\^o's lemma reduces to the ordinary fundamental theorem of
calculus. This is the connection that makes the It\^o substrate
...
\textbf{Wave-closure summary.}
\texttt{wave\_3b\_itoBeta\_5\_itoLemma\_closure}.
...
\textbf{Substrate-data predicate.}
\texttt{IsCramerLowerBound mgf I} declares the Cram\'er
\emph{lower} bound: under appropriate conditions,
...
from the program's deep-research appendix). The typeclass connects
five pieces of project content into one coherent interface:
...
given $(\beta, I)$ pair, but does not yet assert the substantive
quantitative content behind the predicates. When the substantive
content lifts (\S\ref{sec:mathlib-upstream}), the typeclass
instances re-prove without needing rearchitecting.
...
rate function with the LDP substrate's predicate-substrate trivial
inhabitations. The cumulative effect is that \emph{the program's
existing linear-response Gaussian rate function is registered as
LDPCompatibleSKEFT}, and downstream consumers can declare hypotheses
of the form \texttt{[LDPCompatibleSKEFT $\beta$ I]} and pick up
both the LDP infrastructure and the SK--EFT-axiomatic compatibility
content via typeclass inference.
...
\texttt{linearResponseRateFunctionCentered} is what closes Phase~6n
...
substrates. Whether the substantive quantitative content behind
each predicate matches the substantive quantitative content of the
LDP framework (Cram\'er, Sanov, Varadhan as classically stated) is
what the upstream Mathlib4 PR cycle (\S\ref{sec:mathlib-upstream})
will check at the substantive-content lift stage.
...
program's content-bundle targets~\cite{Roehm2026Strategy}. We are
explicit that, at the time of this Phase~6o.$\zeta$ release, no
downstream Lean theorem in the D3, D5, or E1 bundles
\emph{currently} invokes the \texttt{LDPCompatibleSKEFT} typeclass
or any of the It\^o-substrate predicates: the cross-bridges below are
\emph{positioned} for future absorption-pass adoption (per the
program's late-Phase-6 absorption protocol; D.2-additive events
are scheduled but not executed at the time of this release). The
...
The total PR cycle is therefore $\sim 12$--14 PRs (depending on the
...
positioned for a $\sim$12--14-PR upstream Mathlib4 cycle staged for
the post-Phase~6o window. The substrate-vs-quantitative-content
engineering choice that drives the release scope (substrate-data
predicates ship now; substantive quantitative content lifts in the
upstream PR cycle) is, to our knowledge, the first explicit
articulation of a software-substrate release pattern for
formally-verified-mathematics infrastructure paired with explicit
coordination with the upstream community --- a model we hope the
program's I2 + I3 release pair makes more legible for future
formal-verification software releases.

\begin{thebibliography}{99}

\bibitem{Roehm2026Strategy} J.~Roehm,
\emph{Paper Strategy: SK--EFT Hawking Bundle Architecture (14 Bundles)},
