...
\paragraph{Relation to existing libraries.}
\textbf{(i)} The Isabelle/HOL Archive of Formal Proofs
``Probability Theory'' topic (32 entries as of 2026-05-08) covers
Martingales (2023), Concentration Inequalities (2023),
Kolmogorov--Chentsov (2025), and Disintegration Theorem (2023), but
not the It\^o integral, It\^o's lemma, Novikov's condition, the
Cram\'er upper or lower bound, Sanov's theorem, the contraction
principle, or Varadhan's lemma.
\textbf{(ii)} The Coq community's curated awesome-coq library list
records ALEA (randomized-algorithm reasoning) and no Itô/SDE/LDP
formalization.
...
expectation in Banach spaces, and does not ship the It\^o integral,
It\^o's lemma, Novikov's condition, the Cram\'er upper or lower
bound, Sanov's theorem, the contraction principle, or Varadhan's
lemma. This library contributes predicate-level scaffolding for the
It\^o stochastic integral, It\^o's lemma, Novikov's condition, Sanov's
theorem, the contraction principle, and Varadhan's lemma, extending
the Mathlib4 measure-theory + probability infrastructure with the
substrate-data-level operationalizations the SK-EFT Hawking program
requires. (HOL Light and Mizar were not surveyed for this release; the
ecosystem comparison is restricted to Lean / Mathlib4, Isabelle/HOL
AFP, and the Coq community's curated awesome-coq list.) The substantive
content behind these predicates is classical and was established in
the analytic literature decades ago; what this release contributes to
the Lean ecosystem is the formal scaffolding by which a Lean~4 paper
can claim a result \emph{up to the substrate level} without asserting
more than the substrate carries.
...
substrate). The resulting cross-bridge is a typeclass whose
...
compatibilities, and the inf-projection inequality plus zero-at-zero for
the contraction principle. This cross-bridge is the
...
\begin{itemize}
\item \textbf{12 modules} ($\sim$800 lines of Lean source;
  exact count 775 across the 12 modules as of the Phase~6o.$\zeta$
  release, post-Stage-13 substantive substrate refactor).
\item \textbf{15 \texttt{Prop}-typed predicates} across the 12 modules
  (14 \texttt{def Is*} declarations plus the
...
\item \textbf{9 per-module wave-closure summary theorems plus 2
  overall-wave closures} (one per It\^o / LDP namespace). Four It\^o
  modules and all five substantive LDP modules carry a per-module
  closure; \texttt{Novikov.lean} and \texttt{ItoLemma.lean} carry none,
  \texttt{Novikov.lean} instead hosting the overall
  \texttt{wave\_3b\_ito\_overall\_closure}, and
  \texttt{LDPCompatibleSKEFT.lean} hosting
  \texttt{wave\_3b\_ldp\_overall\_closure}. Every one of the eleven is
  named in \S\ref{sec:ito} or \S\ref{sec:ldp}, so this count follows
  from the names rather than from hand-maintenance.
...
\figuredeferred{fig:i3-module-dag}{the twelve-module dependency DAG; \texttt{lean\_deps.json} has the edges and the renderer is the missing piece}
...
\textbf{Non-vacuity witness.}
\texttt{isSemimartingaleDecomposition\_zero\_witness} and
\texttt{isDoobMeyerUnique\_zero\_witness} discharge the two predicates
separately at the identically-zero process. This module is one of the
two carrying more than one substrate obligation, which is why its
wave-closure below is a genuine conjunction rather than a restatement.
...
\textbf{Non-vacuity witness.}
\texttt{isItoIsometry\_zero\_witness}: the zero integrand's zero
integral satisfies the isometry, both sides being zero.
...
\textbf{Non-vacuity witness.}
\texttt{cramerIID\_subGaussian\_witness} discharges the predicate at the sub-Gaussian case at the zero log-MGF, whose Legendre--Fenchel transform is the zero rate function.

\textbf{Wave-closure summary.}
\texttt{wave\_3b\_ldp\_alpha\_1\_cramerIID\_closure}. This module carries a single substrate obligation, so
its per-module closure restates the witness rather than conjoining
several; \texttt{Semimartingale.lean} and
\texttt{QuadraticVariation.lean} show the conjunctive form. The five
LDP per-module closures are gathered by
\texttt{wave\_3b\_ldp\_overall\_closure} in
\texttt{LDPCompatibleSKEFT.lean}.
...
\textbf{Non-vacuity witness.}
\texttt{sanov\_methodOfTypes\_witness} discharges the predicate at the binary alphabet ($n = 2$), the smallest alphabet on which the method of types is not degenerate.

\textbf{Wave-closure summary.}
\texttt{wave\_3b\_ldp\_alpha\_2\_sanov\_closure}. This module carries a single substrate obligation, so
its per-module closure restates the witness rather than conjoining
several; \texttt{Semimartingale.lean} and
\texttt{QuadraticVariation.lean} show the conjunctive form. The five
LDP per-module closures are gathered by
...
\textbf{Non-vacuity witness.}
\texttt{isContractionPrinciple\_witness} discharges the predicate at the constant map, under which the pushed-forward rate function is the zero rate function.

\textbf{Wave-closure summary.}
\texttt{wave\_3b\_ldp\_alpha\_3\_contraction\_closure}. This module carries a single substrate obligation, so
its per-module closure restates the witness rather than conjoining
several; \texttt{Semimartingale.lean} and
\texttt{QuadraticVariation.lean} show the conjunctive form. The five
LDP per-module closures are gathered by
...
\textbf{Non-vacuity witness.}
\texttt{isCramerLowerBoundEsscher\_witness} discharges the predicate at the Esscher tilt at zero, where the tilted measure is the source measure.

\textbf{Wave-closure summary.}
\texttt{wave\_3b\_ldp\_beta\_1\_cramerLowerBound\_closure}. This module carries a single substrate obligation, so
its per-module closure restates the witness rather than conjoining
several; \texttt{Semimartingale.lean} and
\texttt{QuadraticVariation.lean} show the conjunctive form. The five
LDP per-module closures are gathered by
...
\textbf{Non-vacuity witness.}
\texttt{isVaradhanUpperBound\_witness} discharges the predicate at the zero test function, for which the variational formula's two sides coincide at zero.

\textbf{Wave-closure summary.}
\texttt{wave\_3b\_ldp\_beta\_2\_varadhan\_closure}. This module carries a single substrate obligation, so
its per-module closure restates the witness rather than conjoining
several; \texttt{Semimartingale.lean} and
\texttt{QuadraticVariation.lean} show the conjunctive form. The five
LDP per-module closures are gathered by
...
The four LDP-side anonymous-constructor tuples carry the substantive
structural components added in today's strengthening pass: the
Cram\'er upper-bound predicate now requires $(\mathrm{Continuous}\ \mathrm{mgf},\ \mathrm{mgf}(0) \leq 1)$
as the MGF-normalisation companion to rate-function continuity and
zero-at-zero; the Sanov predicate adds an MGF reflexivity component
$(\mathrm{mgf}(0) = 0)$; the contraction principle now constrains
the push-forward map $f$ as well as $I_X$ and $I_Y$ to be continuous;
the Varadhan predicate adds $(I(0) = 0,\ F(0) = 0)$ centered-functional
bounds. These components are precisely what makes the four LDP-side
\texttt{Prop} bodies refutable rather than vacuous.
...
program's content-bundle targets~\cite{Roehm2026Strategy}. \textbf{One
consumer exists; D3, D5 and E1 do not yet.} The D9 bundle's
\texttt{fdt\_rare\_event\_tail\_is\_ldp\_certified} invokes
\texttt{LDPCompatibleSKEFT} on the centered linear-response rate function,
so \texttt{SKEFTHawking.LDP} declarations now appear in a downstream
bundle's dependency closure. No D3, D5, or E1 Lean module invokes
\texttt{LDPCompatibleSKEFT} or any of the It\^o-substrate
...
deferral is deliberate: no upstream pull request is drafted at this
stage. Material that may eventually be upstreamed is developed in-tree
first and follows Mathlib naming and style conventions throughout, so
that preparing a later pull request requires no structural rework.
...
(announced in I2 §7's AI-tool-assistance-disclosure subsection
and carried forward into I3), the
...
\figuredeferred{fig:i3-upstream-map}{per-module PR scope against Mathlib's existing coverage; the Mathlib-side coverage audit is a manual survey that has not been redone since the v4.32.0 bump}
...
The library builds against Lean~4 v\leantoolchain (per the project's
...
\emph{SK--EFT Hawking publication architecture (project paper-strategy frame)},
