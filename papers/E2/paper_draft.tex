...
flips between the two electron-electron lengths in circulation here, so we
state the gradient and transport inputs as the assumptions they are.
\end{abstract}
...
$\hbar/(4\pi k_B)$~\cite{KSS2005,Romatschke2010}. We adopt these with the
scope conditions stated below.
...
Equation~\eqref{eq:diss} also settles which way the geometry should be
pushed. The binding rate is viscous, not momentum-relaxing, and the window
$\omega_H/\Gamma_H$ scales as $1/\kappa$ because $\Gamma_H \propto \kappa^2$:
it is $2.5$ at the bilayer nozzle against $0.46$ and $0.23$ for monolayer
constrictions of $100$ and \SI{50}{\nano\meter}, both below unity, where
the quanta damp faster than they oscillate. Shrinking the constriction to
raise $T_H$ therefore closes the detection window rather than opening it.
...
fluxes ($I^{\rm HR} = 3$ and $5$ at \SI{20}{\giga\hertz} detuning follow
from their two $\kappa$ to within rounding). Under Eq.~\eqref{eq:T_H} their
...
sign of the net shift. The machine-checked chain
...
the physical inputs; the statements it does establish are quoted above in
full so that the difference is visible.
