...
same $\pi/6$, which is now the single canonical value in this project's
constants, imported rather than retyped wherever it is used. We display
$c_1$ rather than absorb it, so that adopting a different profile's
coefficient rescales every dispersive number here by $c_1/(\pi/6)$ and
leaves the qualitative conclusions untouched.
