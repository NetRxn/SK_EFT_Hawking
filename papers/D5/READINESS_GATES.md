...
1. **§9 Track B 8/8 unanimous NO-GO closure** — the *first complete-mechanism-family unanimous NO-GO closure* in Phase 6m. Splits across two structurally distinct routes: 4 quantitative-Bayesian bounds aggregated in `EntropicGravityDarkEnergy.all_quantitative_bounds_exceed_jeffreys_decisive` (Verlinde 2017, Tsallis HDE, Barrow HDE, Odintsov-D'Onofrio-Paul, all |log𝓑| ≥ 5 Jeffreys-decisive) + 4 structurally-NO-GO via mechanism-specific obstructions (Padmanabhan/CosMIn no-Lagrangian-scalar; Hossenfelder-Verlinde post-Yoon-Guha; Cadoni-Tuveri DEC via GD theorem; HDE event-horizon wrong-sign w_a). r_d-anchoring rescue robust across all 8.
