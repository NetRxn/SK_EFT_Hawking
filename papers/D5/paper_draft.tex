...
ADW effective potential $V_{\rm eff}(C) = (C^4/4) \ln(C/\Lambda) -
C^4/16$ from the gap equation. Critical point at $C_0 = \Lambda
e^{-1/4}$ (Lean: \texttt{critical\_coupling\_pos}); depth
$-\Lambda^4/(4e)$. In perfect Minkowski vacuum the Volovik tetrad
self-tuning absorbs this to $\Lambda_{\rm eff} = 0$. Lifted from paper17.}
\end{figure}
...
\caption{\label{fig:zhitnitsky-theta-scan}
Zhitnitsky topological-DE energy density scan: $\rho_{\rm DE} = \theta^2
\Lambda_{\rm QCD}^6 / M_P^2$ versus the topological angle $\theta$. The
naturally-$\mathcal{O}(1)$ value $\theta = 1$ (rightmost band) lies
$\sim 240\times$ above the observed $\rho_{\rm DE,obs} = 2.8 \times
10^{-11}\,\text{eV}^4$ (horizontal line); $|\theta| \lesssim 10^{-9}$
constrained by the neutron-EDM bound (vertical line) gives
$\rho_{\rm DE} \sim 10^{-29}\,\text{eV}^4$. Both bounds sit
$\sim 120$ orders below the Planck-natural $M_P^4$. Lifted from paper32.}
\end{figure}
...
live in \texttt{CosmologicalConstantEmergent.lean} +
...
across the natural $(\Lambda_{\rm UV}, N_f)$ parameter band. At
$(\Lambda_{\rm UV} \simeq M_{\rm Pl},\ N_f = 16)$ the prediction
exceeds $\Lambda_{\rm obs} \simeq 2.6 \times 10^{-47}\,\text{GeV}^4$ by
$> 10^{120}$. Verdict bands: resolved (green; $\Lambda^{\rm emerg}
\approx \Lambda_{\rm obs}$), intermediate (yellow), reproduced
(red; classical CC problem regenerated at the natural microscopic
point). Lifted from paper42b.}
\end{figure}
