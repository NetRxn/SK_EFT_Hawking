...
\emph{first complete-mechanism-family unanimous NO-GO closure within
the SK-EFT Hawking project} (Phase~6m)
via two structurally distinct routes: four candidates yield
quantitative disfavour ledgers, of which two (Tsallis~HDE,
Odintsov-D'Onofrio-Paul) reach Jeffreys-decisive at the
Bayes-factor level ($|\!\log\mathcal{B}| \geq 5$); one (Verlinde~2017)
is excluded at $>5\sigma$ by galaxy-cluster mass-density tests under
nominal profile assumptions (Halenka-Miller, PRD 102, 084007 (2020);
the exclusion weakens once profile systematics are included); and one
(Barrow~HDE) reaches Burnham-Anderson moderate-disfavour at the
information-criteria level (primary-source-verified
$\Delta\!\text{AIC} = +4.7$ from Luciano-Paliathanasis-Saridakis 2026
Table~II, below the conventional Bayes-factor decisive cutoff but in
the ``considerably less support'' regime). The quantitative route is
formalized in the Lean aggregators
\texttt{both\_decisive\_bayes\_bounds\_exceed\_jeffreys\_decisive}
(2 of 4, the Bayes-factor cohort) and
\texttt{all\_quantitative\_bounds\_disfavoured} (4 of 4
under mixed, per-candidate unit-coherent thresholds:
$\sigma$ / $\log\mathcal{B}$ / $\Delta\!\text{AIC}$ /
$\log\mathcal{B}$); the
remaining four (Padmanabhan/CosMIn no-Lagrangian-scalar; Hossenfelder-Verlinde
post-Yoon-Guha; Cadoni-Tuveri DEC via the GD theorem; HDE event-horizon
wrong-sign $w_a$) are structurally NO-GO via mechanism-specific
obstructions. The closure is robust under the $r_d$-anchoring rescue. (c)~Track~C Jacobson-thermodynamic GR is the
...
BEC. A unified 7-class GD taxonomy organises this bundle's
$\bundleTheorems$ substantive theorems under a 3-tier applicability
gradient.
...
throughout. The bundle ships at $\bundleTheorems$ Lean theorems across
$\bundleModules$ modules with zero active sorry and zero project-local
axioms (the formerly-tracked gapped-interface statement is now the
tracked Prop \texttt{TPFConjecture}).
...
entropic-gravity DE, with all four explicitly-quantitative disfavour
ledgers at moderate-or-greater disfavour on their native scales (two
Bayes-factor Jeffreys-decisive; one nominal-assumptions $5\sigma$
galaxy-cluster exclusion; one AIC-moderate) and all eight closures
robust under $r_d$-anchoring rescue.
...
/ zero active sorry / zero project-local axioms; the formerly-tracked
\texttt{gapped\_interface\_axiom} from Phase~5s, unrelated to the dark
sector, was retired into the tracked Prop \texttt{TPFConjecture} on
2026-05-19) and the canonical formula
...
hidden charge. The minimal Lagrangian solutions are Wang's topological~\cite{Wan2019}
order T-0 (a $K$-gauge TQFT with no propagating particle content) and
three sterile Weyl fermions S-0; both are tagged by the
\texttt{DarkSectorSynthesis.lean} substrate definitions
\texttt{candidate\_T0} / \texttt{candidate\_S0}. A third option C-1
(Wan-Wang mixed-charge with a dark $\text{SU}(3)$, defined in
\texttt{HiddenSectorMixedCharge.lean}) is shipped under the
...
The Abrikosov--Diakonov--Wetterich (ADW)~\cite{ADW2019,Diakonov2011,Vergeles2025} tetrad-condensation gap
equation, formalized in \texttt{ADWMechanism.lean} and
\texttt{CosmologicalConstant.lean}, exhibits a Volovik-style~\cite{Volovik2006,KV2008} equilibrium
...
For provenance: the quoted significances are single-parameter $w_0$
distances of the KV effective-CDM prediction $(w_0, w_a) = (-1, 0)$
from the DESI~DR2 $w_0 w_a$CDM posteriors~\cite{DESI2025}, whose BAO
constraints extend the DR1 analysis~\cite{DESI2024},
$w_0 = -0.838 \pm 0.055$, $w_a = -0.62^{+0.22}_{-0.19}$
(DESI+CMB+Pantheon$+$, Eq.~(26) of arXiv:2503.14738) and
$w_0 = -0.752 \pm 0.057$, $w_a = -0.86^{+0.23}_{-0.20}$
(DESI+CMB+DESY5, Eq.~(28)), i.e.\
$|{-1} - \bar{w}_0| / \sigma_{w_0} \simeq 2.9$ and $4.4$ respectively;
DESI's own two-parameter $\Delta\chi^2_{\rm MAP}$ preference for
$w_0 w_a$CDM over $\Lambda$CDM in the same data combinations is
$2.8\sigma$ and $4.2\sigma$.
...
\caption{\label{fig:phase5x_viability_matrix}
Phase~5x candidate viability matrix: 4-of-5 candidates pass basic
viability filters, the FG-torsion candidate is excluded (encoded as
\texttt{phase5x\_candidates\_viability\_matrix} in
\texttt{DarkSectorSynthesis.lean}; honesty-table label MCC since the
flag values are hand-assigned per W2/W4/W7 deep-research bounds).
Lifted from paper17.}
\end{figure}
...
\caption{\label{fig:sfdm-money-plot-left}
Money plot (left): DM--galaxy offset vs $M = v_{\rm infall}/c_s$ for
SFDM (step function at $M{=}1$, the defining signature), SIDM (smooth
monotonic rise at $\sigma/m = \SI{1}{\centi\meter\squared\per\gram}$),
and CDM (null). Five canonical mergers plotted as diamonds. Lean:
\texttt{sfdm\_offset\_step\_function}. Lifted from paper17.}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig28_adw_effective_potential.png}
\caption{\label{fig:adw-veff}
ADW effective potential $V_{\rm eff}(C/\Lambda)/\Lambda^4$ from the
Coleman--Weinberg gap equation, plotted for three coupling ratios:
$G/G_c = 0.5$ (pre-geometric, dashed), $G/G_c = 1.0$ (critical, dotted),
and $G/G_c = 2.0$ (full tetrad, solid). The full-tetrad branch develops
a non-trivial minimum at $C/\Lambda \simeq 0.45$, signalling spontaneous
emergence of the tetrad VEV. Critical-coupling positivity is established
in Lean by \texttt{ADWMechanism.critical\_coupling\_pos}. In perfect
Minkowski vacuum the Volovik tetrad-determinant self-tuning absorbs the
condensate energy to $\Lambda_{\rm eff} = 0$. Lifted from paper17.}
\end{figure}
...
\texttt{BBN.lean} with $\bbnThms$ substantive theorems / 0 sorry /
...
\texttt{bbn\_violators\_share\_n\_eff\_failure\_mode} establishes the
universal $N_{\rm eff}$-channel mode-of-failure across the violator
subset under default thermalization; the corresponding
\texttt{at\_least\_three\_phase5x\_candidates\_bbn\_conformant} closes the
$\geq 3$-of-5 unconditional pass.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_bbn_conformance_matrix.png}
\caption{\label{fig:bbn-conformance}
BBN conformance matrix for the five Phase~5x DM candidates against
Planck~$2\sigma$ slack $\Delta N_{\rm eff} \leq 0.34$: T-0, C-1, and
FractonPWave pass unconditionally; S-0 (sterile) gives $\Delta N_{\rm
eff} = 3.0$ (factor $\sim 7.8\times$ excess) and FGTorsion gives
$\Delta N_{\rm eff} \geq 1.0$ as radiation-thermalized e-loops, both
conditional on thermalization at $T_{\rm BBN} \approx
\SI{1}{\mega\electronvolt}$. Lifted from paper29.}
\end{figure}
...
paper (Phase~6c W1) into D5. It formalises, via the
Van Waerbeke--Zhitnitsky~\cite{VanWaerbeke2025} topological-DE mechanism, a structural bridge
between two long-standing Standard-Model puzzles: the smallness of the
QCD topological angle $|\theta| \lesssim 10^{-9}$ (Pendlebury~2015
neutron EDM) and the observed cosmological-constant energy density
$\rho_{\rm DE} \approx 2.8 \times 10^{-11}\,\text{eV}^4$
(Planck~2018 + DESI~2024). The bridge ships in
\texttt{StrongCPTopologicalDE.lean} as $8$ substantive theorems
(macro $\strongCpDeThms{}$).

\subsection{Zhitnitsky topological-DE prediction (no free parameters)}
\label{subsec:zhitnitsky}

The mechanism predicts $\rho_{\rm DE} \sim \Lambda_{\rm QCD}^6 / M_P^2$
with no tunable parameters. At the PDG QCD scale
$\Lambda_{\rm QCD} = \SI{0.1}{\giga\electronvolt}$ and Planck mass
$M_P = 1.22 \times 10^{19}\,\text{GeV}$, the prediction is
$\rho_{\rm DE} \approx 6.7 \times 10^{-9}\,\text{eV}^4$, sitting within
${\sim}240\times$ of the observed value (well within the
three-orders-of-magnitude window quoted in the Zhitnitsky proposal),
and approximately $120$ orders of magnitude below the Planck-natural
estimate $M_P^4 \approx 10^{112}\,\text{eV}^4$. The 120-order
suppression is the substantive structural payoff. The Lean formula
\texttt{zhitnitskyDE\_eV4} encodes the prediction, and
\texttt{zhitnitsky\_DE\_far\_below\_planck} bounds it strictly below
$10^{-8}\,\text{eV}^4$ via \texttt{norm\_num}.

\subsection{$\theta$-vacuum invariant + naturally-$\mathcal{O}(1)$ violation}
\label{subsec:theta-vacuum-load-bearing-invariant}

The $\theta$-vacuum is encoded as a Lean structure carrying the
experimental neutron-EDM constraint as a load-bearing invariant. A
naturally-$\mathcal{O}(1)$ value $\theta = 1$ violates this invariant
by nine orders of magnitude relative to the
$|\theta| \lesssim 10^{-9}$ bound. The Lean module
\texttt{ThetaVacuumFiniteT.lean} adds the finite-temperature behaviour
(cosine-based free energy global minimum at $\theta = 0$ via
\texttt{Real.cos\_le\_one}; $\pi^2 / 3$ misalignment correction;
SN1987A axion bound below the Planck-DM cosmological window).

\subsection{Combined-mechanism falsifier (mutual-exclusivity)}
\label{subsec:combined-mechanism-falsifier}

The substrate program's ADW-tetrad-condensation $\rho_{\rm vac}$
contribution (§\ref{subsec:adw-cc}) and the Zhitnitsky topological
contribution are both candidates for the observed $\rho_{\rm DE}$. The
Lean theorem \texttt{H\_BothActiveGivesInconsistency} (tracked Prop)
encodes the mutual-exclusivity falsifier: if both mechanisms are
simultaneously active and additive without cancellation, the combined
DE density exceeds $\rho_{\rm DE,obs}$ by at least one order of
magnitude. The Lean Prop body uses
...
\subsection{Substrate-silent on $\bar\theta$ (cross-bridge to paper~43)}
\label{subsec:substrate-silent-thetabar}

The paper-43 anomaly cross-bridge
(\texttt{Z16AnomalyForcesThetaBar.lean}) carries the
\emph{substrate-silent} verdict on $\bar\theta$: the substrate program's
$\mathbb{Z}_{16}$ machinery does not fix $\bar\theta$ via
channel--flavour orthogonality. Strong-CP is therefore isolated as the
unique remaining BSM hierarchy that the program's $\mathbb{Z}_{16}$
content cannot constrain; the Branch~$\gamma$ falsifier requires the
CDVW + Abel-2020 PSI neutron-EDM bound (sensitivity factor
${\sim}10^{11}$ above $\bar\theta < 10^{-9}$). The substantive D5
implication: the substrate program admits Zhitnitsky topological-DE as
a CC-channel mechanism that does \emph{not} require fine-tuning of
$\bar\theta$; the prediction is parameter-free given $\Lambda_{\rm QCD}$
and $M_P$.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_zhitnitsky_de_theta_scan.png}
\caption{\label{fig:zhitnitsky-lambda-qcd-scan}
Zhitnitsky topological-DE prediction across $\Lambda_{\rm QCD}$ with no
free parameters. \emph{Left:} predicted $\rho_{\rm DE}(\Lambda_{\rm QCD})
= \Lambda_{\rm QCD}^6 / M_P^2$ on log-log scale, $\Lambda_{\rm QCD} \in
[1\text{ MeV},\,1\text{ GeV}]$. The amber band marks the observed
$\rho_{\rm DE,obs} \simeq 2.8 \times 10^{-11}\,\text{eV}^4$ (Planck~2018);
the vertical dotted line marks the PDG value
$\Lambda_{\rm QCD} \approx 100\,\text{MeV}$. \emph{Right:} bar chart
of the cosmological-constant-problem suppression: the Zhitnitsky-predicted
$\rho_{\rm DE}$ at PDG $\Lambda_{\rm QCD}$ sits $\sim 120$ orders of
magnitude below the Planck-natural $M_P^4$ estimate, with the observed
$(2.3\,\text{meV})^4$ value sitting close to the prediction (within
$\sim 2$ orders without free parameters). Lifted from paper32.}
\end{figure}
...
decidable predicates in Lean~4~\cite{Lean4,Mathlib} (\texttt{EquivalencePrinciple.lean}),
and proves the structural
...
\item \emph{Satisfy WEP / EEP / SEP} (4 mechanisms):
  ...
  (Pretko~2017)~\cite{Pretko2017,Glodkowski2024}, Berezhiani--Khoury~\cite{Berezhiani2015,BK2025} Thomas--Fermi superfluid DM
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_ep_violation_matrix.png}
\caption{\label{fig:ep-violation-matrix}
EP-violation matrix across six emergent-gravity mechanisms vs three EP
levels (WEP, EEP, SEP). Two violators (vestigial differential coupling at
$\eta = 1$, already excluded by MICROSCOPE; vestigial relics at $\eta
\sim 10^{-18}$, at STEP design sensitivity) plus four conformant
mechanisms (Einstein-Cartan torsion-trace, fracton subdiffusion,
Berezhiani-Khoury SFDM, $\mathbb{Z}_{16}$ singlets) all at
\texttt{violationLevel = none}. Lean:
\texttt{vestigial\_phase\_eta\_violates\_microscope\_bound}. Lifted
from paper34.}
\end{figure}
...
Decision-Gate-E.4 closure is in D3~§21; the D5 excerpt focuses on the
dark-sector implication.

\subsection{Decision Gate E.4: the classical CC problem is reproduced}
\label{subsec:42b-decision-gate-e4}

The substantive paper~42b result is a quantitative Lean theorem: at the
natural microscopic point $(\Lambda_{\rm UV} \simeq M_{\rm Pl},\
N_f = 16)$, the heat-kernel-induced emergent cosmological constant
\begin{equation}
\Lambda^{\rm emerg}(\Lambda_{\rm UV}, N_f)
= a_0(N_f) \cdot \Lambda_{\rm UV}^4
\end{equation}
\emph{exceeds} the Planck~2018 observed value
$\Lambda_{\rm obs} \simeq 2.6 \times 10^{-47}\,\text{GeV}^4$ by more than
$10^{100}$ -- the exponent the Lean theorem actually carries
(\texttt{lambdaEmergMicroscopic\_at\_planck\_natural\_far\_exceeds\_observed}
bounds the ratio below by $10^{100}$; the familiar heuristic figure is
$\sim10^{124}$, which is NOT what is proved). The classical
cosmological-constant problem is
\emph{reproduced} in the emergent-gravity formulation; no fine-tuning
of $(\Lambda_{\rm UV}, N_f)$ resolves it within the natural high-energy
parameter band. The Lean theorem and supporting closed-form expressions
live in \texttt{MicroscopicCoefficientMatch.lean} +
\texttt{HeatKernelExpansion.lean}; the proof is closed at zero sorry
and a companion Python pipeline reproduces the natural-parameter scan
and verdict-band figure (resolved / intermediate / reproduced phases).

\subsection{D5 implication: heat-kernel $a_0$ does not substitute for
ADW Volovik self-tuning}
\label{subsec:42b-d5-implication}

The Sakharov--Adler induced Newton-constant calibration
($G_N^{\rm emerg}$ from heat-kernel $a_2$) gives $G_N$ at the right
scale to within a $\mathcal{O}(1)$ prefactor (D3~§17 + Decision Gate~E.2).
But the cosmological-constant scale
$\Lambda_{\rm obs} \sim \rho_{\rm vac}^{1/4} \sim \SI{}{\milli\electronvolt}$
\emph{requires} either the §\ref{subsec:adw-cc} Volovik
tetrad-determinant self-tuning, the §\ref{sec:paper32} Zhitnitsky
topological-DE mechanism, or an analogous substrate-level cancellation;
the heat-kernel expansion alone does not deliver it. This is the D5
``CC-channel constraint'' verdict: the $a_0$ coefficient is the
\emph{wrong} candidate for the observed CC; the substrate program's
positive predictions sit elsewhere (vestigial-tetrad self-tuning at
meV; topological-vacuum suppression at 120 orders below $M_P^4$).

Together with Wave~1's Decision Gate~E.2 (heat-kernel
$\leftrightarrow$ linearized-EFE Newton-constant calibration), Decision
Gate~E.4 closes the structural assessment of the heat-kernel approach to
the dark-sector CC channel: the program reproduces the classical CC
problem on this channel, leaving the §\ref{subsec:adw-cc} +
§\ref{sec:paper32} mechanisms as the load-bearing CC predictions.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_lambda_emerg_parameter_scan.png}
\caption{\label{fig:lambda-emerg-scan}
Emergent cosmological constant $\Lambda^{\rm emerg}(\Lambda_{\rm UV},
N_f) = a_0(N_f) \cdot \Lambda_{\rm UV}^4$ from heat-kernel expansion
across the natural $(\Lambda_{\rm UV}, N_f)$ parameter band.
\emph{Left panel:} log-log $\Lambda^{\rm emerg}(\Lambda_{\rm UV})$ at
fixed $N_f \in \{1, 4, 16, 100\}$ (steel-blue solid / amber dashed /
berry dotted / cool-grey dash-dotted), with $\Lambda_{\rm obs}
\simeq 2.6 \times 10^{-47}\,\text{GeV}^4$ as a horizontal reference and
$M_{\rm Pl}$ + the resolution locus as vertical reference lines.
\emph{Right panel:} continuous Cividis heatmap of
$\log_{10}(\Lambda^{\rm emerg}/\Lambda_{\rm obs})$ over the
$(\Lambda_{\rm UV}, N_f)$ plane, with two contour overlays: the
\emph{resolved} boundary $|\!\log_{10}(\Lambda^{\rm emerg}/\Lambda_{\rm obs})|
= 1$ (steel-blue) and the \emph{reproduced} boundary
$\log_{10}(\Lambda^{\rm emerg}/\Lambda_{\rm obs}) \simeq 60$ (amber).
At $(\Lambda_{\rm UV} \simeq M_{\rm Pl},\ N_f = 16)$ the prediction
exceeds $\Lambda_{\rm obs}$ by $> 10^{100}$ (the rigorous bound), the classical
cosmological-constant problem reproduced in the emergent-gravity
formulation. Lifted from paper42b.}
\end{figure}
...
(Verlinde 2017~\cite{Verlinde2017dSEmergent}, Padmanabhan-CosMIn,
Hossenfelder-Verlinde, Cadoni-Tuveri DEC,
...
\subsection{Per-mechanism Lean closures}
\label{subsec:phase6m-track-b-bounds}

The eight Track~B mechanisms split into four with explicitly-quantitative
disfavour ledgers and four structurally NO-GO mechanisms
(Padmanabhan/CosMIn, Hossenfelder-Verlinde, Cadoni-Tuveri~DEC, HDE
event-horizon). The four with ledgers are two Bayes-factor
Jeffreys-decisive ($|\!\log\mathcal{B}| \geq 5$: Tsallis~HDE,
Odintsov-D'Onofrio-Paul; aggregated in
\texttt{both\_decisive\_bayes\_bounds\_exceed\_jeffreys\_decisive}), one
$\sigma$-significance (Verlinde~2017, excluded at $>5\sigma$ by
galaxy-cluster mass densities under nominal profile assumptions), and one
AIC-moderate (Barrow~HDE, $\Delta\!\text{AIC} = +4.7$); all four are
aggregated unit-coherently
($\sigma$ / $\log\mathcal{B}$ / $\Delta\!\text{AIC}$ / $\log\mathcal{B}$)
in \texttt{all\_quantitative\_bounds\_disfavoured}. The 8 mechanism-level
closures are listed first; two auxiliary supports for the Cadoni-Tuveri
DEC entry follow.

\begin{itemize}
\item \texttt{verlinde\_2017\_no\_go\_via\_cluster\_mass\_densities\_halenka\_miller}:
  Verlinde 2017~\cite{Verlinde2017dSEmergent} is excluded at $>5\sigma$
  by galaxy-cluster mass-profile shape tests: cores and outskirts of
  23 relaxed clusters with weak-lensing total-mass and X-ray gas-mass
  profiles, under nominal profile assumptions; the same analysis finds
  the exclusion weakens once profile systematics are
  included~\cite{HalenkaMiller2020} (PRD \textbf{102}, 084007 (2020);
  arXiv v1 2018). The CMB and Bullet-cluster legs are structural
  critiques carried from the Phase~6m dossiers
  (no scalar perturbation theory for the CMB; apparent-DM tracks
  baryons vs.\ the lensing--X-ray offset) and are not attributed to
  Halenka-Miller.
...
\item \texttt{hossenfelder\_verlinde\_no\_go\_post\_yoon\_guha}:
  ...
  after Yoon-Guha 2304.07301~\cite{YoonGuha2023} (matter+radiation FLRW stabilizes the dS
...
\item \texttt{cadoni\_tuveri\_dec\_no\_go\_via\_gd\_theorem\_w\_eq\_minus\_1}:
  ...
  $w = -1$. Two auxiliary Lean supports back this closure:
  \texttt{dec\_cpl\_excluded\_from\_desi\_dr2\_1sigma} (the DEC CPL
  parameterization $(w_0, w_a) = (-1, 0)$ excluded from the DESI~DR2
  $1\sigma$ confidence region; cross-bridge to Phase~5y
  \texttt{desiDR2\_1sigma}) and
  \texttt{dec\_fails\_orthogonality\_principle} (DEC fails Phase~5y
  \texttt{IsViable} via the factor-\#1 chain through the Phase~5y
  \texttt{viability\_iff\_all\_four} chain).
...
\item \texttt{tsallis\_hde\_no\_go\_bayes\_factor\_tyagi\_haridasu\_basak}:
  Tyagi-Haridasu-Basak 2504.11308~\cite{TyagiHaridasuBasak2025} reports $\Delta\!\log\mathcal{B}
  \sim -8$ to $-13$ for the Gravity-Thermodynamic framework
  (DESI~DR2 + Pantheon$+$); extracted as the Tsallis-limit reading,
  $|\!\log\mathcal{B}| \gtrsim 6$ exceeds Jeffreys-decisive. The
  registry-anchored point estimate $6.2$ is conservative within the
  reported $-8$ to $-13$ aggregate range.
\item \texttt{barrow\_hde\_disfavoured\_information\_criteria\_luciano\_paliathanasis\_saridakis}:
  Luciano-Paliathanasis-Saridakis 2506.03019~\cite{LucianoPaliathanasisSaridakis2506} (JHEAp 49, 100427, 2026)
  applies the Akaike Information Criterion (AIC) on the SN+BAO,
  SN+OHD+BAO joint datasets and reports a max
  $\Delta\!\text{AIC} = +4.7$ disfavouring Barrow HDE relative to
  $\Lambda$CDM (Table~II, Barrow Entropy SN+BAO column;
  primary-source-verified 2026-05-08 via direct PDF body fetch). This
  falls in the Burnham-Anderson ``considerably less support'' regime
  ($\Delta\!\text{AIC} \geq 4$) but \emph{below} the conventional
  Bayes-factor Jeffreys-decisive cutoff ($\geq 5$). Methodology is
  information-criteria (AIC) only (neither BIC nor Bayes factors);
  the theorem-name's prior \texttt{bayes\_factor} suffix has been
  retired and replaced with the methodology-honest
  \texttt{disfavoured\_information\_criteria} form, and the historical
  registry-anchored placeholder $5.5$ has been replaced with the
  primary-source value $4.7$. The companion Lean theorem
  \texttt{barrow\_aic\_delta\_below\_jeffreys\_decisive} records the
  $4.7 < 5$ inequality explicitly.
...
Seven of the eight per-candidate theorems establish their NO-GO from
that candidate's own quantitative falsifier (six numerical exclusions
plus the Hosssenfelder--Verlinde residual $\omega_{\mathrm{cdm}}$ CMB
exclusion, \texttt{hossenfelder\_verlinde\_residual\_no\_go\_cmb\_absent\_cold\_dark\_matter},
a $>5\sigma$ Planck-2018-anchored bound); the eighth
(Padmanabhan/CosMIn) is a disclosed structural no-go (no scalar
perturbation theory in the CosMIn axiom set; registered as
definitional). That is where the proof load sits, and
\texttt{entropic\_gravity\_seven\_genuine\_per\_candidate\_falsifiers}
records the genuine conjunction. The aggregator \texttt{entropic\_gravity\_no\_go\_count\_eight}
records the 8/8 tally, and \texttt{r\_d\_independent\_count\_eight}
records that each of the eight bounds carries the $r_d$-anchoring-independent
flag (no rescue across any candidate); both aggregators are bookkeeping
over the per-candidate legs, a verified-consistent classification ledger
rather than the proof load itself. The mixed-threshold aggregators
\texttt{both\_decisive\_bayes\_bounds\_exceed\_jeffreys\_decisive}
(2 of 4 quantitative ledgers, namely Tsallis and Odintsov, at
Bayes-factor Jeffreys-decisive) and
\texttt{all\_quantitative\_bounds\_disfavoured} (all 4 at moderate-or-greater
disfavour on their native scales, with Verlinde at the nominal-assumptions
$5\sigma$ cluster level and Barrow at the AIC moderate level) close the mechanism
family at the publication-novelty level: this is the first
...
The closure is $r_d$-independent: each of the eight quantitative or
structural disfavour ledgers (two Bayes-factor decisive bounds + one
$\sigma$-significance cluster exclusion + one
AIC moderate-disfavour bound + four structural NO-GO mechanisms) is
computed with the sound-horizon anchor allowed to vary across the
$r_d \in [144, 153]$ Mpc range that the literature spans
\cite{ParkRatra2025rd,WangYinTension2025rd}, and the
disfavour persists. The mechanism family is excluded
...
DESI~DR2 + literature-statistical-evidence level (mixed
$\sigma$-significance / Bayes-factor / information-criteria
methodology).
...
2504.05432~\cite{PlazaKraiselburd2025fR} (PRD~112, 023554, 2025) reports
...
in Phase~6e, providing a systematic $\Lambda_J$ vs $\Lambda_{\rm HK}$
comparison across emergent-gravity substrates with the Sakharov
4-criterion as discriminant. The test asks
...
$\Lambda_J = \Lambda_{HK}$, at the substrate level) is captured at the
Boolean-projection layer in
\texttt{sakharov\_induced\_gravity\_criterion\_implies\_lambda\_j\_eq\_lambda\_hk}
(JTGR6, theorem renamed 2026-05-07 from the prior `\texttt{\_iff\_}'-suffix
form to honestly reflect the one-way implication encoded at that layer).
The Phase~6o Wave 4a substrate-derived refactor (Track 4; authorized
2026-05-08 user-call C2) lifts this to substrate-physics-grounded
$\mathbb{R}$-valued data via the new
\texttt{SakharovExtended} structure: the existing four Boolean
conditions are retained unchanged, and the structure additionally
carries primary-source-grounded $\mathbb{R}$-valued $\lambda_J$ /
$\lambda_{\rm HK}$ fields plus a witness-equality Prop discharging
$\lambda_J = \lambda_{\rm HK}$ whenever Boolean condition~(iv) holds.
The substantive (\,$\Rightarrow$\,) implication at the $\mathbb{R}$
level is captured in JTGR12
(\texttt{sakharov\_criterion\_implies\_lambda\_j\_eq\_lambda\_hk\_real});
the ${}^3\!\text{He-A}$ extended substrate witness satisfies
$\lambda_J = \lambda_{\rm HK}$ at $\mathbb{R}$ level (JTGR13,
\texttt{volovikJannes\_he3a\_lambda\_j\_eq\_lambda\_hk\_real}); and the
FLS BEC extended substrate witness structurally separates
$\lambda_J \neq \lambda_{\rm HK}$ at $\mathbb{R}$ level (JTGR14,
\texttt{flsBEC\_lambda\_j\_neq\_lambda\_hk\_real}).

\paragraph{Wave 4a.4 closure: biconditional retired (verdict (B)).}
The deep-research return at
\texttt{Lit-Search/Phase-6o/6o-Wave 4a FLS BEC Depletion-Factor
$\Lambda$\_substrate Return.md} (returned 2026-05-06) closes the
Wave~4a.4 deferred deliverable with verdict~(B): the
(\,$\Leftarrow$\,)~biconditional is \emph{honestly retired} in favour
of a one-way implication $\text{Sakharov 4-criterion} \Rightarrow
\Lambda_J = \Lambda_{\rm HK}$ paired with a substantively-deeper
substrate-physics encoding. Primary-source coverage of (\,$\Leftarrow$\,)
is asymmetric: Volovik--Jannes 2012 \cite{JannesVolovik2012} (JETP
Lett.\ 96, 215, \S\,VII) argues only \emph{forward}, from Atiyah--Bott--
Shapiro topological protection at Weyl points to universal coupling
and the equality $\Lambda_J \propto \Delta_0^4$ on $^3\!\text{He-A}$;
no primary source argues the converse. The Finazzi--Liberati--Sindoni
acoustic-BEC analogue \cite{FinazziLiberatiSindoni2012PRL,
FinazziLiberatiSindoni2012Proc} provides an explicit substrate where
universal coupling fails (depletion-sector atoms do not propagate on
the BEC effective metric) \emph{and} $\Lambda_J \neq \Lambda_{\rm HK}$
by the structurally enforced FLS Eq.~(71) depletion factor
$\sqrt{\rho_0 a^3}$, but this only confirms the forward implication; it
does not test (\,$\Leftarrow$\,). The depletion-factor finding is
independently corroborated by the relativistic-BEC extension of
Belenchia--Liberati--Mohd \cite{BelenchiaLiberatiMohd2014}. Shipping a
genuine biconditional would constitute publication-novelty without
primary-source precedent in either direction beyond the
$^3\!\text{He-A}$ example; the one-way reading is the
maximally-honest substrate-physics statement available.

The Wave~4a.4 closure ships the depletion factor as a load-bearing
$\mathbb{R}$-valued substrate-physics field on \texttt{SakharovExtended}
with the unconditional relation $\Lambda_J = \texttt{depletion} \cdot
\Lambda_{\rm HK}$ (JTGR16,
\texttt{sakharov\_depletion\_factor\_relation}). On
$^3\!\text{He-A}$ the depletion factor is exactly~$1$ (JTGR17,
\texttt{volovikJannes\_he3a\_depletion\_eq\_one}), reflecting universal
coupling at the topologically protected Weyl point. On FLS BEC the
depletion factor satisfies $0 < \texttt{depletion} < 1$ structurally
(JTGR18, \texttt{flsBEC\_depletion\_strictly\_between\_zero\_and\_one})
with placeholder $\sqrt{\rho_0 a^3} \approx 8 \times 10^{-3}$ for
canonical Steinhauer-class $^{87}\text{Rb}$ parameters; the FLS p.~16
domain-of-validity constraint $\rho_0 a^3 \ll 1$
\cite{FinazziLiberatiSindoni2012Proc} enforces this
strict-inequality bound non-tunably. The numeral $8 \times 10^{-3}$ is
not a read-off of FLS Eq.~67/71. Deriving it would require that
source's condensate density $\rho_0$ and scattering length $a$ for
Steinhauer's canonical $^{87}\text{Rb}$ runs, an extraction this work
has not performed; the value is recorded as
\texttt{FLSBEC.DEPLETION\_FACTOR} at provenance tier PROJECTED with no
human-verification record, and its primary-source derivation is
deferred. No claim in this paper depends on its magnitude: JTGR18 and
JTGR19 consume only $0 < \texttt{depletion} < 1$. The
$^3\!\text{He-A}$
vs.\ FLS~BEC depletion-factor asymmetry (JTGR19,
\texttt{volovikJannes\_vs\_flsBEC\_depletion\_asymmetry}) is the load-
bearing $\mathbb{R}$-valued substrate distinction the prior Boolean
4-criterion did not capture. The composed honest-one-way closure
(JTGR20, \texttt{wave\_4a\_4\_honest\_one\_way\_closure}) packages the
(\,$\Rightarrow$\,) implication, the unconditional depletion-factor
relation, the $^3\!\text{He-A}$ unit-depletion identity, and the FLS
BEC strict-bound asymmetry into a single substantive theorem. The
substrate-physics encoding is now load-bearing; the prior P5-anti-pattern
Boolean projection of $\lambda_{\rm Eff}\!=\!\lambda_{\rm HK}$ has been
strictly-extended to a witness-required $\mathbb{R}$-level identity.
...
The Sakharov 4-criterion test (§\ref{sec:phase6m-sakharov}) compares
$\Lambda_J$ vs $\Lambda_{\rm HK}$ across emergent-gravity substrates
with the Sakharov 4-criterion as discriminant, and constitutes a
publishable observation in its own right. The full
short-note outline lives at
...
\item \emph{Tier 1 (GD applies, NO-GO predicted)}: Classes~(c), (d),
  i.e.\ Track~B's eight unanimous NO-GOs.
...
\item \emph{Tier 3 (GD silent, geometric / cosmographic
  applicability)}: Classes~(e), (f), (g), i.e.\ Track~C's five+ CLEARED-R5
  survivors, where the analytic verdict comes from cosmographic
  consistency rather than thermodynamic consistency.
\end{itemize}

The 7-class taxonomy is, to the best of our knowledge, the first
unified organisational framework for emergent-gravity DE mechanisms.
It sets up the
...
The Phase~6m mechanism count is established by the Lean theorem
\texttt{phase6m\_mechanism\_count}, which proves
$|\text{allEntropicGravityCandidates}| + |\text{allJacobsonCandidates}| =
17$, the lengths of the two enumerated lists for Tracks~B and~C
respectively. Track~A's four candidates enter as inductive
\texttt{CausalSetCandidate} constructors rather than as a
\texttt{List}, so they are not summed by \texttt{phase6m\_mechanism\_count};
counting them separately (via case-exhaustion on the inductive type)
brings the full Phase~6m roster to $4 + 8 + 9 = 21$ mechanisms across
all three tracks. All 21 classify into the unified 7-class GD
taxonomy (\texttt{phase6m\_class\_admits\_tier\_assignment}), with the
8/8 unanimous NO-GO closure on Track~B as the headline structural
result.
...
(b)~Track~B's complete-mechanism-family unanimous NO-GO closure on
...
provides a systematic $\Lambda_J$ vs $\Lambda_{\rm HK}$ test across
emergent-gravity substrates with the Sakharov 4-criterion as
discriminant, falsifying acoustic-BEC analog gravity at condition~(ii). The unified 7-class GD taxonomy
...
\paragraph{Polariton substrate placement: bimodal-branch resolution
(Phase~6v.3, 2026-05-26).}
%% Provenance: Wave 6v.3 cross-reference into D5's substrate-constraints
%% framing. Numerical anchor: src/dkm_polariton/polariton_occupation_bound.py.
%% Lean: SKEFTHawking.DKMBootstrap.polariton_dkm_f3_holds_at_pump_below_threshold
%% + polariton_inherits_graphene_uniqueness_result
%% (lean/SKEFTHawking/DKMBootstrap/PolaritonF3Bound.lean).
A complementary substrate-level finding that lives outside the
dark-energy closure but completes the D5 \emph{substrate constraints
by platform} picture: the Phase~6q DKM transport bootstrap's
explicit open question on polariton-platform placement, left
unresolved at Phase~6q close (\texttt{project\_phase6q\_complete\_2026\_05\_23}
memory), has been resolved by Phase~6v Wave~6v.3. The UPenn
nanocavity-polariton 4~fJ switching threshold pins the per-pulse
per-mode photon count at $n_{\rm per\ pulse} \approx 1.44 \times 10^4$,
some $70\times$ below the $n_{\rm threshold} \approx 10^6$ regime
that breaks the F3 operator-growth axiom on continuum-bosonic
substrates (Yin--Lucas, Kuwahara--Saito Lieb--Robinson-for-bosons).
The polariton platform therefore takes the \emph{same} positive-
uniqueness branch of the \texttt{PlatformBimodalOutcome} as graphene
(formal statement \texttt{polariton\_inherits\_graphene\_uniqueness\_result}
in \texttt{lean/SKEFTHawking/DKMBootstrap/PolaritonF3Bound.lean};
kernel-only; no new project-local axioms). The D5 \emph{constraint
landscape by platform} now reads: graphene + polariton on the
positive-uniqueness branch; BEC Bogoliubov continuum-bosonic
substrates on the sharpened-NO-GO branch (Phase~6q
Wave~2b.4). This closes the explicit Phase~6q open question for
the D5 substrate-constraints reading.

The bundle ships at $\bundleTheorems$ Lean theorems / $\bundleModules$
modules / zero active sorry / zero project-local axioms
(the formerly-tracked \texttt{gapped\_interface\_axiom} from Phase~5s,
unrelated to the dark sector, was retired into the tracked Prop
\texttt{TPFConjecture} on 2026-05-19). The Phase~6m closure adds $50$ substantive theorems
...
Physics Reports (in press, accepted 2026), arXiv:2505.23900.
...
Phys.\ Rev.\ D \textbf{102}, 084007 (2020), arXiv:1807.01689.

\bibitem{PlazaKraiselburd2025fR}
F.~Plaza and L.~Kraiselburd,
...
U.~K.~Tyagi, S.~Haridasu, and S.~Basak,
``Constraints on Generalized Gravity-Thermodynamic Cosmology from
...
%%    (deep-research return 2026-05-06; Wave 4a.4 verdict (B) closure) ──

\bibitem{FinazziLiberatiSindoni2012PRL}
S.~Finazzi, S.~Liberati, L.~Sindoni,
``Cosmological Constant: A Lesson from Bose-Einstein Condensates,''
Phys.\ Rev.\ Lett.\ \textbf{108}, 071101 (2012),
DOI: 10.1103/PhysRevLett.108.071101, arXiv:1103.4841.

\bibitem{FinazziLiberatiSindoni2012Proc}
S.~Finazzi, S.~Liberati, L.~Sindoni,
``The analogue cosmological constant in Bose-Einstein condensates:
a lesson for quantum gravity,''
Proc.\ II Amazonian Symposium on Physics (2012), arXiv:1204.3039.
...
DOI: 10.1103/PhysRevD.90.104015, arXiv:1407.7896.
...
JETP Lett.\ \textbf{96}, 215 (2012),
DOI: 10.1134/S0021364012160059, arXiv:1108.5086.

%% ── Phase 6m r_d rescue-range bracketing literature ──

\bibitem{ParkRatra2025rd}
C.-G.~Park and B.~Ratra,
``$r_d$ treatment as DDE-preference driver; $A_L$ freedom,''
arXiv:2501.03480 (2025).

\bibitem{WangYinTension2025rd}
X.~Wang and Z.~Yin,
``CMB / BAO / SN tensions and apparent DDE significance,''
...
%% ─── Lifted from _phase6n_W1c_writeup (§13) — 2026-05-06T15:41:18Z ───
%% \section{Dissipative SK-EFT bootstrap-uniqueness NO-GO (Phase 6n W1c writeup)}
%% \label{sec:d5-_phase6n_W1c_writeup}

%% Insertion point hint from PAPER_DRAFT_MAPPING.md: §13
%% Lift notes: D.2 absorption: Phase 6n W1c NO-GO writeup memo joint with 6o.γ (NO-GO landscape: dissipative SK-EFT bootstrap-uniqueness obstructions)
%% TODO: lift content from papers/_phase6n_W1c_writeup/paper_draft.tex
...
%% ─── End lift from _phase6n_W1c_writeup ───


%% ─── Lifted from _phase6n_W2b_lean_only (§13) — 2026-05-06T15:41:21Z ───
%% \section{Quantum Crooks no-go (Perarnau-Llobet)}
%% \label{sec:d5-_phase6n_W2b_lean_only}

%% Insertion point hint from PAPER_DRAFT_MAPPING.md: §13
%% Lift notes: D.2 absorption: Phase 6n W2b Perarnau-Llobet quantum-Crooks parametric + ℂ-form NO-GO
