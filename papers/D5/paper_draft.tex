...
paper (Phase~6c W1) into D5. It formalises a structural bridge between
two long-standing Standard-Model puzzles --- the smallness of the QCD
topological angle $|\theta| \lesssim 10^{-9}$ (Pendlebury~2015 neutron
EDM) and the observed cosmological-constant energy density
$\rho_{\rm DE} \approx 2.8 \times 10^{-11}\,\text{eV}^4$
(Planck~2018 + DESI~2024) --- via the Van Waerbeke--Zhitnitsky
topological-DE mechanism. The bridge ships in
...
disfavour ledgers --- two Bayes-factor Jeffreys-decisive
($|\!\log\mathcal{B}| \geq 5$: Tsallis~HDE, Odintsov-D'Onofrio-Paul;
aggregated in
...
AIC-moderate (Barrow~HDE, $\Delta\!\text{AIC} = +4.7$) --- all four
aggregated unit-coherently
($\sigma$ / $\log\mathcal{B}$ / $\Delta\!\text{AIC}$ / $\log\mathcal{B}$)
in \texttt{all\_quantitative\_bounds\_disfavoured} --- and four structurally
NO-GO mechanisms (Padmanabhan/CosMIn, Hossenfelder-Verlinde, Cadoni-Tuveri~DEC,
HDE event-horizon). The 8 mechanism-level closures are listed first;
two auxiliary supports for the Cadoni-Tuveri DEC entry follow.
...
\texttt{both\_decisive\_bayes\_bounds\_exceed\_jeffreys\_decisive}
(2 of 4 quantitative ledgers — Tsallis, Odintsov — at
