...
\texttt{flsBEC\_lambda\_j\_neq\_lambda\_hk\_real}). The (\,$\Leftarrow$\,)
direction --- whether $\lambda_J = \lambda_{\rm HK}$ at the substrate
level forces all four Boolean conditions --- is the Wave~4a.4 deferred
deliverable, pending the FLS BEC depletion-factor primary-source return
dispatched at \texttt{Lit-Search/Tasks/submitted/20260508\_FLS\_BEC\_depletion\_factor\_lambda\_substrate.md}.
Risk-disclosed at user-call C2: if the (\,$\Leftarrow$\,) direction does
not hold under available substrate hypotheses, Wave~4a ships the
substantively-deeper substrate encoding and honestly retires the
