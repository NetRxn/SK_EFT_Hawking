...
($D_L = \SI{830}{\mega\parsec}$, $\Delta r = \SI{400}{\kilo\parsec}$)
...
(Phase~6b W1). It applies the $\mu \gtrsim \SI{}{\mega\electronvolt}$
sufficiency condition for the fracton p-wave dipole superfluid phase to
the canonical five-candidate roster, classifies each candidate's BBN
viability, and surfaces the cross-bridge to fracton-NonAbelian
gauge structure.

The viability matrix is encoded in \texttt{BBNUnified.lean}: each Phase~5x
candidate (T-0 topological, S-0 sterile, C-1 mixed, FG torsion,
fracton p-wave) carries a BBN admissibility flag derived from
its symmetry-breaking--scale assignment relative to the BBN epoch
$T \approx \SI{1}{\mega\electronvolt}$. The substantive condition is
$\mu_{\rm condensate} > \SI{1}{\mega\electronvolt}$ for the fracton
candidate to remain in the dipole superfluid phase through nucleosynthesis
without disrupting the deuterium / helium-4 / helium-3 / lithium-7
abundance ratios. T-0 (no particle content) and S-0 (sterile, mass
$\gtrsim \SI{}{\mega\electronvolt}$) are vacuously admissible; the FG
torsion candidate's BBN safety follows from its CDM-obstruction
($w = 1/3$, radiation-like) failing to dilute slower than radiation.

Cross-bridge: the BBN safety condition for the fracton p-wave provides
the calibration handle for the $\mu \gtrsim \SI{}{\mega\electronvolt}$
constraint that anchors the §\ref{subsec:sfdm-merger} cluster-merger
forecast. The two conditions are independent (BBN constrains the early-time
energy density; the cluster-merger forecast constrains the late-time
gravitational signature), but they are mutually consistent in the
BK-fiducial parameter window.
...
This section consolidates the paper~34 equivalence-principle (EP)
classification deep paper (Phase~6c W3). It evaluates each Phase~5x
candidate's EP-violation profile and yields the substrate program's
EP verdict: the vestigial-gravity phase relic is the unique Phase~5x
candidate predicted to produce a measurable $\eta$-violation at the
STEP / MICROSCOPE / Galileo-Galileo-Galilei sensitivity floor.

The EP-violation matrix in
\texttt{EquivalencePrincipleClassification.lean} maps each Phase~5x
candidate to its violation level (\texttt{none} / \texttt{weak} /
\texttt{strong}) per WEP / EEP / SEP. T-0 (topological), S-0 (sterile),
C-1 (mixed-charge), FG torsion, and fracton p-wave all lie at
\texttt{violationLevel = none} per their substrate-singlet character;
the vestigial-relic phase, by contrast, carries an
$\eta$-violation $\sim 10^{-15}$ at MICROSCOPE sensitivity through the
\texttt{vestigial\_phase\_eta\_violates\_microscope\_bound} theorem
(numerical content: $1.0 > 10^{-15}$ via norm\_num). The
quantitative SEP-violation bound from the Hughes-Drever / LLR / nEDM
window is recorded in \texttt{VestigialPhaseEPViolation.lean}.

The substantive content is the Phase 5y orthogonality theorem applied
to the EP channel: vestigial-relic-EP-violation is in a substrate channel
distinct from the WEP-violation channels of T-0 / S-0 / C-1 / FG / fracton,
so the candidates are mutually compatible (no double-counting) but
predict distinct experimental signatures. STEP and MICROSCOPE provide
...
\item \emph{Sorkin everpresent-$\Lambda$ M1 + M2}: NO-GO at R5 via
  ...
  the raw Tier-(1) bound by at least $100\times$;
  \texttt{CSDE1}, \texttt{CSDE2}, \texttt{CSDE3} in
  \texttt{CausalSetDarkEnergy.lean}).
\item \emph{Phenomenological BDG} (Barrow-Diakonov-Gielen
  ...
  $\alpha_{\rm BDG} > 0$ (\texttt{CSDE7}, \texttt{CSDE8}).
\item \emph{Causet d'Alembertian DE}: NO-GO via 4D gradient instability
  ...
  drives a sub-Hubble inconsistency (\texttt{CSDE4} in
  \texttt{CausalSetDarkEnergy.lean}; cross-references the
  Krishna 2024 review).
\end{itemize}

The Track~A NO-GO is recorded in \texttt{track\_a\_fully\_no\_go\_at\_r5}
(\texttt{CSDE10}) as the conjunction of the four per-candidate verdicts.

\subsection{Caveat 1: GD obstruction inapplicable to causal-set DE}
\label{subsec:phase6m-track-a-caveat-gd}

The Phase~5y Gibbons--Duhem (GD) obstruction theorem
(\texttt{GibbsDuhemTheorem} in \texttt{DESIComparison.lean}) does not
...
(\texttt{CSDE5}) and the cross-bridge
\texttt{track\_a\_outside\_gibbs\_duhem\_domain} (\texttt{CSDE5b}) which
calls the Phase~5y \texttt{InDESIRegion} machinery directly: Track~A is
outside the Phase~5y \texttt{GibbsDuhemTheorem} applicability domain.
...
\texttt{barrow\_bound\_prescription\_dependent} (\texttt{CSDE6}) and
\texttt{barrow\_bound\_gap\_at\_least\_40\_percent} (\texttt{CSDE6\textquotesingle})
...
\texttt{CSDE7}. The Lean theorem
\texttt{bdg\_sigma\_lambda\_alpha\_bdg\_over\_sqrt\_v} establishes the
defining property; \texttt{bdg\_sigma\_lambda\_decreasing} (\texttt{CSDE8})
...
quantitative $\geq 40\%$ gap from \texttt{CSDE6\textquotesingle};
...
systematic Λ-comparison on a common substrate in the literature, and
constitutes a publishable observation in its own right. The full
short-note outline lives at
...
Phys.\ Rep.\ \textbf{1027}, 1--136 (2025), arXiv:2505.23900.

\bibitem{Volovik2006}
G.~E.~Volovik, ``Vacuum energy: myths and reality,''
...
``Gravitational slingshot for the cosmological constant,''
Phys.\ Rev.\ D \textbf{99}, 105009 (2019).

\bibitem{Diakonov2011}
D.~Diakonov,
``Towards lattice gauge gravity,''
Mod.\ Phys.\ Lett.\ A \textbf{26}, 2439 (2011), arXiv:1109.0091.
...
\bibitem{DESI2024}
DESI Collaboration, ``DESI 2024: cosmological constraints from the
measurements of baryon acoustic oscillations,''
arXiv:2404.03002.

\bibitem{DESI2025}
DESI Collaboration, ``DESI~DR2 results: BAO measurements,''
arXiv:2503.14738.
...
``QCD topological dark energy,'' arXiv:2506.14182 (2025).

%% ── Phase 6m three-track sources ──

\bibitem{PlazaKraiselburd2025}
N.~Plaza and L.~Kraiselburd,
``Statistical evidence for $f(R)$ dark energy from DESI BAO + Pantheon+,''
Phys.\ Rev.\ D \textbf{112}, 023554 (2025), arXiv:2504.05432.

\bibitem{TyagiHaridasuBasak2025}
S.~Tyagi, S.~Haridasu, and S.~Basak,
``Tsallis holographic dark energy under DESI~DR2 + Pantheon+ analysis,''
arXiv:2504.11308.

\bibitem{LucianoPaliathanasisSaridakis2025}
G.~G.~Luciano, A.~Paliathanasis, and E.~N.~Saridakis,
``Barrow holographic dark energy under DESI~DR2 + Pantheon+ analysis,''
arXiv:2506.03019.

\bibitem{HalenkaMiller2018}
V.~Halenka and C.~J.~Miller,
``Testing emergent gravity with mass densities of galaxy clusters,''
Mon.\ Not.\ Roy.\ Astron.\ Soc.\ \textbf{482}, 4513 (2018),
arXiv:1807.04702.
...
\bibitem{ArataLiberatiNeri2026}
M.~Arata, S.~Liberati, and A.~Neri,
``SO(3)-restricted Jacobson construction with ADM constraints in
Lorentz-violating gravity,'' arXiv:2603.28851.
