...
in Mathlib in early 2026 (commit \texttt{\mathlibrev}, files dated
...
library ships fifty-two theorems and lemmas across seven modules,
...
This signature-distinguishing falsifier is not vacuous: both
predicates are
...
\end{equation*}
for the manifold Lie bracket. Mathlib4 at \texttt{\mathlibrev} ships only
...
\texttt{\mathlibrev}). The negation-distribution lemma is itself derived
...
After the cuts the library ships \textbf{fifty-two theorems and
lemmas across seven modules} (counted by line-start
\texttt{theorem~}/\texttt{lemma~} grep), zero \texttt{sorry}, zero new
project axioms. Supporting structural declarations
($\texttt{IsLorentzianSignature}$ / $\texttt{IsLorentzianMetric}$ /
$\texttt{IsContinuousLorentzianFamily}$ / $\texttt{Christoffel}$ /
$\texttt{IsTorsionFree}$ / $\texttt{IsLeviCivita}$ as
\texttt{def} / \texttt{structure} declarations) ship alongside the
named theorems and lemmas but are not counted in the 52 — they are
substrate for the counted results. The pattern
...
\newblock \emph{Mathlib4 commit \mathlibrev}, April 2026.
...
\newblock \emph{Mathlib4 commit \mathlibrev}, March 2026.
...
\newblock \emph{Mathlib4 commit \mathlibrev}, 2024--2026.
