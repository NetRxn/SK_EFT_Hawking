...
900~theorems and 2~axioms across 58~modules (zero \texttt{sorry} in main
modules), of which 254~are proved by the Aristotle automated theorem prover. The work is
...
formalization totals 900~theorems and 2~axioms across 58~Lean modules
(zero \texttt{sorry} in main modules), of which 254 are proved by the Aristotle
automated theorem prover~\cite{Aristotle2025}. The work is organized
...
\texttt{Z16Classification.lean} (21~theorems + 1~axiom). Conditional
...
$|{\rm Ext}^4_{A(1)}(\mathbb{F}_2,\mathbb{F}_2)| = 2^4 = 16$
(\texttt{SteenrodA1.lean}, 17~theorems).
...
Total: 900~theorems + 2~axioms, 58~modules, zero \texttt{sorry} (main modules).}
...
Prior infrastructure & 518 & Proved + 2~axioms \\
Phase~5b additions & 152 & Proved (axioms discharged/removed) \\
\midrule
\textbf{Total} & \textbf{900 + 2~ax} & \textbf{Zero sorry} \\
\bottomrule
\end{tabular}
\caption{Formalization summary. All theorems are machine-checked by
\texttt{lake build}. The two remaining axioms are: GS no-go theorem (unbounded spectral
theory) and gauge erasure universality (non-abelian center discrete).
The former $\mathbb{Z}_{16}$ cobordism axiom was discharged (tautological as stated).}
\label{tab:summary}
\end{table}

\noindent\textbf{What is proved:} All 900 theorems, including $[H,Q_A]=0$,
Wilson mass uniqueness, Pauli algebra, Onsager relations, contraction,
$\mathbb{Z}_{16}$ conditional theorems, A(1) Steenrod algebra, GS
conditions, TPF evasion, SM anomaly in $\mathbb{Z}_{16}$, generation
constraint, Drinfeld center computations, and all bridge connections.

\noindent\textbf{What is axiomatized (2 axioms):} GS no-go theorem
(unbounded spectral theory), gauge erasure universality (non-abelian center
discrete). The former Pin$^+$ bordism axiom was discharged.

\noindent\textbf{What remains open:} The 4+1D gapped interface
conjecture (TPF), the gauging step from vector-like SMG to chiral gauge
coupling, and the full $\mathbb{Z}_{16}$ cobordism proof.
...
lattice chirality problem in Lean~4, comprising 900~theorems and
2~axioms across 58~modules with zero \texttt{sorry} (main modules). The three-pillar
...
\bibitem{Tong2022} D.~Tong, arXiv:2204.13264 (2022).
