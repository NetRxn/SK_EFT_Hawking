...
\begin{abstract}
We present a formal verification survey of the lattice chirality
problem---the obstruction to placing chiral fermions on a lattice with
the correct continuum limit. The formalization in Lean~4 with Mathlib comprises
$\totaltheorems{}$~theorems with $\axiomcount{}$~project-local axioms
(the 4+1D gapped-interface statement is carried as the tracked-hypothesis Prop
\texttt{TPFConjecture}, formerly the axiom \texttt{gapped\_interface\_axiom})
across $\leanmodules{}$~modules
(zero \texttt{sorry}), of which $\aristotleproved{}$~are proved by the
Aristotle automated theorem prover. The work is organized around three
...
exactly the Golterman-Shamir conditions identified in Pillar~1. The
lattice chiral fermion construction is formally verified in Lean~4.

\textbf{Pillar~3 (Algebraic Framework):} The Onsager algebra is
formalized in Lean~4 via the Dolan-Grady
...
bordism is formalized in \texttt{Z16Classification.lean} with zero
axioms; the A(1) Steenrod subalgebra computation supplies the
machine-checked algebraic core (Ext computation over the Steenrod
subalgebra $A(1)$ in Lean~4). Bridge theorems connect the Witten
...
formalization totals $\totaltheorems{}$~theorems
($\axiomcount{}$~project-local axioms; the gapped-interface statement is the tracked Prop \texttt{TPFConjecture})
across $\leanmodules{}$~Lean modules, of which $\aristotleproved{}$ are proved by the Aristotle
automated theorem prover~\cite{Aristotle2025}. Phase~2 (3+1D extension) adds
\texttt{GaugingStep.lean} (14~theorems: gauging obstruction, SMG phases,
not-on-site symmetry formalization) and \texttt{SPTClassification.lean}
(20~theorems: SPT phase types, the gapped-interface conjecture as the
tracked Prop \texttt{TPFConjecture}, conditional chiral gauge theorem). The work is organized
...
2D Ising model~\cite{Onsager1944}, is formalized in
\texttt{OnsagerAlgebra.lean} (24~theorems). The
...
subalgebra is formalized in Lean~4 and used to partially discharge the
axiom via the finite computation
...
\texttt{ChiralityWallMaster.lean} (16~theorems):
...
Total: $\totaltheorems{}$~theorems, $\axiomcount{}$~axioms, $\leanmodules{}$~modules.}
...
% papers/paper8_chirality_master/tables.py. Pillar → module mapping
% is author-curated (edit tables.py to change). Counts come from
% lean_deps.json via lean_module_summary.
\input{tables/table1_formalization_summary.tex}
\caption{Formalization summary. All theorems are machine-checked by
\texttt{lake build}. Two former axioms are now proved as theorems:
the GS no-go theorem and gauge erasure universality (non-abelian center
discrete). A third former axiom---$\Omega_5^{\mathrm{Spin}^{\mathbb{Z}_4}}
\cong \mathbb{Z}_{16}$---was converted to a trivially-true placeholder
theorem (\texttt{dai\_freed\_spin\_z4}) pending the addition of cobordism
theory to Mathlib; the cobordism computation itself is an external
input, as is standard in the $\mathbb{Z}_{16}$ literature.}
\label{tab:summary}
\end{table}

\noindent\textbf{What is proved:} $\totaltheorems{}$ theorems across $\leanmodules{}$ Lean modules,
...
Steenrod Ext computation, GS conditions, TPF evasion, SM anomaly in
$\mathbb{Z}_{16}$, generation constraint, Drinfeld center computations,
Fidkowski-Kitaev 2+1D gapped interface, and all bridge connections.
...
cobordism algebraic core is machine-checked (Ext computation). The 4+1D
gapped-interface conjecture is carried as the tracked-hypothesis Prop
\texttt{TPFConjecture} (formerly the axiom \texttt{gapped\_interface\_axiom};
the project now declares $\axiomcount{}$~axioms), supported by machine-checked
1+1D and 2+1D evidence.
...
(\texttt{FKGappedInterface.lean}, 20~theorems)---a machine-checked
...
conjecture (TPF, the project's central tracked-hypothesis Prop \texttt{TPFConjecture}), the gauging step from
...
We have presented a comprehensive formal verification of the lattice
chirality problem in Lean~4, comprising $\totaltheorems{}$~theorems
($\axiomcount{}$~project-local axioms; the gapped interface is carried as the tracked Prop \texttt{TPFConjecture}) across $\leanmodules{}$~modules. The three-pillar
...
Onsager algebra is formalized in Lean~4, and all bridge connections
between the three pillars are machine-checked. Resolution
...
\textit{Nucl.~Phys.~B} \textbf{193}, 173 (1981).
