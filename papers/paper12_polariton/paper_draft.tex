...
places the system in the near-adiabatic regime with ${\sim}20\%$
...
> 0.5$ for $\omega < 0.3\kappa$, requiring only ${\sim}100$ probe
photons per mode for $5\sigma$ detection. All predictions derive from
a formally verified computation pipeline (1214 Lean~4 theorems,
0~axioms, verified by \texttt{lake build}). Polariton-specific
results verified in \texttt{PolaritonTier1.lean} (6~theorems, zero
sorry): \texttt{spatial\_attenuation\_ge\_one},
\texttt{tier1\_regime\_monotone}, \texttt{bec\_recovery\_limit}.
...
$10^9$-fold temperature advantage, with predicted $T_H \sim 0.1{-}1~\text{K}$,
...
derive from a Lean~4 formalization with 1214 verified theorems
...
\midrule
Paris long (100~ps) & 61~mK & 100~ps & 5.0 & \textbf{Yes} \\
Paris ultralong (300~ps) & 61~mK & 300~ps & 15 & \textbf{Best} \\
Paris standard (3~ps) & 61~mK & 3~ps & 0.15 & Marginal \\
...
\item Spatial attenuation $\geq 1$:
  \texttt{spatial\_attenuation\_ge\_one}
  (\texttt{PolaritonTier1.lean}, Aristotle \texttt{da7cb04d})

\item Tier~1 monotonicity:
  \texttt{tier1\_regime\_monotone}
  (\texttt{PolaritonTier1.lean})
\end{itemize}
...
temperatures $10^9$ times higher than achieved in atomic BEC.
All predictions are formally verified (1214 Lean~4 theorems,
...
\bibitem{Burkhard2025} D.~Burkhard \emph{et al.}, arXiv:2511.12339 (2025).
