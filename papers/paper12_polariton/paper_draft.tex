...
a formally verified computation pipeline (4049+ Lean~4 theorems,
1~axiom [\texttt{gapped\_interface\_axiom}, not used by this paper;
since retired into the tracked Prop \texttt{TPFConjecture}],
verified by \texttt{lake build}). Polariton-specific results verified in
\texttt{PolaritonTier1.lean} (6~theorems, zero
...
derive from a Lean~4 formalization with 4049+ verified theorems
...
% papers/paper12_polariton/tables.py. DO NOT hand-edit values.
\input{tables/table1_cs_measurements.tex}
...
% Autogenerated by scripts/render_paper_tables.py from
% papers/paper12_polariton/tables.py. DO NOT hand-edit values.
\input{tables/table2_paris_params.tex}
...
frequency $\omega/\kappa$. The dashed line marks the single-shot
$5\sigma$ gain threshold $G = 0.5$ (equivalently
$N_{\rm probe} = 25/G^2 = 100$ probe photons per mode); the
threshold is crossed at the universal frequency
$\omega/\kappa = \ln(3)/(2\pi) \approx 0.175$. The shaded region
is the optimal probe window.
%% [historical correction 2026-05-04: prior caption read "G = 0.01"
%% and ">10^4 probe photons" — recompute G(0.175κ) =
%% 1/(exp(2π·0.175)-1) = 1/2 = 0.5; recompute N_probe = 25/G²
%% = 100 at G=0.5, not 10^4. Corrected post Stage-10 r1 review of
%% E1 bundle lift; see docs/review_finding_supersessions.json.]
}
\label{fig:gain}
\end{figure}
...
% Autogenerated by scripts/render_paper_tables.py from
% papers/paper12_polariton/tables.py. DO NOT hand-edit values.
\input{tables/table3_platform_comparison.tex}
...
with the same SLM profile remain borderline by the
$\Gamma_{\text{pol}}/\kappa = 0.143$ classifier ($> 0.1$ perturbative cutoff),
approaching but not yet reaching the fully perturbative regime. The steep-horizon
...
All predictions are formally verified (4049+ Lean~4 theorems across
