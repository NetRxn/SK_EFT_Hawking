...
Dissipative correction $\delta_{\text{diss}}$ (Beliaev, $T{=}0$) & (dimensionless) & $2.4 \times 10^{-5}$ & $1.6 \times 10^{-3}$ & $1.4 \times 10^{-5}$ \\
\hline
\end{tabular}
...
thermal cloud~\cite{Zaremba:1999} dominate:
\begin{equation}
\label{eq:gamma_estimates_expanded}
\gamma_1^{\text{(kin)}} \sim \frac{\hbar}{m} n \sigma \sim 10^{-3} \text{ to } 10^{-1} \text{ s}^{-1},
\end{equation}
where the wide range reflects uncertainties in the scattering cross-section
$\sigma$ and the thermal-cloud fraction. Substituting into
\eqref{eq:delta_diss_explicit}:
\begin{equation}
\label{eq:delta_diss_zaremba}
\delta_{\text{diss}}^{\text{(kin)}} \sim \frac{\gamma_1^{\text{(kin)}} \kappa}{\pi c_s^2} \sim 10^{-4} \text{ to } 10^{-3}.
\end{equation}
This is $\sim10\times$ larger than the pure Beliaev prediction and dominates at
experimentally realized temperatures.

\paragraph*{Experimental reach.} Both regimes give
$\delta_{\text{diss}} \ll 10^{-2}$, \textbf{below} the current sensitivity of
Steinhauer's apparatus (limited to $\Delta T / T \sim 10^{-2}$ by shot noise and
trap inhomogeneities). A measurement of $\delta_{\text{diss}}$ would distinguish
between the two regimes by its temperature scaling:
$\delta_{\text{diss}}^{\text{Bel}}$ is $T$-independent, while
$\delta_{\text{diss}}^{\text{(kin)}}$ scales with the thermal-cloud fraction.
