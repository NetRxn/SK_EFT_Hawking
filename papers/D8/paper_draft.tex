...
\begin{abstract}
PLACEHOLDER: the abstract is written last, after the introduction.
\end{abstract}
...
PLACEHOLDER: the introduction is written last, once the territory it maps exists.
...
The four reusable ingredients behind Theorem~\ref{thm:substrate} are the
$\varepsilon_0$-net, which supplies a finite family of words approximating every
target to a fixed base precision; the balanced group-commutator recursion of
Dawson and Nielsen~\cite{DawsonNielsen2006}, which converts a
precision-$\varepsilon$ approximation into a precision-$O(\varepsilon^{3/2})$
one at a fivefold word-length cost; a Baker--Campbell--Hausdorff estimate
bounding the commutator's deviation from the corresponding Lie-bracket flow;
and a Cartan-type argument passing from a one-parameter subgroup to density in
the whole group. None of these mentions the alphabet.
...
the Brylinski--Brylinski criterion~\cite{Brylinski2002}, which states that any
entangling two-qubit gate, together with all single-qubit gates, is universal.
The resulting compilation theorem holds for every even grid resolution
$N > 0$ and is unconditional.
