...
full-tetrad phase, sub-MICROSCOPE but within reach of next-generation
satellite EP tests. The remaining four mechanisms --- Einstein--Cartan
...
classification ships in Lean~4 with $24$ substantive theorems, zero
...
sub-MICROSCOPE but within reach of next-generation satellite tests
(STEP-class, $\eta \sim 10^{-18}$ target).
...
($\eta_{\max} = 1.0 > 10^{-15}$, \texttt{norm\_num}-backed),
...
identical classification as a dictionary lookup, $32$ \texttt{pytest}
