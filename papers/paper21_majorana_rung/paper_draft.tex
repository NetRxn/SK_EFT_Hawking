...
\texttt{MajoranaRung.H\_MR\_FromADWSubstrate\_BCS\_LNV}:
\begin{verbatim}
...
The conjunction with \texttt{H\_LeptonNumberViolated} encodes the
symmetry obstruction; $G_M > 1$ encodes supercriticality (in units where
the critical coupling is normalized to unity). The Wave 2b Lean module
ships the obstruction theorem
...
\item A \texttt{DecouplingRegime} structure encoding $0 < E$, $0 < \LambdaADW$,
  $10 E < \LambdaADW$ (robust hierarchy), plus three predicates excluding
  cosmological-scale failure modes (vestigial gravity~\cite{Volovik2024Spinor};
  q-theory; FRG strong-coupling). At $E = M_W$ and
  $\LambdaADW = 10^{14}$~GeV, all three failure-mode predicates are trivially
  satisfied.
...
\item A consistency witness \texttt{H\_DecouplingBoundDim6\_consistent} showing
  ...
  \texttt{SubstrateData}.
\item A \texttt{decoupling\_isBigOWith} restatement using Mathlib's
...
\midrule
\texttt{MajoranaRung.lean} (Wave 2 + 2b §3a) & $19$ & $0$ \\
\texttt{NeutrinoMixing.lean} (Wave 2 + 2a §4 split) & $11$ & $0$ \\
\texttt{MajoranaRungDecoupling.lean} (Wave 2b new) & $7$ & $0$ \\
\midrule
Wave 2 total (W2 + W2a + W2b) & $37$ & $0$ \\
\bottomrule
\end{tabular}
...
post-Wave-2b is \totaltheorems\ theorems across \totalmodules\
