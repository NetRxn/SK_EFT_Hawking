...
optimal dual witness. No `sorry`, no axiom. (See §3f.)
...
## 3f. Phases 6AI–6AJ: diamond-norm strong duality and fidelity-domain data processing

- **Watrous diamond-norm SDP strong duality (`DiamondSDPDuality.lean`).** `diamondDist_eq_choiSDP`:
...
- **Fidelity-domain data processing (`FidelityKrausDP.lean`).** The trace-distance chain
...
## 3g. Phase 6AK: the coherence-limited fidelity ceiling

Where 6AG–6AJ certify *consistency* (a bound a channel quantity must satisfy), Phase 6AK adds
*physical-limit* substrate — closed-form ceilings an achievable quantity provably cannot cross. The
first result (`CoherenceFidelity.lean`) computes the average gate fidelity of the **coherence channel**
`Φ_t = 𝒜_γ ∘ 𝒟_p`: amplitude damping with `γ = 1 − e^{−t/T₁}` composed after dephasing with
`p = (1 − e^{−t/T₂})/2`, the standard `T₁/T₂` model of a duration-`t` qubit gate. Contracting the four
Kraus traces and feeding the result through the 6AG identity `avgGateFidelity_eq` collapses both
fidelities to a **sum of decaying exponentials**:
...
`ℂⁿ` with `U₀ = 1` and Hilbert–Schmidt orthogonality `tr(Uᵢᴴ Uⱼ) = n·δᵢⱼ`, the channel
`Φ_p(ρ) = ∑ᵢ pᵢ Uᵢ ρ Uᵢᴴ` has exact diamond distance to the identity equal to its total error
probability `1 − p₀` (`diamondDist_errorBasisKraus_eq`), via the `n`-general Bell-block argument
(`Bᵢ Bⱼ = n δᵢⱼ Bᵢ`, optimal dual witness `W = ∑_{i≥1} pᵢ Bᵢ` with `Tr₂ W = (1−p₀)·1`). Two instances
ship: the single-qubit Pauli channel is the `n = 2` case (`diamondDist_pauliKraus_eq_of_errorBasis`,
re-deriving 6AH.2), and the **two-qubit Pauli channel** (16 weights `σᵢ ⊗ σⱼ`) is the `n = 4` case
...
(`diamondDist_spamBitFlip_eq`: `q`). The process (entanglement) fidelity `F_pro(Φ) = (1/d²)∑ₖ|tr Kₖ|²`
is exposed as a named quantity (`processFidelity`) with the 6AG identity restated for it
(`avgGateFidelity_eq_processFidelity`: `F_avg = (d·F_pro + 1)/(d+1)`); for the SPAM channel `F_pro = 1 − q`.
Kernel-only; zero new project-local axioms.

The fourth 6AK result (`QECSuppression.lean`) formalizes the abstract code-distance error-suppression
relation underlying fault tolerance. For physical error rate `p`, threshold `p_th`, prefactor `A`, the
distance-`d` logical error bound `A·(p/p_th)^⌊(d+1)/2⌋` has the two threshold properties: below threshold
(`p ≤ p_th`) it is monotone decreasing in the code distance (`logicalErrorBound_antitone_distance`), and
strictly below threshold (`p < p_th`) it tends to `0` as the distance grows
...
Mathlib, so we formalize the **rate-bound function** `plobBound η = −log₂(1−η)` and its rigorous
loss-penalty properties — the tractable surrogate capturing the physical no-go: the cap vanishes at
total loss (`plobBound 0 = 0`), is strictly increasing in transmissivity (`plobBound_strictMonoOn`), is
bounded below by the linear loss penalty `η/ln 2` (`linear_le_plobBound`), and decays to `0` along a
fiber whose transmissivity falls exponentially with distance (`plobBound_tendsto_zero_of_exp_decay`).
This is the analytic behaviour of the bound, honestly scoped — not a CV-capacity derivation.
Kernel-only; zero new project-local axioms.
