...
| # | Claim | Strength |
|---|---|---|
| N1 | **We are aware of no PRIOR formalization, in any proof assistant, of a Chern number or Chern class OF A VECTOR BUNDLE** (the topological invariant). ⚠️ Carve-out, mirrored from the paper 2026-07-31 (D11 Stage-13 4.1): this group's own `ChernBridge.lean` defines a categorical Chebyshev *marker* (`categoricalChernExpansion`, `realSpaceChernAt`) that a companion bundle describes in Chern language. It is a real-valued Chebyshev evaluation, not an integer bundle invariant, so it does not falsify N1 — but the claim must not stand unqualified against our own tree. In Mathlib all 14 occurrences of the string `Chern` are `Chernoff`; a case-insensitive sweep for `ChernClass\|PontryaginClass\|StiefelWhitney\|EulerClass\|characteristic class` returns no matches. | **Highest** — direct read of the pinned artifact |
