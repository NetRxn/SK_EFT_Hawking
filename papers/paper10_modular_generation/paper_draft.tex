...
This is formalized in \texttt{WangBridge.lean}
(\paperTenWangBridgeThms{}~theorems, zero \texttt{sorry}):
...
(\paperTenModularInvarianceThms{}~theorems): the $q$-parameter shift
\texttt{qParam\_shift} is proved
...
The Kitaev facet is formalized here as its arithmetic core:
\texttt{KitaevSixteenFold.lean} defines the chiral central charge of the
$\nu$-th class-D phase, $c_-(\nu) = \nu/2$, and proves its period is
exactly 16 --- \texttt{kitaevCentralCharge\_period16} gives
$c_-(\nu+16) = c_-(\nu) + 8$, and
\texttt{kitaevCentralCharge\_faithful} shows the sixteen values
$c_-(0),\ldots,c_-(15)$ are pairwise distinct modulo~8, so no smaller
period suffices. What is \emph{not} formalized is the identification of
that $\mathbb{Z}_{16}$ with the Rokhlin signature $\mathbb{Z}_{16}$ or
the Dai--Freed anomaly $\mathbb{Z}_{16}$: that requires the Smith
homomorphism and the computed
$\Omega_4^{\mathrm{Pin}^+} \cong \mathbb{Z}_{16}$, which are absent from
Mathlib. A shared $\mathbb{Z}_{16}$ constrains; it does not derive.
...
Both enumerations state the same four conjuncts, whose individual
status we disclose rather than leave to the reader. (i)~The SM Weyl count
$\sum_f \mathrm{components}(f) = 16$ is substantive, sourced from
\texttt{total\_components\_with\_nu\_R}. (ii)~$(16 : \mathbb{Z}/16) = 0$
is an arithmetic tautology discharged by \texttt{decide}, and carries no
content beyond fixing the modulus. (iii)~The signature divisibility
$16 \mid \sigma$ is the only conjunct on which the two theorems differ: in
\texttt{sixteen\_convergence\_full} it is a verbatim echo of that
theorem's own assumed Rokhlin hypothesis, whereas in
\texttt{sixteen\_convergence\_unconditional} it is proved over the
\texttt{SmoothSpinManifold4} interface, given the tracked topological
input. (iv)~The Kitaev periodicity
$c_-(\nu+16) = c_-(\nu) + 8$ is substantive, sourced from
\texttt{kitaevCentralCharge\_period16}. Neither theorem asserts that the
four occurrences of 16 are the same 16; each is an enumeration, and we
report it as one.
...
Lean~4 proofs verified by \texttt{lake build}
(v\leantoolchain{}, Mathlib \texttt{\mathlibrev{}}).
Automated proofs by the Aristotle theorem prover (Harmonic).
\end{acknowledgments}

\begin{thebibliography}{99}
\bibitem{Wang2024}
J.~Wang, \emph{Family Puzzle, Framing Topology, $c_-=24$ and $3(E_8)_1$
Conformal Field Theories: $48/16 = 45/15 = 24/8 = 3$}, arXiv:2312.14928 (2023).
