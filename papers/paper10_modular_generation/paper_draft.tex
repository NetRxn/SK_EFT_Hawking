...
generation bound is fully machine-checked. The Rokhlin signature bound
$16 \mid \sigma$ for closed smooth spin 4-manifolds---which underwrites
the ``16'' convergence---is itself an unconditional kernel-pure theorem
($8 \mid \sigma$ via van der Blij plus the single topological factor
$2 \mid \sigma/8$), not an assumed input; an independent machine-checked
...
The signature half of this convergence is now an unconditional
formally-verified theorem. The structure
...
$2 \mid \sigma/8$ (\^A-genus even / Arf invariant); from these,
\texttt{SmoothSpinManifold4.rokhlin}~$:~16 \mid \sigma$ is proved as a
kernel-pure theorem (standard axioms only; no global Rokhlin hypothesis,
no new \texttt{axiom}). Its signature is the genuine signature of the
intersection form, $\sigma := \mathrm{latticeSig}\,(\text{form})$, and the
algebraic bound $8 \mid \sigma$ is itself \emph{proved} (van der Blij)
...
the $16 \mid \sigma$ conjunct now a theorem rather than an assumed input.
...
by design; we cite the unconditional form for the substantive claim.
...
Rokhlin's theorem $16 \mid \sigma$ is, in this formalization, a proved
kernel-pure theorem (\texttt{SmoothSpinManifold4.rokhlin}), not an assumed
hypothesis and not an axiom; the historical result (Rokhlin 1952) is
recovered from even-unimodularity plus the topological factor $2 \mid
\sigma/8$. The bound is sharp: the K3 surface has
...
\texttt{SpinRokhlinInterface.lean} (unconditional $16 \mid \sigma$ and
...
The unconditional $16 \mid \sigma$ derivation above does \emph{not} route
through the Adams spectral sequence; it proceeds via the lattice-signature
classification (van der Blij plus the topological factor $2 \mid \sigma/8$).
...
is \emph{deliberately not used} for the unconditional theorem: it relies on
