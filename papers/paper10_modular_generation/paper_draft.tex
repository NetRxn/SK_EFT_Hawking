...
comprises 1179~theorems across 79~Lean~4 modules (zero axioms); the complete
derivation chain from modular forms through fermion counting to the
generation bound is fully machine-checked. This is the first
...
(14 theorems; Rokhlin's theorem enters as a hypothesis, not an axiom).
...
The formal verification now comprises 1179~theorems across 79~Lean~4
modules (zero axioms). Key modules for this paper:
...
and the new \texttt{AlgebraicRokhlin.lean} (Serre's $8 \mid \sigma$ for
even unimodular forms, zero \texttt{sorry}), \texttt{E8Lattice.lean}
(E$_8$ is even unimodular with $\sigma = 8$, $\det = 1$ proved by
\texttt{native\_decide}, Aristotle \texttt{78dcc5f4} --- disproving the naive
algebraic Rokhlin conjecture; the jump from~8 to~16 requires smooth
topology), and \texttt{SpinBordism.lean} (the full Wang chain conditional
on spin bordism data, \texttt{rokhlin\_from\_bordism} proved by Aristotle
\texttt{78dcc5f4}, zero \texttt{sorry}).
