...
$I_0 \sim C\,e^{F(0)} / \sqrt{-F''(0)}$ and
$c_{\rm singlet} = -1$ is the SU(2)-singlet projection from the
$I_0 - I_1$ cancellation
(\texttt{kaul\_majumdar\_log\_decomposition}).

\subsection{The Laplace-method axiom}

The full Laplace-method asymptotic with bounded remainder is not
present in Mathlib~4.29 measure-theoretic tooling. We therefore
axiomatize the key bound:
\begin{equation}
\exists\,C > 0:\;\;
...
(\texttt{gaussianSaddleAsymptotic}). This is the unique new axiom
introduced by Wave~3, classified \texttt{eliminability: hard} in
\texttt{src/core/constants.py::AXIOM\_METADATA} (full elimination
requires Watson's lemma machinery, which would fit naturally in
\texttt{MeasureTheory.Asymptotic.LaplaceMethod} as a future Mathlib
contribution). Two theorems are derived from the axiom:
...
\begin{itemize}
  ...
  \item \texttt{falsifier\_anomalyMatch}: trivially holds at the
        abstract level; substantive content lives at the per-MTC
        sub-corollary level (currently conjectural across all
        candidates).
\end{itemize}
...
\checkmark = passes; ? = conjectural; \textbf{FAIL} = falsifier
triggered. The toric code's F2 failure is the Wave~3 abelian-MTC
falsification: $\log\dmax = 0 \Rightarrow \kappa_C = 0$.}
\label{tab:instances}
\end{table}

The F4 (modular invariance) entries pass via the formalized
$S$-matrices in \texttt{SU2kFusion.lean} (verified unitary for
$k \in \{2, 3, 4, 5, 10\}$ in our pytest suite),
\texttt{FibonacciMTC.lean} (proven, zero sorry, native\_decide over
$\mathbb{Q}(\sqrt{5})$), \texttt{IsingBraiding.lean} (proven, zero
sorry), \texttt{S3CenterAnyons.lean} (proven, zero sorry).
...
\texttt{lean/SKEFTHawking/BHEntropyMicroscopic.lean}: 19 theorems,
0 sorry, 1 new axiom (\texttt{gaussianSaddleAsymptotic}, classified
\texttt{eliminability: hard}), 1 opaque function
(\texttt{verlindeEntropy\_SU2k}). \texttt{lake build} clean across
8267 jobs.
...
\begin{itemize}
  ...
  \item \texttt{kaul\_majumdar\_log\_coefficient} -- structural
        $-3/2$ extraction
  \item \texttt{kaul\_majumdar\_log\_decomposition} --
        $(-1/2) + (-1) = -3/2$
  ...
  \item Five falsifier theorems (F1-F5)
...
\texttt{kaul\_majumdar\_leading\_matches\_G\_N\_emerg} certifies that
at $A = 4 G_N^{\rm emerg}$, $\SBH = 1$, providing the algebraic
shape of the matching condition. The full numerical match requires the
Vergeles-derived $\alpha_{\rm ADW}$ value, which remains a Wave~1
tracked hypothesis (no closed-form $\alpha_{\rm ADW}$ in published
ADW literature; cf.\ \texttt{LinearizedEFE.lean} \S{}H\_VergelesPositivity).
...
sub-corollary as the structural anchor. The Lean module ships 19
theorems, 0~sorry, and one new axiom (the Laplace-method
...
P. Mitra, ``Black-hole entropy without log correction,''
arXiv:1406.5524 (2014).
...
T.~R. Govindarajan, R.~K. Kaul, and V. Suneeta, ``Quantum gravity
on dS$_3$,'' Class. Quantum Grav. \textbf{19}, 4195 (2002),
arXiv:gr-qc/0104010.

\bibitem{Vergeles2025}
S. Vergeles, ``Anderson-Higgs gravity and lattice formulation of
emergent diffeomorphism invariance,'' Phys. Rev. D \textbf{112},
054509 (2025).
