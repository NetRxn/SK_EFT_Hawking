...
$G_N^{\mathrm{Sak}} = \input{tables/g_n_sakharov_planck_gev_inv2.tex}\unskip\
\mathrm{GeV}^{-2}$.
...
= \input{tables/g_n_over_g_c_ratio.tex}\unskip ,
\end{equation}
...
$\input{tables/g_n_sakharov_planck_gev_inv2.tex}\unskip\ \mathrm{GeV}^{-2}$,
...
$|\Delta c/c| \le \input{tables/gw170817_two_sided_cap.tex}\unskip$,
...
to
$\chi_{\mathrm{vest}} = \input{tables/chi_vest_natural_upper.tex}\unskip$,
...
\;<\; \chi_{\mathrm{vest}} \;<\;
\input{tables/chi_vest_natural_upper.tex}\unskip ,
\end{equation}
\noindent
...
$\input{tables/chi_window_ratio_log10.tex}\unskip$, so the machine-checked
...
horizon's error-correcting structure, and the link between them is an identification this
work makes rather than an agreement it derives. The code distance of the horizon code is
\emph{defined} to be the same $\log d_{\max}$ that the area-law coefficient uses
(\texttt{QECHolography\lb{}Bridge.\lb{}HPCode.\lb{}code\lb{}Distance}). On that definition the
first conjunct of the bridge theorem --- code distance positive if and only if
$d_{\max} > 1$, that is, if and only if the category is non-abelian --- is the positivity
of a logarithm and carries nothing beyond it; in particular it is not a statement that two
independently specified admissibility criteria coincide. What the theorem does add is its
second conjunct: positive code distance forces a strictly positive scrambling-time bound
$\log \mathcal{D}^2$, which follows from $d_{\max}^2 \le \mathcal{D}^2$ and is not available
...
\begin{equation}
\label{eq:Mc-planck}
\frac{M_c}{M_{\mathrm{Pl}}} \;=\; \input{tables/m_c_over_planck_mass.tex}\unskip\,.
\end{equation}
...
\alpha = \input{tables/stelle_r_sq_coefficient.tex}\unskip,\qquad
\beta  = \input{tables/stelle_ricci_sq_coefficient.tex}\unskip,\qquad
...
$\input{tables/lambda_observed_gev4.tex}\unskip\ \mathrm{GeV}^{4}$. This is a
...
\right)
\;=\; \input{tables/lambda_overshoot_log10.tex}\unskip ,
\end{equation}
...
|T_{\mathrm{EC}}| \;=\;
\input{tables/torsion_amplitude_gev.tex}\unskip\ \mathrm{GeV}.
\end{equation}
...
$\input{tables/torsion_bound_kostelecky_gev.tex}\unskip\ \mathrm{GeV}$
...
$\input{tables/torsion_bound_hughes_drever_gev.tex}\unskip\ \mathrm{GeV}$
\cite[Eq.~(6)]{KosteleckyRussellTasson2008}. The substrate is tested against
both. Equation~\eqref{eq:torsion-value} sits
$\input{tables/torsion_orders_below_kostelecky.tex}\unskip$ orders of magnitude
