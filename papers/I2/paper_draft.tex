...
\texttt{lean-tensor-categories}}: a 28-module library
($\sim$640 declarations across theorems, definitions,
...
Kassel~\cite{kassel1995}; here too the formalization has
...
yield across four files: 101 declarations.
...
\texttt{QuantumGroupHopf.lean} (16 theorems) defines the generic
...
implicit in \texttt{QSqrt2.lean}'s structure --- is the first
...
\texttt{FibonacciMTC.lean} (18 declarations) wraps the result as a
...
\caption{Dependency graph of the 28 modules that comprise the
...
(steel blue, mid; Section~\ref{sec:numberfields}), and the eleven
concrete MTC instances (amber; Section~\ref{sec:instances}). Sankey
...
broader research community (including the Microsoft Quantum
program's published Majorana-fermion-based proposals); the
Fibonacci universality proof in \texttt{FibonacciBraiding.lean}
is the formal counterpart of the universality result that
motivates topological-QC research. A formally verified gate
compiler can build on the library without re-deriving the
...
\texttt{IsolatedHorizonHypotheses} tracked-Prop bundle
(Phase~6j Wave~1 of the SK-EFT Hawking project, which scopes
the loop-quantum-gravity isolated-horizon assumptions explicitly
rather than treating them as hidden inputs to the MTC
calculation). Section~\ref{sec:reusability} of the SK--EFT
Hawking flagship paper documents this dependency.
