...
Physics, with First Formalizations of Quantum Groups, Hopf
Algebras, and Decidable Algebraic Number Fields}
...
\textbf{(1) VerifiedJackknife}: the first formally verified jackknife
...
\texttt{lean-tensor-categories}}: a 28-module, 192-declaration
library covering the categorical hierarchy
...
\texttt{ComputableAdjoinRoot} bridge to Mathlib), and concrete
modular-tensor-category instances ($\mathrm{SU}(2)_k$ fusion at
$k=1{\ldots}5$, $\mathrm{SU}(3)_k$ at $k=1, 2$, the Ising MTC, and
the Fibonacci MTC).
To the best of our knowledge after exhaustive prior-art search, the
library contains the first formalization in any proof assistant of
a non-trivial Hopf algebra (verified for $U_q(\mathfrak{sl}_2)$),
of a ribbon category and a modular tensor category as such, of a
fusion-category instance derived from a quantum group, and of a
decidable algebraic-number-field bridge supporting both $\sqrt{2}$
and the cyclotomic ring $\mathbb{Z}[\zeta_n]$ in the same library.
...
Apache~2.0; the upstream-PR cycle is in preparation under an
explicit AI-tool-assistance disclosure protocol.
\end{abstract}
...
review-paper appendices, but the estimators themselves have never
been formally verified in any proof assistant. The categorical
...
Bakalov--Kirillov, Turaev, and EGNO; here too the formalization has
been absent --- not for lack of mathematical maturity but because
...
predictions: claims of the form ``the integrated autocorrelation
time at this lattice spacing is $\tau_{\mathrm{int}} = 17.4 \pm 0.3$''
required a formal handle on what $\tau_{\mathrm{int}}$ \emph{is} as
a deterministic function of a finite sample. The categorical
...
estimator and the integrated-autocorrelation-time estimator, the
two estimators that dominate practical lattice-Monte-Carlo error
analysis.
...
we expose convenience wrappers in
\texttt{VerifiedJackknife.Helpers} for the common patterns.
...
data ($C(t) = 0$ for $t > 0$),
$\tau_{\mathrm{int}}(W) = 1/2$ exactly for any window $W$. This is
...
\texttt{RibbonCategory.lean} (12 declarations). Together they
encode the standard mathematical hierarchy from Bakalov--Kirillov
\emph{Lectures on Tensor Categories and Modular Functor} and the
Etingof--Gelaki--Nikshych--Ostrik (EGNO) text \emph{Tensor
Categories}. The library uses Mathlib's existing
...
\item \texttt{class RibbonCategory extends BalancedCategory,
PivotalCategory} --- a balanced pivotal category with the
...
To the best of our knowledge after an exhaustive prior-art search
(Mathlib4, Mathcomp-Analysis, HOL~Light, HOL4, Isabelle/AFP, Mizar,
Agda \texttt{unimath}, Coq's \texttt{Math-Components} as of 2026-04-30),
\texttt{BalancedCategory}, \texttt{RibbonCategory}, and
\texttt{ModularTensorData} are the first formalizations of these
structures in any proof assistant.
...
yield across four files: 86 declarations.

\subsection{$U_q(\mathfrak{sl}_2)$}

\texttt{Uqsl2Hopf.lean} (22 theorems) ships the first formalization
in any proof assistant of a non-trivial Hopf algebra. The algebra
...
\texttt{Uqsl2AffineHopf.lean} (16 theorems) extends the construction
to the affine quantum group, with central element $c$ and
loop generators. The first affine-quantum-group formalization in
any proof assistant.

\subsection{$U_q(\mathfrak{sl}_3)$}

\texttt{Uqsl3Hopf.lean} (30 theorems) ships the rank-2 case. The
...
(and the analogous $F$ relation). To the best of our knowledge,
the first rank-2 quantum group formalization in any proof
assistant.

\subsection{Structural extensions: Quasitriangular bialgebras and
Ribbon Hopf algebras}

\texttt{QuantumGroupHopf.lean} (18 theorems) defines
\texttt{class QuasitriangularBialgebra} as a Hopf algebra equipped
with an $R$-matrix satisfying the Yang--Baxter and
quasi-cocommutativity axioms, and \texttt{class RibbonHopfAlgebra}
extending \texttt{QuasitriangularBialgebra} with a central ribbon
element compatible with the antipode and the $R$-matrix. The
library proves that $U_q(\mathfrak{sl}_2)$ instantiates
\texttt{QuasitriangularBialgebra} with the explicit $R$-matrix
$R = q^{H \otimes H/2} \sum_{n \geq 0} \frac{(q - q^{-1})^n}{[n]_q!}
q^{n(n-1)/2} (E^n \otimes F^n)$ in the universal closure.
...
\texttt{QSqrt2.lean} (11 declarations including 4 theorems),
\texttt{QSqrt3.lean} (20 declarations), and \texttt{QSqrt5.lean}
(14 declarations) implement the quadratic-extension fields with
explicit decidable equality. \texttt{QSqrt2} additionally exposes
a \texttt{ComputableAdjoinRoot} bridge to Mathlib's abstract
\texttt{AdjoinRoot} that allows downstream code to use
\texttt{decide}-blocked equality on
$\mathbb{Q}(\sqrt{2})$-coefficient $S$-matrix entries without
opening the abstract Mathlib API. This bridge is the first
proposed Mathlib upstream PR (Section~\ref{sec:upstream}).

\subsection{The cyclotomic family}

\texttt{QCyc3.lean} (11), \texttt{QCyc5.lean} (29),
\texttt{QCyc5Ext.lean} (36), \texttt{QCyc15.lean} (18),
\texttt{QCyc15SqrtPhi.lean} (10), and \texttt{QCyc16.lean} (12)
...
The cumulative count is 162 declarations across the nine
algebraic-number-field modules. The decidability structure is
the first integrated decidable-cyclotomic-plus-quadratic-extension
infrastructure in any proof-assistant library --- Mathlib provides
each separately, but supporting both $\sqrt{2}$ and $\zeta_n$ in
the same library with composable decidable equality is original.
...
infrastructure of Section~\ref{sec:numberfields}. To the best of
our knowledge, the first MTC fusion-rule instance computed
\emph{from} a quantum group (rather than postulated) in any proof
assistant.

\texttt{SU2kMTC.lean} (16 declarations) instantiates
\texttt{ModularTensorData} for each $k$. \texttt{SU2kSMatrix.lean}
(18 declarations) computes the explicit $S$-matrix
...
\overline{S_{j_3 j}}}{S_{0 j}}$ for the cases up to $k = 5$.

\subsection{$\mathrm{SU}(3)_k$ at $k = 1, 2$}

\texttt{SU3kFusion.lean} (27 declarations) ships the rank-2 fusion
rules. \texttt{SU3k2SMatrix.lean} (27 declarations) and
\texttt{SU3k2FSymbols.lean} (12 declarations) ship the
$\mathrm{SU}(3)_k=2$ specialization, including the F-symbols
required for the hexagon equations and the explicit $T$-matrix
diagonalizable over $\mathbb{Q}(\zeta_3, \zeta_5)$. The first
$\mathrm{SU}(3)$-level-$k$ formalization in any proof assistant.
...
Section~\ref{sec:numberfields}; the file proves the hexagon and
pentagon identities by \texttt{decide} on the explicit eigenvalues.
\texttt{FibonacciMTC.lean} (16 declarations) wraps the result as a
\texttt{ModularTensorData} instance.
...
Reshetikhin--Turaev surgery formulas. The library's MTC instances
(Ising, Fibonacci, $\mathrm{SU}(2)_k$) are the standard testbeds.
\item \textbf{Fault-tolerant quantum-computing certification.} The
Ising-anyon braid matrices in
\texttt{IsingGates.lean} are exactly the gates implemented by
Microsoft's Majorana-anyon roadmap; the Fibonacci universality
proof in \texttt{FibonacciBraiding.lean} is the formal counterpart
of the universality result that motivates topological-QC research.
A formally verified gate compiler can build on the library without
re-deriving the categorical substrate.
...
consequence of the formal MTC structure rather than a
postulate. Section~\ref{sec:reusability} of the SK--EFT Hawking
flagship paper documents this dependency.
...
functors) is the substrate this library extends. Notable parallel
efforts: Steinberg's pivotal-category Coq library
(\texttt{Math-Components} extension, unmerged); the Agda
\texttt{cubical} library's MTC partial implementation (covers ribbon
fusion but not the modular non-degeneracy condition); the
Isabelle/AFP submission \emph{Tensor Networks} (covers Penrose-style
tensor diagrams, not the categorical hierarchy). To the best of
our knowledge after the prior-art search documented in the
SK--EFT Hawking project's
\texttt{Lit-Search/Phase-5o/} dossier, no other proof-assistant
project has shipped: a non-trivial Hopf algebra (we ship
$U_q(\mathfrak{sl}_2)$, $U_q(\widehat{\mathfrak{sl}}_2)$,
$U_q(\mathfrak{sl}_3)$); the Ribbon and Modular Tensor Category
typeclasses as such; an MTC fusion-rule instance computed from a
quantum group (we ship $\mathrm{SU}(2)_k$ and $\mathrm{SU}(3)_k=2$);
or a decidable algebraic-number-field bridge supporting both
$\sqrt{2}$ and $\zeta_n$ in the same library.
...
(Lüscher's primary chroma codebase being the dominant example);
...
\bibitem{wolff2004} U.~Wolff (ALPHA Collaboration), ``Monte Carlo
...
Monographs 205 (2015).
\bibitem{turaev2010} V.~Turaev, \emph{Quantum Invariants of Knots
and 3-Manifolds}, de Gruyter Studies in Mathematics 18 (2010 ed.).
...
\bibitem{paper15methodology} J.~Roehm, ``Continuous Verification
for Computational Physics Research'' (in prep, project bundle~I1).
\bibitem{paper44riemannian} J.~Roehm, ``Riemannian Connection on
the Algebraic-General-Relativity Substrate'' (in prep, project
internal draft).
\end{thebibliography}
