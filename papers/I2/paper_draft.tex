...
(1233 declarations across theorems, definitions,
...
\texttt{KLinearCategory.lean} (40 declarations),
\texttt{FusionCategory.lean} (39 declarations including 24 theorems),
\texttt{SphericalCategory.lean} (38 declarations), and
\texttt{RibbonCategory.lean} (38 declarations). Together they
...
\texttt{SphericalCategory.lean} (38 declarations including
26 theorems) builds the pivotal structure
...
yield across four files: 119 declarations.

\subsection{$U_q(\mathfrak{sl}_2)$}

\texttt{Uqsl2Hopf.lean} (29 declarations) ships a Lean~4
...
\texttt{QuantumGroupHopf.lean} (25 declarations) defines the generic
...
\texttt{QSqrt2.lean} (32 declarations including 11 theorems),
\texttt{QSqrt3.lean} (37 declarations), and \texttt{QSqrt5.lean}
(29 declarations) implement the quadratic-extension fields with
...
\texttt{QCyc3.lean} (14), \texttt{QCyc5.lean} (118),
\texttt{QCyc5Ext.lean} (79), \texttt{QCyc15.lean} (21),
\texttt{QCyc15SqrtPhi.lean} (13), and \texttt{QCyc16.lean} (35)
...
The cumulative count is 378 declarations across the nine
...
(259 declarations including 245 theorems) computes the explicit
$S$-matrix
...
\overline{S_{j_3 j}}}{S_{0 j}}$ at every $k = 1, \ldots, 5$:
representative cases at $k = 1, 2$
...
\texttt{verlinde\_k2\_psi\_sq\_vacuum}), and the \emph{complete}
fusion tables at $k = 3, 4, 5$
(\texttt{verlinde\_k3\_full} / \texttt{verlinde\_k4\_full} /
\texttt{verlinde\_k5\_full}: all $4^3 = 64$, $5^3 = 125$, and
$6^3 = 216$ index triples respectively, each fiber checked against
the truncated Clebsch--Gordan coefficients of
\texttt{SU2kFusion.lean}), together with
$S S^{\mathsf{T}} = I$ unitarity and $\det S \neq 0$ modularity at
each level. The $k = 3, 4$ tables reduce in
$\mathbb{Q}(\varphi) = \mathbb{Q}(\sqrt{5})$ and
$\mathbb{Q}(\sqrt{3})$ respectively, while the $k = 5$ entries live
in the totally real cubic field $\mathbb{Q}(2\cos(\pi/7))$, whose
generator admits no quadratic radical form (casus irreducibilis) and
whose cubic minimal polynomial $c^3 = c^2 + 2c - 1$ is itself proven
from the double- and triple-angle identities and drives all power
reduction. All of these proofs are kernel-pure
(\texttt{decide} / \texttt{norm\_num} / \texttt{ring} /
\texttt{linear\_combination}; no \texttt{native\_decide}).

\subsection{$\mathrm{SU}(3)_k$ at $k = 1, 2$}

\texttt{SU3kFusion.lean} (62 declarations) ships the rank-2 fusion
...
\texttt{FibonacciBraiding.lean} (69 declarations including 44
theorems) is the second-largest instance file in the library
(after \texttt{SU2kSMatrix.lean}). The
...
512-case F-symbol catalog. The hexagon identity is shipped in both
braiding orientations as named, kernel-pure theorems in
\texttt{FibonacciBraiding.lean}: the $d = \tau$ sector
entry-by-entry (\texttt{fib\_hexagon\_R\_00} through
\texttt{fib\_hexagon\_R\_11}) plus the vacuum sector
\texttt{fib\_hexagon\_R\_vacuum} (the eigenvalue relation
$R_\tau^2 = R_{\mathbf{1}}$), and the inverse-braiding counterparts
\texttt{fib\_hexagon\_Rinv\_00} through
\texttt{fib\_hexagon\_Rinv\_11} with
\texttt{fib\_hexagon\_Rinv\_vacuum}, against proven inverses
$R_{\mathbf{1}}^{-1} = \theta_\tau$ and $R_\tau^{-1} = -\zeta_5$;
the pentagon remains hosted in \texttt{FibonacciMTC.lean}, the
hexagon in \texttt{FibonacciBraiding.lean} because the $R$-symbols
are complex phases that live in $\mathbb{Q}(\zeta_5)$ rather than
the real field $\mathbb{Q}(\sqrt{5})$ of the F-matrix.
