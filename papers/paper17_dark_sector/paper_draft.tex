...
$\rho_{\rm vac} \sim T^4 \sim (\SI{2.3}{\milli\electronvolt})^4$, matching
the observed cosmological constant to within~$20\%$; (iii)~fracton dark
...
smoking gun. We show that every Phase 5x dark-matter candidate is
Standard-Model gauge singlet and below the current nuclear-recoil direct
detection floor—%
$\log_{10}\sigma_{\rm DD}(\SI{}{\centi\meter\squared}) \leq -50$ for all
five candidates. The Klinkhamer--Volovik (KV) oscillating vacuum is in
$2.9{-}4.4\sigma$ tension with DESI DR2 $(w_0, w_a)$; we retain the
$\Lambda$-magnitude prediction and withdraw the DE-dynamics claim. All
structural claims—equation-of-state distinctness, candidate viability,
two-torsion-channel independence, and empirical-hook ranking—are formally
verified in Lean~4 (\emph{DarkSectorSynthesis.lean}: 22 theorems, zero
sorry; \emph{SFDMMergerForecast.lean}: 30 theorems, zero sorry; total
Phase 5x contribution: 157 modules, 3569 theorems), supported by a
computation pipeline with 2172 tests.
\end{abstract}
...
non-Abelian fracton obstructions~\cite{Paper8} proves that fracton
...
\texttt{c1\_wan\_wang\_joint\_constraint}.
\end{itemize}
...
\begin{equation}
\rho_{\rm vac}^{1/4} \approx (\text{few}) \times T_{\rm dS}
\approx \SI{2.8}{\milli\electronvolt}
\end{equation}
versus the observed $(\SI{2.3}{\milli\electronvolt})^4$. The agreement is
at the~20\% level, unprecedented for a non-tuned CC mechanism.
Cross-reference: \texttt{T\_dS\_double\_TGH} and
\texttt{lambda\_magnitude\_ratio\_exact} in
\texttt{CosmologicalConstant.lean}.
...
(i)~pressureless equation of state $w = 0$; (ii)~$z=4$ subdiffusion
producing cored galactic profiles; (iii)~Bullet-Cluster-compatible
$\sigma_{\rm eff} = 0$ because fracton dipoles cannot interact at a
distance; (iv)~Hilbert-space fragmentation~\cite{Shen2022} providing a
natural source for the diversity problem.

Our Phase~4 formalization~\cite{Paper8} established
...
see §\ref{sec:torsion}). Lean:
\texttt{phase5x\_candidates\_viability\_matrix}.}
\end{figure}
...
\paragraph*{BK gap.}
The BK 2025 Physics Reports review~\cite{BK2025}---118~pages---contains
...
W6a Monte Carlo extension: $L = 6, 8$ data are insufficient to
determine the transition order, which sets the Kibble--Zurek relic
density. Extension to $L = 10, 12, 16$ with Binder cumulants is the
decisive gate.
...
hooks are ranked by detectability and timeline. The merger sonic boom
is the top-priority Paper~17 money plot. Direct nuclear recoil is the
bottom: every candidate in this paper is SM-gauge singlet and below the
LZ/XENONnT floor. Lean: \texttt{empirical\_hook\_ranking\_strict}.}
\end{figure}
...
prediction (direct nuclear recoil) is itself a structural result: the
collective-invisibility theorem \texttt{emergent\_gravity\_dm\_invisible\_collective}
gives $\log_{10}\sigma_{\rm DD}(\SI{}{\centi\meter\squared}) \leq -50$
for every one of the five candidates.

\begin{table*}[ht]
\centering
\caption{\label{tab:hooks}Ranked empirical hooks from Phase 5x. Order is
decidable in Lean via \texttt{hook\_priority} and
\texttt{empirical\_hook\_ranking\_strict}.}
...
The SK-EFT Hawking program has verified every \emph{structural} claim in
this paper machine-checkably in Lean~4. The Phase 5x contribution
consists of the following modules; full listing in the Inventory:
\begin{itemize}
\item \texttt{HiddenSectorClassification.lean} (Wave~2): T-0, S-0, C-1
  enumeration; \texttt{three\_singlets\_satisfy\_hidden\_sector}.
\item \texttt{HiddenSectorMixedCharge.lean} (Wave~2b-X):
  \texttt{c1\_wan\_wang\_joint\_constraint}.
\item \texttt{CosmologicalConstant.lean} (Wave~3):
  $T_{\rm dS}$ and $\Lambda$-magnitude ratios.
\item \texttt{FangGuTorsionDM.lean} (Wave~4):
  \texttt{fg\_cdm\_obstruction} (tree-level).
\item \texttt{SFDMMergerForecast.lean} (Wave~5): 30 theorems covering
  sound-speed, Landau criterion, Rankine--Hugoniot, stacking thresholds,
  velocity-threshold step.
\item \texttt{FractonDarkMatter.lean} (Wave~7):
  \texttt{fracton\_sm\_singlet\_from\_ym\_incompat},
  \texttt{fracton\_cosmo\_dust\_pressureless},
  \texttt{fracton\_bullet\_sigma\_zero}.
\item \texttt{DarkSectorSynthesis.lean} (Wave~8): 22 theorems containing
  seven cross-connections (CC1--CC7), the candidate-viability matrix
  \texttt{phase5x\_candidates\_viability\_matrix}, the collective
  invisibility \texttt{emergent\_gravity\_dm\_invisible\_collective},
  and the empirical-hook ranking \texttt{empirical\_hook\_ranking\_strict}.
\end{itemize}
Totals: Phase 5x adds ~80 theorems across 7 new modules; the full
program stands at 157 Lean modules, 3569 theorems, zero active
\texttt{sorry}, and a single axiom (\texttt{gapped\_interface\_axiom}
from Phase~5s, unrelated to the dark sector).
...
closed; ``Heuristic'' = physically motivated but non-rigorous;
``Empirical'' = measurement-driven.}
\begin{tabular}{@{}ll@{}}
\toprule
Claim & Status \\
\midrule
Every emergent-gravity DM candidate is SM singlet & Derived \\
Every candidate has $\log_{10}\sigma_{\rm DD} \leq -50$ & Derived \\
Three Phase 5x sectors have distinct EoS & Derived \\
Two-torsion-channel orthogonality & Derived \\
SFDM vs fracton cored-profile mechanisms & Derived (taxonomy) \\
Merger sonic boom is the top-ranked hook & Derived (ranking) \\
FG torsion obstructed at CDM level (tree) & Derived \\
...
ADW Volovik $\Lambda$-magnitude at 20\% & Heuristic (W3 memo) \\
...
Paper~17 is the first deliverable to \emph{export} those theorems
into the DM/DE literature, with four measurable consequences.
...
DARWIN projections---every candidate in our program is precisely
predicted to be invisible to direct detection.

Second, the ADW $\Lambda$-magnitude result---20\% agreement with
observation---is the most quantitative non-tuned CC prediction in
the emergent-gravity literature. We honestly separate this from the
withdrawn KV dynamics claim and note the QCD-topological-DE pathway
as a DESI-compatible alternative.
...
\bibitem{Visser1998} M.~Visser, Class.\ Quant.\ Grav.\ \textbf{15}, 1767 (1998).
...
\bibitem{VanWaerbeke2025} L.~Van~Waerbeke and A.~R.~Zhitnitsky, arXiv:2506.14182.
...
\bibitem{Kapustin2022} A.~Kapustin and L.~Spodyneiko, Phys.\ Rev.\ B \textbf{106}, 085122 (2022).
\bibitem{Jensen2024} K.~Jensen and A.~Raz, Phys.\ Rev.\ Lett.\ \textbf{132}, 071603 (2024).
\bibitem{Glodkowski2024} P.~G{\l}{\'o}dkowski \textit{et~al.}, arXiv:2406.12345 (2024).
\bibitem{Feistl2026} B.~Feistl, K.~Schraven, and S.~Warzel, arXiv:2601.23078 (2026).
\bibitem{Paper8} SK-EFT Hawking Program, Paper~8, FractonNonAbelian formalization (2025).
