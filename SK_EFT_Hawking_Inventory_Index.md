...
**Last synced:** 2026-04-22 session 5 (**Phase 5x Wave 9 Paper 17 draft SHIPPED + validate.py paper_provenance fix**).
...
|------|-------|-----------------|
| Lean theorems | **3569** (post-Phase-5x W5+W8; verified via `update_counts.py` 2026-04-22 session 4; +96 from W5+W8, 52 substantive new theorems — 30 W5 + 22 W8) | counts.json — package-module-bound count |
| Placeholders (True := trivial) | **98** | Module summaries + content placeholders; see PLACEHOLDER_THEOREMS in constants.py |
| Aristotle-proved | **322** (machine) | ARISTOTLE_THEOREMS in constants.py; 44 Aristotle runs total |
| **Sorry gaps** | **0** | Project-wide (verified 2026-04-22 session 4 via `lake build SKEFTHawking.ExtractDeps`, 8421 jobs). Uqsl2Hopf, Uqsl2AffineHopf, Uqsl3, Uqsl3Hopf all 0 sorry. CenterFunctor 0 sorry (2 tracked hypotheses as `Prop` defs). CenterFunctorZ2Equiv 0 sorry: 2 tracked `Prop` hypotheses. DarkSectorSynthesis 0 sorry: 1 tracked `Prop` hypothesis `H_VestigialRelicCarriesZ16Charge` parks ℤ₁₆ × vestigial-relic cross-connection pending Phase 6 bordism. **SFDMMergerForecast 0 sorry (2026-04-22 session 4)**: no tracked hypotheses; L4/L5 (hard, MOND force derivation + FDR noise bound) fully deferred to Phase 6 rather than parked as hypotheses. |
| **Axioms** | **1** | gapped_interface_axiom in SPTClassification.lean |
| Lean modules | **157** | All `.lake/build`-registered `.lean` files in `lean/SKEFTHawking/*` (verified via `update_counts.py` 2026-04-22 session 4). +1 Phase 5x W5 (2026-04-22 session 4): SFDMMergerForecast (30 thms — BK fiducial constants, sound-speed formula, Landau criterion, 5 canonical merger table, Rankine-Hugoniot SFDM γ=2, stacked S/N scaling, decidable numerical S/N witnesses, backreaction direction, smoking-gun step function). +1 Phase 5x W8: DarkSectorSynthesis (22 thms). +1 Phase 5x W4-strengthening: FangGuTorsionDM (10 thms). +1 Phase 5x W7: FractonDarkMatter (25 thms). +1 Phase 5x W3 partial: CosmologicalConstant (8 thms). +1 Phase 5x W2b Track X: HiddenSectorMixedCharge (6 thms + 3 helpers). +1 Phase 5x W2: HiddenSectorClassification (9 thms). +1 Phase 5w: GrapheneNoiseFormula (8 thms). |
| Lean definitions | **2909** | counts.json |
| Python source modules | **64** (update_counts.py 2026-04-22 session 4; includes src/dark_sector/{z16_hidden_sector,adw_cosmological_constant,fracton_dm,synthesis,sfdm_sk_eft,sfdm_merger_forecast}.py) | |
| Test files | **56** (update_counts.py 2026-04-22 session 4; +test_sfdm_merger_forecast.py = 86 tests; +test_dark_sector_synthesis.py = 70 tests) | +test_fracton_dm.py (86); +5 Phase 5w: test_graphene_metric (26), _hawking (21), _spectrum (21), _transport (17), test_platform_comparison (14) = 99 total graphene tests |
| Test count | **2174 pass**, 16 skip, 0 fail (2026-04-22 session 5; +2 Paper 17 figure smoke tests) | `pytest tests/ -q` |
