...
Lean modules under `lean/SKEFTHawking/`, grouped by subdirectory or topical family. Live count: the AUTOGEN table above.
...
| `lean/SKEFTHawking/Control/` | 4 | **Phase 6EE — the control + composite-ceiling layers, capstone of the `6E*` verified-device-physics-metrology series (bundle D12).** `RotatingWave` (112): the co-rotating reduction as an EXACT split `interactionHamiltonian_decomp : H_I = H_RWA + V` — the RWA is the act of discarding `V`, and this identity is what makes the discard quantifiable — plus `rotFrame_ode` pinning the frame as the exponential, the closed-form Bloch–Siegert antiderivative and its uniform bound `‖S t‖ ≤ 2(Ω/ω)·ℓ¹`, the FTC link, the GENERAL squaring identity `rwaGenerator_sq` (`(Δ²+Ω²m²)/4`, with resonance a corollary and `rwaRotationAngle_lt_generalRotationAngle` showing detuning strictly increases the rate), the propagator-difference bound `rwa_propagator_difference_bound` — with `_inhabited` exhibiting a DEGENERATE witness (identity drive ⇒ `rwaGenerator = 0`, `U_rwa ≡ 1`, difference is unobservable global phase), `rwa_propagator_difference_bound_physical` performing the identification `L = U_rwa⁻¹`, `U = U_exact`, `P = i·H_RWA`, `Q = −i·H_I` as a declaration, and **`diagonal_drive_propagator_bound`** APPLYING it at an `a·1 + d·σ_z` drive with `d ≠ 0` — nonzero generator, nonzero remainder, closed-form exact propagator (`diagonalExactPropagator` + `_ode`), every constant computed (`norm_zRotation`, `norm_rwaPropagator_diagonal`, `norm_diagonalExactPropagator` all `= 1`; `Kq` via `norm_interactionHamiltonian_le`), and an OBSERVABLE error (`zRotation_not_scalar`: the discarded factor is not a global phase). Review 5 ruled the round-4 witness degenerate — it was the same identity drive with detuning added, the closed-form co-rotating propagator pinned by `rwaPropagator_ode` — which is where `rwaGenerator_sq` is actually consumed — and validity/honest-failure witnesses. `BanachAveraging` (8): the averaging estimate over an abstract Banach algebra — `norm_integral_mul_mul_le` bounds `∫L·G·U` using only a bound on the ANTIDERIVATIVE of `G`, with *general* factor bounds (pinning them to 1 would exclude every ℓ^∞ rotation); `norm_propagator_sub_le` lifts it to the propagator difference. Abstract because the estimate is not 2×2-specific and because on concrete `Matrix` an instance diamond blocks `HasDerivAt.mul`. `DriveCalibration` (36): signed drive elements read off the operator as matrix ENTRIES and proved equal to their Pauli forms; DURATION calibration identities carrying fail conditions as binders, with the *consequences* proved (`no_duration_achieves_nonzero_angle_of_zero_element`, `magnitude_calibration_rotates_backwards`); the PHASE calibration layer — `rwaAxisPhasor` tied to the generator by `transverseElement_rwaGenerator` (a call, not a docstring claim), the factorisation `rwaAxisPhasor_eq = (Ω/2)·conj⟨0|O|1⟩·e^{iφ}`, and `envelope_phase_alignment` showing the achieved axis azimuth is the commanded phase shifted by the projected element's argument, with the `m = 0` fail condition and a σ_y quarter-turn mis-pointing witness; the suppression bound with a traceless 11× witness; Kramers degeneracy from first principles with a non-vacuity witness on `EuclideanSpace`. `CompositeReadoutCeilings` (35): fidelity ceilings over the relaxation/thermal floors and the 6EA/6EB/6EC floors — the latter three DERIVED by calling their upstream theorems, not assumed; the additive form rests on genuine measure-theoretic disjointness (`add_le_branch_error_of_disjoint`), and `combined_ceiling_add_lt_max` compares the two composites under one shared hypothesis set with a numeric gap witness (`1/200`). Witness pairs for all five ceilings, each BITES half derived from *its own* ceiling theorem so it concludes about `assignmentFidelity` rather than about a bound expression. 191 extracted declarations (180 authored + 11 compiler-generated equation lemmas), **191/191 kernel-pure, zero project axioms**. Reviewed by SIX fresh-context adversarial passes. Review 2 found a BLOCKER in code review 1 had already blessed (the propagator bound's hypotheses provably excluded every Schrödinger propagator pair); review 3 found 3 MAJOR / 5 IMPORTANT showing round 2's own fixes were bridged to their referents by prose; review 4 found 2 MAJOR / 5 IMPORTANT, by then almost entirely in the documentation layer; review 5 found 2 MAJOR / 4 IMPORTANT, both on round 4's own headline fix; review 6 found 3 MAJOR / 5 IMPORTANT, including that these Inventory entries had gone stale again. Every BLOCKER/MAJOR/IMPORTANT remediated in-session — the phase now ships the co-rotating propagator `rwaPropagator` pinned by its own ODE and by `rwaPropagator_unique`; `interactionPicture_ode` (the frame change GENERATES the dynamics, forcing the `−(ω/2)σ_z`); `driveOp_conjTranspose` + `exists_driveOp_of_isHermitian`; `calibrated_duration_transverse_propagator_full` + `trace_blind_to_rotation_direction`; `norm_counterRotating_le` + `integral_counterRotating_naive_bound` (the naive-Duhamel contrast, shipped rather than asserted); and `relaxation_photon_ceiling` — the first CROSS-LAYER composite (device ⊕ detection) on one attributed readout, **not** the first two-floor composite outright, since `relaxation_thermal_ceiling` predates the phase. ⚠️ Disclosed: the 6EB/6EC BITES witnesses fire at zero matched budget (signal-free readout) — honest but degenerate. | `ls lean/SKEFTHawking/Control/` |
...
Every bundle in `scripts/bundle_registry.BUNDLE_CODES` has a `papers/<code>/` directory. Live per-bundle readiness: `docs/BUNDLE_READINESS_HEATMAP.md`. Open:
...
**Aristotle-proved theorem and run counts: see the AUTOGEN table above.** Authority: `ARISTOTLE_THEOREMS` registry in `src/core/constants.py`. Run table in `SK_EFT_Hawking_Inventory.md` Section 3.
...
**Validation + counts:**
- `scripts/validate.py` — full validation suite; `--list` enumerates; `--check <name>` runs one.
