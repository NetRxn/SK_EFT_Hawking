...
|------|-------|-----------------|
| Lean theorems | **3993** (3882 substantive + 111 placeholder; +20 from Phase 5z Wave 1 ScalarRungInterpretation; counts.json delta also captures Lean's elaboration-level lemma counting) | counts.json 2026-04-24 — package-module-bound count |
| Placeholders (True := trivial) | **111** (unchanged across Phase 5y) | Module summaries + content placeholders; see PLACEHOLDER_THEOREMS in constants.py |
| Aristotle-proved | **322** (machine; unchanged across Phase 5y — no Aristotle runs in closure) | ARISTOTLE_THEOREMS in constants.py; 44 Aristotle runs total |
...
| Lean modules | **167** (was 166; +1 Phase 5z Wave 1: `ScalarRungInterpretation`) | counts.json 2026-04-24 |
| Lean definitions | **3297** (was 3260; +37 from ScalarRungInterpretation structures + noncomputable defs + match-predicate Prop) | counts.json 2026-04-24 |
| Python source modules | **68** | +2 Phase 5z W1 (`scalar_rung/higgs_prediction.py`, `scalar_rung/ew_mass_matrix.py`); `__init__.py` not counted as a module |
| Test files | **59** | +1 Phase 5z W1 (`tests/test_scalar_rung.py`, 24 tests / 5 classes) |
| Test count | **2840 pass**, 14 skip, 0 fail (Phase 5z W1; +24 scalar_rung on top of 2816 baseline; pre-existing torch-dep tests excluded) | `pytest tests/ -q --ignore=tests/test_stencil_dirac.py` |
