...
**Last synced:** 2026-04-23 session 7 (**Phase 5w Wave 10c greybody + quasi-1D strengthening COMPLETE** — `QuasiOneDReduction.lean` T2/T3/T5 refactored from positivity lemmas to bound-propagation theorems with abstract-variable hypotheses; 18 new tests (`TestGreybodyZeroFreq` + companions); paper 16 §IV updated "Γ=1 upper bound, principal uncertainty" → "Γ₀ ≈ 0.9994, quasi-1D correction ≤ 1.8% at ω_H"; fig 104/105 regenerated with realistic Γ(ω); 4.5% vs 1.8% Lean comment inconsistency resolved to 1.8% per deep research §2.3). Session 6: Phase 5t Wave 2 FermiHubbardDimer.lean SHIPPED — Track A COMPLETE; Phase 5x W9 Paper 17 draft + validate.py fix also in the session-6 group.
...
|------|-------|-----------------|
| Lean theorems | **3587** (3488 substantive + 99 placeholder; +18 Phase 5t W2 FermiHubbardDimer 2026-04-23 session 6 — T1-T7 singlet + T10a-c triplet + corollaries + summary marker) | counts.json — package-module-bound count |
| Placeholders (True := trivial) | **99** (+1 `fermi_hubbard_dimer_wave2_summary`) | Module summaries + content placeholders; see PLACEHOLDER_THEOREMS in constants.py |
| Aristotle-proved | **322** (machine) | ARISTOTLE_THEOREMS in constants.py; 44 Aristotle runs total |
| **Sorry gaps** | **0** | Project-wide (verified 2026-04-23 session 6 via `lake build SKEFTHawking.ExtractDeps`, 8422 jobs). Uqsl2Hopf, Uqsl2AffineHopf, Uqsl3, Uqsl3Hopf, FermiHubbardDimer all 0 sorry. CenterFunctor 0 sorry (2 tracked hypotheses as `Prop` defs). CenterFunctorZ2Equiv 0 sorry: 2 tracked `Prop` hypotheses. DarkSectorSynthesis 0 sorry: 1 tracked `Prop` hypothesis `H_VestigialRelicCarriesZ16Charge` parks ℤ₁₆ × vestigial-relic cross-connection pending Phase 6 bordism. **SFDMMergerForecast 0 sorry (2026-04-22 session 4)**: no tracked hypotheses; L4/L5 fully deferred. **FermiHubbardDimer 0 sorry (2026-04-23 session 6)**: T8/T11/T12 and all of Layer 2 (T13-T15) fully deferred. |
| **Axioms** | **1** | gapped_interface_axiom in SPTClassification.lean |
| Lean modules | **158** (+1 Phase 5t W2 2026-04-23 session 6: FermiHubbardDimer — 3×3 singlet-sector Hamiltonian + chiral operator + 6×6 full Hamiltonian; T1-T7 core + T10a-c triplet eigenvectors; 0 sorry, 0 new axioms, ~230 LOC; mirrors deep research `Lit-Search/Phase-5t/Effective Fermi-Hubbard Dimer…`). +1 Phase 5x W5: SFDMMergerForecast. +1 Phase 5x W8: DarkSectorSynthesis. +1 W4-strengthening: FangGuTorsionDM. +1 W7: FractonDarkMatter. +1 W3 partial: CosmologicalConstant. +1 W2b-X: HiddenSectorMixedCharge. +1 W2: HiddenSectorClassification. +1 Phase 5w: GrapheneNoiseFormula. |
| Lean definitions | **2909** | counts.json |
| Python source modules | **65** (+1 Phase 5t W2 2026-04-23 session 6: src/fermi_hubbard/dimer.py mirroring Lean matrix definitions) | |
| Test files | **57** (+1 Phase 5t W2 2026-04-23 session 6: test_fermi_hubbard_dimer.py = 74 tests over 6 test classes, random + parametric grids) | |
| Test count | **2300 pass**, 16 skip, 0 fail (2026-04-23 session 7; +74 Phase 5t W2, +18 Phase 5w W10c greybody/quasi-1D coverage in test_graphene_spectrum.py) | `pytest tests/ -q` |
