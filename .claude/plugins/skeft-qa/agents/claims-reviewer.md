...
`cd "$REPO" && uv run python scripts/sentence_state.py validate …`); the `uv run` /
`scripts/` examples below are repo-relative and only work from `$REPO`.
...
## Routing fields (ADR-012 D1) — required on every finding
