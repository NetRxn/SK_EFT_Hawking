...
## Relative-MV LES + local-homology batch (2026-07-21, wt3) — one entry is a VERIFICATION-TRUST law

- **⚠ `lean_verify` can report a FALSE `sorryAx` from a STALE elaboration** — seen right after fixing an elaboration error on a theorem that was in fact clean. **A `lean_verify` sorryAx report is not authoritative on its own.** Re-run `lean_diagnostic_messages` (and `lake build` if a dependency changed) to force re-elaboration, then re-verify. Cuts both ways: it can raise a false alarm, and in principle a stale-clean read could mask a real one — so the merge gate stays a fresh `lake env lean` `#print axioms`, never `lean_verify` alone.
- **`ChainComplexLESInt`'s family implicit `{M : ℕ → Type}` does not unify** against `fun n => RelativeChainInt U n × RelativeChainInt V n`. Diagnostic is `failed to synthesize Membership … (Submodule ?m (?m n))`, **not** a timeout — so it is a *unification* variant of the `{X : TopCat}` pin law with a different symptom. Pin it: `(M := bChainInt U V)` at every use site.
- **`Hml.mk` is a plain `def`, not a bundled map**, so `map_add` will not fire. `show … Hml.mkHom …` first; same for `RelHomologyInt.mk`.
- **Instantiating an abstract-in-`N` LES engine forces the PRODUCT complex**, and `Hₙ(C(M,U) ⊕ C(M,V))` is *not* defeq to `Hₙ(M,U) × Hₙ(M,V)`. Budget a "homology of a product complex = product of homologies" bridge (`bHmlProdEquivInt`) plus lemmas identifying the engine's induced maps with the project's own (`prod_Hmap_diagInt`, `Hmap_sum_prodInt`) — otherwise the LES lands in a parallel copy of the maps and no consumer can use it.
- **Star-shapedness beats convexity for complement retracts.** `SingularConvexComplementRetract` needs `0 ∈ interior A`; a flat disk in ℝ⁴ has empty interior (`zero_notMem_interior_flatDisk` proves the obstruction is real). The radial retraction needs only star-shapedness about `0` plus `‖·‖ < 1`, so the general statement costs nothing and covers points/arcs/cones/balls. Engine: `smul_mem_compl` with the dichotomy decided by `‖p‖ ≥ 1` vs `< 1` — no case analysis on the homotopy parameter.
- Pin renames at this toolchain: `le_or_lt` → `le_or_gt`; `EuclideanSpace.norm_single`/`.single_apply` → `PiLp.norm_single`/`PiLp.single_apply`.
